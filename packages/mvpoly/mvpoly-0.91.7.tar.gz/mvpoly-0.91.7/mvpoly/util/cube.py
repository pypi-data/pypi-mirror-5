...
def int(p, d) :
    ...
    def int2(p, k) :
        ...
        q[idx] = p / meshgridn(pv, k, np.array(range(1,pv[k]+1), dtype=float))
        return q
...
def horner(p, dtype=float, *args) :
    ...
    *args* = *X*, *Y*, ..., (as many as the dimension 
    ...
    def coerce_np_array(arg) :
        return np.array(arg if list_like(arg) else [arg])

    args = [coerce_np_array(arg) for arg in args]

    shp = args[0].shape
    assert all(shp == s for s in map(lambda arg : arg.shape, args)), \
