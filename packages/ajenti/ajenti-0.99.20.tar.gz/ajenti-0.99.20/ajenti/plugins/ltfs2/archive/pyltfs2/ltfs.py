...
class Entry (Base):
    ...
    parent = relation('Entry', uselist=False)
...
class LTFS (object):
    ...
    def create_tape(self, xml, name, custom_a='', custom_b='', custom_c='', custom_d=''):
        ...
        def recurse(cwd, element):
            ...
            for eroot in ([element] + list(element.findall('contents'))):
                for edir in eroot.findall('directory'):
                    recurse.counter += 1
                    dir = Entry(
                        parent=cwd,
                    ...
                    recurse(dir, edir)
                for efile in eroot.findall('file'):
                    recurse.counter += 1
                    file = Entry(
                        parent=cwd,
