...
class LTFSArchivePlugin (SectionPlugin):
    def init(self):
        ...
        url = 'mysql+mysqlconnector://dbuser:syslink@localhost/syslink_tapelibrary'
    ...
    def navigate(self, tape, cwd):
        ...
        files = self.ltfs.ls(tape, cwd)
        self.nav_files = []
        for file in files:
            if self.nav_files:
                nf = self.nav_files[-1]
                if nf.name == file.name:
                    if nf.uid > file.uid:
                        continue
                    self.nav_files.remove(nf)
                self.nav_files.append(file)
            else:
                self.nav_files.append(file)
