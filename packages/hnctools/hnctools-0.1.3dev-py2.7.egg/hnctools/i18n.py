...
def add_renderer_variables(event):
    if event['renderer_name'] != 'json':
        request = event['request']
        event.update({'_' : request.translate, 'ungettext' : request.ungettext, 'i18n':I18N(request)})
    return event


def extend_request(config, domain, available_locales, default_locale_name, tsf = None):
    ...
    config.add_request_method(I18N, "i18n", reify = True)
