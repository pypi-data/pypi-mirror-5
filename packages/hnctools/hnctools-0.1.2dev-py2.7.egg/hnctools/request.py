...
def add_renderer_variables(event):
    if event['renderer_name'] != 'json':
        ...
        event.update({'_' : request.translate, 'ungettext' : request.ungettext, 'i18n': i18n })
    return event



def extend_request(config, translationFactory = None, available_locales = None, default_locale_name = None):
    def furl(request):
        try:
    ...
    if translationFactory:
        config.add_request_method(i18n.set_lang(translationFactory), "set_lang")
        config.add_request_method(i18n.getFullLocale)
        config.add_request_method(i18n.getLangName)
        config.set_locale_negotiator(i18n.DefaultLocaleNegotiator(available_locales, default_locale_name))

        config.add_subscriber(i18n.add_localizer(translationFactory), 'pyramid.events.NewRequest')
        config.add_subscriber(add_renderer_variables, 'pyramid.events.BeforeRender')
    else:
        log.warning("REQUEST CONFIGURED WITHOUT LOCALE SUPPORT")
