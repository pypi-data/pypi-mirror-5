...
# log debug output to /var/log/durus.log by default
log.addHandler(logging.FileHandler('/var/log/durus.log'))
log.setLevel(logging.DEBUG) # set to DEBUG if debugging..

# max_queued_connections_limit: Increase the maximum amount of queued connections 
# default: 16
# recommended: 128
# production: recommended
max_queued_connections_limit = 128
...
class Server(object):
    ...
    def dispatch(self):
        ...
        while 1:
            client_socket, client_address = yield conn.accept()
    ...
    def _report_load_record(self, storage):
        if storage.d_load_record and is_logging(log, 5):
            log.debug('\n'.join('%8s: %s' % (value, key)
                             for key, value
    ...
    def handle_commit(self, client, db_name):
        ...
        logging_debug = is_logging(log, 10)
