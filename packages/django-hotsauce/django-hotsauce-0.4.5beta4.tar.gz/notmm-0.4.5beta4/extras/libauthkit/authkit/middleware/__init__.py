...
# FIXME: Get rid of Paste artefacts
from paste.util.import_string import eval_import
from paste.deploy.converters import asbool

from authkit.permissions import no_authkit_users_in_environ, AuthKitConfigError
...
class AuthKitUserSetter(object):
    ...
    REMOTE_USER on each request. The class is overridden by the individual
    handlers.
    """
    pass
...
def get_authenticate_function(app, auth_conf, format, prefix):
    """
...
def middleware(app, app_conf=None, global_conf=None, prefix='authkit.', 
               handle_httpexception=False, middleware=None, **options):   
    """
    ...
    ``middleware``
        ...
        set then ``authkit.setup.methods`` should not be set.
    ...
    if middleware:
        ...
        #assert isinstance(all_conf['setup.method'], list), 'expected a list instance'
        methods = [method.strip() for method in all_conf['setup.method']]
