...
your own requirements then you will have no need for this implementation. It is
...
class AuthKitNoSuchUserError(AuthKitConfigError):
    pass
    
class AuthKitNoSuchRoleError(AuthKitConfigError):
    pass

class AuthKitNoSuchGroupError(AuthKitConfigError):
    pass
    
class AuthKitNotSupportedError(AuthKitConfigError):
    pass

class AuthKitError(AuthKitNotSupportedError):
    pass
...
class UsersFromString(UsersReadOnly):
    """
    A ``Users`` class which obtains user information from a string with lines
