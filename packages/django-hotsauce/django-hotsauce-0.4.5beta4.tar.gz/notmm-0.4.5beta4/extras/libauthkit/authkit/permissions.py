...
from authkit.authorize.middleware import middleware
...
class AuthKitConfigError(NotAuthenticatedError): 
    """
...
no_authkit_users_in_environ = AuthKitConfigError(
    ...
    'forgotten to specify a ``Users`` object or are using the default '
...
class Permission(object):
    ...
    .. Notes ::
        ...
        - A permission object to perform an ``and`` operation is possible and has
        been implemented as the ``And`` permission class.  
...
class RemoteUser(RequestPermission):
    ...
    def check(self, app, environ, start_response):
        if (not 'REMOTE_USER' in environ.keys() or environ['REMOTE_USER'] is None):
            raise NotAuthenticatedError('Not Authenticated')
        elif (self.accept_empty is False and not environ['REMOTE_USER']):
