...
log.addHandler(logging.FileHandler('/var/log/django.log'))
...
class BannerBase(object):
    ...
    def show(self, registered=False):
        ...
        if not registered:
            self.debug("Django-hotsauce %s (UNREGISTERED edition)" % BASEVERSION)
        else:
...
def daemonize(wsgi_app, bind_addr, logging=False, autoreload=True, debug=False):
    """
