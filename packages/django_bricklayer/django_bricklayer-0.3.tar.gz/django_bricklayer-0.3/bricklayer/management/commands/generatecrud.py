...
class Command(AppCommand):
    ...
    option_list = AppCommand.option_list + (
        ...
        make_option('-c', 
                    ...
                    action="store_true",
                    default=False,
                    help='Generate only the create view'),
        make_option('-r', 
                    dest='read',
                    action="store_false",
                    default=True,
                    help='Generate only the detail and list views'),
        make_option('-u', 
                    dest='update',
                    action="store_false",
                    default=True,
                    help='Generate only the update view'),
        make_option('-d', 
                    dest='delete',
                    action="store_false",
                    default=True,
                    help='Generate only the delete'),
        ...
        for model in models:
            ...
            if not exists_path(app_path, 'views.py') or is_blank(app_path, "views.py"):
                ...
                context  = Context({'app': app_name,
                                    ...
                                    'has_form': options['forms'],
                                    'generic_views': generic_views,
                                    'create': model+"Create" not in views_module,
                                    'detail': model+"Detail" not in views_module,
                                    'list':   model+"List" not in views_module,
                                    'update': model+"Update" not in views_module,
                                    'delete': model+"Delete" not in views_module})
