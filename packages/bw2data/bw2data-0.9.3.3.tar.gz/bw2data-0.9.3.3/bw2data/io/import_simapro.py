...
without_slashes_pattern = ' (?P<geo>[A-Z]{2,10}) [SU]$'
ws_re = re.compile(without_slashes_pattern)


class SimaProIsFrustrating(StandardError):
    pass
...
def detoxify(string, log):
    ...
    if not found:
        found = ws_re.findall(string)
        if not found:
            raise SimaProIsFrustrating("\nCan't parse process name:\n\t%s\nPlease let the Brightway2 developers know so we can fix this!" % string)
        log.warning(u"Name '%s' doesn't have SimaPro™ slashes - matched without slashes" % string)

    geo = found[0][0]
    name = re.sub(detoxify_pattern, '', string)
    print string, name, geo
    return [name, geo]
...
class SimaProImporter(object):
    def __init__(self, filepath, delimiter="\t", depends=['ecoinvent 2.2'], overwrite=False, name=None, geo="GLO"):
        ...
        self.name = name
        self.geo = geo
        self.log, self.logfile = get_io_logger()
