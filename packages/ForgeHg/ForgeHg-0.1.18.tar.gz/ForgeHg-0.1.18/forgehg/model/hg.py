...
class HgImplementation(M.RepositoryImplementation):
    ...
    def log(self, revs=None, path=None, exclude=None, id_only=True, **kw):
        ...
        exclude = exclude or []
        match = cmdutil.match(self._hg, pats=pats)
        noop = lambda ctx, fns: None
        branches = defaultdict(list)
        for name, head in self._hg.branchtags().iteritems():
    ...
    @LazyProperty
    def branches(self):
        return [Object(name=k, object_id=self._hg[v].hex()) for k,v in self._hg.branchtags().items()]
