...
Default_val_calculate= 'r,th', '0.5*m*(vr**2+(r*vth)**2)', 'm*g*(h-r*cos(th))' , '0.5*c1*vr**2,0.5*c2*vth**2','m,g,h,a,c1,c2','50,10,6,1.2,0,0','6.1,0,0.2,0','-350,0'
...
class Windows():
    def simulation_window(self):
        if self.calculatecheck.get()==0:
            tkMessageBox.showerror(title="Calculation Status",message="Calculation Unsuccessful!                                      Press 'calculate' first.")
        else:
            root = tk.Toplevel()
