...
class BaseUserModel(AbstractBaseUser, PermissionsMixin, TimeStampedModel):
    ...
    def send_activation_key(self):
        ...
        key_expiration = timezone.now() + timedelta(
            days=getattr(settings, 'ACCOUNT_ACTIVATION_DAYS', 7)
        )

        key = self.activation_keys.create(user=self, expires=key_expiration)
        key.send_email()
