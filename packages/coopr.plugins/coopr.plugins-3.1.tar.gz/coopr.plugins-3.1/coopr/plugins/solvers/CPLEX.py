...
class CPLEX(OptSolver):
    ...
    pyutilib.component.core.alias('cplex', doc='The CPLEX LP/MIP solver')
...
class CPLEXSHELL(ILMLicensedSystemCallSolver):
    ...
    def _presolve(self, *args, **kwds):
        ...
        elif (self.warm_start_solve is True) and (isinstance(args[0],basestring) is False) and (has_discrete_variables(args[0]) is True):
        ...
        if (len(args) > 0) and (isinstance(args[0],basestring) is False):
            ...
            if self.warm_start_solve is True:
                ...
                if has_discrete_variables(args[0]) is True:
    ...
    def executable(self):
        ...
        if executable is None:
            pyutilib.component.core.PluginGlobals.env().log.warning("Could not locate the 'cplex' executable, which is required for solver %s" % self.name)
            self.enable = False
            return None
