...
class PICO(OptSolver):
    ...
    pyutilib.component.core.alias('pico', doc='The PICO LP/MIP solver')
...
class PICOSHELL(SystemCallSolver):
    ...
    def executable(self):
        ...
        if executable is None:
            pyutilib.component.core.PluginGlobals.env().log.warning("Could not locate the 'PICO' executable, which is required for solver %s" % self.name)
            self.enable = False
            return None
