...
class ASL(SystemCallSolver):
    ...
    pyutilib.component.core.alias('asl', doc='Interface for solvers using the AMPL Solver Library')
    ...
    def executable(self):
        ...
        if executable is None:
            pyutilib.component.core.PluginGlobals.env().log.warning("Could not locate the '%s' executable, which is required for solver %s" % (self.options.solver, self.name))
            self.enable = False
            return None
