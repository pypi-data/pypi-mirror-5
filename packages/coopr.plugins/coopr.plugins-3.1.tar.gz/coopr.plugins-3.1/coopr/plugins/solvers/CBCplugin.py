...
from coopr.plugins.solvers.mockmip import MockMIP
...
def configure_cbc():
    ...
    if _cbc_version < (2,7,0,0):
        logger.warning("found CBC version ("+('.'.join(str(i) for i in _cbc_version))+") < 2.7; ASL support disabled.")
        logger.warning("consider upgrading CBC")
        return
...
class CBC(OptSolver):
    ...
    pyutilib.component.core.alias('cbc', doc='The CBC LP/MIP solver')
    ...
    def __new__(cls, *args, **kwds):
        ...
        if mode == 'nl':
            if cbc_compiled_with_asl:
                # CBC doesn't not accept all asl style command line options
...
class CBCSHELL(SystemCallSolver):
    ...
    def executable(self):
        ...
        if executable is None:
            pyutilib.component.core.PluginGlobals.env().log.warning("Could not locate the 'cbc' executable, which is required for solver %s" % self.name)
            self.enable = False
            return None
