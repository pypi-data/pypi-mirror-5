...
#輸入：
#trainer = 要訓練的ssvm trainer
#fround = 第一輪要撒多少點在C和G交織成的參數平面(預設是13)
#sround = 第二輪要撒多少點在C和G交織成的參數平面(預設是9)
#tround = 第三輪要撒多少點在C和G交織成的參數平面(預設是5)
#C_start = 參數C的起始點
#C_end = 參數C的結束點
#G_start = 參數G的起始點
...
###############################################################################
def Hibiscus(trainer, fround=13, sround=9, tround=5, C_start=-6, C_end=10, G_start=-12, r=1, v=10, k=1, s=0):
    if C_end - C_start <= 0:
        print "===Error in hibiscus : C_end must greater than C_start==="
        return False
    if G_start >= 1:
        print "===Error in hibiscus : G_start must less than 1==="
        return False
    if (fround or sround or tround) is not isinstance( v, int ) or (fround or sround or tround) <3 or (fround or sround or tround) >30:
        print "===Error in hibiscus : uniform table is wrong==="
        return False
    UDTable = numpy.array([
    [],
    [],
...
###############################################################################
def GridSearch(trainer, C_start=-6, C_end=10, G_start=-12, r=1, v=10, k=1, s=0):
    if C_end - C_start <= 0:
        print "===Error in gidsearch : C_end must greater than C_start==="
        return False
    if G_start >= 1:
        print "===Error in gidsearch : G_start must less than 1==="
        return False
    trainer.make(r, v)
    Best_C = 0
    Best_G = 0
