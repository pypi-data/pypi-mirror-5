...
def main():
  ...
  if args.demandedplot == 4:
    ...
    mpl.hist(base_pos, bins=20, color='blue', alpha=0.6, label="Valid users", normed=True)
    mpl.hist(base_neg, bins=10, color='red', alpha=0.8, label="Zero-effort impostors", normed=True)
    #mpl.hist(base_neg, bins=10, color='red', alpha=0, normed=True)
    mpl.hist(over_neg, bins=20, color='black', alpha=0.4, label="Spoofing attacks", normed=True)
    #mpl.hist(over_neg, bins=10, color='black', alpha=0, normed=True)  
  ...
  if args.demandedplot == 5:
    ...
    # plot the line
    mpl.axvline(x=thres_baseline, ymin=0, ymax=1, linewidth=3, color='green', linestyle='--', label="EER threshold")
