...
class ConfigurationManager(object):
    ...
    def _overlay_expand(self):
        ...
        while new_keys_discovered:  # loop until nothing more is done
            ...
            print 'NNN', 'storage0.storage_classes' in str(keys)
            ...
            print "overlay -----"
            for key in keys:
                if 'storage_classes' in key:
                    if 'storage0' in key:
                        print 'BAD DEAL', key, keys
                    else:
                        print key
                if key not in known_keys:  # skip all keys previously seen
                    ...
                    for a_value_source in self.values_source_list:
                        try:
                            ...
                            if key.endswith('storage_classes'):
                                if key in val_src_dict:
                                    print key, type(a_value_source), val_src_dict[key]
                                else:
                                    print key, type(a_value_source), 'NO'
                            #print 'looking for %s in %s' % (key, val_src_dict)
            ...
            for key in keys:
                ...
                if key not in known_keys:  # skip all keys previously seen
                    ...
                    print "Q!!Q key now known", key
                    known_keys.add(key)
                    if 'destination' in self.option_definitions:
                        print 'destination present:', self.option_definitions.__dict__
                        if 'storage0' in self.option_definitions.destination:
                            print 'storage0 present:', self.option_definitions.destination.__dict__
                            if 'crashstorage_class' in self.option_definitions.destination.storage0:
                                print 'crashstorage_class present:', self.option_definitions.destination.storage0.crashstorage_class.default
                                if self.option_definitions.destination.storage0.crashstorage_class.default == 'socorro.external.postgresql.crashstorage.PostgreSQLCrashStorage':
                                    print '!!!!!!!!!!! ok'
                                else:
                                    print "########### we're fucked"
                    an_option = self.option_definitions[key]
                    if isinstance(an_option, Aggregation):
                        print "!QQ! ignoring Aggregation:", key
                        continue  # aggregations are ignored
                    print 'working with option', an_option.name, an_option.default
                    ...
                    try:
                        ...
                        n = new_req.safe_copy()
                        print "CCCC - key:", key
                        print "CCC0 - option.name:", an_option.name
                        print "CCC1 - option.value:", an_option.default
                        try:
                            print "CCC2 - current_namespace", current_namespace.__dict__["_doc"], current_namespace.__dict__
                        except:
                            print "CCC2a - current_namespace", current_namespace.__dict__
                        print "CCC3 - new_req:", n.__dict__["_doc"], n.__dict__
                        current_namespace.update(new_req.safe_copy())
                        print "CCC4 - merged:", current_namespace.__dict__["_doc"], current_namespace.__dict__
                    except AttributeError, x:
                        # there are apparently no new Options to bring in from
                        # this option's value
                        pass
                nk = [x for x in self.option_definitions.keys_breadth_first()]
                print 'QQQQ', key, 'storage0.storage_classes' in str(nk)
            nk = [x for x in self.option_definitions.keys_breadth_first()]
            print 'PPPP', 'storage0.storage_classes' in str(nk)
            print "  end"
        return known_keys
