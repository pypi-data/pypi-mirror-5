...
from marcellus.util import strtotime
                ...
                if operador in ['#', '!']:
                    ...
                    if isinstance(col.type, DATE) or isinstance(col.type, TIME) or isinstance(col.type, DATETIME):
                        ...
                        # es nulo
                            ...
                            sql.append('"%s" = \'%s\'' % (c.encode('utf-8'), campos[1].strip()))
                    ...
                    for termino2 in terminos2:
                        ...
                        if isinstance(col.type, DATE) or isinstance(col.type, TIME) or isinstance(col.type, DATETIME):
                            ...
                            if operador in ['=', '<>']:
                                ...
                                expresion = '"%s" %s \'%s\'' % (c.encode('utf-8'), operador.encode('utf-8'), termino2)
                        ...
                        # texto (o cualquier otra cosa)
                            ...
                            if operador == '=':
                                ...
                                expresion = "CAST(\"%s\" as Text) ~* '^%s$'" % (c.encode('utf-8'), termino2)
                    ...
                    for exp in explicitos:
                        ...
                        if not (isinstance(col.type, DATE) or
                            ...
                            if operador in ['=', '<>']:
                                ...
                                # "Fecha inicio" = '01/01/2010'
                                expresion = '"%s" %s \'%s\'' % \
                                    (c.encode('utf-8'), operador.encode('utf-8'), exp.encode('utf-8'))
                    ...
                    if item.strip():
                        ...
                        for col_name, tipo in zip(self.cols, self.types):
                            ...
                            # rest of types
                                ...
                                if col_name != 'id' and not col_name.startswith('id_') and \
                                        not col_name.startswith('_') and \
                                        not is_date and \
                                        not is_time:
                                    try:
                                        args_ = (col_name, col_name, self.sin_acentos(item.encode('utf-8')),)
                                        texto  = "(CAST(\"%s\" as Text) <> '' AND " \
                                                 "UPPER(CAST(\"%s\" as Text)) SIMILAR TO UPPER('%%%s%%'))" % args_
                    ...
                    if item.strip():
                        ...
                        for col_name in self.cols:
                            if col_name != 'id' and not col_name.startswith('id_') and not col_name.startswith('_'):
                                try:
                                    texto = "(CAST(\"%s\" as Text) <> '' AND " \
                                            "UPPER(CAST(\"%s\" as Text)) LIKE UPPER('%%%s%%'))" % \
