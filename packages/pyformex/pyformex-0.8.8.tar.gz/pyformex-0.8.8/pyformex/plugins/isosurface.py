# $Id: isosurface.py 2539 2012-11-04 17:09:05Z bverheg $
##
...
def polygoniseCube(pos,val,iso):
    ...
    #print "Cubeindex = %s, val = %s" % (cubeindex,edgetable[cubeindex])
    ...
    for vert,andv,corners in vert_table:
        if edgetable[cubeindex] & andv:
            p1,p2 = pos[corners]
            val1,val2 = val[corners]
            vertlist[vert] = vertexinterp(iso,p1,p2,val1,val2)
...
def vertexinterp(iso,p1,p2,val1,val2):
    ...
    iso is the level value of the surface
    """
    if abs(iso-val1) < 0.00001:
        return p1
    if abs(iso-val2) < 0.00001:
        return p2
