...
if __name__ == "__main__":
    ...
    logging.info("broker_url '%s' " % sys.argv[1])
    queue_name = sys.argv[2] if len(sys.argv) > 2 else 'celery'
    logging.info("queue_name '%s' " % sys.argv[2])
    broker_api_url = 'http://guest:guest@localhost:55672/api/' #http://localhost:55672/#/

    broker = Broker(broker_url, broker_api_url=broker_api_url)
    #print(broker.queues([queue_name]))
    logging.info(broker.queues([queue_name]))
