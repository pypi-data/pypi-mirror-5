...
class TestRun(unittest.TestCase):
    ...
    def test_run_valid(self):
        ...
        TEST_REPO.request_build('test', 'C', 'M', 'Q', TEST_REPO.heads[0], branch='master')
        self.assertTrue(run(Config(path=os.path.join(FILES, 'builder.cfg'), envs=['M'])))
        self.assertIsNone(TEST_REPO.get_build('M', 'Q'))
        self.assertTrue(run(Config(path=os.path.join(FILES, 'builder.cfg'), envs=['M'])))

    def test_run_valid_test(self):
        """Verify a builder can be run in test mode."""
        self.assertTrue(run(Config(path=os.path.join(FILES, 'builder.cfg')), test=True))

    def test_run_valid_test_step(self):
        """Verify a builder step can be run in test mode."""
        self.assertTrue(run(Config(path=os.path.join(FILES, 'builder.cfg')), test='U'))
