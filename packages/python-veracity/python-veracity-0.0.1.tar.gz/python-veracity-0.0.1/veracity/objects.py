...
def requires_build(func):
    """Decorator for methods that require an initialized build configuration.
    """
    def wrapped(self, *args, **kwargs):
        """Wrapped method to show a better error message.
        """
        try:
            return func(self, *args, **kwargs)
            ...
            if "is null" in msg:
                raise VeracityScriptException("build configuration not defined ({0})".format(msg))
            else:
                raise
...
class Repository(_Base):
    ...
    def pull(self, remote=None):
        ...
        if remote:
            logging.debug("pulling from repo '{r}'...".format(r=remote))
            vv.run('pull', remote, dest=self.name)
        else:
            try:
                logging.debug("pulling from default...")
    ...
    def push(self, remote=None):
        ...
        remote = remote or self.remote
        if remote:
            logging.debug("pushing to repo '{r}'...".format(r=remote))
            vv.run('push', remote, src=self.name)
        else:
            try:
                logging.debug("pushing to default...")
...
class BuildRequest(_BaseRepositoryObject):
    ...
    def __repr__(self):
        return self._repr(self.bid, self.cid, name=self.name,
                          series=self.series, environment=self.environment,
                          status=self.status, branch=self.branch,
                          repo=self.repo)

    def __str__(self):
        return "'{b}' of '{c}'".format(b=self.bid[:10], c=self.cid[:10])
