...
SCRIPTS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'files')
...
def run(*args, **kwargs):
    ...
    try:
        ...
        msg = "\n\nIN: {0}\n\nRAN: {1}\n\nOUTPUT:{2}\n".format(os.getcwd(), erc.full_cmd,
                                                               erc.stdout.strip() or erc.stderr.strip())
    ...
    if style == 'builds':
        ...
        if 'builds' in data:
            for record in data['builds']:
                yield record['recid'], record['csid'], record['name'], record['series'], record['environment']
        elif 'build' in data:
            record = data['build']
            yield record['recid'], record['csid'], record['name'], record['series'], record['environment']
        else:
