...
class Builder(_BaseRepositoryObject):
    ...
    def __init__(self, repo, config):
        ...
        """
        super(Builder, self).__init__(repo=repo, create=False)
        self.config = config
        self.count = 0
    ...
    def run(self):
        ...
        for environment in self.config.environments:
            for status in self.config.starts:
                ...
                self.count += 1
                with self.repo.checkout(tempfile.mkdtemp(), rev=request.changeset.rev) as work:
    ...
    def create_virtualenv(self):
        ...
        if self.config.virtualenv:
            ...
            command = "virtualenv {0}".format(self.config.virtualenv)
    ...
    def delete_virtualenv(self):
        ...
        if self.config.virtualenv:
            ...
            shutil.rmtree(self.config.virtualenv)
    ...
    def call(self, path, command):
        ...
        if command:
            ...
            if self.config.virtualenv:  # pragma: no cover - OS determines coverage
                if os.name == 'nt':
                    command = "{v}\\Scripts\\activate && {c}".format(v=self.config.virtualenv, c=command)
                else:
                    command = ". {v}/bin/activate && {c}".format(v=self.config.virtualenv, c=command)
            logging.info("$ {0}".format(command))
    ...
    while True:
        for builder in builders:
            if isinstance(test, basestring):
                ...
                logging.info("running {b}...".format(b=builder))
                builder.run()
