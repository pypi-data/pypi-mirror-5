...
class Poller(_BaseRepositoryObject):
    ...
    def __init__(self, repo, config):
        ...
        """
        super(Poller, self).__init__(repo=repo, create=False)
        self.config = config
        self.count = 0
            ...
            if self.call(command):
                ...
                for branch in branches:
                    for changeset in self.repo.get_heads(branch):
                        ...
                        request = rebuild
                        if not request:
                            logging.info("checking if '{c}' ({b}) requires build...".format(c=changeset.rev, b=branch))
                            for build in self.repo.builds:
                                if build.changeset.rev == changeset.rev:
                                    logging.info("already built: {0}".format(changeset.rev))
                                    break
                            else:
                                request = True
                        if request:
                            self.count += 1
                            logging.info("requesting builds of '{0}'...".format(changeset.rev))
    ...
    def test(self):
        ...
        for series in self.config.series:
            ...
            branches, command, rebuild, _start, sleep = self.config.get_poll(series, all_branches=self.repo.branches)
            now = time.time()
    ...
    def name(total, now, series, environment, changeset, branch):  # pylint: disable=R0913
        ...
        name = "{n} {c}".format(t=total, n=timestamp, s=series, e=environment, c=changeset, b=branch)
    ...
    while True:
        for poller in pollers:
            if test:
                ...
                logging.info("running {p}...".format(p=poller))
                poller.run()
