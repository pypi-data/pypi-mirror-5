...
class Config(object):
    ...
    @property
    def exits(self):
        """Get a list of exit and fail state aliases.
        """
        aliases = []
        for data in self.config[BUILDER].itervalues():
            for setting, value in data.iteritems():
    ...
    @staticmethod
    def _get_command(data):  # pragma: no cover - different coverage by OS
...
def parse_config(path, error, walk=False, repos=None, series=None, envs=None):
    ...
    path = path or find_config(os.getcwd(), walk=walk)
    if path:
...
def find_config(cwd, walk=False):
    ...
    @return: path or None
    """
    for root, _dir, _filename in os.walk(cwd):
        logging.debug(root)
