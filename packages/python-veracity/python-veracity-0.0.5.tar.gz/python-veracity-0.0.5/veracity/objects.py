...
class Repository(_Base):
    ...
    def request_build(self, name, series, environment, status, cid, branch=None):
        ...
        bid, cid, _name, _series, _environment = next(vscript.parse(vscript.report_build(self, 'new', *args),
                                                                    style='builds'))
    ...
    def get_build(self, environment, status, order=None):
        ...
        args = self, 'get-pending-build', environment, order or BuildRequest.OLDEST, status
        try:
    ...
    def get_builds(self, limit=settings.MAX_BUILDS):
        ...
        if limit:
            args = self, 'list', 'null', 'null', limit
        else:
            args = self, 'list'
