...
from veracity.test import TEST_REPO, REASON, log


def setUpModule():  # pylint: disable=C0103
    """Create a temporary, randomly-named repository for this module's tests.
    """
    global TEMP_REPO  # pylint: disable=W0601
    if TEST_REPO:
        TEMP_REPO = TEST_REPO.clone(TEST_REPO.name + '-' + str(randrange(999)), autosync=False)
        TEMP_REPO.user = 'testuser'
    else:
        TEMP_REPO = None
