...
class GvGen:
    ...
    #
    def propertiesStringList(self, propertiesDict):
        stringlist = []
        for key, value in propertiesDict.iteritems():
            if value.startswith("<") and value.endswith(">"):
                # html-like label
                stringlist.append("%s=%s" % (key, value))
            else:
                stringlist.append('%s="%s"' % (key, value))
    ...
    def propertiesAsStringGet(self, node, props):
        ...
        if self.__has_children(node):
            propStringList = self.propertiesStringList(allProps)
            propStringList = [v + '\n;' for v in propStringList]
    ...
    def propertiesLinkAsStringGet(self, link):
        ...
        properties = props and ','.join(self.propertiesStringList(props)) or ""
        return properties
