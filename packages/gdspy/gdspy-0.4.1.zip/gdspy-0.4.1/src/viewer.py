...
class LayoutViewer(Tkinter.Frame):				
	...
	def _update_canvas(self, *args):
		...
		for i in layers:
			...
			if i in lbl_dict:
				label = lbl_dict[i]
				self.canvas.create_text(label.position[0] / self.scale, label.position[1] / -self.scale, text=label.text, anchor=[Tkinter.NW, Tkinter.N, Tkinter.NE, None, Tkinter.W, Tkinter.CENTER, Tkinter.E, None, Tkinter.SW, Tkinter.S, Tkinter.SE][label.anchor], fill=self.outlines[i], activefill='#FFF', tag=('L' + str(i), 'TEXT'))
