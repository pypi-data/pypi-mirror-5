...
template')
    ...
    def establish_connection(url, ip_address=None):
        ...
        if ip_address:
            ...
            headers = dict([('Host', parsed_uri.netloc)] + STD_HEADERS.items())
        else:
            uri = url
            headers = STD_HEADERS
    ...
    def process_download_other(self, data, meta_file_name):
        ...
        while True:
            ...
            if not self.time_to_get_first_byte:
                ...
                self.time_to_get_first_byte = first_byte_time - start
            if (not self.encoding_rate
                and tries <= config_pytomo.MAX_NB_TRIES_ENCODING):
                self.compute_encoding_rate(meta_file_name)
                tries += 1
            ...
            if not self.data_duration:
                try:
                    self.data_duration = get_data_duration(meta_file)
                except ParseError, mes:
