...
ifdef(/*pyx*/,/*    */cdef public object value)
    ...
    ifdef(/*pyx*/,cp)def check_tree_invariant(self):
        ...
        if self.right is not None:
            assert self.key ifdef(/*m4uniq*/,<)ifdef(/*m4dup*/,<=) self.right.key
...
# Note that we only do predecessor and successor for treaps, not for duptreaps
ifdef(/*m4uniq*/,
