...
from tg import expose, flash, require, url, lurl, request, redirect, validate, tmpl_context, config, abort
...
class RootController(TGController):
    ...
    def attach(self, **kw):
        ...
        hooks = config['hooks'].get('smallpress.on_attachment_upload', [])
        for func in hooks:
            func(article, attachment)
    ...
    def rmattachment(self, attachment):
        ...
        hooks = config['hooks'].get('smallpress.on_attachment_remove', [])
        for func in hooks:
            func(article, attachment)
    ...
    def publish(self, article):
        ...
        hooks = config['hooks'].get('smallpress.on_article_publishing', [])
        for func in hooks:
            func(article, True)
    ...
    def delete(self, article):
        hooks = config['hooks'].get('smallpress.on_article_delete', [])
        for func in hooks:
            func(article)
