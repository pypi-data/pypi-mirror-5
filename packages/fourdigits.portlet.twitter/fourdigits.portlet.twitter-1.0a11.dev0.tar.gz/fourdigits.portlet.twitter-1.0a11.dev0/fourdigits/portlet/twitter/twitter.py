...
__version__ = '1.0'
'''With modifications to get a proxy address for a plone product'''
'''fabiosurrage@gmail.com'''
...
from fourdigits.portlet.twitter.controlpanel.interfaces import IFourdigitPortletTwitterSetting
try:
  # Python >= 2.6
...
class Api(object):
  ...
  def _calculate_status_length(cls, status, linksize=19):
    ...
    for x in status.split(' '):
      if not(x.startswith('http://') or x.startswith('https://')):
        ...
        links.append(dummy_link_replacement)
      pass
    shortened = ' '.join(links)
  ...
  def _FetchUrl(self,
    ...
    if registry is None:
      ...
      settings = registry.forInterface(IFourdigitPortletTwitterSetting, check=False)
      if settings.proxyInfo:
        proxy_address = settings.proxyInfo[0]
      else:
        proxy_address = None
    ...
    if proxy_address:
      proxy = urllib2.ProxyHandler({"https" : proxy_address, "http" : proxy_address})
...
class _FileCache(object):
  ...
  def _GetTmpCachePath(self):
    # Use PID instead of username
    # username = self._GetUsername()
    # cache_directory = 'python.cache_' + username
    cache_directory = 'python.cache_' + str(os.getpid())
