...
SETUP_INFO = dict(
    ...
    install_requires = ['infi.execute', 'lxml', 'infi.registry', 'infi.recipe.console_scripts', 'distribute', 'infi.pypi_manager', 'zc.buildout', 'infi.pyutils', 'git-py', 'Archive'],
    namespace_packages = ['infi', 'infi.recipe'],

    package_dir = {'': 'src'},
    package_data = {'': ['postinst.in', 'signtool.exe', 'distribute_setup.txt', 'control.in', 'changelog.in', 'rules.in', 'prerm.in', 'template.wxs', 'silent_launcher-x86.exe', 'rpmspec.in', 'md5sums.in', 'silent_launcher-x64.exe']},
