...
struct myFex{
	__device__ void operator()(int *neq, double *ts, double *y, double *dy/*, void *otherData*/) {
        ...
        __shared__ double a;
        __shared__ float k;
        __shared__ double tdot;
        __shared__ double ha;
        __shared__ double eta;

        if(threadIdx.x==0){
			double t = ts[0];

			a = exp(t);
			k = tex2D(args_tex,1, tid);
			...
			tdot = -0.0692 / pow(a, 2) * xe * omega_b * h / rh;
        ...
        int work = (lmax-1-3)/blockDim.x;

//        if(tid==0)
//        	printf("work: %i\n", work);

//		for (l = 3; l < lmax-1; l++) {
        l = 3+threadIdx.x*work;
        int max = l+work;
		for (; l < max; l++) {
			//printf("tid: %i l: %i\n", tid, l);
...
//			printf("theta lmax computed\n");
//	        memcpy(g_y, y, sizeof(double)*neq[0]);
//	        memcpy(g_dy, dy, sizeof(double)*neq[0]);
//	        printf("data copied\n");
		}
...
struct myJex
{
	__device__ void operator()(int *neq, double *ts, double *y, int ml, int mu, double *dy, int nrowpd/*, void *otherData*/)
	{
		...
		double a = exp(t);
		//float k = tex2D(ks_tex,1, tid);
		float k = tex2D(args_tex,1, blockIdx.x);

//		double h = params[0];
//		double rh = params[2];
//		double omega_gam = params[4];
//		double omega_r = params[6];
//		double omega_b = params[7];
//		double omega_dm = params[8];
