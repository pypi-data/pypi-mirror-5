...
NOTE:
    ...
    Now that optparse is deprecated as of Python 2.7,
    the next major enhopa version will consider this bazaar closed
    and apply its enhancements directly to the last optparse
    version.
"""
__version__ = (0,
...
def check_xchoice(option, opt, value):
    try:
        ...
        choices = ", ".join(map(repr, option.choices))
        raise OptionValueError(
            _("option %(option)s: invalid choice %(value)r"
              " (choose from %(choices)s)"
              ) % locals())
...
class EnhancedHelpFormatter(optparse.IndentedHelpFormatter):
    ...
    Untestet: help text keys %choices and %xchoices
    """
    def __init__(self,
                 indent_increment=2,
