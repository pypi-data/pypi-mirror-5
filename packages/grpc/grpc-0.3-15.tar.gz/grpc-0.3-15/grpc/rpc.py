...
from gevent import GreenletExit, spawn, Timeout, sleep
    ...
    def call(self, obj_id, name, args, kw, no_result=False,
        ...
        if proxy:
            ...
            if isinstance(objs, (tuple, list)):
                obj_ids = []
                for o in objs:
                    obj, is_pickle = _reg_obj(o)
                    pickle = pickle or is_pickle
                    obj_ids.append(obj)
            else:
                obj, is_pickle = _reg_obj(objs)
                pickle = pickle or is_pickle
                obj_ids = obj
            args = (obj_ids, ) + args[1:]
            dtype |= DT_PROXY
    ...
    def _handle_request(self, parts):
        ...
        try:
            ...
            if dtype & DT_PICKLE:
                ...
                args, kw = loads(argkw)
            if dtype & DT_PROXY:
                def _get_proxy(export):
                    if isinstance(export, RpcProxy):
                        return export
                    if isinstance(export, (tuple, list)):
                        export_cls = PROXYS[export[0]]
                        return export_cls.proxy_unpack(export, svc=self)
                    return self.get_proxy(export)
                export_ids = args[0]
                if isinstance(export_ids, (tuple, list)):
                    proxys = []
                    for e in export_ids:
                        proxys.append(_get_proxy(e))
                else:
                    proxys = _get_proxy(export_ids)
