    ...
    def get_jids(self):
        ...
        for addr in addrs:
            try:
                ...
                if warn_bad:
                    logger.warning("Bad JID in the certificate: {0!r}: {1}"
                                                            .format(addr, err))
        return result
...
class BasicCertificateData(CertificateData):
    ...
    def _decode_names(self):
        ...
        for key, old in list(self.alt_names.items()):
            new = []
            for name in old:
                try:
                    name = name.decode("utf-8")
