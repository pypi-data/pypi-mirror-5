...
class RosterTool(EventHandler, XMPPFeatureHandler):
    ...
    def print_item(self, item):
        print("    JID: {0}".format(item.jid))
        if item.name is not None:
            print("    Name: {0}".format(item.name))
        print("    Subscription: {0}".format(item.subscription))
        if item.ask:
            print("    Pending {0}".format(item.ask))
        if item.approved:
            print("    Approved")
        if item.groups:
            groups = ",".join(
                            ["'{0}'".format(g) for g in item.groups])
        ...
        if not presences:
            ...
            print("    ONLINE:")
            for jid, presence in list(presences.items()):
                if presence.show:
                    show = ": [{0}]".format(presence.show)
                elif not presence.status:
                    show = ""
                else:
                    show = ":"
                if presence.status:
                    status = " '{0}'".format(presence.status)
                else:
                    status = ""
                print("      /{0}{1}{2}".format(jid.resource, show, status))

    @event_handler(RosterReceivedEvent)
    def handle_roster_received(self, event):
        if self.args.action == "show":
            if self.args.presence:
                logging.info("Waiting for incoming presence information...")
    ...
    def print_roster(self):
        ...
        if roster.version is None:
            ...
            print("  Version: '{0}'".format(roster.version))
        if len(roster):
            print("  Items:")
            for item in list(roster.values()):
    ...
    def handle_disconnected(self, event):
        if self.client.roster and self.args.roster_cache:
            logging.info("Saving roster to {0!r}"
                                            .format(self.args.roster_cache))
    ...
    def handle_available_presence(self, stanza):
        ...
        if stanza.show:
            show = ": [{0}]".format(stanza.show)
        elif not stanza.status:
            show = ""
        else:
            show = ":"
        if stanza.status:
            status = " '{0}'".format(stanza.status)
        else:
            status = ""
        print("  {0} is now ONLINE{1}{2}".format(jid, show, status))
        print()
    ...
    def handle_unavailable_presence(self, stanza):
        ...
        if stanza.status:
            ...
            status = ""
        print("  {0} is now OFFLINE{1}".format(jid, status))
        print()
