...
def load_nc(file,var,dt,date1):
	...
	n = len(data)
        ...
        f.close()
        return data,var,n,time



def load_txt(archive,var,dt,date1):
	"""Archive .dat and .txt  without date 
  	...
  	"""
        ...
        time = np.arange(n)*dt + date1  
	return data,var,n,time
#data,var,n,time = load_txt('sst_nino3.dat','SST_NINO3',0.25,1871)
...
def cwt (data, dt, variance, n, pad, dj,s0,j1,lag1,param,mother): 
	...
	mean_wavelet = np.ones(nw)*mean_wavelet
        result = {'ondaleta':ondaleta,'wave':wave ,'period': period,'scale':scale ,'coi':coi,'power': power,'sig95':sig95, 'global_ws':global_ws,
			'global_signif':global_signif,'joint_wavelet':joint_wavelet,'imag_wavelet':imag_wavelet,'nw':nw ,'mean_wavelet':mean_wavelet ,'dj':dj, 'j1':j1, 'dt':dt}
	return result
...
def wavelet_plot(var,time,data,dtmin,result):
	...
	subplot(3,2,5)
	lev = wavetest.levels(result)									# call levels - power levels
	...
	pylab.show()
        return
#wavelet_plot('SST_NINO3',time,data,0.03125,result)
