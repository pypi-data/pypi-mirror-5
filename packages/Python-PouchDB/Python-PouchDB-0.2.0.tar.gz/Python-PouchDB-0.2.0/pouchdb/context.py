...
class JSContext(object):
	...
	def inspect(self):#pragma: no cover
		...
		self._i = QtWebKit.QWebInspector()
	...
	def _reload(self):
		...
		self._page.mainFrame().setHtml(html, QtCore.QUrl(self._baseUrl))
		...
		while not getattr(callback, "_called", False):
			...
			for error in self._page.getMessagesAndReset():
				for ignoredPattern in self._ignoredErrors:
					if ignoredPattern not in error:
						errors.append(error)
			if errors:
