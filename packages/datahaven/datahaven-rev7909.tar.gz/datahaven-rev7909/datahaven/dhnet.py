...
MAXIMUM_SENDING_STREAMS_NUMBER = 3
...
class Application:
    ...
    def onAction(self, contactName):
        ...
        def onConnectUser( err, sock ) :
            ...
            global _InActionFilesDict 
            if not _InActionFilesDict.has_key(keyid):
                notify().sent(keyid, filename, transferID, 'failed', None, 'not found outgoing queue to that user')
                return
            if len(_InActionFilesDict[keyid]) == 0:
                notify().sent(keyid, filename, transferID, 'failed', None, 'no files to that user found in the outgoing queue')
                return
...
#        self.files2send.append((filename, transferID, description))
#
#    def doWaitNewFiles(self):
#        if len(self.files2send) == 0:
#            self.currentOp = self.reactor.callLater(0.1, self.doWaitNewFiles)
#            return
#        self.filename, self.transferID, self.description = self.files2send.pop(0)
    ...
    def doSend(self):
        ...
        self.currentOp = self.reactor.callLater(0.001, self.doSend)
...
#        print 'FileSender.connectionError', errMsg
    ...
    def smart_close_stream(self):
        global _SendingStreams
        global OPENED_SENDING_STREAMS_NUMBER
        if len(_SendingStreams.get(self.peer, [])) > OPENED_SENDING_STREAMS_NUMBER:
            self.close_stream()
...
class FileReceiver(object):
    ...
    def dataReceived(self, data):
        ...
        if self.bytesReceived == self.length:
            ...
            # self.close_other_receiving_streams()
    ...
    def close_other_receiving_streams(self):
        global _ReceivingStreams
        global OPENED_RECEIVING_STREAMS_NUMBER
        if len(_ReceivingStreams.get(self.peer, [])) > OPENED_RECEIVING_STREAMS_NUMBER:
            self.close_stream()
