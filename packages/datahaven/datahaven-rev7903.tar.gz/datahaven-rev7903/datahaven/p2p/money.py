...
def SetGuiObject(obj):
    global GuiObject
    GuiObject = obj

def SetWebObject(obj):
    global WebObject
    WebObject = obj
...
def Receipt(tup):
    ...
    if GuiObject is None:
        return
    SaveReceipt(tup)
    GuiObject.updateReceipts()

def Balance(amount):
    global GuiObject
    if GuiObject is None:
        return
    GuiObject.updateBalance(amount)

def UpdateCorrespondents():
    global GuiObject
    if GuiObject is None:
        return
    GuiObject.updateIdList()
...
def InboxReceipt(newpacket):
    ...
    if WebObject is not None:
        WebObject.UpdateReceipts()

    if GuiObject is None:
        return
    GuiObject.updateReceipts()
    GuiObject.updateBalance()
