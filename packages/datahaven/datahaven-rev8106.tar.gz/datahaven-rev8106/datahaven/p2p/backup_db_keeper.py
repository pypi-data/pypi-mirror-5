...
class BackupDBKeeper(Automat):
    ...
    def SupplierResponse(self, packet):
        ...
        dhnio.Dprint(6, 'backup_db_keeper.SupplierResponse %s, %d more suppliers' % (packet.OwnerID, len(self.requestedSuppliers)))

    def SupplierAcked(self, packet):
        self.sentSuppliers.discard(packet.OwnerID)
        dhnio.Dprint(6, 'backup_db_keeper.SupplierAcked %s, %d more suppliers' % (packet.OwnerID, len(self.sentSuppliers)))
