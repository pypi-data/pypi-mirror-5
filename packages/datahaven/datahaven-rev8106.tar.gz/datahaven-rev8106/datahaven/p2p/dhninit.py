...
def settings_patch():
    ...
    # TODO - let's remove this things in future
    
    # I want to rename backup_info.xml to backup_db
    if os.path.exists(settings.BackupInfoFileFullPathOld()):
        if os.path.exists(settings.BackupInfoFileFullPath()):
            if os.access(settings.BackupInfoFileFullPathOld(), os.W_OK):
                try:
                    os.remove(settings.BackupInfoFileFullPathOld())
                except:
                    dhnio.DprintException()
        else:
            if os.access(settings.BackupInfoFileFullPath(), os.W_OK):
                try:
                    os.rename(settings.BackupInfoFileFullPathOld(), settings.BackupInfoFileFullPath())
                except:
                    dhnio.DprintException()
                    
    # want to enable cspace transport for everybody
