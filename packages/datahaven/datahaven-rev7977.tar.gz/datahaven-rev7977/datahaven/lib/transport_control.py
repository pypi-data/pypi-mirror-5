...
def outbox(outpacket, doAck, wide=False, retries=settings.MaxRetries()):
    ...
    dhnio.Dprint(12, "transport_control.outbox [%s] to %s doAck=%s wide=%s protos=%s" % (
        outpacket.Command, nameurl.GetName(outpacket.RemoteID), str(doAck), str(wide), str(_PeersProtos.get(destinationID, set()))))
...
def sendStatusReport(host_, filename, status, proto='', error=None, message=''):
    ...
    for workitem in _SendQueue:
        if filename == workitem.filename:
            # let's remember that this proto is working to this guy
    ...
    if workitem.wide:
        ...
        if tcp_contact and localIP:
            ...
            if host.strip():
                ...
                workitem_sent = True
        # we prefer cspace against udp, check if we did not sent the packet yet via tcp
        if not workitem_sent:
            if 'cspace' in _SupportedProtocols and cspace_contact and 'cspace' in working_protos:
                protocol, host, port, filename2 = nameurl.UrlParse(cspace_contact)
                if host.strip():
                    SendWorkItem(workitem, protocol, host, port)
                    workitem_sent = True
        # now check udp after cspace and tcp
        if not workitem_sent:
            if 'udp' in _SupportedProtocols and udp_contact and 'udp' in working_protos:
        ...
        if not workitem_sent: 
            for contactmethod in destinationIdentity.getContacts():
                ...
                if host.strip():
                    # we always support tcp
                    if protocol in _SupportedProtocols or protocol == 'tcp':
                        # no need, it seems we already catch this above, let's try to send to his world IP 
                        # if protocol == 'tcp' and localIP:
                        #     host = localIP
                        SendWorkItem(workitem, protocol, host, port)
                        workitem_sent = True
