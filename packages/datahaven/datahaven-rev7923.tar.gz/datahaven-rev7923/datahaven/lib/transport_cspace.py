...
def send(filename, keyid, description=''):
    ...
    transferID = registerTransfer('send', keyid, filename, size, getfilesize(filename), description)
...
def callback(command, data):
    ...
    if command == 'sent':
        ...
        try:
            ...
            dhnio.DprintException()
            return
        transferID = registerTransfer('receive', keyid, filename, -1, getfilesize(filename), '')
