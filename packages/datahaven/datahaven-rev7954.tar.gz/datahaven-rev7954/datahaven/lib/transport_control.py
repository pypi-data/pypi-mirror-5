...
def inbox(filename, proto='', host=''):
    ...
    dhnio.Dprint(8, "transport_control.inbox [%s] from %s" % (Command, nameurl.GetName(OwnerID)))
...
def SendFail(request, response=''):
    ...
    result = dhnpacket.dhnpacket(commands.Fail(), request.OwnerID, misc.getLocalID(), request.PacketID, response, request.CreatorID)
    outboxNoAck(result)
...
def sendStatusReport(host_, filename, status, proto='', error=None, message=''):
    ...
    for workitem in _SendQueue:
        if filename == workitem.filename:
            ...
            # if not _SendingStats.has_key(workitem.remoteid):
                # _SendingStats[workitem.remoteid] = [0, 0]

            if status is 'finished':
                _SentFailedCountDict[workitem.remoteid] = 0
                # _SendingStats[workitem.remoteid][0] += 1
                dhnnet.ConnectionDone(filename, proto, 'sendStatusReport %s' % host)
            else:
                _SentFailedCountDict[workitem.remoteid] += 1
                # _SendingStats[workitem.remoteid][1] += 1
...
def SendQueueRemove(workitem, check_counts=False, why='removing from send queue'):
    global _SendQueue
    dhnio.Dprint(14, 'transport_control.SendQueueRemove %s to [%s] because %s' % (os.path.basename(workitem.filename), nameurl.GetName(workitem.remoteid), why))
...
def DoSend():
    ...
    # here we decide which method to use
    workitem_sent = False
    # we always support tcp
    # now check if we know his local IP and he supports tcp too - than use it and send to his local IP
    # but if we want to send this packet to all his contacts - skip and check all contact methods
    localIP = identitycache.GetLocalIP(workitem.remoteid)
    tcp_contact = destinationIdentity.getProtoContact('tcp')
    if not workitem.wide and tcp_contact and localIP:
        ...
        for contactmethod in destinationIdentity.getContacts():
            ...
            if host.strip():
                if protocol in _SupportedProtocols or protocol == 'tcp':
                    ...
                    workitem_sent = True
                    # sometimes we want to send the file to all contact methods - so check this here
...
def CancelFile(transferID):
    ...
    if not fti:
        dhnio.Dprint(2, "transport_control.CancelFile ERROR can not find transfer %s" % transferID )
        return 
...
def register_transfer(proto, send_or_receive, remote_address, callback, filename, size, description):
    ...
    dhnio.Dprint(14, '######  transport_control.register_transfer %d [%s %s %s %s]' % (
...
def unregister_transfer(transfer_id):
    ...
    dhnio.Dprint(14, '******  transport_control.unregister_transfer %d [%s %s %s %s]' % (
