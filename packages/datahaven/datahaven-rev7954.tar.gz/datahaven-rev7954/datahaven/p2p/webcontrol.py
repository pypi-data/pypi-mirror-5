...
class InstallPage(Page):
    ...
    def renderInputNamePage(self, request):
        ...
        src += 'If your computer is fast enough or you need extra secure storage of your data - select 2048.</p>\n'
        src += '</td></tr>\n<tr><td align=center>\n'
...
#        src += '</td>\n'
    ...
    def renderContactsPage(self, request):
        ...
        src += '<br><input type="checkbox" name="betatester" value="%s" %s />\n' % (
            'True', ('checked' if self.betatester=='True' else ''))
...
#                    blocks, percent = backups.GetBackupBlocksAndPercent(backupID)
    ...
    def _list(self, request):
        ...
        for backupDir, backupRuns, totalBackupsSizeForDir, is_running, recentBackupID in backup_db.GetBackupsByFolder():
            ...
            delete_folder_link = '%s?urlyes=%s&urlno=%s&text=%s&back=%s' % (
                '/'+_PAGE_CONFIRM, 
                misc.pack_url_param('%s?action=delete&backupdir=%s&back=%s' % (request.path, misc.pack_url_param(backupDir), request.path)),
                misc.pack_url_param(request.path),
                misc.pack_url_param(msg),
                request.path)
            src += '<a href="%s">' % delete_folder_link
            src += '<img src="%s" width=%d height=%d />' % (
                iconurl(request, 'icons/delete02.png'), 24, 24 )
            src += '</a>'
            src += '</div>'
            ...
            if len(backupRuns) > 0:
                ...
                for backupRun in backupRuns:
                    ...
                    src += '<td nowrap align=left>&nbsp;\n'
                    msg = 'Do you really want to delete backup <br>%s<br> of the folder <br> %s.' % (backupID, backupDir)
                    delete_backup_link = '%s?urlyes=%s&urlno=%s&text=%s&back=%s' % (
                        '/'+_PAGE_CONFIRM, 
                        misc.pack_url_param('%s?action=deleteid&backupid=%s&back=%s' % (request.path, backupID, request.path)),
                        misc.pack_url_param(request.path),
                        misc.pack_url_param(msg),
                        request.path)
                    src += '<a href="%s">' % delete_backup_link
                    # src += '<a href="%s?action=deleteid&backupid=%s&back=%s">' % (
                    #     request.path, backupID, request.path)
                    src += '<img src="%s" width=%d height=%d />' % (
...
class CustomerPage(Page):
    ...
    def renderPage(self, request):
        ...
        bytesGiven = int(float(spaceDict.get(idurl, 0)) * 1024 * 1024)
