...
def init(baseDir=None, onlineCallback=None, offlineCallback=None):
    global _Controller
    global _BaseDir
...
def run(params=[], process_finished_callback=None):
    ...
    if dhnio.isFrozen() and dhnio.Windows():
        ...
        cmdargs = [sys.executable, progpath]
        cmdargs.extend(params)
...
def send(filename, keyid, description=''):
    ...
    if not os.access(filename, os.R_OK):
        ...
        return 
    if A().state is 'ONLINE':
        size = os.path.getsize(filename)
        transferID = registerTransfer('send', keyID, filename, size, zero, description)
...
def callback(command, data):
    ...
    if command == 'sent':
        try:
            ...
            dhnio.Dprint('transport_cspace.callback ERROR data=%s' % str(data))
            dhnio.DprintException()
            return
       ...
       transferID = registerTransfer('receive', keyID, filename, -1, zero, '')
...
class TransportCspace(automat.Automat):
    ...
    def doExecute(self, arg):
        ...
        kill()
        if run(params=['--logfile', '"%s"' % settings.CSpaceLogFilename(), 
    ...
    def doRegister(self, arg):
        ...
        def _register(username, password):
            if run(process_finished_callback=_process_finished, 
...
#            dhnio.Dprint(6, "transport_cspace.list ERROR service not started")
    ...
    def send(self, keyID, filename, transferID, description):
        ...
        if server is not None:
            try:
                server.send(str(keyID), filename, transferID, description)
            except:
                dhnio.DprintException()
                return
        else:
...
def main():
    ...
    def initDone(state, options, args):
        ...
        if options.send:
            ...
            reactor.callLater(5, send, args[0], args[1])
            return
        if options.receive:
