...
def SendingSpeedLimit():
    return 1 * 1024 # 1KB/sec is about 3.5MB/hour
...
def SendTimeOut():
    ...
    #return 60                     # 60 seconds send timeout for ssh/tcp/q2q
    #return 30
...
def DefaultBackupBlockSize():
    ...
    #return 1024*99
    #return 500000
    #return (1024*1023) + 512 # :-)
    #return 1024 * 511 + 256 + 128 + 64 + 32 + 16 + 8 + 4 + 2
    #return 1024 * 256
    return 512 * 1024
...
def BackupDBSynchronizeDelay():
    # TEST
    return 5
    return 60 * 5 # save backup_db no more than one time per 5 min
...
def CurrentVersionURL(repo='test'):
    return UpdateLocationURL(repo) + 'version.txt'
...
def setBackupBlockSize(block_size):
    ...
    # uconfig().set('backup.backup-block-size', str(_BackupBlockSize))
    # uconfig().update()
     
# What size DHNpackets we want
# BlockSizeTarget is this times eccmap.nodes()
def PacketSizeTarget():          
    return getBackupBlockSize()
