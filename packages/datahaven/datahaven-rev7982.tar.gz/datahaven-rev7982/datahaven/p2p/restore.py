...
class restore(automat.Automat):
    timers = {'timer-01sec':    (0.1, ['RUN',  ]),
              'timer-1sec':     (1,   ['REQUEST',]),
              'timer-5sec':     (5,   ['REQUEST',]), } 
    ...
    def isBlockValid(self, NewBlock):
        if NewBlock is None:
            return False
        return NewBlock.Valid()
    
    def isLastBlock(self, NewBlock):
    ...
    def doReadRaid(self, arg):
        ...
        threads.deferToThread(
            raidread.raidread,
                ...
                self.BackupID, 
                self.BlockNumber, 
                settings.getLocalBackupsDir()).addBoth(
    ...
    def doSavePacket(self, NewPacket):
        ...
        dhnio.Dprint(6, "restore.doSavePacket saved as " + filename)
    ...
    def doRestoreBlock(self, filename):
        ...
        try:
            ...
            blockdata = ''
            newblock = None
        self.automat('block-restored', newblock)
    
    def doWriteRestoredData(self, NewBlock):
    ...
    def doReportAborted(self, arg):
        dhnio.Dprint(6, "restore.doReportAborted")
        self.Done = True
