...
class MySendingProtocol(protocol.Protocol):
    ...
    def transferFailed(self, err):
        global _SendStatusFunc
        dhnio.Dprint(6, 'transport_tcp.MySendingProtocol.transferFailed NETERROR host=%s file=%s error=%s' % (str(self.factory.host), str(self.factory.filename), str(error)))
        try:
            self.fin.close()
        except:
