...
This module has several goals:
  * Allow loading either PySide or PyQt4
...
class QtProxyImporter:
    ...
    def __init__(self):
        ...
        self._importing = None  # Package name of module being imported
    ...
    def find_module(self, fullname, path=None):
        ...
        # If we are in the progress of importing, we do not accept new imports
        if self._importing:
            return None
        
        # Even if an export is explicitly from PySide or PyQt4, we
        # handle it with this importer, so we can yield the system
        # module if appropriate. But the module will always be from
        # PySide or PyQt4 as requested.
        if basename in ('PySide', 'PyQt4'):
            if VERBOSE: print('find_module (%s): %s' % (self._importing, fullname))
            self._import_qt(basename)
            if (self._qtPackage and self._import_path and 
                                        basename == self._qtPackage.__name__):
                return self  # We deal with it
            else:
                return None  # import as normal
    ...
    def load_module(self, fullname):
        ...
        if os.path.isdir(qtdir):
            ...
            if VERBOSE: print('load_module normally: %s' % fullname)
    ...
    def _determine_preference(self):
        ...
        # Read qt.conf
        qt_conf = os.path.join(os.path.dirname(sys.executable), 'qt.conf')
    ...
    def _import_qt(self, toolkit=None):
        ...
        # Make qtPackage global and only proceed if its None
        if self._qtPackage is not None:
            return
        
        # Do not proceed if we are in the process of importing
        # This can happen due to an import in PySide/PyQt4 itself;
        # qtPackage is not set yet, but a new import is invoked
        if self._importing:
            return
