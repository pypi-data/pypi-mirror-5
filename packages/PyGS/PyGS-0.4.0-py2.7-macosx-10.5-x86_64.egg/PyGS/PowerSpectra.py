...
class PowerSpectra(object):
    ...
    def PowerSpec_1d_fast(self, quantity="redshift", bins=None, min_wave=0.0001, 
                          max_wave=0.02, pad=1, in_wavelength=True, weight=True):
        """
        ...
        if 'weight' not in self.survey_data.columns and weight:
        ...
        if weight:
            hist, edges = np.histogram(self.survey_data[quantity], bins, 
                                       weights=self.survey_data[weight])
        else:
            hist, edges = np.histogram(self.survey_data[quantity], bins)

        #Calculate bin centres for weighting
        centres = []
        for i, edge in enumerate(edges):
            if i == len(edges) - 1:
                break
            centres = centres + [(edges[i + 1] + edge) / 2]
        ...
        plt.ylabel("Power")
        if in_wavelength:
            plt.savefig(self._dirs['PowerSpecs_1D'] + quantity + '_bins' + str(bins) + '_pad' + str(pad) + '_wavelength.eps')
        else:
            plt.savefig(self._dirs['PowerSpecs_1D'] + quantity + '_bins' + str(bins) + '_pad' + str(pad) + '_frequency.eps')
        plt.show()

        return ps, freq * 2.0 * np.pi, wavelength

    def PowerSpec_1d_slow(self, quantity="redshift", bins=None, min_wave=0.0001, max_wave=0.02, n_waves=1000, weight=0):
        """
        Computes the 1D power spectrum and plots it.
        
        This method uses a slower DFT method, rather than an FFT. However, the bonus is that the increments in
        wavelength or regular and completely customizable (producing smooth plots). And also can do weights.
        ...
        if bins:
            ...
            if not weight:
                ...
                ps = dft.dft_one(np.asfortranarray(self.survey_data[weight]), 
                                 np.asfortranarray(self.survey_data[quantity]), 
                                 wavelengths)
        ...
        plt.ylabel("Power")
        plt.show()
        plt.savefig(self._dirs['PowerSpecs_1D'] + quantity + '_bins' + str(bins) + '_wavelength_smooth.eps')
