...
def main():
  ...
  parser.add_argument('--features-dir', metavar='STR', type=str,
      dest='features_dir', default='pca/features_projected', help='The subdirectory where the features are stored. It will overwrite the value in the configuration file if any. Default is the value in the configuration file.')
  parser.add_argument('--algorithm-dir', metavar='FILE', type=str,
      dest='algorithm_dir', default='lda_euclidean', help='The relative directory of the algorithm that will contain the models and the scores.')
  ...
  features_dir = os.path.join(args.output_dir, config.protocol, algorithm_dir, config.features_projected_dir)
