...
def download(username, password, directory, limit=None, force=False, delete_age=None, album_id=None):
    ...
    photos = []
    if album_id:
        photos = client.photos = client.GetFeed('/data/feed/api/user/%s/albumid/%s?kind=photo' % (username, album_id)).entry
    else:
        photos = client.GetUserFeed(user=username, kind="photo", limit=limit).entry
...
def run():
    ...
    parser.add_argument('--album', dest='album_id', help='A Picasa Album ID to download from. If not provided, all albums will be searched.')
    ...
    download(args.username, args.password, os.path.abspath(args.directory), args.limit, args.force, args.delete_age, args.album_id)
