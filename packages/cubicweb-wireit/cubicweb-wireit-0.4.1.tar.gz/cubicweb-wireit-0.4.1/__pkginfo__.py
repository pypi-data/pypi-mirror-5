...
for dname in ('jq_upload', 'wireit', 'wireit_patches'):
    for dirpath, dirname, fnames in walk(join('data', dname)):
        data_files.append([join(THIS_CUBE_DIR, dirpath),
                           [join(dirpath, fname) for fname in fnames]
                           ])
