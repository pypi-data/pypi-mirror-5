...
class MailSender:
    ...
    def add_attach(self,filePath):
        ...
        # gzip'd or compressed files.
            ...
            filecontent.add_header('Content-Disposition', 'attachment', filename=os.path.basename(filePath))
            self.msg.attach(filecontent)
        else:
            raise Exception("the attached file is not existed")
    
    def send_mail(self,mail_from,mail_to_list=[],is_with_tls=False):
        """ send mail """
        self.msg["From"] = mail_from
        #COMMASPACE = ', '
        for recipient in mail_to_list:
            self.msg.add_header('To',recipient)
            #self.msg['To']=COMMASPACE.join(mail_to_list)
        ...
        try:
            server.sendmail(self.msg['from'], mail_to_list, self.msg.as_string())
            server.close()
        except Exception, e:
