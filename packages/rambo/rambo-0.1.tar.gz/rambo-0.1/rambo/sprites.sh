...
temp_css_files=$(ls $TEMP_CSS_FILES 2> /dev/null | wc -l)
    ...
    if [ "$temp_css_files" -gt "0" ]
        ...
        rm -f $TEMP_CSS_FILES
            ...
            echo "\033[31mSorry, couldn't find any output CSS. Exiting.\033[0m"
            exit
    fi
}
        ...
        if [ -e "$_ief" ]; then
            ...
            if [ ${#files[@]} -gt 0 ]; 
                then 
                    ...
                    python rambo/pypacker.py -i "$_ief" -c "$sprite_css_output_dir" -a "$sprite_sass_output_dir" -s "$sprite_output_dir"  -m grow --test
                else
                    echo "\033[90mSkipping $_ief because it's empty. Put some .png images in the directory to make a sprite.\033[0m"
            fi
        else
            echo "\033[31mYou need images to make a sprite, fool.\033[0m"
        fi
    done
}
