...
def filter_fn(line):
    ...
    return tmp.startswith('Disjunct') or tmp.startswith('DEPRECATION') or tmp.startswith('DiffSet') or line.startswith('    ') or tmp.startswith("Differential") or tmp.startswith("DerivativeVar") or tmp.startswith("InputVar") or tmp.startswith('StateVar')
...
class Test(unittest.TestCase):
    ...
    def test5(self):
        ...
        self.assertFileEqualsBaseline(currdir+"test5.out", currdir+"test5.txt",filter=filter,tolerance=_diff_tol)
