...
class TestExpressionData(pyutilib.th.TestCase):
...
class TestExpression(pyutilib.th.TestCase):
    ...
    def test_init_abstract_indexed(self):
        model = AbstractModel()
        model.ec = Expression([1,2,3],initialize=1.0)
        model.obj = Objective(expr= lambda m: 1.0+summation(m.ec,index=[1,2,3]))
        inst = model.create()
...
e : Size=1, Index=None, Default=None
    Key  : Expression
    None : sum( 2.0 , x )
E : Size=2, Index=E_index, Default=None
    Key : Expression
      1 : sum( 1 , pow( x , 2.0 ) )
      2 : sum( 1 , pow( x , 2.0 ) )
"""
