...
from coopr.pyomo.transform import *
...
class Test(unittest.TestCase):
    ...
    def test_nonnegativity_transformation_1(self):
        ...
        self.model.a = Var()
        #
        # TODO: Use the commented definition of b once the domain bug in
        # Var objects related to RangeSet domains has been resolved.
        #
        #self.model.b = Var(within=self.model.A)
        self.model.b = Var()
        self.model.c = Var(within=NonNegativeIntegers)
        ...
        print("------------------ Instance -------------------")
        instance=self.model.create()
        #transformed = apply_transformation("nonnegative_transform", instance)
        ...
        # continuous
        print(transformed.a.domain)
        for ndx in transformed.a._index:
            self.assertTrue(isinstance(transformed.a[ndx].domain, RealSet))
        for ndx in transformed.b._index:
            #self.assertTrue(isinstance(transformed.b[ndx].domain, IntegerSet))
            self.assertTrue(isinstance(transformed.b[ndx].domain, RealSet))
        for ndx in transformed.c._index:
            self.assertTrue(isinstance(transformed.c[ndx].domain, IntegerSet))
        for ndx in transformed.d._index:
            self.assertTrue(isinstance(transformed.d[ndx].domain, IntegerSet))
        for ndx in transformed.e._index:
            self.assertTrue(isinstance(transformed.e[ndx].domain, BooleanSet))
        for ndx in transformed.f._index:
            self.assertTrue(isinstance(transformed.f[ndx].domain, BooleanSet))
    ...
    def test_nonnegative_transform_3(self):
        ...
        instance_sol = solver.solve(instance)
        transformed_sol = solver.solve(transformed)
