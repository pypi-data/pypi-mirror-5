...
class ArrayParam6(unittest.TestCase):
    ...
    def test_index1(self):
        ...
        self.model.B = Param( B_index, [True,False],
                              initialize=B_init, repn=self.repn )
        self.instance = self.model.create()
    ...
    def test_dimen1(self):
        ...
        model.y = Param(model.B, initialize=((1,1,7),2), repn=self.repn)
...
class SingletonTester(ParamTester):

    def setUp(self, **kwds):
        #
        # "Sparse" singleton Param, no default
        #
        self.model.Z = Set(initialize=[1,3])
    ...
    def test_value_singleton(self):
        ...
        if not self.sparse_data:
            self.assertRaises(ValueError, value, self.instance.A)
            self.assertRaises(ValueError, float, self.instance.A)
            self.assertRaises(ValueError, int, self.instance.A)
        else:
            val = self.data[None]
...
class SingletonParam_mutable_noDefault\
          (SingletonTester, unittest.TestCase):
    ...
    def setUp(self, **kwds):
        ...
        #
        self.model = AbstractModel()
        SingletonTester.setUp(self, mutable=True, **kwds)
        
        self.sparse_data = {}
        self.data = {None:None}

class SingletonParam_mutable_floatDefault\
          (SingletonTester, unittest.TestCase):
    ...
    def setUp(self, **kwds):
        ...
        #
        self.model = AbstractModel()
        SingletonTester.setUp(self, mutable=True, initialize=1.3, **kwds)
