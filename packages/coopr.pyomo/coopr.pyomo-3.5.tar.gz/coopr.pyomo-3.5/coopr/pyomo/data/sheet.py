...
try:
    #
    # If win32com is available, then use the PyUtilib Excel interface
    #
    import win32com
...
if sys.platform.startswith('win'):
    try:
        #
        # If pyodbc is available, then use it for XLS, XLSX, XLSB and XLSM files
        #
        import pyodbc

        from db_table import pyodbc_db_Table

        if not xls_interface:
            xls_interface = True
