...
__all__ = ["collectAbstractComponents", "partial", "process_canonical_repn"]
...
def collectAbstractComponents(model):
    ...
    for comp in cp.subcomponent_map(active=True):
...
def _getAbstractIndices(comp):
    ...
    """
    if comp._index_set is not None:
        # Multiply indexed component
        return comp._index_set
    elif type(comp._index) != type({}):
