...
from coopr.pyomo.base.block import active_subcomponents_data_generator
...
def symbol_map_from_instance(instance):
   ...
   for block in instance.all_blocks():
      for varvalue in active_subcomponents_data_generator(block,Var):
...
class SymbolMap(object):
    ...
    def getEquivalentObject(self, symbol, instance):
        ...
        while obj is not None:
            if obj.component() is obj:
                idx = None
            else:
                idx = obj.index()
            path.append((obj.component(), idx))
            obj = obj.parent()
