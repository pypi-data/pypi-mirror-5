...
from coopr.pyomo.base.objective import _ObjectiveData
...
class Model(SingletonBlock):
    ...
    def __init__ ( self, name='unknown', _deprecate=True, **kwargs ):
        ...
        #
        SingletonBlock.__init__(self, **kwargs)
        self.name=name
    ...
    #def constraints(self):
        #return self.subcomponents(Constraint)
    ...
    #def objectives(self):
        #return self.subcomponents(Objective)
    ...
    def clone(self):
        ...
        for block in self.all_blocks():
            for comp in itervalues(block.subcomponents()):
                comp._parent = None
        #
        ...
        assert instance._parent is None
        return instance

    def reset(self):
        # TODO: check that this works recursively for nested models
        for obj in itervalues(self.subcomponents()):
            obj.reset()
        ...
        for i in xrange(len(results.solution)):
            ...
            for label, entry in iteritems(input_soln_variable):
                ...
                if var_value_index.__class__ is tuple:
                    tmp[(var_value.component().cname(),)+var_value_index] = (var_value.cname(False, tmp_name_dict), entry)
                else:
                    tmp[(var_value.component().cname(),var_value_index)] = (var_value.cname(False, tmp_name_dict), entry)
            ...
            for label, entry in iteritems(input_soln_constraint):
                ...
                if con_value_index.__class__ is tuple:
                    tmp[(con_value.component().cname(),)+con_value_index] = (con_value.cname(False, tmp_name_dict), entry)
                else:
                    tmp[(con_value.component().cname(),con_value_index)] = (con_value.cname(False, tmp_name_dict), entry)
            for key in sorted(tmp.keys()):
                value = tmp[key]
            ...
            for label in input_soln.objective.keys():
                ...
                if obj_value.index().__class__ is tuple:
                    tmp[(obj_value.component().cname(),)+obj_value.index()] = (obj_value.cname(False, tmp_name_dict), entry)
                else:
                    tmp[(obj_value.component().cname(),obj_value.index())] = (obj_value.cname(False, tmp_name_dict), entry)
            for key in sorted(tmp.keys()):
                value = tmp[key]
    ...
    def _load_model_data(self, modeldata, namespaces, **kwds):
        ...
        for component_name, component in iteritems(self.subcomponents()):

            if component.type() is Model:
                continue
            elif component.type() is Var:
                var = component
                if var._defer_domain:
                    ...
                    if var.domain == []:
                        ...
                        if len(var.domain) != var.domain[-1] - var.domain[0] + 1:
                            ...
                            #
                            # WEH - this does not seem to be needed, so I'm omitting it.
                            #
                            #self.active_subcomponents(Var)[component_name] = var
            ...
            if report_timing is True:
                ...
                clone_counters = (
                    ...
                    coopr.pyomo.base.intrinsic_functions.generate_intrinsic_function_expression.clone_counter,
                    )
            ...
            if report_timing is True:
                ...
                if isinstance(comp, IndexedComponent) or isinstance(comp, SparseIndexedComponent):
                    clen = len(comp)
                else:
    ...
    def _initialize_component(self, modeldata, namespaces, component_name, profile_memory):
        declaration = self.subcomponent(component_name)
        ...
        if __debug__ and logger.isEnabledFor(logging.DEBUG):
            _blockName = "Model" if self.parent() is None \
    ...
    def write(self, filename=None, format=ProblemFormat.cpxlp, solver_capability=None, io_options={}):
        ...
        # problem writers are presently derived from the AbstractProblemWriter class, which in turn
        # is derived from ManagedPlugin and (indirectly) Plugin. thus, upon creation, all instances
        # of this class are thrust into the PluginGlobals environment, and are therefore referenced 
        # for eternity unless explicitly deactivated. JPW believes that problem writers really 
        # shouldn't be a service, if we have to go through this pain to clean up...
        problem_writer.deactivate()
