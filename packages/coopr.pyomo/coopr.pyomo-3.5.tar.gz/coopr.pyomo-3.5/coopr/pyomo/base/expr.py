...
__all__ = ['_ExpressionBase', 'complements']
...
def clone_expression(exp):
    if exp.is_expression():
        ...
        # except the _ExpressionData class (in which clone does nothing 
...
class _ExpressionBase(NumericValue):
    ...
    def to_string(self, ostream=None, verbose=None, precedence=0):
        ...
        if ostream is None:
            ostream = sys.stdout
        _verbose = TO_STRING_VERBOSE if verbose is None else verbose
...
class _ComplementarityExpression(_ExpressionBase):
    """An expression that defines a complementarity constraint"""
    ...
    def __init__(self, e1, e2):
        """Constructor"""
        _ExpressionBase.__init__(self, None)
        if e1.__class__ is _InequalityExpression:
            self._e1 = e1
            self._e2 = e2
        elif e2.__class__ is _InequalityExpression:
            self._e1 = e2
            self._e2 = e1
        else:
            raise TypeError("When creating a complementarity constraint, "\
                  "at least one argument must be an inequality expression.")
        # TODO: check that either we have two single inequalities or one
        # double inequality and an expression


def complements(e1, e2):
    """
    Returns an expression that represents a complementarity constraint.
    """
    return _ComplementarityExpression(e1,e2)
