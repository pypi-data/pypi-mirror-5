...
def create_model(data):
    ...
    if len(data.options.data_files) > 1:
        ...
        if suffix == "dat":
            ...
            if "modeldata" in dir(userdata):
                ...
                if len(ep) == 0:
                    msg = "Neither 'modeldata' nor 'pyomo_create_modeldata'"  \
    ...
    if not data.options.save_model is None:
        ...
        if data.options.save_model == True:
            ...
            format=None
        (fname, symbol_map) = instance.write(filename=fname, format=format, io_options={"symbolic_solver_labels" : data.options.symbolic_solver_labels})
        if not data.options.quiet:
