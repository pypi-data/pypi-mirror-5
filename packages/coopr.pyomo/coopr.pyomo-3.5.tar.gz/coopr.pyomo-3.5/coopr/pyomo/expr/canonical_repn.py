...
from coopr.pyomo.base.expression import _ExpressionData, _ExpressionElement, Expression
from coopr.pyomo.base.connector import _ConnectorValue, _ConnectorElement
...
#     global temp_nonl
    ...
    #
        ...
        # 
            ...
            return collect_general_canonical_repn(exp.value, idMap)
        #
        # ERROR
        #
        else:
    ...
    # Variable
    #
    elif exp_type is _VarData or exp.type() is Var:
        key = idMap.setdefault(id(exp), len(idMap))
...
class LinearCanonicalRepn(object):
    ...
    def __str__(self):
        ...
        return "LinearCanonical{ %s%s }" % \
            ( "%s + " if self.constant else "",
              " + ".join("%s*%s"%(self.linear[i], v) for v,i in ordered_vars) )
...
def _collect_linear_sum(exp, idMap, multiplier, coef, varmap):
    ...
    for arg in exp._args:
        ...
        if arg.__class__ is _VarData and arg.fixed is False:
            ...
            _linear_collectors[arg.__class__](arg, idMap, multiplier * six.next(arg_coef_iterator), coef, varmap)
...
def _collect_branching_expr(exp, idMap, multiplier, coef, varmap):
...
def _collect_identity(exp, idMap, multiplier, coef, varmap):
    exp = exp.value
    if exp.is_fixed():
        coef[None] += multiplier * value(exp)
    else:
        _linear_collectors[exp.__class__](exp, idMap, multiplier, coef, varmap)
...
_linear_collectors = {
    ...
    _ConnectorElement    : _collect_linear_connector,
    ...
    _ExpressionElement  : _collect_identity,
...
def canonical_is_constant(repn):
    ...
    if isinstance(repn, dict):
        return max(iterkeys(repn)) == 0 and None not in repn
    else:
...
def canonical_is_nonlinear(repn):
    ...
    if isinstance(repn, dict):
        return max(iterkeys(repn)) > 1 or None in repn
    else:
        return False
...
def canonical_is_linear(repn):
    ...
    if isinstance(repn, dict):
        return max(iterkeys(repn)) == 1 and None not in repn
    else:
...
def canonical_is_quadratic(repn):
    ...
    if isinstance(repn, dict):
        return max(iterkeys(repn)) == 2 and None not in repn
    else:
        return False
