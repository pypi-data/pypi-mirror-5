...
def Set_ParmValue(ph, ParmName, NewVal):
   ...
   for scenario in rootnode._scenarios:
      ...
      getattr(instance, ParmName).value = NewVal[index]
      instance.preprocess() # needed to re-evaluate expressions
#   print "Set",ParmName,"=",NewVal
...
def PurifyIndVar(ph, IndVarName):
   rootnode = ph._scenario_tree._stages[0]._tree_nodes[0]
   for scenario in rootnode._scenarios:
      instance = ph._instances[scenario._name]
      ...
      for index in pm:
        ...
        if   abs(delta)    < 1.e-10: delta = 0
        elif abs(delta-1.) < 1.e-10: delta = 1
        else: 
            print("** delta["+str(index)+"," + scenario._name + "] = "+str(delta))
            sys.exit() 
