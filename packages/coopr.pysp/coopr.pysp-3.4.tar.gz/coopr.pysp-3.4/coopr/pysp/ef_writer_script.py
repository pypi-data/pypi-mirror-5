...
from coopr.opt.base import SolverFactory, ConverterError
...
def run_ef_writer(options, args):
    ...
    pieces = options.output_file.rsplit(".",1)
    if len(pieces) != 2:
        raise RuntimeError("Could not determine suffix from output filename="+options.output_file)
    ef_output_file_suffix = pieces[1]        

    scenario_tree, binding_instance, scenario_instances, symbol_map = write_ef_from_scratch(os.path.expanduser(options.model_directory),
    ...
    if options.solve_ef is True:
        ...
        # at this point you have a specific solver - communicate solver capabilities
        # to the writer via the instance.
        binding_instance.has_capability = ef_solver.has_capability
        for scenario_name, scenario_instance in iteritems(scenario_instances):
            scenario_instance.has_capability = ef_solver.has_capability
        ...
        scenario_tree.snapshotSolutionFromInstances()
        ...
        if isinstance(ef_solver_manager,coopr.plugins.smanager.pyro.SolverManager_Pyro) and (options.shutdown_pyro is True):
