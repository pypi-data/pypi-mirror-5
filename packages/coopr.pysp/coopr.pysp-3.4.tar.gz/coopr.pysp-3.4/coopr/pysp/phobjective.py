...
def add_ph_objective_weight_terms(instance_name, instance, scenario_tree):
    ...
    for tree_node in scenario._node_list[:-1]:
        ...
        w_parameter_name = "PHWEIGHT_"+tree_node._name
    ...
    objective.expr.value += weight_expression_component
...
def add_ph_objective_proximal_terms(instance_name, 
    ...
    for tree_node in scenario._node_list[:-1]:
        ...
        if linearize_nonbinary_penalty_terms > 0:
            quad_penalty_term_variable_name = "PHQUADPENALTY_"+tree_node._name
    ...
    objective.expr.value += proximal_expression_component
...
def form_linearized_objective_constraints(instance_name, 
    ...
    for tree_node in scenario._node_list[:-1]:
        ...
        # if linearizing, then we have previously defined a variable associated with the result of the
        # linearized approximation of the penalty term - this is simply added to the objective function.
        linearized_cost_variable_name = "PHQUADPENALTY_"+tree_node._name
        ...
        linearization_constraint_name = "PH_LINEARIZATION_"+tree_node._name
        ...
        if linearization_constraint is not None:
            ...
            linearization_constraint = Constraint(nodal_index_set, linearization_index_set, name=linearization_constraint_name, noruleinit=True)
        ...
        for variable_id in tree_node._variable_ids:
            ...
            if variable_id not in tree_node._minimums:
                ...
                raise RuntimeError("No minimum value statistic found for variable="+variable_name+indexToString(index)+" on tree node="+tree_node._name+"; cannot form linearized PH objective")
            if variable_id not in tree_node._maximums:
                variable_name, index = tree_node._variable_ids[variable_id]
                raise RuntimeError("No maximums value statistic found for variable="+variable_name+indexToString(index)+" on tree node="+tree_node._name+"; cannot form linearized PH objective")

            xbar = xbar_parameter[variable_id].value
