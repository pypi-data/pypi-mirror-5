...
def run(args=None):
    ...
    # create a ph object for finding the solution. we do this even if we're solving the extensive form
    # directly, mainly out of convenience - we're leveraging the code in ph_for_bundle to create the
    # scenario tree and scenario instances.
    print("")
    print("Loading scenario instances and initializing scenario tree for xhat scenario bundle.")
    xhat_ph = ph_for_bundle(0, num_scenarios_for_solution, reference_model, full_scenario_tree, scenario_tree_instance, index_list, options, unarchived_model_directory, unarchived_instance_directory)
    xhat_obj = None
    ...
    if options.solve_xhat_with_ph is False:
        ...
        if options.verbose is True:
            ...
            print("Time="+time.asctime())
        xhat_ph._scenario_tree.snapshotSolutionFromInstances()
    ...
    for k in range(1, n_g+1):
        if AllInOne is True:
            ...
            gk_ph = ph_for_bundle(start_index, stop_index, reference_model, full_scenario_tree, scenario_tree_instance, index_list, options, unarchived_model_directory, unarchived_instance_directory)
        else:
            ...
            reference_model, scenario_tree_for_soln, scenario_tree_instance, unarchived_model_directory_mrp, unarchived_instance_directory_mrp = load_models(options)

            if (reference_model is None) or (scenario_tree_for_soln is None) or (scenario_tree_instance is None):
                raise RuntimeError("***ERROR: Failed to initialize reference model and/or the scenario tree.")

            gk_ph = create_ph_from_scratch(options, reference_model, scenario_tree_for_soln, unarchived_model_directory_mrp, unarchived_instance_directory_mrp)
        ...
        gk_ph._scenario_tree.snapshotSolutionFromInstances()
...
#
def ph_for_bundle(bundle_start, bundle_stop, reference_model, full_scenario_tree, scenario_tree_instance, index_list, options, unarchived_model_directory, unarchived_instance_directory):
    ...
    scenario_tree_for_soln = ScenarioTree(scenariotreeinstance=scenario_tree_instance,
    ...
    ph = create_ph_from_scratch(options, reference_model, scenario_tree_for_soln, unarchived_model_directory, unarchived_instance_directory)
    return ph
