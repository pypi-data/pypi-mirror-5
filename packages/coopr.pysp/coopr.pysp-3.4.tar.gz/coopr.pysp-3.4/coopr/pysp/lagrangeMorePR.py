...
###################################
   ...
   def LagrangeMorePR(args=None):
      print "lagrangeMorePR begins ",str(datetime.datetime.now())
...
# options used
      alphaTol      = options.alpha_tol          # tolerance used to separate b-values
...
# initialize
      ...
      inputFile.close()
      addstatus = str(len(ScenarioList))+' scenarios read from ' + csvPrefix+'ScenarioList.csv'
      ...
      inputFile.close()
      addstatus = str(len(PRoptimal))+' PR points read from '+ csvPrefix+'PRoptimal.csv (envelope function)'
      if verbosity > 0: print addstatus
      Result.status = Result.status + '\n' + addstatus
...
## read scenarios to select for each PR point on envelope function
      ...
      for line in inputFile.readlines():
         ...
         if verbosity > 1: print selections
         OptimalSelections.append(selections)
      inputFile.close()
      Result.OptimalSelections = OptimalSelections

      addstatus = str(len(OptimalSelections)) + ' Optimal selections read from ' \
...
# ...read probabilities
        ...
        inputFile.close()
        if verbosity > 0: print "\t",len(probList),"probabilities"
        if verbosity > 1: print probList
        PRlist = GetPoints(options, PRoptimal, probList)
        if verbosity > 1: 
           print "PRlist:"
           for interval in PRlist: print interval
...
#####################################################################################
      ...
      for interval in PRlist:
         ...
         if verbosity > 1: 
            print "target probability =",b,"< bU = PRoptimal[" + str(i) + "][1]" \
         ...
         if verbosity > 1: 
            print "i =",i,": Starting with bU =",bU,"having ", \
                len(OptimalSelections[i]), "selections:"
            print OptimalSelections[i]
...
# flip scenario selections from bU until we reach b (target probability)
         ...
         for scenario in ScenarioList:
            ...
            if verbosity > 1: print "\tflipped",sname,"with prob =",sprob,"...bNew =",bNew

         if verbosity > 1:
            print "\tflipped selections reach",bNew, " >= target =", b," (bL =",bL,")"
         if bNew <= bL + alphaTol or bNew >= bU - alphaTol: 
            if verbosity > 0:
...
# check that scenarios are fixed as they should be
            ...
            for scenario in ScenarioList:
               ...
               sprob = scenario[1]
               instance = ph._instances[sname]
               print "fix",sname,"=",getattr(instance,IndVarName).value,\
                  "is",getattr(instance,IndVarName).fixed," probability =",sprob
...
# solve (all delta fixed); lambda=0, so z = Lagrangian
         ...
         if verbosity > 0: 
            print "solve ends",str(datetime.datetime.now()),"SolStat =",SolStat, \
                " b =",b," z =",z, " (adding to more PR points)"
      ...
      addstatus = str(len(Result.morePR)) + ' PR points written to '+ csvPrefix + 'PRmore.csv'
      if verbosity > 0: print addstatus
      Result.status = Result.status + '\n' + addstatus 
...
##########################
   try:
      ...
      conf_options_parser.add_option("--alpha-tol",
                                     help="Tolerance for testing equality to alpha. Default is 10^-2",
                                     action="store",
                                     dest="alpha_tol",
                                     type="float",
                                     default=1e-2)
...
# create ph objects for finding the solution. we do this even if we're solving the extensive form
...
##################################################################################### 
   ...
   try:
     print "Envelope:"
     print PrintPRpoints(Result.PRoptimal)
     print "\nAdded:"
     PrintPRpoints(Result.morePR)
   except:
     print "Result object not defined"
     sys.exit()   
...
# Find more PR points (above F^*)
   if options.verbosity > 1: 
      print "entered FindPRpoints seeking",options.max_number,"points"
   Intervals = []

# Find intervals, each with width > 2alpha_tol, such that cdf[i] is near its midpoint for some i
   if options.verbosity > 1: 
      print "Collecting intervals having width >",2*options.alpha_tol
   for i in range(1,len(PRoptimal)):
      width = PRoptimal[i][1] - PRoptimal[i-1][1] 
      if width <= 2*options.alpha_tol: continue
      midpoint = (PRoptimal[i][1] + PRoptimal[i-1][1]) / 2.
      Intervals.append( [i, width, midpoint] )
      if options.verbosity > 1: 
         print "Interval: ",i,", width =",width," midpoint =",midpoint
...
# split widest interval to have another PR point
      ...
      if interval[1] < 2*options.alpha_tol: 
         status = 'greatest width = ' + str(w) + ' < 2*alpha_tol = ' + str(2*options.alpha_tol)
         print status
      ...
      if options.verbosity > 1: print "splitting interval:",interval
...
#                                     new midpoint of right
      ...
      if options.verbosity > 1: print "Number of intervals =",len(Intervals)
   if options.verbosity > 1: 
      print "\n--- end while with ",len(Intervals),"intervals:"
      for interval in Intervals:  print "\t",interval
