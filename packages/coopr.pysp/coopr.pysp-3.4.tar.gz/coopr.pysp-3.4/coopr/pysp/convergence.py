...
class TermDiffConvergence(ConvergenceBase):
    ...
    def computeMetric(self, ph, scenario_tree, instances):
        ...
        for stage, tree_node, variable_id, variable_datas, is_fixed, is_stale in scenario_tree_node_variables_generator(scenario_tree, includeDerivedVariables=False, includeLastStage=False):

            if is_stale is False:
                
                for var_data, scenario_probability in variable_datas:

                    term_diff += scenario_probability * fabs(var_data.value - tree_node._averages[variable_id])
...
class NormalizedTermDiffConvergence(ConvergenceBase):
    ...
    def computeMetric(self, ph, scenario_tree, instances):
        ...
        for stage, tree_node, variable_id, variable_datas, is_fixed, is_stale in scenario_tree_node_variables_generator(scenario_tree, includeDerivedVariables=False, includeLastStage=False):
            ...
            if (is_stale is False) and (fabs(average_value) > 0.0001):

                for var_data, scenario_probability in variable_datas:
                    
                    normalized_term_diff += scenario_probability * fabs((var_data.value - average_value)/average_value)
