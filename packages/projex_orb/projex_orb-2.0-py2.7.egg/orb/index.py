...
class Index(object):
    ...
    def __call__(self, cls, *values, **options):
        ...
        cache_key = (data, hash(LookupOptions(**options)))
        ...
        if self._unique:
            results = cls.selectFirst(**options)
        else:
            results = cls.select(**options)
        
        # cache the results
        if self._cached and \
           (results is not None or self._cachedExpires):
    ...
    def cachedExpires(self):
        """
        Returns the time in seconds that this index will hold onto a client
    ...
    def toXml(self, xparent):
        ...
        xindex.set('cachedExpires', str(self.cachedExpires()))
