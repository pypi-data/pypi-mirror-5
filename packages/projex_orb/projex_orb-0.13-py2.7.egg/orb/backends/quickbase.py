...
class Quickbase(Connection):
    ...
    def _fieldIds(self, schema):
        ...
        if not field_ids:
            ...
            xresp   = self.request(table_id, 'GetSchema')
    ...
    def _select( self, table_or_join, lookup, options ):
        ...
        elif ( Join.typecheck(table_or_join) ):
            logger.warning('Joining is not yet supported for Quickbase')
            return []
        ...
        for schema, columns in schemas:
            ...
            if ( lookup.columns is not None ):
                ...
                fids = map(lambda x: field_ids.get(x, '0'), fields)
                request['clist'] = '.'.join(fids)
            
            # collect specific entries
            if ( lookup.where ):
                request['query'] = self.queryCommand(schema, lookup.where)
            ...
            # collect the records
            xresponse = self.request(table_id, 'DoQuery', request)
            ...
            for xrecord in xresponse:
                ...
                for xfield in xrecord:
                    ...
                    if ftype == ColumnType.Bool:
                        fval = fval == '1'
                    
                    elif ftype in (ColumnType.Integer, 
                                   ColumnType.Enum,
                    ...
                    elif ftype in (ColumnType.Double, ColumnType.Decimal):
                        try:
                            fval = float(fval)
                        except:
                            fval = 0.0
                    
                    elif ftype == ColumnType.Date:
                        try:
                    ...
                    elif ftype == ColumnType.Datetime:
                        try:
                            fval = datetime.datetime.strptime(fval,
                                                           '%Y-%m-%d %H:%M:%S')
                        except:
                            fval = datetime.datetime()
                    
                    elif ftype == ColumnType.Time:
                        try:
    ...
    def count(self, table_or_join, lookup, options):
        ...
        table_id = schema.tableName()
        if not lookup.where:
            xresponse = self.request(table_id, 'GetNumRecords')
            return int(xresponse.find('num_records').text)
        else:
            opts = {}
            opts['query'] = self.queryCommand(schema, lookup.where)
            xresponse = self.request(table_id, 'DoQueryCount', opts)
    ...
    def queryCommand( self, schema, query ):
        ...
        """
        if ( query.isNull() ):
            logger.debug( 'Quickbase.queryCommand: NULL QUERY.' )
            return ''
        ...
        if ( QueryCompound.typecheck(query) ):
            ...
            join = 'AND'
            if ( query.operatorType() == QueryCompound.Op.Or ):
                join = 'OR'
        ...
        if ( not col ):
            warn = errors.ColumnNotFoundWarning(colname)
            logger.warning(warn)
            return ''
        ...
        # extract the primary key information from a record set
            ...
            value = ','.join(map(str, value.values(settings.PRIMARY_FIELD)))
    ...
    def recordCommand( self, column, value ):
        ...
        # necessary
        if ( not Table.recordcheck(value) ):
            return value
    ...
    def request(self,
        ...
        # debug the lookup
        logger.debug(url)
        logger.debug(unicode(headers))
        logger.debug(projex.text.encoded(data))
        ...
        try:
            ...
            try:
                ...
                response = None
            raise ConnectionError(unicode(error))
        ...
        try:
            ...
            raise ResponseError(unicode(error))
    ...
    def buildRequest(**fields):
        ...
        def add_sub_element(field, value):
            if isinstance(value, tuple):
                attrib, value = value
                attrib = dict((k, unicode(v)) for k, v in attrib.iteritems())
            else:
                attrib = {}
            sub_element = ElementTree.SubElement(request, field, **attrib)
            if not isinstance(value, basestring):
                value = projex.text.encoded(value)
...
if ( urllib2 ):
    Connection.register('Quickbase', Quickbase)