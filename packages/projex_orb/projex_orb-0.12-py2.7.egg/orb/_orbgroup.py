...
class OrbGroup(object):
    ...
    def merge( self ):
        ...
        try:
            xorb = ElementTree.parse(str(filename)).getroot()
    ...
    def property( self, key, default = '' ):
        ...
        :return     <str>
        """
        return self._properties.get(str(key), str(default))
    ...
    @staticmethod
    def fromXml( xgroup, referenced = False ):
        """
        ...
        if modname:
            ...
            grp = Orb.instance().group(grpname)
            if ( not grp ):
                return None
                
            grp.setDatabaseName(xgroup.get('dbname', ''))
            ...
            if xschemas is not None:
                for xschema in xschemas:
                    ...
                    schema.setGroupName(grp.name())
                    schema.setDatabaseName(grp.databaseName())
