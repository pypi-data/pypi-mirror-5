...
from orb.common          import ColumnType, SelectionMode, DatabaseFlags
...
try:
    ...
    text = 'For Mongo backend, download the pymongo module'
...
class Mongo(Connection):
    ...
    def _select( self, table_or_join, lookup, options ):
        ...
        for schema, columns in schemas:
            ...
            if ( lookup.where ):
                specs = self.queryCommand(schema, lookup.where)
    ...
    def cleanValues( self, schema, data ):
        ...
        for key, value in data.items():
            if ( isinstance(value, datetime.datetime) ):
                ...
                if ( not (col and col.columnType() == ColumnType.Date) ):
                    continue
                ...
                col = schema.column(key)
                if ( not (col and col.columnType() in [ColumnType.String, 
                                                       ColumnType.Password,
                                                       ColumnType.Email])):
                    continue
                
                data[key] = projex.text.encoded(value)
    ...
    def insert( self, record, options ):
        ...
        if ( options.dryRun ):
            ...
            safe   = options.flags & DatabaseFlags.SafeInsert != 0
            result = db[tableName].insert(values, safe = safe)
            record._updateFromDatabase({'_id': projex.text.encoded(result)})
    ...
    def queryCommand( self, schema, query ):
        ...
        # extract the primary key information from a record set
            ...
            value = value.values(settings.PRIMARY_FIELD)
        
        # make sure we're working with ObjectId's (kind of a pain)
        elif ( col.columnType() == ColumnType.ForeignKey and
    ...
    def update( self, record, options ):
        ...
        for colname, changevals in changes.items():
            ...
            elif ( Table.recordcheck(newValue) ):
...
if ( pymongo ):
    Connection.register( 'Mongo', Mongo )