...
def random_redshift_array(
    ...
    **Key Arguments:**
        ...
        - ``redshiftVolumeLimit`` -- the redshift limit of the volume to be included
    ...
    **Return:**
    ...
    redshiftDistribution = np.arange(0., redshiftLimit, redshiftResolution)
    closestNumber=lambda n,l:min(l,key=lambda x:abs(x-n))

    # GIVEN THE REDSHIFT LIMIT - DETERMINE THE VOLUME LIMIT
    distanceDictionary = da.convert_redshift_to_distance(redshiftLimit)
    mpcLimit = distanceDictionary["dl_mpc"]
    volumeLimit = (4./3.)*np.pi*mpcLimit**3

    # GENERATE A LIST OF RANDOM DISTANCES
    redshiftList = []
    for i in range(sampleNumber):
        randomVolume = npr.random()*volumeLimit
...
def convert_lightcurves_to_observered_frame(
    ...
    **Return:**
    ...
    for item in range(len(snTypesArray)):
        ...
        log.debug('thisModel, observeFrameFilter: %s, %s' % (thisModel,observeFrameFilter ,))
        explosionDay = rawLightCurveDict[thisModel][observeFrameFilter]['Explosion Day Relative to Peak']
        ...
        absLightCurve = rawLightCurveDict[thisModel][observeFrameFilter]['poly'] + peakMagPolyList[item]
    ...
    for item in range(len(redshiftArray)):
        ...
        explosionDay = rawLightCurveDict[thisModel][observeFrameFilter]['Explosion Day Relative to Peak']*timeDilation
