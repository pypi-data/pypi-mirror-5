...
def plot_sn_discovery_map(log,
    ...
    **Return:**
    ...
    plt.scatter(
        tooFaint,
        tooFaintRedshift,
        label="""Detected - too faint to constrain as PISN""",
        s=50,
        c='#dc322f',
        marker='o',
        cmap=None,
    ...
    plt.scatter(
        shortCampaign,
        shortCampaignRedshift,
        label="""Detected - campaign to short to constrain as PISN""",
        s=50,
        c='#268bd2',
        marker='o',
        cmap=None,
    ...
    title = "PISNe Detected Within the Suvrey Year"
    box = ax.get_position()
    ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
    # Put a legend to the right of the current axis
...
def plot_sn_discovery_ratio_map(log,
    ...
    **Return:**
    ...
    if len(tooFaintRedshift) > 0:
        dataDictionary["Detected - too faint to constrain as PISN"] = tooFaintRedshift
    if len(discoveredRedshift) > 0:
        dataDictionary["Discovered"] = discoveredRedshift
    if len(shortCampaignRedshift) > 0:
        dataDictionary["Detected - campaign to short to constrain as PISN"] = shortCampaignRedshift

    ################ >ACTION(S) ################
    imageLink = dp.plot_polar(
       log,
       title="Redshift Map of PISNe Simulated within the Survey Volume",
...
def determine_sn_rate(
    ...
    **Key Arguments:**
        ...
        - ``PISNToCCSNRateFraction`` -- the ratio of the PISN rate to the rate of CCSNe
        ...
        - ``pathToOutputPlotDirectory`` -- path to add plots to
        - ``redshiftLimit`` -- the redshist limit out to which to determine the rate
        - ``redshiftResolution`` -- the redshist resolution ued to determine the rate

    **Return:**
        - ``imageLink`` -- MD link to the output plot
        - ``totalRate`` -- the final PISN rate calculated
    """
    ################ > IMPORTS ################
    ## STANDARD LIB ##
    ## THIRD PARTY ##
    ...
    sdssArea = 5713.
    sdssSFRRedshiftLimit = 0.033
    sdssSFR = 14787.21
    CCSNRateFraction = 0.007
    PISNToCCSNRateFraction = 1./10000.
    PISNRateFraction = CCSNRateFraction*PISNToCCSNRateFraction

    anchor = redshiftResolution/sdssSFRRedshiftLimit
    ...
    redshiftLimit = int(redshiftLimit*multiplier)

    shellPISNRateDensityList = []
    shellRedshiftList = []
    ...
    for shellRedshift in range(redshiftResolution, redshiftLimit, redshiftResolution):
        ...
        shellPISNRateDensity = shellSFRDensity*PISNRateFraction
        shellPISNRateDensity = shellPISNRateDensity*areaRatio
        timeDilation = 1./(1.+shellMiddle)
        shellPISNRateDensity = shellPISNRateDensity*timeDilation
        # log.info('shellPISNRateDensity %s' % (shellPISNRateDensity,))
        shellPISNRateDensityList.append(shellPISNRateDensity)
        ...
        PISNShellRate = discoveryFraction*shellPISNRateDensity
        tooFaintShellRate = tooFaintFraction*shellPISNRateDensity
        shortCampaignShellRate = shortCampaignFraction*shellPISNRateDensity
        PISNShellRateList.append(PISNShellRate)
    ...
    if len(tooFaintRedshiftListArray) > 0:
        ...
        #polynomialDict["Detected - too faint to constrain as PISN"] = tooFaintRateCurve
        orginalDataDictionary["Detected - too faint to constrain as PISN"] = [tooFaintRedshiftListArray, tooFaintFractionListArray]
    else:
    ...
    if len(shortCampaignRedshiftListArray) > 0:
        ...
        #polynomialDict["Detected - campaign to short to constrain as PISN"] = shortCampaignRateCurve
        orginalDataDictionary["Detected - campaign to short to constrain as PISN"] = [shortCampaignRedshiftListArray, shortCampaignFractionListArray]
    else:
    ...
    if pleasePlot:
        imageLink = dp.plot_polynomial(
                log,
                title='PS1-PISN Survey - Precentage of PISN Detected',
    ...
    shellPISNRateDensityArray = np.array(shellPISNRateDensityList)
    shellRedshiftArray = np.array(shellRedshiftList)
    # log.info('shellRedshiftArray %s' % (shellRedshiftArray,))
    # log.info('shellPISNRateDensityArray %s' % (shellPISNRateDensityArray,))
    thisPoly = np.polyfit(shellRedshiftArray, shellPISNRateDensityArray, 3)
    PISNRateCurve = np.poly1d(thisPoly)
    # log.info('flat %s' % (flatPoly,))
    polynomialDict = { "PISN Rate" : PISNRateCurve }
    orginalDataDictionary = { "PISN Rate" : [shellRedshiftArray, shellPISNRateDensityArray] }
    imageLink = dp.plot_polynomial(
            log,
            title='PS1 MDF PISN Rate Density',
            polynomialDict=polynomialDict,
            orginalDataDictionary=orginalDataDictionary,
            pathToOutputPlotsFolder=pathToOutputPlotFolder,
            xRange=[0.01, 1., 0.01],
    ...
    if discoveryRateCurve:
        pleasePlot = True
        discoveredPISNCurve = PISNRateCurve*discoveryRateCurve
        discoveredPISNCurve = np.poly1d(discoveredPISNCurve)
        #polynomialDict['Discovered PISN Rate'] = discoveredPISNCurve
        orginalDataDictionary['Discovered PISN Rate'] = [shellRedshiftArray, shellPISNRateDensityArray*discoveryFractionListArray]
    if tooFaintRateCurve:
        pleasePlot = True
        tooFaintPISNCurve = PISNRateCurve*tooFaintRateCurve
        tooFaintPISNCurve = np.poly1d(tooFaintPISNCurve)
        #polynomialDict["Detected - too faint to constrain as PISN"] = tooFaintPISNCurve
        orginalDataDictionary["Detected - too faint to constrain as PISN"] = [shellRedshiftArray, shellPISNRateDensityArray*tooFaintFractionListArray]
    if shortCampaignRateCurve:
        pleasePlot = True
        shortCampaignPISNCurve = PISNRateCurve*shortCampaignRateCurve
        shortCampaignPISNCurve = np.poly1d(shortCampaignPISNCurve)
        #polynomialDict["Detected - campaign to short to constrain as PISN"] = shortCampaignPISNCurve
        orginalDataDictionary["Detected - campaign to short to constrain as PISN"] = [shellRedshiftArray, shellPISNRateDensityArray*shortCampaignFractionListArray]

    if pleasePlot:
        imageLink = dp.plot_polynomial(
                log,
                title='PS1-PISN Survey - Relative Detected Rates',
                polynomialDict=polynomialDict,
                orginalDataDictionary=orginalDataDictionary,
                pathToOutputPlotsFolder=pathToOutputPlotFolder,
                xRange=[0.01, 1., 0.01],
...
def log_the_survey_settings(
    ...
    **Return:**
    ...
    redshiftLimit = yamlContent["Volume Limit Redshift"]
    #log.debug('redshiftLimit %s' % (redshiftLimit,))
    ...
    PISNToCCSNRateFraction = yamlContent["PISN to CCSN Ratio"]
    observeFrameFilter = yamlContent["Observed Frame Filter for K-corrections"]
...
| Filter | Magnitude |
|:---|:----|
""" % (now, observeFrameFilter)
    for k, v in limitingMags.iteritems():
        settings_log += """| %s | %s |\n""" % (k,v,)
    settings_log += """

A total of **%s** PISNe where simulated in the survey, out to a **redshift of %s**. A constant galactic extinction of `E(B-V) = %s` is used (the mean extinction of the 10 PS1 Medimum Deep Fields). The MDFs are visible for a fraction of %s of the survey year and the typical fraction of survey time lost to weather of %s is accounted for. Here are the relative rates and peak magnitude distributions of the SN used in the survey:

| SN Type | Relative Rate | Peak Magnitude | Sigma Peak |
|:---|:---|:---|:---|
""" % (sampleNumber, redshiftLimit, extinctionConstant, observableFraction, weatherLossFraction,)
...
The PISN rate for the survey volume is estimated by assuming a rate of **%s** times that of the CCSN rate (itself a fraction of **%s** of the total SFR).

""" % (peakMagLimit, campaignLengthLimit, PISNToCCSNRateFraction, CCSNRateFraction)
    return settings_log
