...
class AccDocFormat10ReaderProFormaTest(TestCase, FormatReaderGenericTest):
    def setUp(self):
        """
    ...
    def test_load_data(self):
        """
        Tests the Format_1_0_Reader reads correctly an XML pro-forma document.
        """
        xml_root = etree.fromstring(xml_data)
        acc_doc = self.reader.read_accounting_doc(xml_root,
                                                  filename="X0.xml",
                                                  cfg=self.cfg)
        self.assertEqual(acc_doc.__class__, ProForma)
        self.assertEqual(acc_doc.validity_date, "Z0")
        self._check_doc_generic_content(acc_doc, check_id=False)
