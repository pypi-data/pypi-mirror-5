...
class EventValidatorSection(object):
    ...
    def __iter__(self):
        for item in self.previous:
            ...
            period.no_starttime = item.get('no_starttime', False)
            ...
            if period.no_starttime == True:
                period.start = period.start.replace(hour = 20, minute=0)
            
            period.no_endtime = item.get('no_endtime', False)
            item['period'] = period
