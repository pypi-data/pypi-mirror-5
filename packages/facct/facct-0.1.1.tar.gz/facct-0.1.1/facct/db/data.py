...
import config
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'bill'))
import gen_tex
...
def set_prestation_dates(orga, data):
    ...
    for f in glob.glob(os.path.join(get_bill_path(orga), '20*', '*[0-9].csv')):
        csv_data_tmp.extend(get_csv_data(orga, os.path.splitext(f)[0], tables[table_name],
                keep_first=True, csv_file_name=os.path.normpath(f),
                formater=retrieve_date))
