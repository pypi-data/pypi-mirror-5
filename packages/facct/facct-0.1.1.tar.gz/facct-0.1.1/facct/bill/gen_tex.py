...
def get_data_dir():
    return config.get_internal_data_dir()
...
def write_pdf(orga, year, month, company_name, unicode_TeX):
    ...
    tmpl_file = os.path.join(get_data_dir(), '%(bill_tmpl)s.tex' % dico)
...
def getSameDayNextMonth(date):
    return datetime.date(date.year + date.month//12, (date.month%12)+1, date.day)
...
''' % { 'year' : self._year,
    ...
    def get_VAT(self):
        ...
        return decimal.Decimal('%.3f' % round(
                    self._total_excluded_tax * decimal.Decimal(0.196), 2))
    ...
    def get_dico(bill):
        return { 'year' : bill._year, 
            ...
            'total VAT' : bill.get_total_excluded_tax(),
            'VAT'       : '%.2f' % bill.get_VAT(), 
...
class TeX:
    ...
    def getVarDef(self, db):
        def getVarDef_fromVal(k, v):
            return r'\gdef\%s%s{%s}' % (db.get_name(), 
                    ...
                    str(v).replace('_','\_'))
        return '\n'.join([getVarDef_fromVal(k, v) for k,v in db.get_dico().items()])

    def create_utf8_translation_table(self, utf8_file):
        translation_table = {}
    ...
    def get_unicode_TeX(self, tex_data, dir_root):
        utf8_file = os.path.join(get_data_dir(), 'utf8ienc.dtx')
        translation_table = self.create_utf8_translation_table(utf8_file)
        if sys.version_info[0] == 2:
            if not platform.os.name == 'nt':
                tex_data = tex_data.decode('utf-8')
            ...
            if platform.os.name == 'nt':
                #tex_data = tex_data.decode('utf-8')
                pass
        return tex_data.translate(translation_table)
...
def bill_creation(orga, bill, company, dir_root):
    ...
    '\n'.join([tex.calendar(bill)]+[tex.getVarDef(db) for db in db_list]), dir_root)
...
class Company():
    def __init__(self, orga):
        ...
        for r in reader:
            ...
            capital = locale.format( '%d', int(self._dico['capital']), grouping=True)
            self._dico.update({
