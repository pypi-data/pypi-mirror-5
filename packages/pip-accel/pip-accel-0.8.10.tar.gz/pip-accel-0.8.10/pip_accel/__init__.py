...
def unpack_source_dists(arguments):
    ...
    Returns a list of tuples with three strings each: The name of a requirement
    (package), its version number and the directory where the unpacked source
    distribution is located. If ``pip`` fails, an exception will be raised by
    ``pip``.
    """
    unpack_timer = Timer()
...
def build_binary_dists(requirements):
    ...
    :py:func:`unpack_source_dists()` function.

    Returns ``True`` if it succeeds in building a binary distribution,
    ``False`` otherwise (probably because of missing binary dependencies like
    system libraries).
    """
...
def install_requirements(requirements, install_prefix=ENVIRONMENT):
    ...
    Expects a list of tuples in the format of the return value of
...
def run_pip(arguments, use_remote_index):
    ...
    Returns a pip ``RequirementSet`` object, unless an exception raised by
    ``pip`` (in which case the exception will bubble up).
    """
    command_line = []
...
class CustomInstallCommand(InstallCommand):
    ...
    """
    ``InstallCommand.run()`` returns a ``RequirementSet`` object which
    `pip-accel` is interested in, however ``pip.basecommand.Command.main()``
    swallows the requirement set (based on my reading of the pip 1.3.x source
    code). This is why we subclass ``InstallCommand`` to wrap the run() method
    :-). Yes, this is a bit sneaky, but I don't fancy reimplementing
...
def add_extension(download_path, archive_path):
    ...
    on PyPi, but that would add a binary dependency to `pip-accel` and I don't
    want that :-).
    """
