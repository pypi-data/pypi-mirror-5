...
from art3dutils.utilities import (
...
class Quarter(Entity):
    ...
    def fetch_available_buildings(self):
        ...
        for building in self.buildings:
            if len(building.get_apartments(True)) > 0:
...
class Building(Entity):
    ...
    def get_floors(self, floor_numbers=None):
    ...
    def get_apartments(self, available_only=False, floors=None):
...
class Section(Entity, Mapping):
    ...
    def get_apartments(self, available_only=False, room_count=None):
        query = session.query(Apartment)\
                       .filter(apartments.c.project_title==self.project_title,
                               apartments.c.quarter_number==self.quarter_number,
    ...
    def get_floors(self, available_only=False):
        ...
        if available_only:
            ...
            for floor in all_floors:
                if len(floor.get_apartments(available_only=True)):
...
projects = Table('project', metadata,
    Column('title', VARCHAR(255), primary_key=True, nullable=False),
    mysql_engine='MyISAM', mysql_charset='utf8')

quarters = Table('quarter', metadata,
    Column('project_title', VARCHAR(255), ForeignKey('project.title'),
            primary_key=True),
...
buildings = Table('building', metadata,
    Column('project_title', VARCHAR(255), ForeignKey('quarter.project_title'),
            primary_key=True),
    Column('quarter_number', Integer, ForeignKey('quarter.number'),
            primary_key=True),
    Column('number', Integer, primary_key=True, nullable=False,
            autoincrement=False), mysql_engine='MyISAM', mysql_charset='utf8'
    )

sections = Table('section', metadata,
    Column('project_title', VARCHAR(255), ForeignKey('building.project_title'),
            primary_key=True),
    Column('quarter_number', Integer, ForeignKey('building.quarter_number'),
            primary_key=True),
    Column('building_number', Integer, ForeignKey('building.number'),
            primary_key=True),
    Column('number', Integer, primary_key=True, nullable=False,
            autoincrement=False), mysql_engine='MyISAM', mysql_charset='utf8'
    )
...
floors = Table('floor', metadata,
    Column('project_title', VARCHAR(255), ForeignKey('section.project_title'),
            primary_key=True),
    Column('quarter_number', Integer, ForeignKey('section.quarter_number'),
            primary_key=True),
    Column('building_number', Integer, ForeignKey('section.building_number'),
            primary_key=True),
...
apartments = Table('apartment', metadata,
    Column('project_title', VARCHAR(255), ForeignKey('floor.project_title'),
            primary_key=True),
    Column('quarter_number', Integer, ForeignKey('floor.quarter_number'),
            primary_key=True),
    Column('building_number', Integer, ForeignKey('floor.building_number'),
            primary_key=True),
    Column('section_number', Integer, ForeignKey('floor.section_number'),
            primary_key=True),
...
mapper(Quarter, quarters, properties={
    'buildings': relationship(Building, backref='quarter',
        primaryjoin=and_(
            buildings.c.quarter_number==quarters.c.number,
            buildings.c.project_title==quarters.c.project_title
        ))})
mapper(Building, buildings, properties={
    'sections': relationship(Section, backref='building',
        primaryjoin=and_(
            sections.c.building_number==buildings.c.number,
            sections.c.quarter_number==buildings.c.quarter_number,
            sections.c.project_title==buildings.c.project_title
        ))})
mapper(Section, sections, properties={
    'floors': relationship(Floor, backref='section',
        primaryjoin=and_(
            floors.c.section_number==sections.c.number,
            floors.c.building_number==sections.c.building_number,
            floors.c.quarter_number==sections.c.quarter_number,
            floors.c.project_title==sections.c.project_title
        ))})
mapper(Floor, floors, properties={
    'apartments': relationship(Apartment, backref='floor',
        primaryjoin=and_(
            apartments.c.floor_number==floors.c.number,
            apartments.c.section_number==floors.c.section_number,
            apartments.c.building_number==floors.c.building_number,
            apartments.c.quarter_number==floors.c.quarter_number,
            apartments.c.project_title==floors.c.project_title
        ))})
