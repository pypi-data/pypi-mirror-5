...
class PollVoteView(FormView):
    ...
    def post(self, request, *args, **kwargs):
        self.pk = kwargs.get('pk')
        form = self.form = PollVoteForm(request.POST)
        user = request.user
        poll = Poll.objects.get(pk=self.pk)
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if not instance.multiple_choice:
            form.fields['choice'] = forms.MultipleChoiceField(widget=forms.RadioSelect)
        
        form.fields['choice'].choices = ((choice.pk, choice.choice_text) for choice in instance.choice_set.all())
