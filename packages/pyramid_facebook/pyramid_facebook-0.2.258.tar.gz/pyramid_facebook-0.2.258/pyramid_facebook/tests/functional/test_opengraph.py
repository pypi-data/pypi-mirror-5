...
class TestOpenGraph(TestView):
    ...
    def setUp(self):
        # redefine config and app objects so that one test can change
        # the config without impacting other tests--unit testing yo
        self.config = Configurator(settings={
            'facebook.app_id': '112233',
        })
        self.config.include('pyramid_facebook.opengraph')
    ...
    def test_basic(self):
        resp = self.app.get('/store/packs/rainbows_pack', status=200)
        ...
        for prop, expected in [
            ('fb:app_id', '112233'),
            ('og:type', 'og:product'),
            ('og:url', 'http://localhost/store/packs/rainbows_pack'),
            ('og:title', 'Rainbows Pack'),
            ('og:description', 'Use Rainbows to play more'),
          ]:
    ...
    def test_url_with_query_string(self):
        resp = self.app.get('/store/packs/rainbows_pack?promo=labor_day',
                            status=200)

        self.assertEqual(
            resp.html.head.find('meta', property='og:url')['content'],
            'http://localhost/store/packs/rainbows_pack?promo=labor_day')

    def test_unknown_pack(self):
        self.app.get('/store/packs/unicorns_pack', status=404)

    def test_default_view(self):
        self.app.put('/store/packs/rainbows_pack', status=405)
