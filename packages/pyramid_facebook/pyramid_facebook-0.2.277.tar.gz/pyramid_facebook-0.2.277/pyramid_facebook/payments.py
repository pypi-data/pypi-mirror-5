...
from pyramid_facebook.events import OrderCreated, OrderCanceled
...
def put_order(context, request):
    ...
    try:
        ...
        log.exception('facebook_payments_order payment=%s.', payment)
        status = 'canceled'
        try:
            request.registry.notify(OrderCanceled(request, payment))
        except Exception:
            log.critical('CanceledOrder event notification failed')
    else:
        log.debug('facebook_payments_order completed payment=%s', payment)
        status = 'completed'
