...
def _is_string(thing):
    ...
    else: return isinstance(thing, str)
...
def _sanitise(*args):
    ...
    def _check_option(arg, value):
        ...
        if not _util._py3k:
            if not isinstance(arg, list) and isinstance(arg, str):
                arg = str(arg)
    ...
    if args is not None:
        ...
        for arg in args:
            ...
            if (not _util._py3k and isinstance(arg, str)) \
...
def _get_options_group(group=None):
    ...
    if group and group in list(locals().keys()):
        return locals()[group]
...
class ImportResult(object):
    ...
    _counts = collections.OrderedDict(
        list(zip(_fields, [int(0) for x in range(len(_fields))])) )
    ...
    def _handle_status(self, key, value):
        ...
        if key == "IMPORTED":
            ...
            reasons = []
            for code, text in list(self._ok_reason.items()):
            ...
            for x in range(len(self.counts)):
                self.counts[list(self.counts.keys())[x]] = int(import_res[x])
        elif key == "KEYEXPIRED":
