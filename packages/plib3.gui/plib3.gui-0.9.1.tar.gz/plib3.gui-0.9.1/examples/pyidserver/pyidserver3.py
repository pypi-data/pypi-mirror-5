        ...
        def __iter__(self):
            try:
                ...
                while self._items:
                    ...
                    # to exit, as is done in PLIB3.IO
...
def run_idserver(arg, dns_only, protocol, portnum, fileobj):
    ...
    if ':' in arg:
        ...
        try:
            ...
            do_output(fileobj,
                      "URL includes invalid port {:d}, ignoring.".format(
                      portstr))
        else:
            if p != portnum:
                if portnum != 0:
    ...
    if dns_only:
        do_output(fileobj, "Doing DNS lookup on {} ...".format(arg))
    else:
        proto_msg = port_msg = ""
        if protocol == "":
    ...
    if ipaddr == arg:
        ...
        do_output(fileobj, "Domain name for {} is {}.".format(ipaddr, url))
    else:
        # URL was a domain name, normal lookup
        url = arg
        do_output(fileobj, "IP address for {} is {}.".format(url, ipaddr))
    
    if not dns_only:
        do_output(fileobj, "Contacting {}{}{} ...".format(arg, proto_msg, port_msg))
...
def run_main(arg, outfile=None, errfile=None,
    ...
    try:
        ...
        errfile.write("{}{}".format(str(sys.exc_info()[1]), os.linesep))
