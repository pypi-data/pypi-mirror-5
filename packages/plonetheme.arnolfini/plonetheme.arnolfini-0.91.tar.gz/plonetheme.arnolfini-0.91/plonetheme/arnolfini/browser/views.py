...
from plone.app.event.browser.event_listing import EventListing as AppEventListing
...
class CommonBrowserView(BrowserView):
    ...
    def getTagsAsClass(self, item):
        ...
        for tag in item.Subject:
            classes.append("tag_%s"%tag)
        
        return " ".join(classes)
    
    def getReviewState(self, item):
    ...
    def getEventFromOccurence(self, occ):
        context = occ.context
        if isinstance(context, Occurrence):
    ...
    def getFormattedEventDate(self, event):
        ...
        reg = queryUtility(IRegistry, context=getSite(), default=None)
        if reg:
        ...
        #start = event.start()
        #end = event.end()
        start = IEventAccessor(event).start
        end = IEventAccessor(event).end
        ...
        if start is None or end is None:
            ...
            samedate = start.strftime('%d - %m - %Y') == end.strftime('%d - %m - %Y')
        ...
        dates = "%s"%(samedate and (start is not None and start.strftime('%A %d %B %Y') or end.strftime('%A %d %B %Y')) or "%s to %s"%(start.strftime('%A %d %B %Y'), end.strftime('%A %d %B %Y')))
        
        openingHour = start is not None and start.strftime('%H:%M') or ""
        closingHour = end is not None and end.strftime('to %H:%M') or ""
...
class AppEventListingView(CommonBrowserView, AppEventListing):
    """
    plone.app.event event listing vie for the calendar
    """
