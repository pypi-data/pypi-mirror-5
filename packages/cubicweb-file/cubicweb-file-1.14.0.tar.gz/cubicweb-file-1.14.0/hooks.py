    ...
    def __call__(self):
        if 'data' in self.entity.cw_edited:
            ...
            if maxsize and self.entity.data_format.startswith('image/'):
                ...
                try:
                    self.entity.cw_edited['data'] = iimage.resize(maxsize)
                except UnResizeable:
            ...
            if self._cw.vreg.config['compute-sha1hex']:
                data = self.entity.cw_edited['data'].getvalue()
                self.entity.cw_edited['data_sha1hex'] = self.entity.compute_sha1hex(data)
