...
class Command(Assertion):
    ...
    def run(self):
        def decorate_out_with_logger(out, suffix, activate):
            ...
            return FileTee(out,
                           LoggerAsFile(out_logger.info, prefix))
...
class CommandExitsWith(Matcher):
    ...
    def describe_to(self, description):
        description.append_text("returncode to be %s" % (self.expected))
...
class RanForTime(Matcher):
    ...
    def describe_to(self, description):
        description.append_text("execution time to be %ss" % (self.expected))
