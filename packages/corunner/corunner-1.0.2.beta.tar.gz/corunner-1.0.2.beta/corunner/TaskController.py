...
class TaskController:
	...
	'''
	Check whether all nodes have finished, also marked those timed out nodes.
	'''
	def __isAllEnd(self):
		end = True
		now = time.time()
		...
		for snode in self.snodes:
			...
			if self.logger.isEnabledFor(logging.INFO):
				self.logger.info("NONE:%5d  STARTED:%5d  ALIVE:%5d  DEAD:%5d  END:%5d"%(nStatusNone, nStatusStarted, nStatusAlive, nStatusDead, nStatusEnd))
		return end
