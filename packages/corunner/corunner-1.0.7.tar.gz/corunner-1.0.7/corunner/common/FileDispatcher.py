...
class FileDispatcher:
	...
	def __copy(self, localPath, remotePath, targetIP, targetPort, mkdir, isOut):
		...
		if mkdir:
			if isOut:
				mkdirCommand = ["ssh", "-o", "ConnectTimeout=%d" % self.connectTimeout, "-p", str(targetPort), targetIP, "mkdir", "-p", remotePath]
				ShellUtil.execute(mkdirCommand)
			else:
				ShellUtil.execute(["mkdir", "-p", localPath])
		command = ["scp", "-o", "ConnectTimeout=%d" % self.connectTimeout, "-P", str(targetPort)]
		if os.path.isdir(localPath):
			command.append("-r")
		if isOut:
