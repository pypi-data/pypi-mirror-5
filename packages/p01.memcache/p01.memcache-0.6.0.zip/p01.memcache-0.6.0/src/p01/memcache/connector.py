...
##############################################################################
"""
$Id: connector.py 3783 2013-07-03 00:48:53Z roger.ineichen $
"""
...
import memcache
ERRORS = (IOError, RuntimeError, memcache.Client.MemcachedKeyError, socket.error)

try:
    import umemcache
    ERRORS += (umemcache.MemcachedError,)
except ImportError:
    pass


class Connector(object):
    """Dispatch calls to the used memcache client using a given factory
    
    This wrapper will reconnect on errors and supports a retry loop
    """

    def __init__(self, clientFactory, address, debug=0, timeout=4,
        retries=3, delay=2):
    ...
    def _create_connector(self):
        self._client = self.clientFactory(self.address,
    ...
    def set(self, bKey, data, lifetime):
        return self._client.set(bKey, data, lifetime)

    def get(self, bKey):
        return self._client.get(bKey)

    def delete(self, bKey, lifetime=0):
        return self._client.delete(bKey)
    ...
    def close(self):
        if self._client is not None and self._client.socket is not None:
...
class UltraConnector(Connector):
