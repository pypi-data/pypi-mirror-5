...
class MemcacheClient(object):
    ...
    _connectorFactory = p01.memcache.connector.Connector
    ...
    # connection setup
    timeout = FieldProperty(interfaces.IMemcacheClient['timeout'])
    delay = FieldProperty(interfaces.IMemcacheClient['delay'])
    retries = FieldProperty(interfaces.IMemcacheClient['retries'])
    lifetime = FieldProperty(interfaces.IMemcacheClient['lifetime'])

    # connection pool queue
    queuetime = FieldProperty(interfaces.IMemcacheClient['queuetime'])
    pooltime = FieldProperty(interfaces.IMemcacheClient['pooltime'])
    maxPoolSize = FieldProperty(interfaces.IMemcacheClient['maxPoolSize'])

    # server handling
    blacktime = FieldProperty(interfaces.IMemcacheClient['blacktime'])
    ...
    def _checkout_connection(self):
        ...
        while True:
            try:
                ts, client = self.pool.get(blocking, self.queuetime)
            except Queue.Empty:
                if blocking:
                    #timeout
