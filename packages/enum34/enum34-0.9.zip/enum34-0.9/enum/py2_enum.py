...
class EnumMeta(type):
    ...
    def __new__(metacls, cls, bases, classdict):
        ...
        enum_class._member_names = []               # names in random order
        enum_class._member_map = {}                 # name->value map
        ...
        for member_name in __order__:
            ...
            if not use_args or not args:
                '''
                enum_member = __new__.__new__(enum_class)
                '''
                ...
                '''
                enum_member = __new__.__new__(enum_class, *args)
                '''
        ...
        if not order_specified:
            enum_class._member_names = [
                e[0] for e in sorted(
                [(name, enum_class._member_map[name]) for name in enum_class._member_names],
                 key=lambda t: t[1]._value
                        )]
        ...
        if Enum is not None:
            ...
            if save_new:
                setattr(enum_class, '__member_new_', enum_class.__dict__['__new__'])
    ...
    def _find_new_(classdict, member_type, first_enum):
        ...
        # check all possibles for __member_new_ before falling back to
        # __new__
        for method in ('__member_new_', '__new__'):
