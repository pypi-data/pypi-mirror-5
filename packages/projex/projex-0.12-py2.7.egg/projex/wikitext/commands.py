...
def render(plain, urlHandler=None, templatePaths=None, options=None):
    """
    ...
    for i, line in enumerate(lines):
        ...
        for grp, url in EXPR_IMG.findall(line):
            ...
            opts = (url, last_word, urlsplit[1], url)
            templ = '<a href="%s"><img class="thumb" alt="%s" style="%s" '\
                    'src="%s"/></a>' % opts
        ...
        for grp, coloring in EXPR_COLOR.findall(line):
            ...
            templ = '<span style="color:%s">%s</span>' % (splt[0], splt[1])
            line = line.replace(grp, templ)
