...
class DocgenUrlHandler(wikitext.UrlHandler):
    ...
    def resolve( self, key ):
        ...
        # replace a static lookup
            ...
            return (key.replace('$STATIC', self.rootUrl() + '/_static'), True)
    ...
    def resolveImage( self, key ):
        ...
        :return     (<str> url, <bool> exists)
        """
        return self.resolve('$STATIC/img/%s' % key)
...
def generate(  module,
    ...
    for path, folders, files in os.walk(outpath):
        if ( '.svn' in path ):
            continue
            
        arcpath = path.replace(outpath, '')
...
def generateUserDocs(path,
    ...
    for entry in sorted(os.listdir(path)):
        ...
        # add wiki pages to the system
            ...
            wiki_file.close()
                        title    = text.capitalizeWords(entry.split('.')[0])            contents = wikitext.render(wiki_contents, url_handler)            
            crumbs = []
    ...
    if ( not has_index_file ):
        ...
        wiki_templ = wikitext.render(wiki_txt, url_handler)
