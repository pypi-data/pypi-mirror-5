...
def displayName( date, 
    ...
    date_name = named(date, options)
    
    if ( date_name != Names.Sometime ):
        output = projex.text.pretty(Names[date_name])
    else:
        output = date.strftime(format)
    
    if isinstance(date, datetime.datetime) and \
       date_name & (Names.Today | Names.Tomorrow | Names.Yesterday):
        output += ' @ ' + date.strftime('%H:%M %p').strip('0')
