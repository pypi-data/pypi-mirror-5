...
class TestServeStaticFile():
    ...
    def setup(self):
        ...
        self.mtime = os.path.getmtime(self.tmpname)
        os.close(fh)
    ...
    def test_sets_last_modified_header(self):
        with FrescoApp().requestcontext() as c:
            response = serve_static_file(c.request, self.tmpname)
    ...
    def test_full_response_when_if_modified_before_mtime(self):
        with FrescoApp().requestcontext(**self.make_ims(0)) as c:
            response = serve_static_file(c.request, self.tmpname)
            response.content.close()
    ...
    def test_304_when_if_modified_after_mtime(self):
        with FrescoApp().requestcontext(**self.make_ims(self.mtime + 1)) as c:
            response = serve_static_file(c.request, self.tmpname)
    ...
    def test_filewrapper_used_if_present(self):
        with FrescoApp().requestcontext() as c:
            ...
            serve_static_file(c.request, self.tmpname)
