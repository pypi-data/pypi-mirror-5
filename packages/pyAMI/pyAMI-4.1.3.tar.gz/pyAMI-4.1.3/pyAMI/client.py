...
try:
    ...
    try:
        ...
        pass
        # FIXME: this catch is overly broad
        # should log a warning with the full message if
        # doesn't match "ImportError: No module named lxml"
...
class AMIResult(object):
    ...
    def output(self, xslt='text'):
        ...
        helps_tags = self.dom.getElementsByTagName('help')

        if len(helps_tags) > 0:
            if len(helps_tags[0].childNodes) > 0:
                return helps_tags[0].childNodes[0].nodeValue
            else:
                return 'No help implemented for this command' 
        ...
        if xslt == 'xml':
            ...
            try:
                ...
                raise ValueError("could not open '%s'" % XSLT[xslt])
            else:
                xslt_content = f.read()
                f.close()

                if USE_TINY_XSLT:
                    return tiny_xslt.transform(xslt_content, self.dom.toxml())
                    ...
                    raise ImportError("tiny_xslt or lxml must be installed to "
                                      "perform XSLT transformations")

        elif xslt in XSLT:
            xslt_url = urlparse.urljoin(endpoint.get_XSL_URL(), XSLT[xslt])

            xslt_content = urllib2.urlopen(xslt_url).read()
...
class AMIClient(object):
    ...
    def execute(self, args):
        args_without_help = args#[arg for arg in args if arg != 'help']        

        args_without_help = self.set_user_credentials(args_without_help)
        ...
        if self.verbose:
            ...
            print "query:"
            print
            print ' '.join(args_without_help)
        if len(args_without_help) == 1 and args_without_help[0] == "UploadProxy":
            self.upload_proxy()
            f = StringIO(
            ...
            args_without_help = self._parse_args(args_without_help)
            if 'command' not in args_without_help:
                raise AMI_Error("You must provide a command")
            args_without_help = self.config.include(args_without_help)
            request = execAMICommand_arrayRequest()
            request._args = []
            for name, value in args_without_help.items():
