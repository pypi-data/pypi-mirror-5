...
class XMLQuoter(Quoter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # that flummox the normal superclass instantiation
        opts = self.options = self.__class__.options.push(kwargs)
        self._flatargs(args)
        self._register_name(opts.name)
    ...
    def __getattr__(self, name):
        try:
            ...
            return XMLQuoter(tag=name, name=name)
...
# TODO: When a quoter is called without clear contents but with other attributes, return an object
# implementing those attributes, rather than barfing because no content provided (ie another form
# of auto-generation of quoting objects)
