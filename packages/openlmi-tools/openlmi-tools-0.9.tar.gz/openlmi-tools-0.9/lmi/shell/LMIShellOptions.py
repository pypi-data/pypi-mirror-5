...
class LMIShellOptionsHelpWithVersionFormatter(argparse.HelpFormatter):
    ...
    def _format_usage(self, usage, actions, groups, prefix):
        """
        Returns a string with pre-formatted LMIShell usage. This method prepends
...
class LMIShellOptions(object):
    """
    Class representing a LMIShell command line options. In the constructor, all command
    line options before a script name are passed to the LMIShell. First position argument
    belongs to the script and the rest of command line options is passed to the script run
    under the LMIShell.
    ...
    def interactive(self):
        """
        :returns: flag, which tells if the LMIShell should be initially run in the
            interactive mode
        :rtype: bool
        """
    ...
    def interact(self):
        """
        :returns: flag, which indicates, if the LMIShell should enter an interactive mode,
            after executing a provided script. The behavior is similar to python
            interpreter
        :rtype: bool
        """
