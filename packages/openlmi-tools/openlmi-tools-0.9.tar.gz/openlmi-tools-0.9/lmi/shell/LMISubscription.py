...
class LMISubscription(LMIBaseObject):
    ...
    :param bool permanent: indicates, if the indication should be deleted on the
        LMIShell's quit
    """
    ...
    def delete(self):
        ...
        :py:attr:`.LMISubscription._cim_filter_tpl` contains a flag, that the filter
        object was created temporarily, it will be deleted by this call. If
        :py:attr:`.LMISubscription._cim_handler_tlp` contains an flag, that the handler
        object was created temporarily, it will be deleted as well.

        This is called from :py:class:`LMIConnection` object, which holds an internal list
        of all subscribed indications by the LMIShell (if not created by hand).
        """
