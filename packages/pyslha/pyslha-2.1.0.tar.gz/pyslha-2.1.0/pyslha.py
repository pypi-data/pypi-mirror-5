...
Please cite PySLHA via the URL http://insectnation.org/projects/pyslha if it is
useful to you in the production of an academic paper. A PySLHA publication will
be provided at some point... watch this space!
    ...
    def __repr__(self):
        ...
        def _format_kv(k, v):
            if type(k) is not tuple:
                ...
                s = ",".join("%r" % subindex for subindex in k)
            s += " : %s" % repr(v)
            return s
        s += " { " + "; ".join(_format_kv(k, v) for k, v in self.items()) + " }"
        return s
    ...
    for pid, particle in decays.iteritems():
        ...
        for d in particle.decays:
            if d.br > 0.0 or not ignorenobr:
                products_str = sep.join(map(str, d.ids))
