...
#        self.emis_grid = self.atom.getEmissivity(self.tem, self.den)
    ...
    def getGrid(self, lev_i=None, lev_j=None, wave= -1, to_eval=None):
        ...
        """
        if wave != -1 :
            lev_i, lev_j = self.atom._Transition(wave)[0:2]
    ...
    def plotImage(self, to_eval=None, lev_i1=None, lev_j1=None, lev_i2=None, lev_j2=None, 
        ...
        if to_eval is None:
            if wave1 != -1 :
                lev_i1, lev_j1 = self.atom._Transition(wave1)[0:2]
            if wave2 != -1 :
                lev_i2, lev_j2 = self.atom._Transition(wave2)[0:2]
    ...
    def plotContours(self, to_eval, low_level=None, high_level=None, n_levels=20,
        ...
        Parameters:
            ...
            - linestyles:    used for the contour lines (default is continuous line: '-')
            - clabels:    Boolean, control if line labels are printed (True by default)
            - log_levels:    if True (default), log of the line ratio is used.
            - title:        plot title
            - **kwargs:        send to plt.contour
        """ 
        if not pn.config.INSTALLED['plt']:
            pn.log_.error('Matplotlib not available', calling=self.calling)
            return None
...
def getEmisGridDict(elem_list=None, spec_list=None, atom_list=None, restore=True, pypic_path=None,
    ...
    """
    Return a dictionary of EmisGrid objects refereed by their atom string (e.g. 'O3').
    ...
    if restore or save:
        if pypic_path is None:
            pn.log_.error('Unable to find pypic_path directory', calling = calling)
            return None
        if not os.path.isdir(pypic_path):
            if createPypicsDir:
                try:
                    os.makedirs(pypic_path)
                except:
                    pn.log_.error('Unable to create {0} directory', calling = calling)
                    return None
                pn.log_.message('Creating the {0} directory'.format(pypic_path), calling=calling)
            else:
                pn.log_.error('Directory {0} does not exist, you may use the createPypicsDir=True option'.format(pypic_path), 
                              calling=calling)
                return None
    emis_grids = {}
    atoms = []
