...
import os
import pyneb as pn
from pyneb.utils.misc import int_to_roman, strExtract, parseAtom, quiet_divide
from pyneb.utils import chebyshev
from pyneb.utils.init import LINE_LABEL_LIST, BLEND_LIST, SPEC_LIST
from pyneb.utils.physics import sym2name, gsDict, gsLevelDict, gsFromAtom
...
def kappa_correct(delta_E, tem, kappa):
    return np.ones_like(tem)
    ...
    def printSources(self):
        ...
        for com in self.comments:
            if 'SOURCE' in com:
                ...
                sources[number] = self.coments[com]
            if 'NOTE' in com:
                number = com.split('NOTE')[1]
                notes[number] = self.coments[com]
        for key in sources:
            try:
                print('{0} : {1} : {2}'.format(self.atom, notes[key], sources[key])) 
            except:
                print('{0} : {1}'.format(self.atom, sources[key]))
    ...
    def getA(self, lev_i= -1, lev_j= -1, wave= -1):
        ...
        if (lev_i == -1):
            if (lev_j == -1):
                ...
                return (self._A[lev_j - 1])
        else:
            return (self._A[lev_j - 1, lev_i - 1])       
    ...
    def getOmega(self, tem, lev_i= -1, lev_j= -1):
        ...
        if (lev_i == -1) and (lev_j == -1):
            ...
            if np.ndim(tem_eval) > 0:
                ...
                tem_eval[left] = TemArray[0]
                right = (tem_eval > TemArray[-1])
                tem_eval[right] = TemArray[-1]
            else:
...
class CollDataAscii(object):

    def __init__(self, elem=None, spec=None, atom=None, OmegaInterp='Linear'):
        self.log_ = pn.log_
    ...
    def _loadAscii(self):
        ...
        if 'N_LEVELS' in self.comments:
            ...
            self.NLevels = np.max(self.lev_j)
        ...
        if 'SPECTRUM' in self.comments:
            if int(self.comments['SPECTRUM']) != self.spec:
                pn.log_.error('The spectrum I read in the file {0} is {1}, but you are requesting {2}'.format(self.collFitsFile, self.comments['SPECTRUM'],
                                                                                                    self.spec), calling=self.calling)
        if 'ATOM' in self.comments:
            if self.comments['ATOM'] != sym2name[self.elem]:
                pn.log_.error('The element name I read in the file {0} is {1}, but I was expecting {2}. Check the keyword ATOM'.format(self.collFitsFile, self.comments['ATOM'],
                                                                                                    sym2name[self.elem]), calling=self.calling)
...
class Atom(object):
    ...
    def __init__(self, elem=None, spec=None, atom=None, OmegaInterp='Cheb'):
        ...
        self.atomFileType = pn.atomicData.getDataFile(self.atom, data_type='atom').split('.')[-1]
        if self.atomFileType == 'fits':
            self.AtomData = AtomDataFits(elem=elem, spec=spec, atom=atom)
        elif self.atomFileType == 'dat':
            self.AtomData = AtomDataAscii(elem=elem, spec=spec, atom=atom)
        ...
        self.collFileType = pn.atomicData.getDataFile(self.atom, data_type='coll').split('.')[-1]
        if self.collFileType == 'fits':
            ...
            self.CollData = CollDataAscii(elem=elem, spec=spec, atom=atom, OmegaInterp='Linear')
        try:
    ...
    def getOmega(self, tem, lev_i= -1, lev_j= -1):
        """
        If self.gamma is not None, a correction to getOmega is applied using the relation from Mendoza & Bautista, 2014.
        """

        kappa = pn.config.kappa 
        if kappa is None:
            return self.CollData.getOmega(tem, lev_i, lev_j)
        ...
        if (lev_i == -1) and (lev_j == -1):
            ...
            correc =  kappa_correct(delta_E, tem, kappa)
...
#        print 'maxError on ratio %f' % maxError
    ...
    def plotGrotrian(self, tem=1e4, den=1e2, thresh_int=1e-3, unit='eV'):
        """
        ...
        for i_multi in np.arange(len(multiplets)):
            ...
            for i_level in multiplets[i_multi]:
                latex_sw = gsLevelDict[self.gs][i_level]
            ...
            if (stat_weights != [self.getStatWeight()[k] for k in multiplets[i_multi]]):
                ...
                pn.log_.warn(to_print.format(i_multi + 1, self.atom, multiplets[i_multi], stat_weights, [self.getStatWeight()[k] for k in multiplets[i_multi]]), calling=self.calling + '.plot')
        ...
        for i_multi in np.arange(len(multiplets)):
            ...
            for i in np.arange(len(multiplets[i_multi])):
                ...
                plt.plot([x_0, 1.5 * x_0, 1.6 * x_0, x_0 + x_span], [y, y, energies[multiplets[i_multi][i]], energies[multiplets[i_multi][i]]], lw = 1.2, color = color_list[np.mod(multiplets[i_multi][i], len(color_list))])
                plt.text(x_span + x_0 + dx, y, '{0:7.4f}'.format(energies[multiplets[i_multi][i]]), size='small', horizontalalignment='left', verticalalignment='center')
                try:
                    plt.text(dx, y, gsLevelDict[self.gs][i_tot], size='medium', verticalalignment='center')
                    i_tot += 1
                except:
                    pass
                shift += 1
        ...
        for j in np.arange(self.NLevels - 1) + 2:
            for i in np.arange(j - 1) + 1:
                ...
                if emis > (emis_max * thresh_int):
                    N_seg = 1000. #number of segments to draw emission line, to make it looh smooth
