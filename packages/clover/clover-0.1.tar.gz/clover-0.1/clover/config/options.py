...
class Options(object):
    ...
    class externs(Option):
        """ Files that contain externs that should be included in the compilation. 
            ...
            (This article from the Closure Tools blog explains why adding unneccessary externs can inhibit the 
            Compiler's ability to rename properties.)
    ...
    class print_input_delimeter(Option):
        """ Equivalent to the command-line --formatting=PRINT_INPUT_DELIMITER flag for the Closure Compiler. Defaults
         to false.
        """
    ...
    class test_template(Option):
        """ A jinja template to use for jsunit tests. The template will receive the following variables:
        
            baseJsUrl - the url to closures base.js
            testJsUrl - the url to the unit test file
            
            
            The default template is:

            {namespace org.clover}
            ...
            */
            ...
            {/template}
            The value of the test-template option is not the content of your template, but the path to the .soy file 
            that defines your template. Your template must be named org.clover.test, just like the default template. 
