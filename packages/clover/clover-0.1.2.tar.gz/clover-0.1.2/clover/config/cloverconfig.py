...
class CloverConfig(DictConfig):
    ...
    def get_build_manifest(self):
        ...
        if self._build_manifest is None:
            ...
            manifest_all_sources = self._build_manifest.get_all_sourcefiles()
            expired = []
            for source in manifest_all_sources:
                if source.is_cache_expired():
                    expired.append(source)

            if expired:
                logger.debug('Expired sourcefiles: %r', pprint.pformat(expired))
