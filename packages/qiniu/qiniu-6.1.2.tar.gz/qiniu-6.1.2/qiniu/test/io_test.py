...
class Test_get_file_crc32(unittest.TestCase):
	...
	def test_get_un_utf8(self):
		key = u"我很好".encode("utf-8")
		#print "origin: %s" % key
		data = urllib.urlopen('http://cheneya.qiniudn.com/hello_jpg')
		ret, err = io.put(policy.token(), key, data)
		assert err is None
		...
		#base_url = rs.make_base_url("cheneya.qiniudn.com", u"我很好".encode("utf-8"))
		base_url = rs.make_base_url("cheneya.qiniudn.com", ret['key'])
		data1 = urllib.urlopen(rs.GetPolicy().make_request(base_url))
		print data1.read()
		assert data1 == data
