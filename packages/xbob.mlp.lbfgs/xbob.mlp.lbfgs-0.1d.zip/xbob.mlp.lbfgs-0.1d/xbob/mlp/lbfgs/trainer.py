...
class Trainer(bob.trainer.overload.MLPBaseTrainer):
  ...
  def __init__(self, grad_norm, max_eval=0):
    ...
    """
    bob.trainer.overload.MLPBaseTrainer.__init__(self, 1)
  ...
  def dj(self, theta, machine, X, y):
    ...
    **all** :math:`\theta`'s. Use the training dataset.
    ...
    for l in range(Ne-1):
      b.append(self.error[l] * activation_prime(self.output[l]))
    b.append(self.error[-1] / X.shape[0] * activation_prime(self.output[-1]))
