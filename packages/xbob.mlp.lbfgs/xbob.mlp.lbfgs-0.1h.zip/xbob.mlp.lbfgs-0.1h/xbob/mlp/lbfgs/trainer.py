...
class Trainer(bob.trainer.overload.MLPBaseTrainer):
  ...
  def __init__(self, grad_norm, max_eval=0):
    ...
    """
    bob.trainer.overload.MLPBaseTrainer.__init__(self, 1, bob.trainer.SquareError())
  ...
  def dj(self, theta, machine, X, y):
    ...
    **all** :math:`\theta`'s. Use the training dataset.
    ...
    Ne = len(self.error)
    b = []
    # Note that delta <-> self.error
    for l in range(Ne-1):
      ar = numpy.ndarray(shape=self.error[l].shape, dtype=numpy.float64)
      for i in range(ar.shape[0]):
        for j in range(ar.shape[1]):
          ar[i,j] = self.error[l][i,j] * machine.hidden_activation.f_prime(self.output[l][i,j])
      b.append(ar)
      #b.append(self.error[l] * machine.hidden_activation.f_prime(self.output[l]))
    ar = numpy.ndarray(shape=self.error[-1].shape, dtype=numpy.float64)
    for i in range(ar.shape[0]):
      for j in range(ar.shape[1]):
        ar[i,j] = self.error[-1][i,j] / X.shape[0] * machine.output_activation.f_prime_from_f(self.output[-1][i,j])
    b.append(ar)
    #b.append(self.error[-1] / X.shape[0] * machine.output_activation.f_prime(self.output[-1]))

    d = []
    arg_a0 = numpy.vstack((numpy.ones(X.shape[0]), X.T))
    d.append((numpy.dot(arg_a0, b[0]) / y.shape[0]).flatten())
    for l in range(Ne-1):
      arg_a = numpy.vstack((numpy.ones(self.output[l].shape[0]), self.output[l].T))
      d.append((numpy.dot(arg_a, b[l+1]) / y.shape[0]).flatten())
