...
class PickledObject(str):
    ...
    object or not (if the object is an instance of this class then it
    must [well, should] be a pickled one).
...
class PickledObjectField(models.Field):
        ...
        *not* try to pickle and encode it.
        ...
        if self.has_default():
            if callable(self.default):
                return self.default()
            return self.default
        # If the field doesn't have a default,
        # then we punt to models.Field.
    ...
    def to_python(self, value):
        ...
        if value is not None:
            try:
                ...
                # If the value is a definite pickle; and an error
                # is raised in de-pickling it should be allowed
                # to propogate.
    ...
    def get_db_prep_value(self, value):
        ...
        if value is not None and not isinstance(value, PickledObject):
            ...
            # Alternatively, we could have just registered PickledObject
            # with the psycopg marshaller (telling it to store it like
            # it would a string), but since both of these methods result
            # in the same value being stored, doing things this way
            # is much easier.
    ...
    def get_db_prep_lookup(self, lookup_type, value, connection=None, prepared=False):
        ...
        # The Field model already calls get_db_prep_value before doing
        # the actual lookup, so all we need to do is limit the
        # lookup types.
        try:
