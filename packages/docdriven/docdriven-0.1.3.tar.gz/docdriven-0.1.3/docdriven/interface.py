...
def initialize(doc_filepath, print_feed=False):
    if print_feed:
        print '@@@@@@@@@@@@@@@@@@@@@@@@@ DDD FEED @@@@@@@@@@@@@@@@@@@@@@@@@'
    global driven_down_doc
    driven_down_doc = DrivenDownDoc()
    root_dir, basename = os.path.split(doc_filepath)
    feed_file(driven_down_doc, doc_filepath, root_dir, print_feed, rcrsn_lvl=0)
    if print_feed:
        print '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
...
def feed_file(driven_down_doc, filepath, root_dir, print_feed, rcrsn_lvl=0):
    ...
    for line in feeder_file:
        ...
        if match:
            ...
            if match.groups()[3] is not None:
                feed_line(driven_down_doc, ('#### '+match.groups()[3]+'\n'), from_filekey, print_feed, rcrsn_lvl+1)
                if match.groups()[3] == 'runtime':
                    feed_line(driven_down_doc, ('~'*42)+'  python\n', from_filekey, print_feed, rcrsn_lvl+1)
                else:
                    feed_line(driven_down_doc, ('~'*44)+'  json\n', from_filekey, print_feed, rcrsn_lvl+1)
                feed_file(driven_down_doc, include_filepath, root_dir, print_feed, rcrsn_lvl+1)
                feed_line(driven_down_doc, ('~'*50)+'\n', from_filekey, print_feed, rcrsn_lvl+1)
            else:
                feed_file(driven_down_doc, include_filepath, root_dir, print_feed, rcrsn_lvl+1)
        else:
            feed_line(driven_down_doc, line, from_filekey, print_feed, rcrsn_lvl)
    feeder_file.close()

def feed_line(driven_down_doc, line, from_filekey, print_feed, rcrsn_lvl):
    if print_feed:
        print ' '*(4*rcrsn_lvl)+'| '+line,
