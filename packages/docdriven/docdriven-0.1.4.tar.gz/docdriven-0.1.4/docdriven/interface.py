...
class AbstractDocUnit(object):
    ...
    def as_string(self, response=True, status=True):
        ...
        if len(self.sections[RUNTIME]) > 2:
            doc_string += '\n#### runtime'
            doc_string += '\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  python\n'
            doc_string += self.pretty_json(self.sections[RUNTIME])
            doc_string += '\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n'
        else:
            doc_string += '\n#### runtime/\n'
        ...
        if len(self.sections[STATUS]) > 0:
            doc_string += '\n#### status'
            doc_string += '\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  json\n'
            doc_string += self.sections[STATUS]
            doc_string += '\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n'
        else:
            doc_string += '\n#### status/\n'
...
class ChapterDocUnit(AbstractDocUnit):
    ...
    def print_nav_to_file(self, parent_dir, response=True):
        chapter_filepath = os.path.join(parent_dir, 'chapter/'+self.get_keypath(section=CHAPTER)+'.ddd')
        nav_out = open(chapter_filepath, 'w')
        nav_out.write('# '+self.title+'\n')
        self.write_nav_files(os.path.join(parent_dir, 'chapter/'), nav_out)
        for route in self.route_list:
            route.print_nav_to_file(os.path.join(parent_dir, 'chapter/'), nav_out)
        nav_out.close()
...
class RouteDocUnit(AbstractDocUnit):
    ...
    def print_nav_to_file(self, chapter_dir, nav_outfile):
        nav_outfile.write('## '+self.title+'\n')
        self.write_nav_files(chapter_dir, nav_outfile)
...
class DrivenDownDoc(object):
    ...
    def print_nav_to_file(self, parent_dir, response=True):
        nav_path = os.path.join(parent_dir, title2filename(self.title)+'.nav.ddd')
...
def initialize(doc_filepath, debug_output=None):
    ...
    if print_feed:
        print '@@@@@@@@@@@@@@@@@@@@@@@@@ DDD FEED @@@@@@@@@@@@@@@@@@@@@@@@@'
    global driven_down_doc
    driven_down_doc = DrivenDownDoc()
    root_dir, basename = os.path.split(doc_filepath)
    feed_file(driven_down_doc, doc_filepath, root_dir, line_feed, file_feed, print_feed, rcrsn_lvl=0)
    if print_feed:
        print '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
...
def feed_file(driven_down_doc, filepath, root_dir, line_feed, file_feed, print_feed, rcrsn_lvl=0):
    if file_feed:
        print 'docdriven: '+filepath
    filename = os.path.basename(filepath)
    from_filekey = filename[:filename.rfind('.')] # last index of .
    feeder_file = open(filepath, 'r')
    for line in feeder_file:
        ...
        if match:
            ...
            if not path.endswith(('.json', '.py', '.ddd')):
                raise 'cannot import unrecognized filetype: '+path
            if not path.startswith('chapter/'):
                path = 'chapter/'+path
            include_filepath = os.path.join(root_dir, path)
            if match.groups()[3] is not None:
                feed_line(driven_down_doc, ('#### '+match.groups()[3]+'\n'), from_filekey, line_feed, print_feed, rcrsn_lvl+1)
                if match.groups()[3] == 'runtime':
                    feed_line(driven_down_doc, ('~'*42)+'  python\n', from_filekey, line_feed, print_feed, rcrsn_lvl+1)
                else:
                    feed_line(driven_down_doc, ('~'*44)+'  json\n', from_filekey, line_feed, print_feed, rcrsn_lvl+1)
                feed_file(driven_down_doc, include_filepath, root_dir, line_feed, file_feed, print_feed, rcrsn_lvl+1)
                feed_line(driven_down_doc, ('~'*50)+'\n', from_filekey, line_feed, print_feed, rcrsn_lvl+1)
            else:
                feed_file(driven_down_doc, include_filepath, root_dir, line_feed, file_feed, print_feed, rcrsn_lvl+1)
        else:
            feed_line(driven_down_doc, line, from_filekey, line_feed, print_feed, rcrsn_lvl)
    feeder_file.close()

def feed_line(driven_down_doc, line, from_filekey, line_feed, print_feed, rcrsn_lvl):
    if print_feed:
        print ' '*(4*rcrsn_lvl)+'| '+line
    if line_feed:
        print 'docdriven:   '+line
    driven_down_doc.feed_line(line, from_filekey)

def print_to_files(parent_dirpath, ddd_all_response=True):
    print 'parent_dirpath='+str(parent_dirpath)
    global driven_down_doc
    filepath = os.path.join(parent_dirpath, title2filename(driven_down_doc.title))
    report_filepath = os.path.join(parent_dirpath, 'reports/')
    report_filepath = os.path.join(report_filepath, title2filename(driven_down_doc.title))

    ddd_all_file = open(filepath+'.all.ddd', 'w')
    driven_down_doc.print_all_to_file(ddd_all_file, ddd_all_response)
    ddd_all_file.close()

    driven_down_doc.print_nav_to_file(parent_dirpath, response=ddd_all_response)
    ...
    driven_down_doc.print_status_report(status_report)
    status_report.close()
    ...
    driven_down_doc.print_ch_status_report(ch_status_file)
    ch_status_file.close()


def get_json_doc_structure():
    global driven_down_doc
    return driven_down_doc.get_json_doc_structure()

if __name__ == '__main__':
    routes_filepath = '/Users/Jon/dev/iit/nebula1/cwautopro/routes.ddd'
