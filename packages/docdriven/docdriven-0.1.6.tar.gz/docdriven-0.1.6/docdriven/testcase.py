...
class DocDrivenTestCase(unittest.TestCase):
    ...
    def __init__(self, driven_doc):
        self.driven_doc = driven_doc
    ...
    def setUpClass(cls):
        ...
        if cls.dd.introduction:
            cls.global_config = json.loads(
                cls.dd.introduction.sections[interface.JS_CONFIG])
            cls.global_reply = json.loads(
                cls.dd.introduction.sections[interface.JS_REPLY])
            cls.run_route_list(cls.dd.introduction.route_list, cls.global_config, cls.global_reply)

    @classmethod
    def tearDownClass(cls):
        if cls.dd.conclusion:
            conc_config = json.loads(
                cls.dd.conclusion.sections[interface.JS_CONFIG])
            conc_reply = json.loads(
                cls.dd.conclusion.sections[interface.JS_REPLY])
    ...
    def build_test_method(cls, chapter):
        ...
        def test_method(self):
            ...
            dd_TestCase.run_route_list(chp.route_list, ch_config, ch_reply)
...
default_base_url='http://216.224.141.220:5438'
default_http_headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
def init_DocDrivenTestCase(driven_doc, base_url=default_base_url, http_headers=default_http_headers):
