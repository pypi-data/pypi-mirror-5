...
def drive_dd_file(dd_file_full_path):
    ...
    docdriven_interface.print_to_files(dd_all_response=True)
    ...
    with open(dd_filename) as driven_docs_file:
        ...
        for line in driven_docs_file:
            ...
            if os.path.isdir(filepath):
                ...
                # log.debug('got down here')
    # except IOError:
    #     raise IO
    #     # this means we could not successfully open a DRIVEN_DOCS.txt file in that directory
    #     files = os.listdir(root_dir)
    #     log.debug('DRIVEN_DOCS.txt file not located in '+root_dir)
    #     log.debug('looking for *.dd files in '+root_dir)
    #     for f in files:
    #         if os.path.isdir(f):
    #             continue
    #         else:
    #             if f.endswith('.dd'):
    #                 log.debug('found '+str(f))
    #                 drive_dd_file(f)
