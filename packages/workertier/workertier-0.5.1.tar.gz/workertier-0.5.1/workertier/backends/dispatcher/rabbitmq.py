...
class RabbitMQConnection(Connection):
    ...
    def _consume(self, consumer):
        ...
        self._channel.basic.start_consumer(queue=self.queue, consumer=consumer, cb=cb)
    ...
    def _consume(self):
        while 1:
            with self._acquire_connection() as conn:
                conn.start_consumer(self._messages_queue.put)  #TODO: This should indicate a failure on connect
                while not conn.broken:
                    gevent.sleep()
                logger.warning("Consumer connection was severed")
