...
class PySCeS_Connector(PyscesInputFileParser):
  ...
  def BuildReactions(self):
    ...
    for r_id in self.Mod.__reactions__:
        temp_reactant = []
        temp_product  = []      
        rate_eq = reaction_info[r_id]['RateEq']           # Rate eq. info      
        L_parms   = reaction_info[r_id]['Params']         # Parm from rate eq.         
        ...
        self.rate_eqs.append(reaction_info[r_id])
        init_reagents = sorted(reaction_info[r_id]['Reagents'])# Remove fixed species      
