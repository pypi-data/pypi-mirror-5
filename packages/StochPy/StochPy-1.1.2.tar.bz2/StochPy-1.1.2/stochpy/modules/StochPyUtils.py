...
def doSequentialSim(smod,n_generations,cell_division_times):
    ...
    for i in xrange(1,n_generations):     
        ### divide each species between two daughter cells ###
        for j in xrange(0,len(smod.data_stochsim.species_labels)): 
...
class Utils():
    ...
    def doExample5(self):
        """ Protein turnover example with and without cell division (available at http://stompy.sourceforge.net/examples.html) """
        smod = _stochpy_.SSA(File='CellDivision.psc',dir=_stochpy_.model_dir) # start SSA module with CellDivision model
