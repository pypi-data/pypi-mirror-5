...
class OWMDS(OWWidget):
    settingsList = [
        "graph.PointSize", "graph.proportionGraphed", "graph.ColorAttr",
        "graph.SizeAttr", "graph.ShapeAttr", "graph.NameAttr",
        "graph.ShowStress", "graph.NumStressLines", "graph.ShowName",
        "graph.differentWidths", "graph.stressByTransparency",
        "graph.useAntialiasing" "StressFunc", "applyLSMT",
        "toolbarSelection", "autoSendSelection", "selectionOptions",
        "computeStress", "RefreshMode"
    ]

    contextHandlers = {
        "": DomainContextHandler(
             "",
             [ContextField("graph.ColorAttr", DomainContextHandler.Optional),
    ...
    def setExampleTable(self, data):
        ...
        discAttributes = filter(
            lambda a: a.varType == orange.VarTypes.Discrete,
            attributes
        )
        contAttributes = filter(
            lambda a: a.varType == orange.VarTypes.Continuous,
            attributes
        )
        ...
        self.selectedInput = map(
            lambda d: selectedInput and (d in selectedInput) or not selectedInput,
            data
        )
...
#  from the widget. If somebody has time to fix orngMDS first, he's welcome.
    ...
    def testStart(self):
        ...
        # The name mangling for orange2.5 does not seem to work for
        # orangeom.MDS so I set this explicitly
        self.mds.progress_callback = self.callback

        self.mds.mds.optimize(self.maxIterations,
    ...
    def sendReport(self):
        ...
        if self.graph.ColorAttr or self.graph.stressBySize or \
                self.graph.SizeAttr or self.graph.ShapeAttr or \
                self.graph.NameAttr or self.graph.ShowStress:
            self.reportSettings(
                "Visual settings",
...
class MDSGraph(OWGraph):
    ...
    def updateDistanceLines(self):
        ...
        # draw maximum of 1000 closest pairs
        np = min(int(N * (N - 1) / 2. * self.proportionGraphed / 100.), 1000)
        ...
        if maxdist != mindist:
            ...
            for dist, i, j in hdist:
                ...
                c = self.addCurve("A", black, black, 0, QwtPlotCurve.Lines,
                                  xData=[pti[0], ptj[0]], yData=[pti[1], ptj[1]],
                                  lineWidth=max(1, (maxdist - dist) ** 2 * k))
                c.setZ(10)
    ...
    def setPoints(self):
        if not self.mds:
            return
        if self.ShapeAttr == 0 and self.SizeAttr == 0 and \
                self.NameAttr == 0 and not self.stressBySize and \
                not self.stressByTransparency:
            ...
            for color in set:
                X = [self.mds.points[i][0] for i in dict[QColor(color).getHsv()]
                     if self.showFilled[i]]
                Y = [self.mds.points[i][1] for i in dict[QColor(color).getHsv()]
                     if self.showFilled[i]]
                c = self.addCurve("A", color, color, self.PointSize,
                                  symbol=QwtSymbol.Ellipse, xData=X, yData=Y)
                c.setZ(100)

                X = [self.mds.points[i][0] for i in dict[QColor(color).getHsv()]
                     if not self.showFilled[i]]
                Y = [self.mds.points[i][1] for i in dict[QColor(color).getHsv()]
                     if not self.showFilled[i]]
                c = self.addCurve("A", color, color, self.PointSize,
                                  symbol=QwtSymbol.Ellipse, xData=X, yData=Y,
                                  showFilledSymbols=False)
                c.setZ(100)
        else:
            ...
            for i in range(len(self.colors)):
                ...
                c = self.addCurve("a", cq, self.colors[i][self.ColorAttr],
                                  (max(5, ks * (1 + maxs - stresses[i]))
                                   if self.stressBySize else
                                   self.sizes[i][self.SizeAttr] * 1.0 / 5 * self.PointSize),
                                  symbol=self.shapes[i][self.ShapeAttr],
                                  xData=[self.mds.points[i][0]],
                                  yData=[self.mds.points[i][1]],
                                  showFilledSymbols=self.showFilled[i])

                c.setZ(100)
                if self.NameAttr != 0:
                    c = self.addMarker(self.names[i][self.NameAttr],
                                       self.mds.points[i][0],
                                       self.mds.points[i][1],
                                       Qt.AlignBottom)
                    c.setZ(100)
        ...
        if len(self.mds.points) > 0:
            ...
            span_x = max_x - min_x
            span_y = max_y - min_y
            self.setAxisScale(QwtPlot.xBottom,
                              min_x - 0.05 * span_x, max_x + 0.05 * span_x)
            self.setAxisScale(QwtPlot.yLeft,
                              min_y - 0.05 * span_y, max_y + 0.05 * span_y)
