...
def add_alignments(pan_file, alis, output, min_len=25, self_alis=True):
	...
	try:
		...
		return
	sys.stderr.write('...pangenome loaded\n')
	for ali_file in alis:
		sys.stderr.write('#\n')
		try:
			pangenome = parse_alis(ali_file, pangenome, min_len, self_alis)
...
def singletons(pan_file, ref_chrom, ref_genome, min_len, genomes=[], chroms=[], exclude_genomes=[], exclude_chromosomes=[], outfolder='', output=True, bed_file=''):
	...
	if ref_chrom:
		...
		open(bed_file, 'w').close()
		get_singletons(pangenome, ref_chrom, min_len, genomes, chroms, exclude_genomes, exclude_chromososomes, outfolder, output, bed_file)
	elif ref_genome:
...
#sequence(args.panfile, args.CHROM, args.start, args.stop, args.output, args.linewidth)
def sequence(panfile, chrom_name, start, stop, outfile='', lw=100):
	try:
		pangenome = decompress(panfile)
	except:
		raise ValueError('...unable to open {}!'.format(panfile))
	...
	if outfile:
		...
		seq='\n'.join(seq[i:i+lw] for i in range(0, len(seq), lw)) #insert linebreaks
		dir, f = os.path.split(outfile)
		if dir:
			try:
				os.stat(dir)
