...
class EditOps:
	...
	def divide_ref(self, cut_ind):
		'''return two new EditOps Objects, cut_ind represents cut_ind in corresponding FeatureInstance!'''
		'''call for REFERENCE FeatureInstance in divideSF'''
		length = 0 #,i.e. len(op1)
		self_ops = self.ops
		ops1, ops2 = [], []
		for i in range(len(self_ops)):
			if length >= cut_ind:
				...
				break
			ops1.append(self_ops[i])
			#instead of calling len() on EditOps Object op1
			if type(self_ops[i]) == int:
				 if self_ops[i]>0: length+= self_ops[i]
			else: length+=1

		if length > cut_ind:
			if ops1[-1] >0:
				ops2 = [length-cut_ind] +ops2
				ops1[-1] = ops1[-1] -length + cut_ind #ops1[-1] - (length-cut_ind)
			else:
				ops2 = [cut_ind-length] + ops2 #[-1 * (length-cut_ind)] + ops2
				ops1[-1] = ops1[-1] + length - cut_ind #-1 * (-ops1[-1] - (length - cut_ind))
		return EditOps(ops1, self.reverse), EditOps(ops2, self.reverse)


	def ref_cut_ind(self, overlap):
