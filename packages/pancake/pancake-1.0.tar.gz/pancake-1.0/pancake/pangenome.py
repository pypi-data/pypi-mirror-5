...
from pancake.utils import get_new_alignment
import sys
...
class PanGenome:
	...
	def treat_labeled_overlap(self, ref_fi, overlap):
		...
		for fi in list(sf):
			...
			if fi==ref_fi:
				new_edit_ops = fi.seq.divide_ref(overlap) 
			else:
				new_edit_ops = fi.seq.divide(cut_ind) 
		...
		if new_sf1: self.features.add(new_sf1)
		if new_sf2: self.features.add(new_sf2)
		try:
	...
	def merge_sfs(self, ali, q_start_fi, s_start_fi):
		...
		s_ref_ali = get_new_alignment((q_ref_ali[1], q_ref_ali[0]), (ali.qali, ali.sali)) # alignment of qsf.seq and s_start_fi.seq
		edit_ops = editops_from_ali(s_ref_ali[0], s_ref_ali[1], s_start_fi.seq.reverse)
		if len(old_sf) >1:
			ali_dic = old_sf.alignments_to_entry(s_start_fi) #alignment of s_start_fi.seq and entry
			for entry in ali_dic:
				new_ali = get_new_alignment((s_ref_ali[1], s_ref_ali[0]), ali_dic[entry])
				other_edit_ops = editops_from_ali(new_ali[0], new_ali[1], entry.seq.reverse)
				entry.sh_feat = qsf
				entry.seq = other_edit_ops
				qsf.add(entry)
				if entry.nextFI:
