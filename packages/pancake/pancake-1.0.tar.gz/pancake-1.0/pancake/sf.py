from pancake.utils import rev_comp, get_new_alignment
...
class SharedFeature:
	...
	def alignments_to_entry(self, ref_fi):
		...
		if ref_fi in self.entries:
			ref_alignment = ref_fi.seq.get_alignment(self.seq) #alignment of sf.seq  (revcomp or not) and ref_fi.seq (original)
			result_dic = {}
			for fi in self.entries:
				if fi != ref_fi:
					entry_alignment = fi.seq.get_alignment(self.seq) #alignment of sf.seq (revcomp or not) and ref_fi.seq (original)
					alignment = get_new_alignment(ref_alignment, entry_alignment)
					result_dic[fi] = alignment
			return result_dic
	...
	def treat_empty_seq(self):
		...
		old_reverse = ref_entry.seq.reverse
