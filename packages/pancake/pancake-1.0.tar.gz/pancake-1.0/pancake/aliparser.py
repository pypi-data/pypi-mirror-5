...
#A = ABC.DACBDCAC$
#B = .BCCDAC.DCAC$
def parse_DELTA(delta_file, pangenome, min_length = 0, self_alignments=True):
	ali_count = 0
		...
		for line in in_file:
			if line[0]=='>':
				...
				if qtupel and stupel:
					...
					pangenome.add_alignment(Alignment(genome1, genome2, qtupel, stupel, ali))
					qtupel, stupel = None, None
...
def parse_BLAST(blast_file, pangenome, min_ali_length = 25, self_alignments=True):
				...
				if new_dict['q']['start'] and sbjct and (self_alignments or query!=sbjct):
					...
					if max(q_pos_tuple) - min(q_pos_tuple) +1 >= min_ali_length and max(s_pos_tuple) - min(s_pos_tuple) +1 >= min_ali_length:
						if query != sbjct or (min(q_pos_tuple), max(q_pos_tuple)) != (min(s_pos_tuple), max(s_pos_tuple)):
							ali_ind +=1
							if (ali_ind+1)%500==0:
								sys.stderr.write('...adding alignment {}\n'.format(ali_ind+1))
								pangenome.refine_unlabeled()
								pangenome.refine_adjacent_sfs()
