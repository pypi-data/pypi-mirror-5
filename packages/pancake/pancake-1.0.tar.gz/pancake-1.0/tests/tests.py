...
from pancake.__init__ import create,add_alignments,stats,core
...
def test_sequences():
	...
	pangenome=decompress(pan_file)
	for chrom in pangenome.chromosomes:
		assert pangenome.seq(chrom) == seq_dict[chrom.name], 'Error in test_sequences after alignment inegration!'
