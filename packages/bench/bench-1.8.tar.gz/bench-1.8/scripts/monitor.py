...
def startMainProcess(useShell, command, q):
    initTime = time()
    if useShell:
	p = psutil.Popen(["/bin/sh", "-c", ' '.join(command)])
    else:
	try:
	    p = psutil.Popen(command, shell = False)
	except OSError:
	    print >> sys.stderr, "Cannot execute command: ", command
	    raise
	
    res = [p.pid, p.getcwd(), p.cmdline, None, None, None]
    while p.is_running():
        #print "wait4", p.pid
	# NOTE: waitpid/wait4 can only wait for children, cannot wait for grand children
...
if __name__ == '__main__':
	...
	try: 
	    ...
	    # print a
	    # print r
	    # NOTE: the unit of ru_maxrss is kb, and it is 4 times big.
	    summaryLine = "%.2fu %.2fs %.2fr (rss)%dkb %d %s" % (r[2], r[3], a[-3], int(r[6]/4), a[-2], ' '.join(a[2]))
	    ## print summaryLine
        res[pid] = r
    #print res
