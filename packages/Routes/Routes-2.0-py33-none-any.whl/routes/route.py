...
class Route(object):
    ...
    def _setup_route(self):
        ...
        self._default_keys = frozenset(list(self.defaults.keys()))
    ...
    def _defaults(self, routekeys, reserved_keys, kargs):
        ...
        if 'action' not in routekeys and 'action' not in kargs \
           and not self.explicit:
            kargs['action'] = 'index'
        defaultkeys = frozenset([key for key in list(kargs.keys()) \
        ...
        if 'id' in routekeys and 'id' not in defaults \
           and not self.explicit:
            defaults['id'] = None
        newdefaultkeys = frozenset([key for key in list(defaults.keys()) \
                                    if key not in reserved_keys])
    ...
    def match(self, url, environ=None, sub_domains=False, 
        ...
        result = {}
        extras = self._default_keys - frozenset(list(matchdict.keys()))
    ...
    def generate(self, _ignore_req_list=False, _append_slash=False, **kargs):
        ...
        if not url.startswith('/') and not self.static:
            url = '/' + url
        extras = frozenset(list(kargs.keys())) - self.maxkeys
        if extras:
            ...
            if fragments:
                url += '?'
                url += urllib.parse.urlencode(fragments)
