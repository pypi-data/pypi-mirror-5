...
class Mapper(SubMapperParent):
    ...
    def _create_regs(self, clist=None):
        ...
        if clist is None:
            ...
            elif isinstance(self.controller_scan, collections.Callable):
    ...
    def generate(self, *args, **kargs):
        ...
        cache_key = str(args).encode('utf8') + \
            str(kargs).encode('utf8')
        ...
        keys = frozenset(list(kargs.keys()))
        cacheset = False
        cachekey = str(keys)
        ...
        for route in keylist:
            ...
            for key in route.hardcoded:
                ...
                if kval != route.defaults[key] and not isinstance(route.defaults[key], collections.Callable):
                    fail = True
                    break
            if fail:
                continue
