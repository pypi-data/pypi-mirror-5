...
def test_basic():
    ...
    assert b"matchdict items are [('action', 'index'), ('controller', " + repr(
        'content').encode() + b"), ('id', None)]" in res
...
def test_no_singleton():
    ...
    def start_response_wrapper(status, headers, exc=None):
        pass
    response = b''.join(app(env, start_response_wrapper))
    assert b"matchdict items are [('action', " + repr('index').encode() + \
           b"), ('controller', " + repr('myapp').encode() + b"), ('path_info', 'fred')]" in response
...
def test_path_info():
    ...
    print(res)
    assert b"matchdict items are [('action', " + repr('index').encode() + \
           b"), ('controller', " + repr('myapp').encode() + b"), ('path_info', 'some/other/url')]" in res
...
def test_redirect_middleware():
    ...
    print(res)
    assert b"matchdict items are [('action', " + repr('index').encode() + \
           b"), ('controller', " + repr('myapp').encode() + \
...
def test_method_conversion():
    ...
    assert b"matchdict items are [('action', 'index'), ('controller', " + \
           repr('content').encode() + b"), ('id', None)]" in res
    
    res = app.get('/content/hopper', params={'_method':'DELETE'})
    assert b"matchdict items are [('action', " + repr('index').encode() + \
           b"), ('controller', " + repr('content').encode() + \
           b"), ('type', " + repr('hopper').encode() + b")]" in res
    ...
    assert b"matchdict items are [('action', " + repr('index').encode() + \
           b"), ('controller', " + repr('content').encode() + \
           b"), ('type', " + repr('grind').encode() + b")]" in res
