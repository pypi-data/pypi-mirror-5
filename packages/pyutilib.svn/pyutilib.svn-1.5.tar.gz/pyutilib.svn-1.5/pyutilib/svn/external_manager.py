...
class ExternalManager(object):
    ...
    def generate_repo_definition(self, repo=None, proj=None):
        ...
        def _reduce_inherited(trunkExternals, target):
            modified = False
            targetExternals = target.get('externals', None)
            if not targetExternals:
                return
        ...
        ans = { '__repo__' : repo.url, '__svn15__' : repo.svn15format }
            ...
            for tName, target in repo.projects[pName].targets.iteritems():
                ...
                for e in target.externals.itervalues():
                    if e.target is None:
                        ...
                        _proj = self.projects.get(e.target.project.name, None)
                        if _proj is None or not e.url.startswith(_proj.url):
                            ref = Description_External(e.path, link=e.url)
                        else:
    ...
    def collapse_repo_definition(self, rd):
        ...
        for pName, p in rd.iteritems():
            if pName.startswith('__') and pName.endswith('__'):
                continue
    ...
    def expand_repo_definition(self, rd):
        ...
        if repo:
            ...
            repo.svn15format = rd.get('__svn15__', False)
        for pName, p in rd.iteritems():
            if pName.startswith('__') and pName.endswith('__'):
                continue
    ...
    def generate_changeset_from_definition(self, defn):
        ...
        for r in self.all_repos:
            if defn.get('__repo__') == r.url:
                repo = r
                repo.svn15format = defn.get('__svn15__', False)
                break
        if repo is None:
    ...
    def implement_changeset(self, changeset, defn):
        ...
        defaultExternal15Format = repo.svn15format
        ...
        class externalFile(object):
            def __init__(self, dir):
                ...
                self.dir = dir
                self.name = 'Externals'
                self.svn15format = defaultExternal15Format
        ...
        def _readExternalsFile(eInfo, target_wd):
            ...
            try:
                ...
                data1 = [l.strip() for l in f.readlines() if l.strip()]
                f.close()
                data2 = [l for l in svn_propget('svn:externals', ans.dir) if l]
                if data1 != data2:
                    print data1
                    print data2
        ...
        def _writeExternalsFile(eFile):
            ...
            lNum = 0;
            _addExternalsFile = not os.path.exists( eFile.fileName() )
            ...
            if p.returncode:
                raise SvnError( "svn propset returned error %i:\n%s" %
                        (p.returncode, stderr) )
            if _addExternalsFile:
                p = Popen( [ 'svn', 'add', eFile.fileName() ],  
                           stdout=PIPE, stderr=PIPE )
                stdout, stderr = p.communicate()
                if p.returncode:
                    raise SvnError( "svn add returned error %i:\n%s" %
                                    (p.returncode, stderr) )
        # end _writeExternalsFile()

        def _checkoutWD(item, deepref):
            if isinstance(item, ResolvedTarget):
                ...
                return
            if deepref:
                deepref = deepref.strip('/')
                target_url = '/'.join([target_url, deepref])

            if not target_url.startswith(repo.url):
                raise ExternalError("Target URL does not begin with Repo URL")

            wd = self.config['workingdir']
            url = repo.url
            for d in target_url[len(repo.url):].replace('\\','/').strip('/').split('/'):
                if len(d) == 0:
                    continue
        ...
        for target in changeset['del']:
            if target[1] is None:
                ...
                continue
            wd = _checkoutTarget(target[0])
            allCheckouts.add(wd)
        for target in changeset['add']:
            if target[1] is None:
                # Cannot automatically add new project
                continue
            deepref_idx = target[1].rfind('/')
            if deepref_idx > 0:
                wd, _url = _checkoutWD(target[0], target[1][:deepref_idx])
            else:
                wd, _url = _checkoutWD(target[0], None)
            allCheckouts.add(wd)
        ...
        for target in changeset['add']:
            ...
            if target_def is None:
                ...
                continue
            if not target[0].url.startswith(target[0].raw.repo_url):
                raise Exception("INTERNAL ERROR: extracting project from "
                                "repo url failed")
            _projUrl = target[0].url[len(target[0].raw.repo_url):]
            while _projUrl and _projUrl[0] == '/':
                _projUrl = _projUrl[1:]
            wd = os.path.join(
                self.config['workingdir'], _projUrl, target[1] )
            if not os.path.isdir(wd):
                ...
                # Checkout any intermediate directories to be able to
                # create the new target
                if not os.path.isdir(os.path.dirname(wd)):
                    _checkoutWD(target[0], target[1])

                # Checkout the source target
                base = _checkoutTarget(src)

                log.info("Copying %s from %s" % ( target[1], base ))
                p = Popen( ['svn', 'cp', base, wd],
                           stdout=PIPE, stderr=PIPE )
                stdout, stderr = p.communicate()
                if p.returncode:
    ...
    def branch_config(self, cfg):
        def _get_target(em, cfg, external):
            if type(external) is ResolvedProject:
                ...
                if target is None:
                    log.warning(
"External %s pointing to an unknown target,\n    %s.\n"
"Preserving this URL in the new branch.\n"
"Please verify this reference before applying/committing this branch." 
% ( external.path, external.url ) )
                    new_e = Description_External(
                        external.path, link=external.url)
                    new_e.deep_ref = external.deep_ref
                    new_e.rev = external.rev
                    return new_e
                proj = target.project
        ...
        ans = { '__repo__' : proj.repo.url, 
                '__svn15__' : proj.repo.svn15format }
        ...
        if cfg.get('peg', False):
            ...
            for e_path, old_e in src.externals.iteritems():
                target = e_map[e_path]
                if type(target) is Description_External:
                    # ResolvedExternal indicates an external pointing to
                    # an unknown project
                    externals[e_path] = target
                    continue
            ...
            for e_path in sorted(src.externals):
                ...
                target = e_map[e_path]
                if type(target) is Description_External:
                    # Description_External indicates an external pointing to
                    # an unknown project
                    externals[e_path] = target
                    continue
