...
def getjiraversion(settings):
        ...
        jVersion = jira.search_issues('fixVersion=earliestUnreleasedVersion(' + settings.get("jira","project") + ')')[0].fields.fixVersions[0].name
        return jVersion
...
def main():
        ...
        for argvFlag in sys.argv[1:]:
                ...
                if splitFlag[0].lower() == "site":
                        ...
                        actionTweet = True
                        if splitFlag[2]:
                                if splitFlag[2].lower() == "random":
                                        randomWord = '#' + getrandomword()
                                        randomWordTwo = randomWord
                                else:
                                        randomWord = '#' + splitFlag[2]
                                        randomWordTwo = randomWord
                        else:
                                randomWord = 'at ' + datetimestarted.strftime("%H:%M:%S") + '.'
        ...
        if site:
                ...
                site = os.path.split(entryPointFull)[1]
                if (site == 'deploy'):
                        site = os.path.split(os.path.split(entryPointFull)[0])[1]
        ...
        if settings.has_option("actions", "jira"):
                actionJira = True
        ...
        if actionTweet:
                ...
                if actionJira:
                        ...
                        posttwitter(settings, systemuser + " is deploying " + site + " v" + versionTitle.encode("utf-8") + " to " + environment + " " + randomWord)
                else:
        ...
        if settings.has_option("actions", "deploy_server"):
                ...
                if actionOptimize or actionCSS or settings.has_option("scm", "auto_version"):
                        ...
                        hasOptimizeDirectory = settings.has_option("scm", "optimize_directory")
                        optimizeDir = os.path.join(tempdir, settings.get("scm", "site_directory"))
                        if hasOptimizeDirectory:
                                optimizeDir = os.path.join(optimizeDir, settings.get("scm", "optimize_directory"))
                        for root, subFolders, files in os.walk(tempdir):
                                for file in files:
                                        fullpath = os.path.join(root,file)
                                        if actionOptimize:
                                                # use Closure to optimize JS
                                                if fullpath.lower().endswith((".js")) and optimizeDir.lower() in fullpath.lower():
        ...
        if settings.has_option("actions", "apply_database_migrations"):
                ...
                elif (dbtype == "mysql" and clientOS == "linux2"):
                        import pymysql
                ...
                dbcur.execute("""
                        ...
                        WHERE INFORMATION_SCHEMA.TABLES.table_name = 'db_schema_version'
                        AND (INFORMATION_SCHEMA.TABLES.TABLE_CATALOG = '""" + settings.get("database","database") + "'" +
                        "OR INFORMATION_SCHEMA.TABLES.TABLE_SCHEMA = '" + settings.get("database","database") + "')")
        ...
        if actionTweet:
                if randomWordTwo == "":
                        randomWordTwo = 'at ' + datetime.datetime.now().strftime("%H:%M:%S") + '.'
                if actionJira:
                        posttwitter(settings, "Deployment of " + site + " v" + versionTitle.encode("utf-8") + " to " + environment + " has completed " + randomWordTwo)
                else:
                        posttwitter(settings, "Deployment of " + site + " to " + environment + " has completed " + randomWordTwo)
