...
class CreateObjectsCommand(BaseCommand):
    ...
    def create_export_for_start_urls(self):
        participant_content_type = ContentType.objects.get(app_label=self.app_label, model='participant')
        self.create_export(participant_content_type,
                           'start URLs',
                           ['start_url'])

    def create_export_for_participants(self):
        participant_content_type = ContentType.objects.get(app_label=self.app_label, model='participant')
        self.create_export(participant_content_type,
                           'participants',
                           ptree.admin.get_participant_list_display(self.Participant))

    def create_export_for_matches(self):
        match_content_type = ContentType.objects.get(app_label=self.app_label, model='match')
        self.create_export(match_content_type,
                           'matches',
                           ptree.admin.get_match_list_display(self.Match))
    ...
    def handle(self, *args, **options):
        ...
        self.create_export_for_matches()
        self.create_export_for_participants()
