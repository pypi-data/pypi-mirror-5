...
def get_participant_list_display(Participant, first_fields=None, readonly_fields=None):
    first_fields = ['__unicode__', 'id', 'match', 'has_visited'] + (first_fields or [])
    readonly_fields = ['link',] + (readonly_fields or [])
    return get_list_display(Participant, first_fields, readonly_fields)

def get_treatment_list_display(Treatment, first_fields=None, readonly_fields=None):
    first_fields = ['__unicode__', 'id', 'description', 'experiment'] + (first_fields or [])
    readonly_fields = ['link',] + (readonly_fields or [])
    return get_list_display(Treatment, first_fields, readonly_fields)

def get_experiment_list_display(Experiment, first_fields=None, readonly_fields=None):
    first_fields = ['__unicode__', 'id', 'description'] + (first_fields or [])
    return get_list_display(Experiment, first_fields, readonly_fields)
