...
class SequenceView(View):
    ...
    def prerequisite_is_satisfied(self):
        return True

    waiting_page_template = 'ptree/WaitingPage.html'
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.request.is_ajax():
            return HttpResponse(int(self.prerequisite_is_satisfied()))
        ...
        if not self.prerequisite_is_satisfied():
            return render_to_response(self.waiting_page_template, {'SequenceViewURL': self.request.path})
    ...
    def get_context_data(self, **kwargs):
        ...
        if settings.DEBUG:
            try:
                ...
                match_id = ''
            context[Symbols.debug_info] = 'Match ID: {}, Participant index: {}'.format(match_id, self.participant.index_among_participants_in_match)
...
class GetTreatmentOrParticipant(vanilla.View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if experiment_code:
            ...
            try:
                mturk_worker_id = self.request.GET[Symbols.mturk_worker_id]
                mturk_assignment_id = self.request.GET[Symbols.mturk_assignment_id]
