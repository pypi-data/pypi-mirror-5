...
class Yaml_Config(object):
    ...
    def init_store(self):
        ...
        for scheme in schemes:
            ...
            connection = '%(scheme)s://%(user)s:%(pass)s@%(host)s:%(port)s/%(database_name)s'\
                    % {'scheme': key,
            ...
            setattr(self, scheme.get('storm', 'store'), cache_store)
            setattr(self.app, scheme.get('storm', 'store'), cache_store)
            #if scheme.get('sqlalchemy', '') != '':
            #    if key == 'sqlite':
            #        connection = '%(scheme)s://%(database_name)s'\
            #            % {'scheme': key,
            #               'database_name': os.path.join(self.base_path, scheme.get('db', ''))
