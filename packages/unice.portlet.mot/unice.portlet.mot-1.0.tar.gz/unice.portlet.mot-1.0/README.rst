.. image:: https://secure.travis-ci.org/unice/unice.portlet.mot.png
   :target: https://travis-ci.org/unice/unice.portlet.mot

Introduction
============

This portlet shows a mot item as mot in the portlet.
...
Add `unice.portlet.mot` to your buildout and re-run buildout.
...
the option to add a Mot Portlet.

Next, select a mot item to display. It can be any type of item. You can
select which of the item's fields you want to display in the portlet's mot
area:

* Title (either as a link to the mot item or as plain text)
* Description
* Date
* Body text
* Image

When the mot item doesn't have a selected field, it should work without
...
* portlet header title as rendered on page
  (use mot's title or select custom)
* hide portlet header altogether
* conditionally show a portlet footer with a link to the mot item
