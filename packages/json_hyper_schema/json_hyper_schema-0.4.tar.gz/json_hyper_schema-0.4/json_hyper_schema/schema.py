...
class Schema(dict):
    ...
    def _fill_template(self, link, data):
        result = dict(link)
        if 'schema' in link:
            result['schema'] = Schema(link['schema'], self.base_url)
            
        try:
            result['href'] = link['href'].format(**data)
        except KeyError:
            pass
        
        return result

    def links(self, data, rel=None, media_type=None):
        """
        ...
        """
        links = [self._fill_template(link, data) for link
                 in self.get('links', {})]
        ...
        if media_type:
            links = [link for link in links if link.get('mediaType') == media_type]
    ...
    def example(self):
        ...
        if 'properties' in self:
            return dict(
                (prop, Schema(sub_schema).example) for prop, sub_schema in
                self['properties'].items()
            )

        elif 'items' in self:
            return [
                Schema(self['items']).example,
                Schema(self['items']).example,
                '...'
            ]
        elif 'enum' in self:
            return '(%s)' % ('|'.join([str(item) for item in self['enum']]))
