...
def check():
  def library_installed(lib):
    return subprocess.call("R --vanilla <<< 'library({lib})'".format(lib=lib), shell=True,
                           stdout=subprocess.PIPE,
                           stderr=subprocess.PIPE) == 0

  def install_library(lib):
    return subprocess.call("R --vanilla <<< 'install.packages(\"{lib}\", dependencies=T, repos=\"http://cran.us.r-project.org\")'".format(lib=lib), shell=True,
                           stdout=subprocess.PIPE,
                           stderr=subprocess.PIPE) == 0
  ...
  for lib in ['ggplot2', 'hexbin']:
    if not library_installed(lib):
      sys.stderr.write("R library {} not installed; installing now...".format(lib))
      if install_library(lib):
        sys.stderr.write("success!\n")
      else:
        sys.stderr.write("failed!\n")
        raise Exception("Unable to install R package {lib}. Please install manually with 'install.packages({lib})'".format(lib=lib))
...
def main():
  ...
  parser.add_argument( '--debug',
      action='store_true',
      help='print generated R script to stderr')

  args = parser.parse_args()

  print json.dumps(plot(args))
