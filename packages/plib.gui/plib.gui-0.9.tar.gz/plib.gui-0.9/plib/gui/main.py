...
if gui_test:
    ...
    toolkit_dict.update(
        ('P%s' % klassname, toolkit_helper(toolkit, modname, klassname))
...
def mixin_helper(klassname, bases):
    def f():
        baselist = [
            import_from_module('plib.gui.%s' % modname, basename)
...
toolkit_dict.update(
    ('PSorted%s' % klassname, sorted_helper(toolkit, modname, klassname))
...
def widget_helper(klassname, modname):
    def f():
        return import_from_module(
            'plib.gui._widgets.%s' % modname, klassname)
