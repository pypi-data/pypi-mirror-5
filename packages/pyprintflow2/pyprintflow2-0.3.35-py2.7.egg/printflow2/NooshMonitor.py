...
class NooshMonitor(pyinotify.ProcessEvent):
    ...
    def createDamProjectCategory(self, workgroup, project, client):
        ...
        requestUrl = self.cumulusApiUrl +"/data/" + workgroup['dam_site'] + '/create?item=category&path=' + cgi.escape(category_path)
    ...
    def createDamSpecificationCategory(self, client, project, spec):
        try:
            ...
            requestUrl = self.cumulusApiUrl +"/data/" + project['cumulus_project']['dam_site'] + '/create?item=category&path=' + cgi.escape(category_path)
            disResponse = requests.get(requestUrl)
            result = {}
            response = disResponse.json()
