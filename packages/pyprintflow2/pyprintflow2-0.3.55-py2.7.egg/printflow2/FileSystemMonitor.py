        ...
        for dirname, dirnames, filenames in os.walk(f):
            ...
            for filename in filenames:
                ...
                if (parent_dir == 'approved') or (parent_dir == 'failed') or (parent_dir == 'MisplacedFiles') \
                    ...
                    continue
                pathname = os.path.join(utils.removeNonAscii(dirname), utils.removeNonAscii(filename))
                path_bits = dirname.split('/')
                try:
                    self.process_file(utils.removeNonAscii(dirname), utils.removeNonAscii(filename), utils.removeNonAscii(pathname))
                except:
