...
class NooshMonitor(pyinotify.ProcessEvent):
    ...
    def checkForProjectSpecifications(self, client, workgroup, project):
        offset = 0
        count = 1000
        ...
        while response['status_code'] == 200:
            ...
            for i in range(len(specs)):
                self.processSpecification(client, project, specs[i])
            offset += count
    ...
    def processProject(self, workgroup_last_check, workgroup, project, project_filter, send_email):
        ...
        if project['client_account'] is not None:
            ...
            specs = self.checkForProjectSpecifications(client, workgroup, db_project)
...
<hr /></div>'
    ...
    def processSpecification(self, client, project, spec):
