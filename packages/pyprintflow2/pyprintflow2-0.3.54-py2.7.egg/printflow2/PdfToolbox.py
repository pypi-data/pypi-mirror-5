...
class PdfToolbox(object):
    ...
    def run_preview(self, serial_no, file_path, file_name, guid):
        ...
        if self.ready:
            ...
            preview_folder = unicode(os.path.join(self.webroot_folder, guid.replace('-', '/')))
            ...
            try:
                cmd = ['"'+unicode(self.pdftoolbox_path)+'"', '--license='+str(serial_no), '--secret='+str(self.get_license(serial_no)), \
                       '--saveasimg', '--noprogress', '--pagerange=1', '--imgformat=JPEG', '--resolution=400x400', '--compression=JPEG_high',\
                       '--outputfile="'+unicode(previewFile)+'"', \
                       '"'+str(theFile)+'"']
                cmd_str = ' '.join(cmd)
                ...
                self.logger.error(unicode('Problem calling command: ')+str(cmd))
                traceback.print_exc()
    ...
    def run_job(self, serial_no, file_path, file_name, profile, success_destinationation, fail_destination, guid):
        ...
        if self.ready:
            try:
                ...
                report_file_mask = os.path.join(report_folder, unicode('report_')+file_name+unicode('.pdf'))
                if os.path.exists(report_file_mask):
                    try:
                        os.remove(report_file_mask)
                    except:
                        self.logger.error(unicode('Problem removing tmp pdf report file: ')+report_file_mask)
                        
                report_file_text = os.path.join(report_folder, unicode('report_')+file_name+unicode('.txt'))
                if os.path.exists(report_file_text):
                    try:
                        os.remove(report_file_text)
                    except:
                        self.logger.error(unicode('Problem removing tmp text report file: ')+report_file_mask)
                        
                report_file_html = os.path.join(report_folder, unicode('report_')+file_name+unicode('.html'))
