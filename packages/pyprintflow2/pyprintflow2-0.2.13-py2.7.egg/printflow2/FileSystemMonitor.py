    ...
    def walk(self, f):
        ...
        for dirname, dirnames, filenames in os.walk(f):
            ...
            for filename in filenames:
                ...
                if (parent_dir == 'approved') or (parent_dir == 'failed') or (parent_dir == 'MisplacedFiles') or (filename_bits[-1] == 'json'):
                    continue
                ...
                if client_name is not None:  
                    try:
                        ...
                        if 'flowdock_accesstoken' in client:
                            ...
                            for fi in range(len(client['buyer_emails'])):
                                if not client['buyer_emails'][fi] in mail_list:
                                    mail_list.extend(client['buyer_emails'][fi])
        ...
        try:
            ...
            if status:
                ...
                for fi in range(len(client['file_notify_emails'])):
                    if not client['file_notify_emails'][fi] in mail_list:
                        mail_list.extend(client['file_notify_emails'][fi])
                self.send_html_mail(mail_list, subject, message)
