...
@note: to generate libphsh fortran wrappers (libphsh.pyd) for your platform
       ...
       install into your python distribution. Alternatively, use 'f2py -c
       -m libphsh libphsh.f'. Windows users may have to add appropriate 
       compiler switches, e.g. '--fcompiler=gfortran --compiler=mingw'
...
class Atorb(object):
    ...
    look at the atomi files after printing this out to see everything...
    
    suffice it to say, that the charge density at radius r(i)
    in units of electrons per cubi bohr radius is given by 
    
    sum of j=1...nel, 
    occ(j)*phe(i,j)*phe(i,j)/(4.0*3.14159265....*r(i)*r(i))... 
    
    think of the phe functions as plotting the radial wave-functions
    as a function of radius...on our logarithmi mesh...
    ...
    def get_quantum_info(shell):
        ...
        Description
        -----------
        Get a tuple of quantum information for a given orbital 's', 'p', 'd' or 'f' from
        a given subshell string.
        
        Returns
        -------
        tuple : (int, int, list[float, float], list[float, float])
            (n, l, j=[l-s, l+s], occ=[occ-, occ+])
            
        Notes
        -----
        n is the principle quantum number (n > 0).
        l is the azimuthal quantum number (0 ≤ l ≤ n-1).
        s is the spin quantum number (s±1/2).
        j[] is the total angular momentum quantum numbers for both 
        l-s or l+s, respectively.
        occ[] is the occupancy of the spin-split l-s and l+s levels, 
        respectively. 
    ...
    def gen_input(element, **kwargs):
        ...
        Description
        -----------
        Generate atorb input file from <element> and optional **kwargs arguments.
        Returns filename of input file once generated.
        ...
        method : str, optional
            Exchange correlation method using either 0.d0=Hartree-Fock,
            1.d0=LDA, -alpha = xalpha (default: 0.d0)
        ...
        Example
        -------
        >>>Atorb.gen_input('H',rel=False, filename="atorb_H.txt", output='at_H.i')
        ...
        with open(filename, 'w') as f:
            ...
            f.write('w\n')
            f.write('{0}'.format(output).ljust(30)+'! name of output file \n')
            f.write('q\n')
    ...
    def calculate_Q_density(**kwargs):
        ...
        Description
        -----------
        Calculate the radial charge density of a given element or atorb input file
        ...
        element : int or str, optional
            ...
            file - please use 'help(Atorb.gen_input)' for more information. 
        input : str, optional
        ...
        >>>Atorb.calculate_Q_density(input='atorb_C.txt')
         ENTER Z, NR
         PLEASE ENTER RELATIVITY FACTOR.  (0=NR, 1=REL.)
         PLEASE ENTER EXCHANGE CORRELATION (ALPHA).
         0=HARTREE-FOCK, POSITIVE=LDA, NEGATIVE=XALPHA.
         PLEASE ENTER NFC, NEL, RATIO, ETOL, XNUM
         PLEASE ENTER N L M J S OCC.
              18.008635    -33.678535
               4.451786    -36.654271
               1.569616    -37.283660
               0.424129    -37.355634
        ...
        """
        if 'input' in kwargs:
            input = os.path.abspath(kwargs.pop('input'))
        ...
        hartfock(input) #calculates atomic orbital charge densities for atom
