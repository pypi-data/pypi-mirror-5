...
def f(x, mu1, mu2):
    x1 = x[..., 0]
    x2 = x[..., 1]
    numerator = (1 + (pi ** 2 / 4) * np.square(mu2 - mu1 - (mu1 + mu2) * x2) *
                 np.square(np.sin((pi / 2) * (x1 + 1))))
    denominator = 1 + (mu1 + mu2) * np.cos((pi / 2) * (x1 + 1))
...
if __name__ == "__main__":
    ...
    mesh = Mesh2D(-1, 1, -1, 1, N=75, M=75, qtype_x='lgl', qtype_y='lgl')
    params = Mesh2D(-0.4, 0.4, -0.4, 0.4, N=50, M=50,
                     qtype_x='linear', qtype_y='linear')
    ei = EI(f, mesh.vector, mesh.vector_weights, params.vector)
    ei.solve(M=50)
    test_params = -0.4 + rand(200).reshape((100, 2)) * 0.8
    norms = []
    ...
    plt.figure("2D Example 2 L2 norms", figsize=(8, 6))
