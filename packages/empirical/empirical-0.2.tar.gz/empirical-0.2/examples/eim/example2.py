...
if __name__ == "__main__":
    ...
    mesh = Mesh1D(-1, 1, N=75, qtype='lgl')
    params = np.linspace(1, 4, 100).reshape((100, 1))
    ei = EI(f, mesh.vector, mesh.vector_weights, params)
    ei.solve(M=25)
    test_params = 1 + rand(100) * 3
    norms = []
    ...
    for m in m_eim:
        ...
        mesh_linear = Mesh2D(-1, 1, 1, 4, N=75, M=m, qtype_x='lgl', qtype_y='linear')
        mesh_cc = Mesh2D(-1, 1, 1, 4, N=75, M=m, qtype_x='lgl', qtype_y='cc')
        mesh_lgl = Mesh2D(-1, 1, 1, 4, N=75, M=m, qtype_x='lgl', qtype_y='lgl')
    ...
    plt.figure("Example 2 L2 norms", figsize=(8, 6))
    ...
    plt.savefig("example2_l2_norms.png", dpi=120, pad_inches=0)
    ...
    plt.savefig("example2_basis_functions.png", dpi=120, pad_inches=0)


    points = np.linspace(-1, 1, 101)
    plt.figure("Test Function", figsize=(13, 4))
    plt.subplot(131)
    plt.plot(points, f(points, 1))
    plt.subplot(132)
    plt.plot(points, f(points, 2.5))
    plt.subplot(133)
    plt.plot(points, f(points, 4))
    plt.savefig("example2_test_function.png", dpi=120, pad_inches=0)
