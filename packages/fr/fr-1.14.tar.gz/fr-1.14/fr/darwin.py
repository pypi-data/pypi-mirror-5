...
def get_diskinfo(outunit, show_all=False, debug=False):
...
def get_meminfo(outunit, debug=False):
    ...
    '''
    meminfo = Info()
    ms = run('/usr/sbin/sysctl -a hw.memsize', shell=False)
    su = run('/usr/sbin/sysctl -a vm.swapusage', shell=False)
