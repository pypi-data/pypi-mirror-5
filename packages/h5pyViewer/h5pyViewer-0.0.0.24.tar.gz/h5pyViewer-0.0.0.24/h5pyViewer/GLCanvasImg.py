...
'''
  ...
  def UpdateImg(self):
    ...
    try:
      ...
      self.glImg=glImg=glumpy.image.Image(txrData, colormap=colMap,vmin=colRng[0], vmax=colRng[1])
    else:
      if glImg.data.shape==self.data.shape:
        glImg.data[:]=self.GetTxrData()
      else:
...
class DlgColBarSetup(wx.Dialog):
  def __init__(self,parent):
    ...
    self.edVMin=edVMin=wx.TextCtrl(self,-1,'%g'%glImg._vmin,style=wx.TE_PROCESS_ENTER)
    self.edVMax=edVMax=wx.TextCtrl(self,-1,'%g'%glImg._vmax,style=wx.TE_PROCESS_ENTER)
    colMapLst=[]
  ...
  def OnModify(self, event):
    ...
    glImg=parent.glImg
    glImg._vmin=float(self.edVMin.Value)
    glImg._vmax=float(self.edVMax.Value)
    v=self.cbColMap.Value
    if v:
      cmap=getattr(glumpy.colormap,v)
      glImg._filter.colormap=cmap
      glImg._filter.build()
      cbar=parent.glColBar
