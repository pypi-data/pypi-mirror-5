...
The trove classifiers builder knows how to build the python classifiers.
We can list them as we need in the ?:action=list_classifiers method.
The original data are taken from the pypi page at:
https://pypi.python.org/pypi?%3Aaction=list_classifiers.

  >>> from pprint import pprint

  >>> import mypypi.api
  >>> classifierBuilder = mypypi.api.ClassifierBuilder()
  >>> print '\n'.join(classifierBuilder.list())
...
Our basic component which makes the mirroring possible is the used XML-RPC
connection to the PyPi server. Let's setup such a PYPI proxy connection.

  >>> import mypypi.api
  >>> pypiProxy = mypypi.api.getPYPIProxy()
  >>> pypiProxy
  <FakeXMLRPCPYPI for 'https://pypi.python.org/pypi'>

As you can see we use a testing PyPi server. This Fake XMLRPC PYpi proxy offers
test data and supports the real API. This allows us to run all tests without
to touch the real PyPi server.

PyPi offers different methods for retrive package and release data. Let's show
...
a list of name strings.
  ...
  ['z3c.form', 'z3c.authenticator', 'wsgi_intercept']
...
option called ``show_hidden`` which will allow to show all release versions:
  ...
  ['1.9.0', '1.8.2', '1.8.1', '1.8.0', '1.7.0', '1.6.0', '1.5.0b1', '1.5.0',
   '1.4.0b1', '1.4.0', '1.3.0', '1.2.0', '1.1.0', '1.0c2', '1.0c1', '1.0.0']
...
- comment_text
  ...
  >>> version = '1.9.0'
  >>> pprint(pypiProxy.release_urls(packageName, version))
  [{'comment_text': '',
    'downloads': 458,
    'filename': 'z3c.form-1.9.0.tar.gz',
    'has_sig': False,
    'md5_digest': '5b1404473489764ea209cd28f0974662',
    'packagetype': 'sdist',
    'python_version': 'source',
    'size': 37,
    'url': 'https://pypi.python.org/packages/source/z/z3c.form/z3c.form-1.9.0.tar.gz'}]
...
- classifiers (list of classifier strings)
  ...
  >>> version = '0.5.0'
  >>> pprint(pypiProxy.release_data(packageName, version))
  {'_pypi_hidden': 1,
   '_pypi_ordering': 10,
   ...
   'home_page': 'http://cheeseshop.python.org/pypi/z3c.authenticator',
...
The results are returned as a list of dicts:
  ...
  [{'_pypi_ordering': 115, 'version': '1.9.0', 'name': 'z3c.form',
...
seconds since the epoch.
  ...
  [['pypi', '2005-08-01', 1198792881, 'update home_page, classifiers'],
   ...
   ['zope.app.publisher', '3.5.0a4', 1198850101, 'add source file zope.app.publisher-3.5.0a4.tar.gz']]
