...
class SenditApplicationOutbox(BaseSenditApplicationPage, Table):
    ...
    @property
    def values(self):
        values = ISenditApplicationUsers(self.context).getUserFolder().values()
        return sorted(values, key=lambda x: IZopeDublinCore(x).created, reverse=True)
...
class ISenditOutboxDeleteInfo(Interface):
    ...
    notify_recipients = Bool(title=_("Notify recipients?"),
                             description=_("Do you want to notify recipients of this packet that it won't be available anymore?"),
                             required=True,
                             default=True)
