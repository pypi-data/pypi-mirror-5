...
from ztfy.sendit.packet.interfaces.filter import IMimetypeFilterPluginInfo, IMimetypeFilterPlugin, \
                                                 IMimetypeFilterTarget
    ...
    def filterElement(self, config, filename, data):
        ...
        if extensions:
            ...
            if '.' + ext in extensions:
                raise FilteredPacketException, translate(_("A document containing invalid data has been found. Your packet has been rejected."), context=queryRequest())
        # check MIME types
        magic_type = getMagicContentType(data[:4096])
        if magic_type in config.forbidden_magic_types:
            raise FilteredPacketException, translate(_("A document containing invalid data has been found. Your packet has been rejected."), context=queryRequest())
        ...
        for name in unique((mime_type, magic_type)):
            if name:
                ...
                if extractor is not None:
                    ...
                    if extension:
                        mime_types = config.forbidden_mimetypes
