...
class ISenditApplicationInfo(IBaseContent):
...
class ISenditApplicationSecurityInfo(Interface):
    ...
    administrators = PrincipalList(title=_("System administrators"),
                                   description=_("List of principals allowed to configure the whole application, including system settings and utilities"),
                                   required=False)
    ...
    confirmation_delay = Int(title=_("Confirmation delay"),
                             description=_("Number of days after which unconfirmed registration requests will be deleted"),
                             required=True,
                             default=7)
    ...
    single_auth_plugins = List(title=_("Personal authentication plug-ins"),
                               description=_("List of authentication plug-ins which provide individual principals (instead of others which provide groups)"),
                               required=False,
                               value_type=Choice(vocabulary='AuthenticatorPlugins'))
...
class ISenditApplicationMailingInfo(II18nAttributesAware):
    ...
    mail_service_name = I18nTextLine(title=_("Service name"),
                                     description=_("This name will be the name of the service given in registration and notification messages"),
                                     required=True)
    ...
    mail_subject_header = I18nTextLine(title=_("Mails subject header prefix"),
                                       description=_("This header will be placed in front of each message send by the application"),
                                       required=False)

    mail_signature = I18nHTML(title=_("Mails signature"),
                              description=_("This text will be placed in bottom of each message send by the application"),
                              required=False)
...
class ISenditApplicationFiltersInfo(Interface):
    ...
    excluded_addresses = TextLineListField(title=_("Excluded addresses"),
                                           description=_("Target addresses matching these regular expressions will be excluded. Don't forget to protect points with an antislash!"),
                                           required=False)

    excluded_principals = PrincipalList(title=_("Excluded principals"),
                                        description=_("Any individual or group principal present in this list will be excluded"),
                                        required=False)
...
class ISenditApplicationBackInfo(Interface):
    ...
    custom_banner = ImageField(title=_("Custom banner"),
                               description=_("You can provide a custom image file which will be displayed on top of back-office pages"),
                               required=False)

    custom_logo = ImageField(title=_("Custom logo"),
                             description=_("You can provide a custom image file which will be displayed on top of left column"),
                             required=False)

    custom_icon = ImageField(title=_("Custom icon"),
                             description=_("You can provide a custom image file to be used as back-office favorite icon"),
                             required=False)
