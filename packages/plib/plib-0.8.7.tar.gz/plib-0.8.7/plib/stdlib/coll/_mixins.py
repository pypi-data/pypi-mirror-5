...
class SortMixin(object):
    ...
    def __init__(self, iterable=(), key=None):
        # Initialize the underlying data structure with no items,
        # then fill it up in correct sorted order
    ...
    def _init_seq(self, iterable, key):
        ...
        self._given_key = key
        self._key = key = (lambda x: x) if key is None else key
        decorated = sorted((key(item), item) for item in iterable)
        self._keys = [k for k, item in decorated]
        super(SortMixin, self).extend(item for k, item in decorated)
    ...
    def pop(self, index=-1):
        ...
        super(SortMixin, self).pop(index)
    ...
    def append(self, item):
        raise TypeError("can't append to sorted sequence; use insert instead.")
    
    def extend(self, iterable):
        raise TypeError("can't extend sorted sequence; use insert instead.")
