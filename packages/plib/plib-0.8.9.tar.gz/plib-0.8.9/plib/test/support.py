#! /usr/bin/env python
"""
Module SUPPORT -- standard test-running routine
...
def run_tests(testdir, modules_with_doctests=(), module_relative=False, verbosity=0):
    
    doctests = glob.glob(os.path.join(testdir, "doctests", "test_*.txt"))
    if doctests:
    ...
    if unittests:
        ...
        suites = [
            loader.loadTestsFromName('plib.test.%s%s' % (
                '%s.' % subdir if subdir else '', modname))
