...
class CSVDataFeed(DataFeedBase):
    ...
    @staticmethod
    def convertTick(rawDataFile, formattedDataFilePath):
        if not os.path.exists(rawDataFile):
            raise Error.FileNotExistsError("File {} does not exist.".format(rawDataFile))
 
        if not os.path.exists(os.path.split(formattedDataFilePath)[0]):
            try:
                logger.info("Creating path to %s.", formattedDataFilePath)
                os.makedirs(os.path.split(formattedDataFilePath)[0])
            except:
                logger.error("Failed to create path %s.", os.path.split(formattedDataFilePath)[0])
                return False
     
        logger.info("Loading raw data from %s.", rawDataFile)
        dataframe = pd.read_csv(rawDataFile, header=None, parse_dates=[[0, 1]], names=["date", "time", "last", "lastSize", "Total Volume", "Bid", "Ask", "TickID", "BidSize", "AskSize"], index_col="date_time")
        logger.info("Loaded: \n%s.", dataframe)
        
        date = datetime.datetime(dataframe.index.year[0], dataframe.index.month[0], dataframe.index.day[0])
        endDate = datetime.datetime(dataframe.index.year[-1], dataframe.index.month[-1], dataframe.index.day[-1])
        dateList = []
        while date != endDate:
            logger.info("Parsing date: %s.\n", date)
            dateList.append("%04d%02d%02d" % (date.year, date.month, date.day))
            date += datetime.timedelta(days=1)
        dateList.append("%04d%02d%02d" % (endDate.year, endDate.month, endDate.day))
        
        for date in dateList:
            dataFramePartial = dataframe[date:date]
            if not dataFramePartial.empty:
                formattedDataFile = formattedDataFilePath + "_%s_%s_%s.csv" % (date[:4], date[4:6], date[6:8])
                logger.info("Saving into file %s.", formattedDataFile)
                dataFramePartial.to_csv(formattedDataFile, header=None)
        return True
...
#     CSVDataFeed.convertTick(r"C:\Users\GGGF\git\historicaldata\RawData\ticks\sample.txt", r"C:\Users\GGGF\git\historicaldata\RawData\ticks\+HG#_tick_EminiFutureCopper")
    CSVDataFeed.convertTick(r"C:\Users\GGGF\git\historicaldata\RawData\ticks\AAPL_0.txt", r"C:\Users\GGGF\git\historicaldata\RawData\ticks\+HG#_tick_EminiFutureCopper")
