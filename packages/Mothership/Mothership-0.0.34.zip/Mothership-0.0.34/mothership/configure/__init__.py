...
class Configure:
    def __init__(self, config_file):
        ...
        try:
            ...
            raise ConfigureError('Database configuration error %s' % e)
        ...
        aws = all_configs.get('aws', None)
        if aws is not None:
            self.aws_access_key = aws.get('access_key', None)
            self.aws_secret_key = aws.get('secret_key', None)

        ec2 = all_configs.get('ec2', None)
        if ec2 is not None:
            self.ec2_region = ec2.get('region', 'us-east-1')
            self.ec2_key_pair = ec2.get('key_pair', None)
            self.ec2_instance_type = ec2.get('instance_type', 'm3.xlarge')
            self.ec2_subnet_id = ec2.get('subnet_id', None)
            self.ec2_image_id = ec2.get('image_id', None)
            self.ec2_security_group = ec2.get('security_group', 'default')
            self.ec2_profiles = ec2.get('profiles', [])
