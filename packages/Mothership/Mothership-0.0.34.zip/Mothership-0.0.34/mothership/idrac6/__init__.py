...
def sysinfo(info):
    ...
    r'(System BIOS Version\s+=\s(?P<bios>[.\d]+))',
        ...
        r'(MAC Address\s+=\s(?P<drac>[a-fA-F:0-9]+))',
        r'(NIC(?P<ifnum>\d+)\sEthernet\s+=\s(?P<hwaddr>[a-fA-F:0-9]+))',
        r'(NIC\.Integrated\.1\-(?P<ifnum>\d+)\-1\s+Ethernet\s+=\s(?P<hwaddr>[a-f:A-F:0-9]+))',
        ]
    ...
    for line in info.split('\n'):
        for pattern in parsers:
            ...
            if match:
                if 'ifnum' in match.groupdict().keys():
                    num = match.group('ifnum')
...
def prep_idrac(cfg, host, debug=False, basics=True, ipmi=False, serial=False, telnet=None, gold=False):
    ...
    if basics or ipmi:    # enable IPMI, continued
        ...
        os.system('/usr/bin/ipmitool -I lanplus -H %s -U root -P %s user priv 3 4' % (host, cfg.dpass))
        os.system('/usr/bin/ipmitool -I lanplus -H %s -U root -P %s user priv 4 4' % (host, cfg.dpass))
        if debug: os.system('/usr/bin/ipmitool -I lanplus -H %s -U root -P %s user list' % (host, cfg.dpass))
