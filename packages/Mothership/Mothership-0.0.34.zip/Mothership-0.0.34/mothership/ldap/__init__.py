...
def uadd(cfg, username):
    ...
    try:
        ...
        attributes = [('cn', str(u.username)),
                      ('sn', str(u.last_name)),
                      ('gecos', str(full_name)),
                      ('uid', str(u.username)),
                      ('uidNumber', str(u.uid)),
                      ('gidNumber', str(cfg.ldap_default_gid)),
                      ('homeDirectory', str(u.hdir)),
                      ('loginShell', str(u.shell)),
                      ('mail', str(u.email))]
        if u.ssh_public_key:
            attributes.append(('sshPublicKey', str(u.ssh_public_key)))
...
def udisplay(cfg, username):
    ...
    try:
        raw_res = ldcon.search_s(str(dn), ldap.SCOPE_BASE)
        print raw_res 
        ldcon.unbind()
    except ldap.LDAPError, e:
...
def uupdate(cfg, username):
    ...
    try:
        full_name = u.first_name + " " + u.last_name
        mod_record = [(ldap.MOD_REPLACE, 'sn', str(u.last_name)),
                      (ldap.MOD_REPLACE, 'gecos', str(full_name)),
                      (ldap.MOD_REPLACE, 'uidNumber', str(u.uid)),
                      (ldap.MOD_REPLACE, 'homeDirectory', str(u.hdir)),
                      (ldap.MOD_REPLACE, 'loginShell', str(u.shell)),
                      (ldap.MOD_REPLACE, 'mail', str(u.email)),
                      (ldap.MOD_REPLACE, 'sshPublicKey', str(u.ssh_public_key)),
                     ]
...
def gadd(cfg, groupname):
    ...
    try:
        ...
        attributes = [('description', str(g.description)),
                      ('cn', str(g.groupname)),
                      ('gidNumber', str(g.gid)),
                     ]
        if userlist:
            attributes.append(('memberUid', map(str, userlist)))
...
def gupdate(cfg, groupname):
    ...
    dn += ',dc='.join(d)
    try:
        mod_record = [(ldap.MOD_REPLACE, 'description', str(g.description)),
                      (ldap.MOD_REPLACE, 'gidNumber', str(g.gid)),
                      (ldap.MOD_REPLACE, 'memberUid', map(str, userlist)),
                     ]
        ldcon.modify_s(dn, mod_record)
    except ldap.LDAPError, e:
        raise LDAPError(e)
...
def ldapimport(cfg, realm_path):
    ...
    for result in ldcon.search_s(dn, ldap.SCOPE_SUBTREE, search, attr):
        ...
        userlist = result[1].get('memberUid', None)
...
def update_ldap_passwd(cfg, username):
    ...
    try:
        ...
        try:
            ldcon.modify_s(dn, [(ldap.MOD_REPLACE, 'userPassword', str(newpass))])
        except ldap.LDAPError, e:
            raise LDAPError(e)
        ldcon.unbind()
    except ldap.LDAPError, e:
