...
from pyppi.util import user_can_download, user_can_download_package, user_has_access_to_package
...
@register.assignment_tag(takes_context=True)
def can_list_package(context, package):
    request = context['request']
    has_perm = user_has_access_to_package(request, package)
    logger.debug('Check if {request.user} has access to (list/view) {package}: {has_perm}'.format(**locals()))
    return has_perm
