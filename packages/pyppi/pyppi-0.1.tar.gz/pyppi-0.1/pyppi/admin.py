...
from pyppi.models import Review, Package, Release, Distribution, Classifier, Maintainer, Owner, \
    PlatformName, PythonVersion, Architecture, DistributionType, MirrorSite, MirrorLog, IPRestriction, KnownHost, AccessLogEntry
...
class AccessLogEntryAdmin(ModelAdmin):
    list_display = ('datetime', 'logger', 'class_name', 'level', 'func_name', 'short_message')
    search_fields = ('message', )
    list_filter = ('level', 'logger')
    date_hierarchy = 'datetime'

    def short_message(self, h):
        return h.message[:100]
...
def override():
    ...
    register(AccessLogEntry, AccessLogEntryAdmin)
