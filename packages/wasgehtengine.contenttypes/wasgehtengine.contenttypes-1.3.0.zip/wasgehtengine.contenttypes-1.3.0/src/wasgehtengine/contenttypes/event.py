...
""" The following two functions appends the start-date of events created manually
    via-the-web to their id/path, see https://bugs.launchpad.net/wasgehtengine/+bug/1203370
    
    Unfortunately I did not find a way to set the id directly but the following code
    changes the title (from which Plone generates the id) and reverts this change afterwards.   
"""
@grok.subscribe(IEvent, IObjectCreatedEvent)
def append_date_to_title(object, event):
    object.title =  object.title + '-' + object.start.strftime('%Y-%m-%d')
    
@grok.subscribe(IEvent, IObjectAddedEvent)
def remove_date_from_title(object, event):
    object.title = object.title[:-11]