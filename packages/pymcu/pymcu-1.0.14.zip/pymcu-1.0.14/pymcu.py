...
class mcuModule:
    def __init__(self, port=None, baudrate=115200):
        ...
        self.DPins = [{'Min':1,'Max':19},{'Min':1,'Max':14}]
        self.PWMPins = [{'Min':1,'Max':5,'MinDuty':0,'MaxDuty':1023},{'Min':0,'Max':0,'MinDuty':0,'MaxDuty':0}]
    ...
    def spiEnable(self, clockPolarity, clockFreq, clockSelect, clockSample):
        ...
        if self.mcuType == 0:
            if clockFreq < 16 or clockFreq > 2000:
                sys.stderr.write('SPI Clock Freq. is out of range: [16-2000]\n')
                return
            cFreq = int((100.0 / clockFreq) * 40.0)
    ...
    @checkActive
    def pwmPeriod(self, preScaler, preScalerOffset):
        """Set PWM period
        ...
        if preScaler == 1:
            ...
            preScaler = 0
        postScaler = 1
        t4con = ((postScaler - 1) << 3) + 4 + preScaler
        self.mcuserial.write('P' + chr(t4con) + chr(preScalerOffset))
    ...
    @checkActive
    @checkPins
    def analogPinType(self,pinNum, type):
        """Set the analog pin type to analog(default) or digital
        Usage: analogPinType(pinNum, type)
        Example: analogPinType(1, 'digital')
    ...
    def owWrite(self, pinNum, mode, writeData):
        ...
        if self.mcuType == 0:
            ...
            self.mcuserial.write('O' + chr(mode) + byteArray)
            iw = ""
            while iw != "iw":
    ...
    def owRead(self, pinNum, mode, numBytes ):
        ...
        bit 1: 1 = Send reset after data
        bit 2: 0 = byte sized data, 1 = bit sized data
        Example: owRead(1,1,2) - Using Pin 1, send reset pulse before data, read 2 bytes
        returns a list item of read data"""
        if self.mcuType == 0:
    ...
    @checkActive
    @checkPins
    def pulseCount(self, pinNum, duration):
        """Count the number of pulses that occur during the duration on Digital Pin
        Usage: pulseCount(pinNum, duration)
        duration is in milliseconds, the highest possible frequency that can be counted is 200Khz
        Example: pulseCount(4, 10)
        [PulseCount will halt your program until the pulse count is complete]"""
        if self.mcuType == 0:
            if duration > 0 and duration < 60000:
                bL = duration & 255
                bH = duration >> 8
                self.mcuserial.write('C' + chr(pinNum) + chr(0))
                self.mcuserial.write(chr(0) + chr(bL) + chr(bH))
                self.mcuserial.timeout = int(duration / 1000) + 1
                pl = self.mcuserial.read(4)
                self.mcuserial.timeout = 1
                pl = int('0x' + pl, 0)
                return int(pl)
            else:
                sys.stderr.write('duration out of range, valid range [1-60000]\n')
        else:
            sys.stderr.write('Your current pyMCU board does not support this feature.\n')
    ...
    def reset(self):
        ...
        Example: reset()
        """
        self.mcuserial.write('!' + chr(0) + chr(0) + chr(0) + chr(0))
