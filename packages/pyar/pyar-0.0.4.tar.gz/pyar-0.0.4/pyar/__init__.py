...
#sb.default_dialect(SMARTSQL_DIALECTS.get(_dns_url.drivername, DEFAULT_DIALECT))
...
class Model(ModelBase):
    ...
    def __init__(self):
        ...
        self.logger = util.getLogger('.'.join((__name__, _using)))
    	_dns = pyar.settings.DATABASES.get(_using, None)
        if not _dns:
    ...
    def insert(self, sql):
        try:
            ...
            self.logger.debug(e)
            raise exc.DBOperationError("Database Operation Error.")

    def select(self, sql, flag='select_all'):
        try:
    ...
    def all(self, sql):
        try:
            _res = self.select(sql, 'select_all')
            if len(_res) <= 0:
                return None
            return _res
        except Exception as e:
    ...
    def first(self, sql):
        try:
            _res = self.select(sql, 'select_one')
            if len(_res) <= 0:
                return None
            return _res
        except Exception as e:
    ...
    def count(self, sql):
        try:
            _res = self.select(sql, 'select_count')
            if len(_res) <= 0:
                return 0
            for key in _res:
                return _res[key]
