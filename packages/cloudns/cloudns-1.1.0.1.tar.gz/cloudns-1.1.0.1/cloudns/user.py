...
def _check_res(res):
    ...
    if res['result'] != 'success':
        ...
        raise CloudnsRuntimeError(res['msg'])
...
def _record(res):
    ...
    see `_records' if you have many records.

    """
    try:
        obj = res['response']['rec']['obj']
        return Record(**obj)
    except KeyError:
        raise RecordNotFound()
...
def _count(res):
    try:
        ...
        raise CloudnsServerError(u'No response.count found', res)
...
class User(object):
    ...
    def create_records(self, zone, records):
        ...
        Args:
            records: a python object or a json string. Both should be a list
            of objects with these keys: type, name, content, isp, ttl.
        ...
        if not isinstance(records, basestring):
            raise CloudnsBadUsage(u'create_records: bad records param', records)
        if not records:
    ...
    def update_record(self, record, auto_retry=False):
        ...
        To update a record, first fetch a record with `get_records_by_name' or
        `get_record_by_id', then call new_record = old_record.update(...),
        finally [User|Zone].update_record(new_record).
    ...
    def delete_records(self, rids):
        ...
        return self._dnscall('bulk_rec_delete', rids=rids)
