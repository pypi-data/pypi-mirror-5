...
class AnnotationFile:
    ...
    def __init__( self, filename=None, bg_imgs=None, doreadheader=True,
        ...
        if justreadheader:
            ...
            if doreadbgmodel:
                ...
                self.ReadAnnHeader( doreadbgmodel=True, doreadstartframe=doreadtrx )
                if DEBUG_LEVEL > 0: print "Done reading header."
            else:
                if DEBUG_LEVEL > 0: print "Reading settings ..."
                self.ReadAnnHeader( doreadbgmodel=False,
                                    doreadstartframe=False )
        ...
        if not newfile:
            try:
                ...
                if doreadheader or doreadbgmodel or doreadtrx:
        ...
        if doreadheader and not newfile:
            self.ReadAnnHeader( doreadbgmodel=doreadbgmodel )
        ...
        try:
            ...
            try:
                ...
                if params.interactive:
                    wx.MessageBox( "I/O error writing annotation", "Error", wx.ICON_ERROR|wx.OK )
                raise
        finally:
            if release_lock:
                self.file_lock.release()
    ...
    def WriteAnnHeader( self, start_frame ):
        ...
        self.file.write( 'enforce_minmax_shape:%d\n' % int( params.enforce_minmax_shape ) )
    ...
    def ReadAnnHeader( self, doreadbgmodel=True, doreadstartframe=True,
        ...
        with self.file_lock:
            ...
            while True:
                ...
                elif parameter == 'enforce_minmax_shape':
                    params.enforce_minmax_shape = bool( int( value ) )
    ...
    def WriteMAT( self, filename ):
        ...
        if self.nbuffer < nframes:
            ...
            if params.feedback_enabled:
                a = wx.MessageBox(msgtxt + ' Proceed?',
                                  "Store to file?",
                                  wx.YES|wx.NO)
                if a == wx.NO:
                    return
    ...
    def WriteCSV( self, filename ):
        ...
        for fr in range( nframes ):
            ...
            for ee in ells.itervalues():
                if ee.identity not in used_ids:
                    used_ids.append( ee.identity )
        n_ids = len( used_ids )

        print "N ids:", n_ids
        arr = num.zeros( (nframes, n_ids*6) ) - 1.
        for fr in range( nframes ):
            ells = self.get_frame( off + fr )
            for ee in ells.itervalues():
                i = used_ids.index( ee.identity )
