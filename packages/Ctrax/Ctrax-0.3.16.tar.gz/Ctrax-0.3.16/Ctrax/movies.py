...
If initpath is a directory and not in interactive mode, it's an error."""
        ...
        if os.path.isfile( initpath ):
            ...
            dlg = wx.FileDialog( parentframe, "Select movie", initpath, "", dialog_str, wx.OPEN )
            ...
            raise ValueError( "not in interactive mode but wasn't given a full filename" )
    ...
    def open( self ):
        ...
        if ext not in known_extensions():
            print "unknown file extension; will try AVbin to open"
        ...
        # read AVI
            ...
            try:
                ...
                try:
                    ...
                    if self.interactive:
                        wx.MessageBox("Your movie is most likely compressed. Out-of-order frame access (e.g. dragging the frame slider toolbars around) will be slow. At this time, the frame chosen to be displayed may be off by one or two frames, i.e. may not line up perfectly with computed trajectories.","Warning",wx.ICON_WARNING|wx.OK)
                    else:
...
class Avi:
    ...
    def read_header( self ):
        ...
        if real_file_len > approx_file_len*1.1:
            ...
            old_n_frames = self.n_frames
            self.n_frames = int( num.floor( (real_file_len - cur_pos)/self.buf_size ) )
            print "guessing %d frames in movie, although header said %d"%(self.n_frames,old_n_frames)
...
class CompressedAvi:
    ...
    def __init__(self,filename):
        ...
        if not hasattr( self.source, '_seek' ):
            # Pyglet >= 1.2
            self.source._seek = self.source.seek
        self.ZERO = 1./1000000.
        ...
        if DEBUG_MOVIES: print 'first frame read results in im = ' + str(im)
        if DEBUG_MOVIES: print 'and ts = ' + str(ts)
        self.source._seek(self.ZERO)
        self.start_time = self.source.get_next_video_timestamp()
        if DEBUG_MOVIES: print 'First seek to ZERO succeeded, start_time = ' + str(self.start_time)
        ...
        if DEBUG_MOVIES: print 'duration_seconds = ' + str(self.duration_seconds)
        self._estimate_fps()
        if DEBUG_MOVIES: print 'fps estimated to be' + str(self.fps)
        self.n_frames = int(num.floor(self.duration_seconds * self.fps))
        if DEBUG_MOVIES: print "n_frames estimated to be %d"%self.n_frames
        self.frame_delay_us = 1e6 / self.fps
        self._estimate_keyframe_period()
        if DEBUG_MOVIES: print 'keyframe period estimate to be ' + str(self.keyframe_period_s) + ' s, ' + str(self.keyframe_period) + ' frames'
    ...
    def get_frame(self,framenumber):
        ...
        if DEBUG_MOVIES: print "keyframe before frame %d is %d (ts = %f)"%(framenumber,lastkeyframe,lastkeyframe_s)
        ...
        if framenumber > self.currframe and self.currframe >= lastkeyframe:
            ...
            if DEBUG_MOVIES: print "the best thing to do is just to read in frames until we get to desired %d (currframe = %d)"%(framenumber,self.currframe)
        ...
        if DEBUG_MOVIES: print "we need to seek"
        ...
        if DEBUG_MOVIES: print "frame %d is equivalent to time stamp %f"%(framenumber,wantts)
        while True:
            # try seeking
            if DEBUG_MOVIES: print "start_time = %f"%self.start_time
            if DEBUG_MOVIES: print "trying to seek to %f (actually %f)"%(lastkeyframe_s,lastkeyframe_s-self.start_time)
            try: 
            ...
            if DEBUG_MOVIES: print "ended up seeking to %f"%tscurr

            # are we in the right ballpark?
            if wantts < tscurr:

                lastkeyframe_s = max(self.ZERO,lastkeyframe_s-self.keyframe_period_s)
                if DEBUG_MOVIES: print "still not back far enough, trying to seek to %f"%lastkeyframe_s
                dirmoved = -1
                continue
                ...
                if dirmoved == -1:
                    ...
                    # so we're screwed up
                    # let's just stay
                    if DEBUG_MOVIES: print "uh oh -- we've already gone backward, and now we think we need to go forward, so we're just going to stay"
                    break
                ...
                if DEBUG_MOVIES: print "we're in the right ballpark"
                break
...
def write_results_to_avi(movie,tracks,filename,f0=None,f1=None):
    ...
    # write the index
    write_avi_index(movie,tracks,offsets,outstream)

    # close
    outstream.close()

def write_avi_index(movie,tracks,offsets,outstream):
