...
def inner_procedure(proc):
    ...
    if not proc.external:
        ...
        for inner_proc in proc.inner_procedures:
        ...
        tr_code, tr_decl = transition(proc.start.transition)
        # Generate code for the floating labels
        code_labels = []
        for label in proc.floating_labels:
...
def generate(process):
    ...
    #OUT_SIGNALS.update({sig.lower(): defn for sig, defn in
    ...
    #PROCEDURES.update({p.lower(): defn for p, defn in
    #        process.procedures.viewitems()})
    INNER_PROCEDURES.extend(process.inner_procedures)
    ...
    for input_signal in input_signals: #process.inputSignals.viewkeys():
...
package {process_name} is'''.format(process_name=process_name,
    ...
    for proc in process.inner_procedures:
    ...
    for signal in process.input_signals:
            ...
            if input_def:
                ...
                if input_def.transition:
                    taste_template.append('runTransition({idx});'.format(
                        ...
                        #t=input_def.inputlist[signal['name']]))
                else:
        ...
        taste_template.append('end {sig_name};'.format(sig_name=signal['name']))
    ...
    # Generate code for the floating labels
    code_labels = []
    for label in process.floating_labels:
        code_label, label_decl = floating_label(label)
        code_labels.extend(code_label)
