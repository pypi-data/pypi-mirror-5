...
def paste_floating_objects(scene):
    ...
    for item_list, terminators in COPY_PASTE:
        ...
        for state in states:
            ...
            try:
                new_item = Renderer.render_state(
                        CLIPBOARD, state, terminators, states)
            except TypeError:
                # Discard terminators (explanation given in render_state)
                pass
            else:
        ...
        for text_area in text_areas:
            ...
            new_item = Renderer.render_text_area(scene, text_area)
            symbols.add(new_item)
    return symbols
    ...
    for item_list, _ in COPY_PASTE:
        ...
        for i in [c for c in item_list if not isinstance
            ...
            if isinstance(i, ogAST.Input):
                ...
                new_item = Renderer.render_input(
                        CLIPBOARD,
                        i, parent=None, states=states)
            else:
                new_item = Renderer.render_action(
                        CLIPBOARD,
                        i, parent=None, states=states)
