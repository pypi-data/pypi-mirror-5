    ...
    def focusOutEvent(self, event):
        ...
        if not self.completer or not self.completer.isVisible():
            ...
            if self.oldText != str(self):
                ...
                self.scene().undo_stack.push(undo_cmd)
            # End macro that was started in focusInEvent
            ...
            if text_cursor.hasSelection():
                ...
                self.setTextCursor(text_cursor)
            # Do not repeat checks if no changes were made:
            if change:
...
class Connection(QGraphicsPathItem, object):
    ...
    def reshape(self):
        ...
        if self.child_is_a_connection_point:
            ...
            # FIXME - why is the pos of the child above the connection point
            # when loading?!
            self.end_point.setY(
                    max(self.child.pos().y(), self.start_point.y()))
...
class Symbol(QObject, QGraphicsPathItem, object):
    ...
    # Decide if a symbol can be copied or if only one of its kind can exist
    is_singleton = False
        ...
        if action:
            if action.text() == png_action:
                ...
                if save_fmt == 'png':
                    ...
                    generator.setTitle('OpenGEODE SDL Diagram')
                    ...
                    self.scene().render(pdf_painter, source=rect)
                    pdf_painter.end()
    ...
    def resize_item(self, rect):
        ...
        for child in self.childSymbols():
            # note, comments will not move.. is this what we want? (FIXME)
    ...
    def mouse_release(self, _):
        ...
        if self.mode == 'Resize' and self.old_rect != self.boundingRect():
            ...
            undo_cmd = undoCommands.MoveSymbol(self, self.coord, self.pos())
            self.scene().undo_stack.push(undo_cmd)
            self.cam(self.coord, self.pos())
            self.scene().undo_stack.endMacro()
        self.mode = ''
...
class Comment(Symbol, object):
    ...
    def __init__(self, parent=None, ast=None):
        ...
        # FIXME : should not be needed anymore using QPainterPath
...
#    def paint(self, painter, _, ___):
       # Removed MP 02/05/2013 - Not needed, positions are computed properly
       #       '''
...
#       if self.hasParent:
#           self.update_position()
#       super(VerticalSymbol, self).paint(painter, _, ___)
