...
def path_type(path):
    ...
    if not path or not path[0].lower() in VARIABLES:
        return None, None
    kind = ''
    asn1_name = VARIABLES[path[0].lower()].replace('_', '-')
        ...
        for elem in path[1:]:
            ...
            # Sequence of
                ...
                raise TypeError('Expression ' + '.'.join(path) +
                                ' does not resolve')
        asn1_name = current['ReferencedTypeName'].replace('-', '_')
    kind = find_basic_type(current).get('Kind').replace('-', '_')
    LOG.debug('[Ada Generator] Type of path ' + '!'.join(path) +
             ' is ' + asn1_name + ' (' + kind + ')')
    return kind, asn1_name
...
def write_statement(param, newline):
    ...
    if type_kind.endswith('StringType'):
        if param.var.kind == 'stringLiteral':
            # Raw string
            string = '"' + param.var.stringLiteral[1:-1] + '"'
        else:
...
def decipher_primary_id(primaryId):
    ...
    sep = 'l_' if primaryId[0].lower() in VARIABLES else ''
        ...
        if type(pr_id) is not dict:
            ...
            if parent_kind == 'ChoiceType':
                ada_string = ('asn1Scc{typename}_{p_id}_get({ada_string})'
                        .format(typename=parent_typename.replace('-', '_'),
...
def decipher_sequence(seq, seqType):
    ...
    seq_ty = seqType.get('ReferencedTypeName', seqType)
...
def format_ada_code(stmts):
    ...
    for line in stmts[:-1]:
        ...
        if elems and elems[-1] in ('is', 'then', 'loop'):
            indent += 1
