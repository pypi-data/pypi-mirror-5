...
SPECIAL_OPERATORS = ['length', 'write', 'writeln', 'present', 'set_timer',
        'reset_timer']
...
def sdl_to_asn1(sort):
    ...
    '''
    if not DV:
        raise TypeError('No dataview - type not found')
    for asn1_type in DV.types.viewkeys():
...
def find_basic_type(a_type):
    ...
    basic_type = a_type or {'Kind': 'NoneType'}
    while basic_type['Kind'] == 'ReferenceType':
        # Find type with proper case in the data view
        for typename in DV.types.viewkeys():
            if typename.lower() == basic_type['ReferencedTypeName'].lower():
                basic_type = DV.types[typename]['type']
                break
        else:
            raise TypeError('Type "' + basic_type['ReferencedTypeName'] +
...
def check_and_fix_op_params(op_name, expr_list, context):
    ...
    for idx, param in enumerate(expr_list):
        ...
        if signature is not None:
            param_type = signature[idx].get('type').replace('_', '-')
            # Retrieve the type (or None if it is a sepecial operator)
            dataview_entry = DV.types.get(param_type) or {}
    ...
    if (primary.kind == 'primaryId'
        ...
        if ref_type:
            ...
            if actual_type.get('extraAttr'):
                ...
                primary.primType = actual_type # ref_type
            LOG.debug('... It is!')
            return True
        else:
...
def text_area_content(root, ta_ast, context):
    ...
    for child in root.getChildren():
        if child.type == lexer.DCL:
            ...
            context.timers.extend(timer.text for timer in child.children)
        else:
            warnings.append(
    ...
    for child in root.getChildren():
        if child.type == lexer.ID:
            ...
            try:
                ...
                LOG.debug('[check_and_fix_op_params] bug: '
