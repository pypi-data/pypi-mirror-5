...
from PySide.QtGui import(QGraphicsScene, QApplication, QToolBar, QIcon,
        ...
        QWidget, QDockWidget, QListWidget, QImage, QListWidgetItem, QPrinter,
...
from genericSymbols import(Symbol, Comment, EditableText,
        Cornergrabber)
...
class SDL_Scene(QGraphicsScene, object):
    ...
    def render_process(self, process):
        ...
        for top_level in Renderer.render_process(self, process):
            ...
            if top_level.nested_scene:
                ...
                for sub_top_level in Renderer.render_process(
                                        subscene, top_level.nested_scene):
    ...
    def sdl_to_statechart(self):
        ...
        for state, inputs in process_ast.mapping.viewitems():
            ...
            # Add edges
            transitions = inputs if state != 'START' else [process_ast.start]
    ...
    def export_img(self, filename=None, doc_format='png'):
        ...
        rect.adjust(-5, -5, 5, 5)
        if doc_format == 'png':
            device = QImage(rect.size().toSize(),
...
#       '''
#           When scene gets focus, make sure that no items actually gets
#           focus. Qt default behaviour is to give focus back to the item that
#           had it before focus from the scene was lost, but this has bad
#           side effects when text edition is given focus
#       '''
#       self.clear_focus()
    ...
    def mousePressEvent(self, event):
        ...
        if self.button_selected:
            ...
            try:
                ...
                compatible_insert = (self.button_selected.__name__ in
                        parent_item.allowed_followers)
            except AttributeError:
                # User did not click on a parent symbol
                if not self.button_selected.needs_parent:
                    compatible_insert = True
                    LOG.debug('Insertion is compatible')
                else:
                    compatible_insert = False
                    LOG.debug('Insersion not compatible')
            if not compatible_insert:
                self.messages_window.addItem('Cannot insert this symbol here')
                self.button_selected = None
                return
            with undoCommands.UndoMacro(self.undo_stack, 'Place Symbol'):
                self.place_symbol(
                        ...
                        parent=parent_item,
                        pos=event.scenePos() if not parent_item else None)
            self.button_selected = None
        else:
    ...
    def add_symbol(self, item_type):
        ...
        selection = list(self.selected_symbols())
        try:
            # One item is selected. Use it as parent item
            selection, = selection
            with undoCommands.UndoMacro(self.undo_stack, 'Place Symbol'):
...
class SDL_View(QGraphicsView, object):
    ...
    def set_toolbar(self):
        ...
        self.toolbar.set_actions(bar_items=ACTIONS[self.scene().context])
    ...
    def mouseDoubleClickEvent(self, evt):
        ...
        if evt.button() == Qt.LeftButton:
            try:
                item = self.itemAt(evt.pos()).parentItem()
            except AttributeError:
                LOG.debug('Ignoring double click')
            else:
                if item and item.nested_scene:
    ...
    def save_diagram(self, save_as=False, autosave=False):
        ...
        try:
            ...
            if not autosave:
                ...
                LOG.info('Auto-saving backup file completed:' + filename)
            return True
        except AttributeError:
...
class OG_MainWindow(QMainWindow, object):
    ''' Main GUI window '''
    ...
    def start(self, file_name):
        ...
        statechart_dock = self.findChild(QDockWidget, 'statechart_dock')
...
def opengeode():
    ...
    Clipboard.CLIPBOARD = PySide.QtGui.QGraphicsScene()

    LOG.debug('Starting OpenGEODE version ' + __version__)

    if options.check or options.toAda or options.png or options.pdf:
        ...
        if export_fmt:
            ...
            for doc_fmt in export_fmt:
                ...
                scene.export_img(process.processName, doc_format=doc_fmt)
        elif options.pdf:
            pass
    else:
