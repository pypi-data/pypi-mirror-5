...
class TargetError(TaskError):
  """
    ...
    :param list targets: A list of pants targets that failed
  """
...
class Task(object):
  ...
  def __init__(self, context):
    ...
    default_cache_dir = self.get_workdir(section='build_cache', workdir='build_cache')
    self._build_cache = context.config.get('tasks', 'build_cache', default=default_cache_dir)
  ...
  def invalidate(self, all_=False):
    """
      Invalidates the target cache used by this task.

      The next check of changed is guaranteed to indicate that any target checked has in fact
      changed.

      :param bool all_: If True then all Task caches are invalidated, not just this tasks cache.
    """
    self.context.log.debug('Invalidating caches for %s' % 'all tasks' if all_ else str(self))
    safe_rmtree(self._build_cache if all_ else self._basedir)
  ...
  class CacheManager(object):
    ...
    def _fingerprint_jardeps(self, target, sha):
      ...
      for external_target in alltargets - internaltargets:
        sha.update((external_target.cache_key() if issubclass(external_target, ExternalDependency)
                    else str(external_target)).encode('utf8'))
  ...
  def get_workdir(self, section="default", key="workdir", workdir=None):
    return self.context.config.get(section,
                                   key,
                                   default=os.path.join(self._pants_workdir, workdir or
