...
from twitter.pants.tasks.javadoc_gen import javadoc
from twitter.pants.tasks.scaladoc_gen import scaladoc
...
def is_java_library(target):
  return has_sources(target, '.java') and not is_binary(target)


def is_scala_library(target):
  return has_sources(target, '.scala') and not is_binary(target)
...
class JarCreate(Task):
  ...
  def setup_parser(cls, option_group, args, mkflag):
    ...
    option_group.add_option(mkflag("javadoc"), mkflag("javadoc", negate=True),
                            dest="jar_create_javadoc", default=None,
                            action="callback", callback=mkflag.set_bool,
                            help="[%default] Create javadoc jars (includes scaladoc etc.)")
