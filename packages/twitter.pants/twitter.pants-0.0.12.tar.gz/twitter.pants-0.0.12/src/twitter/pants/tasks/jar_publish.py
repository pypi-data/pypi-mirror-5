...
class PomWriter(DependencyWriter):
  def __init__(self, get_db):
    super(PomWriter, self).__init__(get_db, os.path.join('templates', 'jar_publish', 'pom.mk'))
  ...
  def jardep(self, jar, classifier=None, synth=False):
    return TemplateData(
      ...
      rev=jar.rev,
      scope='runtime' if (classifier or jar.classifier) == 'idl' else 'compile',
  ...
  def internaldep(self, jar_dependency, dep=None, synth=False):
    classifier = 'idl' if (is_codegen(dep) and synth) or isinstance(dep, ThriftLibrary) else None
    return self.jardep(jar_dependency, classifier=classifier, synth=synth)


class IvyWriter(DependencyWriter):
  def __init__(self, get_db):
    super(IvyWriter, self).__init__(get_db, os.path.join('templates', 'ivy_resolve', 'ivy.mk'))
  ...
  def internaldep(self, jar_dependency, dep=None, synth=False):
    classifier = 'idl' if (is_codegen(dep) and synth) or isinstance(dep, ThriftLibrary) else None
    return self._jardep(jar_dependency, classifier=classifier, synth=synth)


def is_exported(target):
  return provides(target) and isinstance(target, (AnnotationProcessor, JavaLibrary, ScalaLibrary,
                                                  ThriftLibrary))
...
class JarPublish(Task):
  ...
  * ``ivy_jvmargs`` - Optional list of JVM command-line args when invoking Ivy.
  ...
  def __init__(self, context, scm=None, restrict_push_branches=None):
    ...
    self._jvmargs = context.config.getlist('jar-publish', 'ivy_jvmargs', default=[])
  ...
  def execute(self, targets):
    ...
    def stage_artifacts(tgt, jar, version, changelog, confs=None, synth_target=None):
      ...
      if class_target:
        ...
        if not jarmap.empty() and (is_java(tgt) or is_scala(tgt)):
        ...
        if self.dryrun:
          ...
          def publish(ivyxml_path):
            ...
            result = binary_util.runjava_indivisible(jvmargs=jvmargs, classpath=self.ivycp,
                                                     opts=opts)
            if result != 0:
              raise TaskError("java -jar %s ... exited non-zero (%i) 'failed to "
                              "push %s'" % (self.ivycp[0], result,
  ...
  def commit_push(self, org, name, rev):
    args = dict(
      org=org,
      name=name,
      rev=rev,
  ...
  def generate_ivysettings(self, publishedjars, publish_local=None):
    template_relpath = os.path.join('templates', 'jar_publish', 'ivysettings.mk')
