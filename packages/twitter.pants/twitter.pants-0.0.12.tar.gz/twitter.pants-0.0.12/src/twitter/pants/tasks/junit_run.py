...
_RUNNER_MAIN = 'com.twitter.common.junit.runner.ConsoleRunner'
...
class JUnitRun(JvmTask):
  ...
  def execute(self, targets):
    if not self.context.options.junit_run_skip:
      ...
      if tests:
        _, classpath = binary_util.profile_classpath(self.junit_profile)
        junit_classpath = self.classpath(classpath, confs=self.confs)
        ...
        if self.coverage:
          _, emma_classpath = binary_util.profile_classpath(self.emma_profile)
          ...
          def instrument_code():
            ...
            with binary_util.safe_args(self.get_coverage_patterns(targets)) as patterns:
              ...
              main = 'emma'
              result = binary_util.runjava_indivisible(classpath=emma_classpath, main=main,
                                                       opts=opts)
              if result != 0:
          ...
          run_tests(junit_classpath, _RUNNER_MAIN)
