...
``:pingpong-lib`` is the *address* of another target. Addresses look,
roughly, like ``path/to/BUILD:targetname``.
This address starts with a colon; that means it's in the same directory;
we can see it earlier in the same ``BUILD`` file:
