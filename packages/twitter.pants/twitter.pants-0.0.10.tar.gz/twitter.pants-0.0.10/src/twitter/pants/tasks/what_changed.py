...
from twitter.pants import get_buildroot, has_sources, TaskError, get_scm, is_codegen
...
class WhatChanged(ConsoleTask):
  ...
  def console_output(self, _):
    ...
    if self._show_files:
      for file_ in touched_files:
        yield file_
    else:
      touched_targets = set()
      for file_ in touched_files:
        for touched_target in self._owning_targets(file_):
  ...
  def _owning_targets(self, file_):
    for build_file in self._candidate_owners(file_):
      is_build_file = (build_file.full_path == os.path.join(get_buildroot(), file_))
      for address in Target.get_all_addresses(build_file):
        ...
        if target and (is_build_file or (has_sources(target) and self._owns(target, file_))):
          yield target

  def _candidate_owners(self, file_):
    build_file = BuildFile(get_buildroot(), relpath=os.path.dirname(file_), must_exist=False)
  ...
  def _owns(self, target, file_):
    ...
    return file_ in self._filemap[target]
