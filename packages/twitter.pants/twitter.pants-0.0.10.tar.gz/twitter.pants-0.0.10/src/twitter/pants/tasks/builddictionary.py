...
def get_syms():
  r = {}
  vc = twitter.pants.base.ParseContext.vanilla_context()
  for s in vc:
...
def entry_for_one_class(nom, klas):
  ...
  try:
    ...
    argspec = inspect.formatargspec(args, varargs, varkw, defaults)
    selfless_argspec = argspec.replace("(self, ", "(")
    selfless_argspec = selfless_argspec.replace("(cls, ", "(")
    funcdoc = klas.__init__.__doc__
  except TypeError:  # __init__ might not be a Python function
    selfless_argspec = None
    funcdoc = None
  return entry(nom,
               classdoc=klas.__doc__,
               argspec=selfless_argspec,
               funcdoc=funcdoc)
...
class BuildBuildDictionary(Task):
  ...
  def execute(self, targets):
    d = assemble()
    template_relpath = os.path.join('templates', 'builddictionary', 'page.mk')
    template = resource_string(__name__, template_relpath)
    outdir = (self.context.config.get("builddict", "outdir") or
              os.path.join(self.context.config.getdefault("pants_distdir"),
                           "builddict"))
    tocs = [tocl(d),
    ...
    with safe_open(os.path.join(outdir, "build_dictionary.rst"), "w") as outfile:
