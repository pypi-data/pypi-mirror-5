...
class SourceRoot(object):
  ...
  def reset(cls):
    ...
    cls._ROOTS_BY_TYPE = defaultdict(OrderedSet)
    cls._TYPES_BY_ROOT = defaultdict(OrderedSet)
    cls._SEARCHED = set()
  ...
  @staticmethod
  def roots(target_type):
    """Returns the set of roots for given target type."""
    return SourceRoot._ROOTS_BY_TYPE[target_type]
  ...
  def __init__(self, basedir, *types):
    ...
    :param types: The target types to register ``basedir`` as a source root for
    """
