...
class ScroogeGen(NailgunTask):
  ...
  def _tempname(self):
    ...
    fallback = os.path.join(buildroot, '.pants.d')
  ...
  def gen(self, partial_cmd, targets):
    with self.changed(targets, invalidate_dependants=True) as changed_targets:
      compiler = partial_cmd.compiler
      import_paths, changed_srcs = compiler.calc_srcs(changed_targets, self.is_gentarget)
      outdir = partial_cmd.outdir
      if changed_srcs:
        opts = []

        for import_path in import_paths:
          opts.extend(['--import-path', import_path])

        opts.extend(['--language', partial_cmd.language])
        ...
        opts.extend(['--gen-file-map', gen_file_map_path])

        returncode = self.runjava_indivisible(jvmargs=['-Dfile.encoding=UTF-8'],
                                              classpath=self.profile_classpath(compiler.profile),
                                              main=compiler.main,
                                              opts=opts,
                                              args=changed_srcs)
        try:
        ...
        self.write_gen_file_map(gen_files_for_source, changed_targets, outdir)
