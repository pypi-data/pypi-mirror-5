...
class IvyResolve(NailgunTask):
  ...
  def setup_parser(cls, option_group, args, mkflag):
    ...
    option_group.add_option(mkflag("settingsxml"), dest="ivy_settings",
                            help = "Pass a different ivysettings.xml to ivy.")

    option_group.add_option(mkflag("cache-dir"), dest="ivy_resolve_cache_dir",
                            help = "Path of the ivy cache directory.")
  ...
  def __init__(self, context, confs=None):
    ...
    # Disable cache in File.getCanonicalPath() to make Ivy work with -symlink option properly on nailgun.
    nailgun_args = ['-Dsun.io.useCanonCaches=false']
    NailgunTask.__init__(self, context, classpath=classpath, workdir=nailgun_dir, nailgun_args=nailgun_args)

    self._ivy_settings = context.options.ivy_settings or context.config.get('ivy', 'ivy_settings')
    self._cachedir = context.options.ivy_resolve_cache_dir or context.config.get('ivy-resolve', 'cache_dir')
    ...
    self._template_path = os.path.join('templates', 'ivy_resolve', 'ivy.mk')
  ...
  def execute(self, targets):
    ...
    with self.changed(self._extract_classpathdeps(targets), only_buildfiles=True) as changed_deps:
      if changed_deps:
  ...
  def _generate_ivy(self, jars, excludes, ivyxml):
    ...
    # We pair an <override/> with each forced dependency to effect transitive forces.
...
<ivy-report version="1.0">
    ...
    for conf in self._confs:
      ...
      xalan_main = 'org.apache.xalan.xslt.Process'
      result = self.runjava_indivisible(xalan_main, classpath=classpath, opts=opts)
      if result != 0:
        raise TaskError("java %s ... exited non-zero (%i)" % (xalan_main, result))
      reports.append(out)
  ...
  def _exec_ivy(self, target_workdir, targets, args):
    ...
    ivy_main = 'org.apache.ivy.Main'

    result = self.runjava_indivisible(ivy_main, opts=ivy_opts)
    if result != 0:
      raise TaskError('java %s ... exited non-zero (%i)' % (ivy_main, result))
