...
You invoke pants with a *goal* (like ``test`` or ``eclipse``) and the
...
Pants knows about goals ("verbs" like *compile* and *test*) and targets (build-able
things in your source code). A typical pants command-line invocation looks like ::
...
``tests/java/com/twitter/common/examples/pingpong/BUILD:pingpong``
...
``dist/``
    By default, build outputs go in the ``dist/`` directory.
...
``java/com/twitter/common/examples/pingpong/BUILD`` defines the binary
program we compiled and ran.  This target is named ``pingpong`` and
...
The ``pingpong`` binary has one dependency, ``pants(':pingpong-lib')``.
That name starting with a colon means it's in the same directory; we
can see it earlier in the same ``BUILD`` file:
...
**name**
  ...
  ``dependencies``:
**dependencies**
  List of things this target depends upon. If this target imports code
