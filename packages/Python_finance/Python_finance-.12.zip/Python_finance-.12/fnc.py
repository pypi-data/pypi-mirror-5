...
def M(x):
    return (1/np.sqrt(2*pi))*(e**(-.5*(x**2)))


#calculate call option value
#input (time to maturity, Stock value, Strike, interest rate, volatility)
def put(tm, S, K, r, v):
    d1 = (1/(v*np.sqrt(tm)))*(math.log(S/K, e)+(r+(v**2)/2)*tm)
    d2 = (1/(v*np.sqrt(tm)))*(math.log(S/K, e)+(r-(v**2)/2)*tm)
...
def Call_Theta(tm, S, K, r, v):
    d1 = (1/(v*np.sqrt(tm)))*(math.log(S/K, e)+(r+(v**2)/2)*tm)
    d2 = (1/(v*np.sqrt(tm)))*(math.log(S/K, e)+(r-(v**2)/2)*tm)
    k = (-S*M(d1)*v)*(1/(2*np.sqrt(tm)))-P(-10,d2)*r*K*e**(-r*tm)
    return k
