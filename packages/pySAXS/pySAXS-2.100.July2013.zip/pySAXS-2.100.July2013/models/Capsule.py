...
from pySAXS.LS.LSsca import Pcylh
...
class Capsule(Model):
    ...
    by OT 10/06/2009
    '''
    def CapsuleFunction(self,q,par):
        """
        q array of q (A-1)
        par[0] Radius(A)
        par[1] Length (A)
        ...
        R=par[0]
        L=par[1]
        return par[2]*(1.e-48)*((par[3]-par[4])**2.)*((numpy.pi*(par[0])**2.*(par[1]))**2.)*Pcylh(q,R,L)
    ...
    from LSsca
    '''
    IntensityFunc=CapsuleFunction #function
    N=0
    q=Qlogspace(1e-4,1.,100.)      #q range(x scale)
    ...
    name="Capsule"          #name of the model
    author="O Spalla"   #name of the authors
