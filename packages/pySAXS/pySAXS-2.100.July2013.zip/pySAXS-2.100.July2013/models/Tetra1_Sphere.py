...
from pySAXS.LS.LSsca import Tetra1_Multiplet
import numpy

class Tetra1_Sphere(Model):
    '''
    
    class monoSphere from LSSca
    by OS 19/11/2011
    '''
    def Tetra1_Function(self,q,par):
        """
        q array of q (A-1)
        ...
        """
        rho=[par[3]-par[2],par[3]-par[2],par[3]-par[2],par[3]-par[2]]
        R=[par[0],par[0],par[0],par[0]]
        if len(par)!=5:
            sys.stderr.write("This function requires a list of 5 parameters")
            return -1.
        else:
            return par[4]*Multiplet(q,Tetra1_Multiplet(par[1]/2.),rho,R)
    ...
    from LSsca
    '''
    IntensityFunc=Tetra1_Function #function
    ...
    name="Multi: Tetrahedra of identical spheres"          #name of the model
