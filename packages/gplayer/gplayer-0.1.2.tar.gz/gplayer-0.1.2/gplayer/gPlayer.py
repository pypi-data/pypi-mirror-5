...
def play(config=None):
	...
	try : 
		if VERSION > oConfig.getConfig('version') :
			raise Exception("Programm-Version: %s > Config-Version: %s" % \
			 (gPlayerHelp.fver(VERSION), gPlayerHelp.fver(oConfig.getConfig('version'))))
	except :
		errorhandling.Error.show()
		sys.exit(2)
	...
	args = gPlayerHelp.parser.parse_args()
	
	if args.showPics == 'help':
		gPlayerHelp.showPicsHelp()
		return

	if args.fullhelp:
		gPlayerHelp.showFullHelp(VERSION)
		return
	...
	if len(args.FILE) <= 0 and \
	  bResume == False :
		sys.stderr.write(gPlayerHelp.getLangText('gui', 'nofile') + '\n') 
		return 
