...
# Thomas Korell <thomas DOT Korell AT gmx DOT de>
# GNU General Public License
###############################################################################

"""Dieses Modul enthält die GUI-Verarbeitung und benötigt die Module
gPlayerAudioFile.py und gPlayerGST.py. """

import sys
import os
import platform
from gplayer import errorhandling
...
try:
	# aus irgendeinem Grunde braucht Windows gtk für gobjects-import in gPlayerGST
	import gtk
	from pygame.locals import *	
	import pygame
	from gplayer.metadaten import audiofile
	from gplayer.gPlayerGST import GstPlayer
	from gplayer.config import readconfig
	from gplayer.gPlayerHelp import SHOWPIC_CHOICES
	import random
	import logging
except ImportError, err:
	errorhandling.Error.show()
...
# -------------------- Texte anzeigen ----------------------------------------
	...
	def blitText(self):
		...
		if not self.screen.get_locked() : 
			...
			except pygame.error, err: logging.warning("gPlayer: Error at pygame.display.update() in function blitText(). %s" % (err))
			self.screen.unlock()
		...
		if len(comments) > 0 :
			for t in comments :
				...
				if not self.screen.get_locked() : 
					...
					except pygame.error, err: logging.warning("gPlayer: Error at pygame.display.update() in function blitComments(). %s" % (err))
					self.screen.unlock()
				h += txtH
	...
	def updateActTime(self):
		...
		if not self.screen.get_locked() : 
			try: self.screen.blit(fontActPos, (w, h)) 
				...
				logging.warning(\
					"gPlayer: Error at self.screen.blit(textActPos, (%s, %s)) in function updateActTime(). %s " % (w, h, err))
				self.doQuit()
		if not self.screen.get_locked() : 
			self.screen.lock()
			try: pygame.display.update((w, h, txtW, txtH))
...
# -------------------- Sync-Texte -----------------------------------------------------------------
	...
	def updateSyncLyrics(self):
		...
		if self.gstPlayer.queryPositionInMilliseconds() >= self.audioFile.syncTime[self.audioFile.syncCount] :
			...
			if self.audioFile.syncCount < (len(self.audioFile.syncText) - 1) :
...
# -------------------- doQuit() -----------------------------------------------------------------
	...
	def clearLastRect(self):
		...
		if not self.screen.get_locked() :
			self.screen.lock()
			try: pygame.display.update((w, h, picW, picH))
			except pygame.error, err: logging.error("gPlayer: Error at pygame.display.update(). %s " % (err))
			self.screen.unlock()
		return
