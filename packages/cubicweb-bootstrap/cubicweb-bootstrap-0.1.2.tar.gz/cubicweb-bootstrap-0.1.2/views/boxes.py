...
boxes.SearchBox.formdef = (
    ...
    u'       class="search-query" placeholder="%(searchlabel)s"/>'
...
@monkeypatch(boxes.EditBox)
def render_body(self, w):
    self.render_items(w, klass='unstlyed')


def contextualbox_render(self, w, cssclass='contextualBox'):
    if self.init_rendering():
        view = self.cw_extra_kwargs['view']
        w(u'<div class="%s %s" id="%s">' % (self.cssclass, view.cssclass,
                                            view.domid))
        with wrap_on_write(w, u'<h5 class="boxTitle">',
                           u'</h5>') as wow:
...
# XXX partial
def contextualbox_render2(self, w, cssclass='contextFreeBox'):
    if self.init_rendering():
        view = self.cw_extra_kwargs['view']
        w(u'<div class="%s %s" id="%s">' % (self.cssclass, view.cssclass,
                                            view.domid))
