...
constructs:
    ...
    * :py:class:`GeometricObject` - abstract base class, not meant to be used
      directly
    * :py:class:`Point`
    * :py:class:`Vector`
...
Each of these elements can interact with the other by calculating
intersections, dividing objects, etc.

Notes
-----
Except for the :py:class:`Point` and :py:class:`Vector` classes which will be
discussed below, all of the other classes define a `__getitem__` method that can
be used to retreive the points defining the :py:class:`GeometricObject` by
indices.

The :py:class:`Point` class defines the `__getitem__` method in a sperate way,
i.e. it returns the Cartesian coordinates of the :py:class:`Point` by indinces.
The :py:class:`Vector` class does the same except it returns the x & y Cartesian
...
def get_perpendicular_to(obj, at_point=None):
    """
    Creates a new :py:class:`Vector` or :py:class:`Line` perpendicular with
    `obj` (:py:class:`Vector` or :py:class:`Line`-like) depending on `at_point`
    parameter.
    ...
    at_point : point-like, optional
        If this is given then a :py:class:`Line` is returned instead,
        perpendicular to `obj` and passing through `at_point`.

    Returns
    -------
    out : vector
        A new :py:class:`Vector` or :py:class:`Line` passing through `at_point`
        with the components in such a way it is perpendicular with `obj`. This
        really means to take the given :py:class:`Vector` (`obj` itself, or the
        one from the :py:class:`Line`-like) and make a new one with components
        <`obj.y`, `obj.x`>.

    Raises
    ------
    TypeError:
        If `obj` is not :py:class:`Vector` nor :py:class:`Line`-like or if
...
class GeometricObject:
    ...
    def __contains__(self, x):
        """
        Searches for x in "itself". If we're talking about a :py:class:`Point`
        or a :py:class:`Vector` then this searches within their components (x,
...
class Point(GeometricObject):
    ...
    a :py:class:`Point`, a `tuple` of length two or a `list` of lenth two
    or even two values can be passed.

    Parameters
    ----------
    *args : {two scalars, point-like}, optional
        :py:class:`Point`-like means that it can be either of `tuple` or `list`
        of length 2.
    ...
    ------
    TypeError
        If the arguments are not the correct type (:py:class:`Point`, list,
    ...
    def __str__(self):
        return super().__str__(x=self.x, y=self.y)
    ...
    def __eq__(self, point):
        ...
        ------
        TypeError
            In case something other than :py:class:`Point`-like is given.
        """

        if Point.is_point_like(point):
            point = Point(point)
            return abs(self.x - point.x) < UNCERTAINTY and \
                   abs(self.y - point.y) < UNCERTAINTY
        return super().__eq__(point)
    ...
    def is_point_like(obj):
        """
        See if `obj` is of :py:class:`Point`-like.

        :py:class:`Point`-like means :py:class:`Point` or a list or tuple of
        length 2.
        ...
        -------
        out : {True, False}
            `True` if obj is :py:class:`Point`-like, else `False`.
        """
    ...
    def is_left(self, obj):
        """
        Determine if `self` is left|on|right of an infinite :py:class:`Line` or
        :py:class:`Point`.
        ...
        -------
        out : {scalar, `None`}
            >0 if `self` is left of :py:class:`Line`,
            =0 if `self` is on of :py:class:`Line`,
            <0 if `self` is right of :py:class:`Line`,

        Raises
        ------
        ValueError
            In case something else than a :py:class:`Line`-like or
            :py:class:`Point`-like is given.
        """

        if isinstance(obj, (list, tuple, Line)):
            obj = Line(obj)
            return ((obj.p2.x - obj.p1.x) * (self.y - obj.p1.y) - \
                    (self.x - obj.p1.x) * (obj.p2.y - obj.p1.y))
    ...
    def distance_to(self, obj):
        """
        Calculate the distance to another :py:class:`GeometricObject`.

        For now it can only calculate the distance to :py:class:`Line`,
        :py:class:`Segment` and :py:class:`Point`.
        ...
        out : (float, point)
            ...
            this :py:class:`Point` to the provided object and the
            :py:class:`Point` of intersection.
        """
    ...
    def belongs_to(self, obj):
        """
        Check if the :py:class:`Point` is part of a :py:class:`GometricObject`.
    ...
    def translate(self, dx, dy):
        """
        See :py:meth:`GeometricObject.translate`.
        """
        self._x += dx
        self._y += dy
    ...
    def move(self, x, y):
        """
        The difference between this and :py:meth:`translate` is that this
        function moves `self` to the given coordinates instead.
        """
        self._x = x
        self._y = y

    def rotate(self, theta, point=None, angle='radians'):
        """
...
class Vector(GeometricObject):
    """
    An abstract :py:class:`Vector` object.
    ...
    *args : {two scalars, vector, point, (list, tuple of length 2)}
        ...
        vice-versa. If `args` is of :py:class:`Vector` type then all of the
        other arguments are ignored and we create a :py:class:`Vector` copy of
        the given parameter. It can also be :py:class:`Point`-like element; if
        there are two :py:class:`Point`-like elements given then the vector will
        have `rho` equal to the distance between the two points and the
        direction of point1 -> point2 (i.e. args[0] -> args[1]). If only one
        :py:class:`Point`-like is given then this object's `x` and `y` values
    ...
    **kwargs : coordinates={"cartesian", "polar"}, optional
    ...
    def __str__(self):
        return super().__str__(x=self.x, y=self.y, rho=self.rho,
    ...
    def __neg__(self):
        """
        Turns `self` to 180 degrees and returns the new :py:class:`Vector`.

        Returns
        -------
        out : vector
            Return a new :py:class:`Vector` with same `self.rho`, but
            `self.phi`-`math.pi`.
        """
    ...
    def __mul__(self, arg):
        """
        Calculates the dot product with another :py:class:`Vector`, or
        multiplication by scalar.

        For more details see :py:meth:`dot`.
        """
        return self.dot(arg)
    ...
    def __sub__(self, vector):
        ...
        It is :py:meth:`__add__` passed with turnerd round vector.
        """
    ...
    def random_direction():
        """
        Create a randomly oriented :py:class:`Vector` (with `phi` in the
        interval ``[0, PI)``) and with unit length.

        Returns
        -------
        out : vector
            A :py:class:`Vector` with random orientation in positive Y direction
    ...
    def dot(self, arg):
        ...
        arg : {scalar, vector}
            ...
            with this :py:class:`Vector`, if it's another :py:class:`Vector`
        ...
        ------
        TypeError
            In case `arg` is not number or :py:class:`Vector`.
        """
    ...
    def cross(self, arg):
        """
        Calculates the cross product with another :py:class:`Vector`, as defined
        in 2D space (not really a cross product since it gives a scalar, not
        another :py:class:`Vector`).

        Parameters
        ----------
        arg : vector
            Another :py:class:`Vector` to calculate the cross product with.
        ...
        ------
        TypeError
            In case `arg` is not a :py:class:`Vector`.
        """
    ...
    def parallel_to(self, obj):
        ...
        Find out if this :py:class:`Vector` is parallel with another object
        (:py:class:`Vector` or :py:class:`Line`-like). Since we are in a 2D
    ...
    def perpendicular_to(self, obj):
        ...
        Find out if this :py:class:`Vector` is perpendicular to another object
        (:py:class:`Vector` or :py:class:`Line`-like). If the dot product
...
class BoundingBox(GeometricObject):
    """
    Represents the far extremeties of another :py:class:`GeometricObject`
    (except for :py:class:`Vector`).

    It is totally defined by two :py:class:`Points`. For convenience it also
    has `left`, `top`, `right` and `bottom` attributes.

    Parameters
    ----------
    obj : geometric object
        The object for which to assign a :py:class:`BoundingBox`.
    ...
    def p1(self):
        """
        (point-like) Get the bottom-left :py:class:`Point`.
        """
        return self._p1
    ...
    def p2(self):
        """
        (point-like) Get the top-right :py:class:`Point`.
        """
        return self._p2
    ...
    def __str__(self):
        return super().__str__(left=self.left, top=self.top, right=self.right,
                               bottom=self.bottom, p1=str(self.p1),
                               p2=str(self.p2))
    ...
    def __getitem__(self, idx):
        ...
        ----------
        idx : scalar
            The index of the :py:class:`Point`.

        Returns
        -------
        out : point
            The selected :py:class:`Point` through the provided index.
        """
...
class Line(GeometricObject):
    """
    An abstract mathematical :py:class:`Line`.

    It is defined by either two points or by a :py:class:`Point` and a
    :py:class:`Vector`.

    Parameters
    ----------
    arg1 : point-like
        The passed in parameters can be either two points or a :py:class:`Point`
        and a :py:class:`Vector`. For more on :py:class:`Point`-like see the
        :py:class:`Point` class.
    arg2 : {point-like, vector}
        If a :py:class:`Vector` is given as `arg2` instead of a
        :py:class:`Point`-like, then `p2` will be calculated for t = 1 in the
    ...
    definition for which we need a :py:class:`Point` and a :py:class:`Vector`.
    If two points are given the :py:class:`Vector`
    :math:`\\boldsymbol{\mathtt{p_1p_2}}` will be calculated and then we can
    define the :py:class:`Line` as:
    ...
    def p1(self):
        """
        [point] Get the 1st :py:class:`Point` that defines the :py:class:`Line`.
        """
        return self._p1
    ...
    def p2(self):
        """
        [point] Get the 2nd :py:class:`Point` that defines the :py:class:`Line`.
        """
        return self._p2
    ...
    def v(self):
        """
        [vector] Get the :py:class:`Vector` pointing from `self.p1` to`self.p2`.
        """
        return self._v
    ...
    def __str__(self, **kwargs):
        return super().__str__(v=str(self.v),
                               p1=str(self.p1), p2=str(self.p2), **kwargs)

    def __getitem__(self, idx):
        """
        Get the :py:class:`Points` that define the :py:class:`Line` by index.

        Parameters
        ----------
        idx : scalar
            The index for :py:class:`Point`.

        Returns
        -------
        ret : point
            Selected :py:class:`Point` by index.
        """
    ...
    def intersection(self, obj):
        ...
        If an intersection is found, the :py:class:`Point` of intersection is
        ...
        out : {geometric object, tuple}
            If they intersect then return the :py:class:`Point` where this
            happened, else return `None` (except for :py:class:`Line` and
            :py:class:`Polygon`: see notes).
        ...
        Notes
        -----
        * :py:class:`Line`: in case `obj` is :py:class:`Line`-like and `self`
          then `self` and the :py:class:`Line` defined by `obj` are checked for
          colinearity also in which case `utils.inf` is returned.
        * :py:class:`Polygon`: in the case of intersection with a
          :py:class:`Polygon` a tuple of tuples is returned. The nested tuple is
          made up by the index of the intersected side and intersection point
          (e.g. ``((intersection_point1, 1), ( intersection_point2, 4))`` where
          `1` is the first intersected side of the :py:class:`Polygon` and `4`
          is the second one). If the :py:class:`Line` doesn't intersect any
    ...
    def has(self, point):
        """
        Inspect if `point` (:py:class:`Point`-like) is part of this `Line`.

        Parameters
        ----------
        point : point-like
            The :py:class:`Point` to test if it's part of this :py:class:`Line`.

        Returns
        -------
        ret : {True, False}
            If it's part of this :py:class:`Line` then return True, else False.
    ...
    def perpendicular_to(self, obj):
        """
        Find out if provided :py:class:`Line` is perpendicular to `self`.
    ...
    def parallel_to(self, obj):
        """
        Find out if provided :py:class:`Vector` or :py:class:`Line`-like is
        parllel to `self`.

        Parameters
        ----------
        obj : {vector, line-like}
            The :py:class:`Vector` or :py:class:`Line`-like to compare
            parallelism with.

        Returns
        -------
        ret : {True, False}
            If `self` and :py:class:`Line` are parallel then retrun `True`,
            else `False`.
        """
...
class Ray(Line):
    """
    A :py:class:`Ray` extension on :py:class:`Line`.

    The only difference is that this has a starting :py:class:`Point` (`p1`)
    which represents the end of the :py:class:`Ray` in that direction.
    ...
    Notes
    -----
    The **Attributes** are the same as for :py:class:`Line`. \n
    `p1` is the starting :py:class:`Point` of the :py:class:`Ray`.
    """

    def intersection(self, obj):
        """
        Tries to find the :py:class:`Point` of intersection.

        The difference between this and the :py:class:`Line` intersection method
        is that this has also the constrain that if the :py:class:`Point` of
        intersection is on the line then it also must be within the
        bounds of the :py:class:`Ray`.
        ...
        out : {gometric object, None}
            :py:class:`GeometricObject` if intersection is possible, else the
            cases from :py:class:`Line`.intersection.
        ...
        if type(obj) is Line:
            return obj.intersection(self)
        intersections = super().intersection(obj)
        if isinstance(obj, Polygon):
            if intersections:
    ...
    def has(self, point):
        ...
        ----------
        point : point-like
            The :py:class:`Point` to check.

        Returns
        -------
        ret : {True, False}
            If the point is on the :py:class:`Ray` then return `True`, else
            `False`.
...
class Segment(Line):
    """
    An extension on :py:class:`Line`.

    This class emposes the `length` property on a :py:class:`Line`. A
    :py:class:`Segment` is a finite :py:class:`Line`.

    Parameters
    ----------
    arg1 : point-like
        The passed in parameters can be either two points or a :py:class:`Point`
        and a :py:class:`Vector` For more on `Point`-like see the `Point` class.
    arg2 : {point-like, vector}
        See `arg1`.
    ...
    Notes
    -----
    Athe **Attributes** are the same as :py:class:`Line` plus the one listed
    above.
    ...
    def length(self):
        """
        [scalar] Get the length of the :py:class:`Segment`.
    ...
    def bounding_box(self):
        """
        [BoundingBox] get the :py:class:`BoundingBox` of `self`.
        """
        return BoundingBox(self)

    def __str__(self):
        return super().__str__(length=self.length)
    ...
    def intersection(self, obj):
        ...
        is that this has also the constrain that if the :py:class:`Point` of
        intersection is on the line then it also must be within the
        bounds of the :py:class:`Segment`.
    ...
    def get_point_on_self(self, frac=None):
        """
        Get a point on this :py:class:`Segment` based on `frac`.

        If no argument is given then the :py:class:`Point` on the
        :py:class:`Segment` will be placed randomly.

        Parameters
        ----------
        frac : float, optional
            If `frac` is given then the new :py:class:`Point`'s position will
            be relative to the length of the :py:class:`Segment` and to the
            first :py:class:`Point` (`self.p1`). `frac` can be only in the
            interval (0, 1).

        Returns
        -------
        out : point
            The new :py:class:`Point`'s position on the :py:class:`Segment`.
...
class Polygon(GeometricObject):
    """
    A general (closed) :py:class:`Polygon` class.

    The :py:class:`Polygon` is made out of points (vertices of type
    :py:class:`Point`) and edges (:py:class:`Segment`). It can be created by
    passing a list of :py:class:`Point`-like objects.

    Parameters
    ----------
    vertices : {list/tuple of point-like}
        The `list` of :py:class:`Point`-like objects that make the
        :py:class:`Polygon`. The `self.edges` of the :py:class:`Polygon` are
        automatically created and stored. If the length of the `vertices` list
        is < 3 this cannot be a :py:class:`Polygon` and a `ValueError` will be
        raised.
    ...
    def vertices(self):
        ...
        The list of :py:class:`Point`-like objects that make up the
        :py:class:`Polygon`. It's lengths cannot be less than 3.
        """
    ...
    def edges(self):
        ...
        These are the `edges` of the :py:class:`Polygon`, which are
        defined by the list of vertices. The :py:class:`Polygon` is considered
    ...
    def area(self):
        """
        [scalar] Get the area of this :py:class:`Polygon`.

        Using the standard formula for the area of a :py:class:`Polygon`.

        Returns
        -------
        area : float
            The calculated area of this :py:class:`Polygon`.

        Notes
        -----
        This function (property) also sets up `self._area` if it's not set.
        This variable (`self._area`) is meant to be just for internal use.
        """

        # first add the first vertice to the list
    ...
    def bounding_box(self):
        """
        [BoundingBox] Get :py:class:`BoundingBox`(`self`).
        """
        return BoundingBox(self)
    ...
    def diameter(self):
        """
        [scalar] Get the `diameter` of the :py:class:`Polygon`.
        """
    ...
    def width(self):
        """
        [scalar] Get the `width` of the :py:class:`Polygon`.
        """
    ...
    def __str__(self):
        return super().__str__(vertices=[str(v) for v in self.vertices[:-1]])

    def __getitem__(self, idx):
        """
        Retreive :py:class:`Points` (`vertice`) by `idx`.

        Parameters
        ----------
        idx : scalar
            The index of the :py:class:`Point` (`vertice`).

        Returns
        -------
        ret : point
            The `vertice` by index.
        """
    ...
    def _compute_diameter_width(self):
        """
        Compute the `diameter` of the :py:class:`Polygon`.

        This is mean for internal use only. The definition of `diameter` and
        it's calculation is based on: `Arnon et al., A Linear Time Algorithm for
        the Minimum Area Rectangle Enclosing a Convex Polygon" (1983), Computer
        Science Technical Reports. Paper 382
        <http://docs.lib.purdue.edu/cstech/382>`_.
        """
    ...
    def has(self, point):
        """
        Determine if `point` is inside :py:class:`Polygon` based on the winding
        number.
    ...
    def get_point_on_self(self, edge_no=None, frac=None):
        """
        Return a random :py:class:`Point` on the given :py:class:`Segment`
        defined by `edge_no`.
        ...
        frac : float, optional
            ...
            left to default (`None`), a random :py:class:`Point` will be
            returned on the specified edge.

        Returns
        -------
        out : point
            The :py:class:`Point` on this edge (:py:class:`Segment`).
        """
    ...
    def divide(self, obj=None, edge_no=None, frac=None, relative_phi=None,
               drelative_phi=0):
        """
        Divide the :py:class:`Polygon`.

        Parameters
        ----------
        obj : line-like, optional
            If no `obj` is given then `edge_no` is used to build from a
            `Ray` from a randomly chosen Point on `self.edges[edge_no]` with
            inward direction and the closest intersection :py:class:`Point` to
            `Ray.p1` is used to divide the :py:class:`Polygon` in two, else all
            of the :py:class:`Points` given by the intersection between the
            :py:class:`Polygon` and obj are used to split the
            :py:class:`Polygon` in any number of polygons.
        edge_no : int, optional
            If given, `self.edges[edge_no]` will be used to build a
            :py:class:`Ray` as explained above, else a random edge number will
            be chosen.
        frac : float, optional
        ...
        relative_phi : float, optional
            ...
            :py:class:`Ray` spawned from `self.edges[edge_no]`. It has to be in
        ...
        drelative_phi : float, optional
            ...
            calculate a random relative direction for the :py:class:`Ray`
        ...
        ret : tuple of size 2
            ...
            `Segments` that were used to divide the initial :py:class:`Polygon`
