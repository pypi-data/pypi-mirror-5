...
def putf(f, obj_path, prefix=None):
    ...
    The file-like object ``f`` must be seekable. Note that after putf returns, f's 
        current position (f.tell()) will be unchanged
    """
...
def remove(obj_paths, prefix=None):
    ...
    obj_paths can be a single object or a list of up to 1000 objects
    """
    conn = _getcloudnetconnection()
    ...
    full_obj_paths =[ _get_effective_obj_path(obj_path, prefix) for obj_path in obj_paths]
...
def get(obj_path, file_path=None, prefix=None, start_byte=0, end_byte=None, _retries=1):
    ...
    *data/names.txt*. 
    ...
    dest_dir, dest_filename = os.path.split(file_path)
    if dest_dir:
        try:
