            ...
            while not shutting_down[0] or finished_tickets or processing_tickets:
                ...
                with ticket_lock:
                    for ticket_dct in finished_tickets.itervalues():
                        ...
                        redirect_keys.append(ticket_dct['redirect_key'])
                ...
                # FIXME: This should be sent to the message processors to run, but there's no good way 
                #  architecturally to currently do that (as message_queue blocks)
                for redirect_key in redirect_keys:
                    if redirect_key:
                        try:
                            input_queue._clean_message(redirect_key)
                        except Exception:
                            logging.warning('Could not remove %s' % redirect_key, exc_info=True)
