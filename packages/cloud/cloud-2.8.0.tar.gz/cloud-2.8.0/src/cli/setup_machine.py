...
def setup_machine(email=None, password=None, api_key=None):
    ...
    if system_browser_may_be_graphical():
    ...
    try:
        if email:            
            'Setup will proceeed using this E-mail: %s' % email
        else:
...
each machine have its own unique key."""
        ...
        if sudouid:
            confname = os.path.join(cloud.cloudconfig.fullconfigpath,cloud.cloudconfig.configname)
