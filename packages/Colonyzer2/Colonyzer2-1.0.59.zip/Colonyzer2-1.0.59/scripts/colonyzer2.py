...
def setupDirectories():
    ...
    try:
        ...
        print("Output directories present")

    outputimages=os.path.join(fullpath,"Output_Images")
    outputdata=os.path.join(fullpath,"Output_Data")
    return((fullpath,outputimages,outputdata))

def getImageNames(outputimages,outputdata,fullpath):
    '''Get filenames for all images which have not yet been analysed.'''
    allfiles=os.listdir(fullpath)
    alldats=os.listdir(outputdata)
    imsDone=list(numpy.unique([dat.split(".")[0] for dat in alldats]))
    imList=[]
    for filename in allfiles:
        imname=filename.split(".")[0]
        if filename[-4:] in ('.jpg','.JPG','.tiff','.TIFF','.tif','.TIF') and imname not in imsDone:
            imList.append(filename)
    imList.sort(reverse=True)
    return(imList)

def getBarcodes(outputimages,outputdata,fullpath,barcRange=(0,15)):
    '''Get filenames for all images which have not yet been analysed.'''
    allfiles=os.listdir(fullpath)
    alldats=os.listdir(outputdata)
    imsDone=list(numpy.unique([dat.split(".")[0] for dat in alldats]))
    ...
    for filename in allfiles:
        ...
        if filename[-4:] in ('.jpg','.JPG','.tiff','.TIFF','.tif','.TIF') and barc not in barcsDone:
