...
from pyparsing import Suppress, CaselessLiteral, ZeroOrMore, OneOrMore, Group, Literal, Keyword, \
...
strands = Group(K('strands:') + Optional(Keyword('+')) + Optional(Keyword('-')))('STRANDS')
background_freqs = Group(K('Background letter frequencies')  + Suppress(restOfLine)
    + K('A') + fnumber + K('C') + fnumber + K('G') + fnumber + K('T') + fnumber)('BACK_FREQS')
matrix_row = Group(
...
def extract_or_none(thing, name, extractor=lambda x: x):
    """Try to extract named value out of thing but return None if it is not there.
    """
    if name in thing:
...
def extract_motif(motif):
    return Motif(
        ...
        letter_probs=extract_or_none(motif['MATRICES'], 'LETTER_PROBS', extract_matrix),
        log_odds=extract_or_none(motif['MATRICES'], 'LOG_ODDS', extract_matrix),
...
def extract_parsed(parsed):
    return Parsed(
        ...
        back_freqs=extract_or_none(parsed, 'BACK_FREQS', lambda x: map(float, x)),
        motifs=extract_or_none(parsed, 'MOTIFS', lambda x: map(extract_motif, x))
    )
...
""" % (
    ...
    meme_info.back_freqs[0], meme_info.back_freqs[1], meme_info.back_freqs[2], meme_info.back_freqs[3], 
)
