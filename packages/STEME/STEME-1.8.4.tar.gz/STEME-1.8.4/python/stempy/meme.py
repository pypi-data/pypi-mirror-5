...
import subprocess, re, time, warnings
...
Start = namedtuple('Start', 'w0 nsites0 cons0 cons w nsites sig em_time niters cons_after_em')
...
def run_meme(fasta, options, extra_args=None):
    ...
    start_re = re.compile('\(start\)\s+([0-9]+)\s+([0-9.]+)\s+(\w+)\s+-->\s+(\w+)\s+w\s+([0-9]+)\s+nsites\s+([0-9.]+)\s+sig\s+([0-9.]+|inf)')
    em_time_re = re.compile('EM \(without discretization\) took ([0-9.]+) seconds for ([0-9]+) iterations. Final consensus: ([ACGTX]+)')
    theta_re = re.compile('\s+(0\.[0-9]+)\s+(0\.[0-9]+)\s+(0\.[0-9]+)\s+(0\.[0-9]+)')
...
#            starts.append(start)
            ...
            start = Start(w0=w0, nsites0=nsites0, cons0=cons0, cons=cons, w=w, nsites=nsites, sig=sig, em_time=em_time, niters=niters, cons_after_em=cons_after_em)
...
class Algorithm(object):
    ...
    def __call__(self, fasta):
        ...
        # run MEME
        self.meme_cmd_args, self.stdoutdata, self.starts, self.Zs, self.thetas, self.lambdas = run_meme(fasta, self.options)

        # parse output
        from Bio import Motif
        for motif in Motif.parse(open(os.path.join(self.options.output_dir, 'meme.txt')), "MEME"):
            for instance in motif.instances:
                start = instance.start-1 # MEME parser seems to count from 1, not 0
                prediction = instance.sequence_name, Interval(start, start + motif.length), instance.strand == '-'
...
if '__main__' == __name__:
    ...
    parser = MEMEParser()
    meme_output = parser.parse(open(os.path.join(options.output_dir, 'meme.txt')))
