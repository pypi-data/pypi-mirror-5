...
""" % (
                algorithm.fasta,
                '\n'.join("%-24s 1.0000 %6d" % (_id, algorithm.input_sequences.seqs.string_length(i)) for i, _id in enumerate(algorithm.input_sequences.ids))
            ))
...
""" % (
    ...
    def write_motif(self, algorithm, motif):
        ...
        Write the motif
        """
        rounded_pspm = (N.exp(motif.model.bs.pssm.log_probs.values()) * 10.).round().astype(int)
...
""" % (
                motif.idx+1,
                '\n'.join([self._output_for_prediction(algorithm, prediction) for prediction in motif.predictions])
            ))
    ...
    def _output_for_prediction(self, algorithm, prediction, site_flanks=10):
        ...
        flank_end = min(prediction.interval.end + site_flanks, algorithm.input_sequences.data.seq_length(seq_index))
        left_flank = algorithm.input_sequences.data.subsequence(seq_index, flank_start, prediction.interval.start - flank_start)
        site = algorithm.input_sequences.data.subsequence(seq_index, prediction.interval.start, prediction.interval.end - prediction.interval.start)
        right_flank = algorithm.input_sequences.data.subsequence(seq_index, prediction.interval.end, flank_end - prediction.interval.end)
        if prediction.rev_comp:
            left_flank, right_flank = reverse_complement(right_flank), reverse_complement(left_flank)
            site = reverse_complement(site)
        if not left_flank: left_flank = '.'
        if not right_flank: right_flank = '.'
        return "%-24s %5s %6d %9.1e %*s %s %-*s" % (
            prediction.seq.split()[0], # only print first part of sequence name, otherwise parsing can get confused later
...
""" % (motif.idx+1, motif.idx+1))
    ...
    def write_pssm(self, algorithm, motif):
        ...
        """
        int_log_odds = (motif.model.bs.pssm.log_probs.values() * 100).round().astype(int)
        self.f.write("""
--------------------------------------------------------------------------------
...
""" % (
                ...
                N.log2((1-motif.model.lambda_)/motif.model.lambda_), N.exp(motif.log_E_value),
                '\n'.join(''.join('%6d' % l for l in int_log_odds_col) for int_log_odds_col in int_log_odds)
            )
        )
...
""" % (motif.idx+1,))
    ...
    def found_motif(self, algorithm, motif, seconds_taken):
        ...
        #self.write_multilevel_consensus(algorithm, motif) # not implemented yet
...
********************************************************************************
""" % seconds_taken)
        # ensure that all internal buffers associated with self.f are written to disk
        self.f.flush()
