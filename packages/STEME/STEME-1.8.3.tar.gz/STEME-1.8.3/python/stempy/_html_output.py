...
class HTMLOutput(object):
    ...
    def found_motif(self, algorithm, motif, seconds_taken):
        ...
        f = open(os.path.join(self.output_dir, 'STEME-motif-%02d.html' % motif.idx), 'w')
        f.write(
            template.render(
                motif = motif,
    ...
    def _write_job(self, algorithm):
        ...
        f.write(
            template.render(
                motifs = [(motif, HTMLOutput._image_url(idx), HTMLOutput._image_url(idx, 'eps')) for idx, motif in enumerate(algorithm.motifs)],
                **variables
            )
        )
