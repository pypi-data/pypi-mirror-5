...
#include <pvalues/calculator.h> // for log_qfast
...
struct pval_pvalue_calculator :
    ...
    pval_pvalue_calculator(
        int alg, const double * pu, bool use_qfast ) :
        alg(
            alg ), use_qfast(
            use_qfast ) {
        switch( alg ) {
        case 0:
            cout << "Using chi^2 approximation.\n";
            break;
        case 1:
            cout << "Using exact method.\n";
            break;
        case 2:
            cout << "Using branch-and-bound method.\n";
            break;
        case 3:
            cout << "Using convex method.\n";
            break;
        default:
            cout << "Unknown method!\n";
            break;
        ...
        if( use_qfast )
            cout << "Using QFAST to combine column p-values.\n";
        else
            cout << "Not using QFAST to combine column p-values.\n";
        ...
        for( int i = 1; i <= K - 1; i++ ){
            Qcomp[ i ] = ( qcomp -= Q[ i ] );
            Qcompmin[ i ] = d_min(
                Q, i + 1, K ); // all Q is now assigned
            lgQcomp[ i ] = log(
                Qcomp[ i ] );
            lgQcompmin[ i ] = log(
                Qcompmin[ i ] ); // all Q is now assigned
        }
    ...
    void
    exact(
        int i, int n_sum, double KL_sum, double Qtau_sum ) {// we are guaranteed that lg[0]=cumlg[0]=0.0
        ...
        for( n[ i ] = N - n_sum; n[ i ] >= 0; n[ i ]-- ) // never descends to level K
        {
            ...
            if( i < K - 1 )
                exact(
                    i + 1, n_sum + n[ i ], KL_sum + KL_term, Qtau_sum
                                    + Qtau_add );
    ...
    expand(
        ...
        for( n[ i ] = N - n_sum; n[ i ] >= 0; n[ i ]-- ) // never descends to level K
        {
            ...
            if( KL_sum + KL_term + KL_minlf >= KL_thresh ){
                logops++;
                add_lgp(
                    Qtau_sum + Qtau_add + n_comp * lgQcomp[ i ]
                                    - cumlg[ n_comp ] );
            }else if( i < K - 1 ){
    ...
    convex(
        ...
        mmin = int(
            Q[ i ] * ( n_comp ) / ( Q[ i ] + Qcomp[ i ] ) );
        a = 0;
        dummy = mmin;
        lmin = binsearch_descend(
            a, dummy, KL_thresh - KL_sum, n_comp, i, lgQcomp );
        dummy = mmin + 1;
        d = n_comp;
        rmin = binsearch_ascend(
            dummy, d, KL_thresh - KL_sum, n_comp, i, lgQcomp );

        mmax = int(
            Q[ i ] * ( n_comp ) / ( Q[ i ] + Qcompmin[ i ] ) );
        dummy = 0;
        b = mmax;
        lmax = binsearch_descend(
            dummy, b, KL_thresh - KL_sum, n_comp, i, lgQcompmin );
        c = mmax + 1;
        dummy = n_comp;
        rmax = binsearch_ascend(
            c, dummy, KL_thresh - KL_sum, n_comp, i, lgQcompmin );
        ...
        { // as written - do NOT mix with approx pval. overshoots, only LATER mends!
            logops += d - a - 1;
            add_lgp(
                Qtau_sum + n_comp * lgQcomp[ i - 1 ] - cumlg[ n_comp ] );
            for( n[ i ] = d - 1; n[ i ] >= a + 1; n[ i ]-- ) // never descends to level K
            { // (loop over all not add)
                sub_lgp(
                    Qtau_sum + n[ i ] * lgQ[ i ] - cumlg[ n[ i ] ] + ( n_comp
                                    - n[ i ] ) * lgQcomp[ i ] - cumlg[ n_comp
                                    - n[ i ] ] );
    ...
    int
    binsearch_descend(
        int &l, int &r, double c, int n_comp, int i, double *lgq_star ) { // input : r>=l and the implicit F is monotonically decreasing
        // output: -1  F(all)<c
        //          0  F(l)>=c>f(r)
        //         +1  F(all)>=c
    ...
    int
    binsearch_ascend(
        int &l, int &r, double c, int n_comp, int i, double *lgq_star ) { // input : r>=l and the implicit F is monotonically increasing
        // output: -1  F(all)<c
        //          0  F(l)<c<=f(r)
        //         +1  F(all)>=c
...
#undef F
    ...
    g2pvalsim(
        ...
        time_t start = clock();
        for( rounds = 1; double(
            clock() - start ) / CLOCKS_PER_SEC < secs; rounds++ ){
            for( int i = 1; i <= K; i++ )
                n[ i ] = 0;
...
create_PVAL_pvalue_calculator(
    const double * pu, int alg, bool use_qfast
) {
    return pvalue_calculator::ptr(
        new pval_pvalue_calculator(
            alg, pu, use_qfast ) );
}
...
{
    fprintf(
        stderr, "Numerical Recipes run-time error...\n" );
    fprintf(
        stderr, "%s\n", error_text );
    fprintf(
        stderr, "...now exiting to system...\n" );
    exit(
        1 );
}
...
void
gser(
    double *gamser, double a, double x, double *gln ) {
    double
    gammln(
        double xx );
    void
    ...
    *gln = gammln(
    ...
    if( x <= 0.0 ){
        if( x < 0.0 )
            nrerror(
                const_cast< char * > ( "x less than 0 in routine gser" ) );
        *gamser = 0.0;
        return;
    }else{
        ap = a;
        ...
        nrerror(
            const_cast< char * > ( "a too large, ITMAX too small in routine gser" ) );
...
void
gcf(
    double *gammcf, double a, double x, double *gln ) {
    double
    gammln(
        double xx );
    void
    ...
    *gln = gammln(
    ...
    if( i > ITMAX )
        nrerror(
            const_cast< char * > ( "a too large, ITMAX too small in gcf" ) );
    *gammcf = exp(
        -x + a * log(
            x ) - ( *gln ) ) * h;
}
...
gammq(
    ...
    if( x < 0.0 || a <= 0.0 )
        nrerror(
            const_cast< char * > ( "Invalid arguments in routine gammq" ) );
    if( x < ( a + 1.0 ) ){
        gser(
            &gamser, a, x, &gln );
        return 1.0 - gamser;
