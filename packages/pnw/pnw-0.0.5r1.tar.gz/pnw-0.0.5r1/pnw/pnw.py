...
""" 
Simple configuration management for the mentally retarded
sofar undocumented....
"""
def setoption(kwargs):
    for key in kwargs:
        if kwargs[key] == 'default':
            Config[key] = Defaults.get(key,None)
        else:    
...
keywords = [u'chunk',u'block',u'end',u'import',u'include',u'path',u'set']
...
class Block():
    ...
    **Public methods:**
    ...
    def parse(self):
        ...
        m=re.match(r'(?P<indent>\s*)'+Config['tagprefix']+Config['tagspecial']+\
                r'(?P<tag>\w*)(\((?P<args>.*)\)){0,1}',self.text)
        if m:
            d = m.groupdict()
            tag = d['tag'] #m.group(2)
            if tag not in tags:
...
def long_version():
    return 'version '+hgbanner()

def banner():
    return u'''This is pnw version %s %s
c2013 Matthias Lesch (ml@matthiaslesch.de) 
Apache License Version 2.0'''%(version,hginfo)
