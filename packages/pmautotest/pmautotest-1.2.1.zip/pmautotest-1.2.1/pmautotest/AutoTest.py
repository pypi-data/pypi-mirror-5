...
class AutoTest:
    ...
    def size_check(self):
        ...
        if re.match(r'Int\d*', self.Library.type):
            ...
            pass  
        self.log.info("Bytes:  Table:%s  Device:%s"%(table_size,dev_size))
    ...
    def check(self):
        ...
        error_total = 0   
        if len_t != len(self.Library.range):
            self.log.warning("Amount of data from device: %s"%len_t)     
            self.log.warning("Amount of range in table : %s"%len(self.Library.range)) 
            self.log.warning("Please check the Table of instance %s."%self.Library.instance)
            print "Please check the Table of instance %s.(Ranges are less \
                    than datas from device."%self.Library.instance
            return
        for i in range(0, len_t):           
            if not check_validity(self.date_from_device[i], str(self.Library.range[i]), \
                    self.model_name, self.Library.instance):
                if re.match(r'Int\d*', self.Library.type) and int(self.date_from_device[i])<0:
                    to_ushort = struct.unpack("H",struct.pack("h",int(self.date_from_device[i])))
                    self.log.info("Change short %s to unsigned short: %s"%(self.date_from_device[i]\
                            ,to_ushort[0]) )               
                    if not check_validity(to_ushort[0], str(self.Library.range[i]), \
                            self.model_name, self.Library.instance):
                        error_total += 1
                        serialNum = i
                        ...
                        self.log.warning("Value: %s(unsigned short:%s"%(self.date_from_device[i],\
                                to_ushort[0]))
                else:
                    error_total += 1
                    serialNum = i
...
def run(debug=False,instance=None):
    log.logged_class(AutoTest,debug)    
    log.logged_class(Excel,debug)
    a = AutoTest()
    for row in range(a.Library.table_sheet.nrows):
        if not a.table_update(row):
            continue            
        if instance is not None:
            if float(a.Library.instance) == instance:
                print "Find instance %s!"%instance
            else:
                continue  
        a.start_wiresnake_engine()
        if not a.device_update():
            continue
        if not a.size_check():
            continue
        a.check()
        if instance is not None:
            print "Test finished (Instance %s )"%instance
            return
    a.show_result() 