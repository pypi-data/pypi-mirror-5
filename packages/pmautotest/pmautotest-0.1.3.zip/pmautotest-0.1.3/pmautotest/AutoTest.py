...
class AutoTest:
	def __init__(self):
		...
		self.autolog = log.Logger("matchLog","info")
	...
	def size_check(self):
		...
		if	dev_size!= table_size:	
			self.log.warning("Table:%sbyte  Device:%sbype --> Not equal"%(table_size,dev_size))
			return False
		return True
	...
	def show_result(self):
		...
		now = strftime("Test time:%a, %d %b %Y %H:%M:%S", gmtime())
		self.log.warning("############### Test Result ###############")
		self.log.warning(now)
		self.log.warning("Chanel: %s"%self.chan)
