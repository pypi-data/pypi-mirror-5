...
from misc import generate_finite_elements
from misc import generate_colloc_points
from misc import update_diffset_indexed_component
from misc import add_equality_constraints
from misc import get_index_information
from coopr.pyomo.base import Block
...
class Collocation_Discretization(object):
    
    def __init__(self, instance):
        self._model = instance
        self._instance = instance.clone()
        self._instance.concrete_mode()
        ...
        self._blocks = list(self._instance.all_blocks())
        self._ncp = {}
        self._nfe = {}
        self._a = {}
        self._tau = {}
    ...
    def reduce_collocation_points(self, var=None, ncp=None, diffset=None):
        """
        ...
        if diffset.type() is not DifferentialSet:
            raise TypeError, "The component specified using the 'diffset' keyword "\
                "must be a differential set"
        ds = self._instance.find_component(diffset.name)
        if ds is None:
        ...
        if var.type() is not Var:
            raise TypeError, "The component specified using the 'var' keyword "\
                "must be a variable"
        tmpvar = self._instance.find_component(var.name)
        if tmpvar is None:
        ...
        self._instance.add_component(list_name,ConstraintList())
        conlist = self._instance.find_component(list_name)
    ...
    def get_discretized_instance(self):
        return self._instance
