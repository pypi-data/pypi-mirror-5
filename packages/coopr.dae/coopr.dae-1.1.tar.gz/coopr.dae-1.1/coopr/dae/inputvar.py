...
from coopr.dae.diffset import DifferentialSet
...
class StateVar(DifferentialVar):
    def __new__(cls, *args, **kwds):
        ...
        if 'diffset' not in kwds:
            largs -= 1
        if largs == 0:
            return _SingletonStateVar.__new__(_SingletonStateVar)
        else:
...
class _SingletonStateVar(_DifferentialVarData, StateVar):
    def __init__(self, *args, **kwds):
...
class InputVar(DifferentialVar):
    def __new__(cls, *args, **kwds):
        ...
        if 'diffset' not in kwds:
            largs -= 1
        if largs == 0:
            return _SingletonInputVar.__new__(_SingletonInputVar)
        else:
...
class _SingletonInputVar(_DifferentialVarData, InputVar):
    def __init__(self, *args, **kwds):
...
class DerivativeVar(DifferentialVar):
    def __new__(cls, *args, **kwds):
        ...
        if args[0].is_indexed():
            ...
            return _SingletonDerivativeVar.__new__(_SingletonDerivativeVar)
...
class _SingletonDerivativeVar(_DifferentialVarData, DerivativeVar):
    def __init__(self, *args, **kwds):
