...
def _rename_names(names):
    ...
    for index, name in enumerate(field_names):
        if (not all(c.isalnum() or c=='_' for c in name)
            ...
            or name in seen):
            field_names[index] = '_{:d}'.format(index)
        seen.add(name)
...
def _make_attrs(typename, field_names, rename):
    if rename:
        field_names = _rename_names(field_names)
    ...
    def _replace(self, **kwds):
        result = self._make(list(map(kwds.pop, self._fields, self)))
        if kwds:
...
def _set_module(result):
    try:
        result.__module__ = sys._getframe(3).f_globals.get('__name__', '__main__')
    except (AttributeError, ValueError):
        pass


def _make_tuple(typename, attrs, _new):
...
def namedtuple(typename, field_names, rename=False):
    ...
    attrs.update({
        '__doc__': '{typename}({arg_list})'.format(**locals()),
    })
    
    def _new(cls, *args, **kwds):
        args = _check_args(cls, args, kwds)
        return tuple.__new__(cls, args)
    _new.__doc__ = "Create new instance of {typename}({arg_list})".format(**locals())
    
    return _make_tuple(typename, attrs, _new)
