...
class XExcelExporter(XExporter):
    def __init__(self):
        super(XExcelExporter, self).__init__('Excel', '.xls')
    ...
    def exportTree(self, tree, filename):
        ...
        title = str(tree.windowTitle())
        if not title:
            title = str(tree.objectName())
        if not title:
            title = 'Sheet 1'
        ...
        for c, col in enumerate(cols):
            sheet.write(0, c, str(hitem.text(col)))
    ...
    def exportTreeItem(self, sheet, cols, item):
        ...
        for c, col in enumerate(cols):
            ...
            if data:
                ...
                sheet.write(self._currrow, c, str(item.text(col)))
