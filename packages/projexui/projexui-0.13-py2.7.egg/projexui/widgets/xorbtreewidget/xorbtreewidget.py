...
class XOrbTreeWidget(XTreeWidget):
    ...
    def __init__( self, parent=None):
        ...
        self._worker.loadedRecords[object].connect(self.__loadRecords)
        self._worker.loadedRecords[object, object].connect(self.__loadRecords)
        self._worker.loadedGroup.connect(self.createGroupItem)
        self._worker.columnLoaded.connect(self.__loadColumns)
        self._worker.connectionLost.connect(self.__connectionLost)
        ...
        self.columnHiddenChanged.connect(self.__updateColumnValues)
    ...
    def _loadBatch(self, item, batch):
        ...
        if self.isThreadEnabled() and batch.isThreadEnabled():
            ...
            self._worker.loadRecords(batch)
    ...
    def __loadColumns(self, record, columnName, value):
        """
    ...
    def __loadRecords(self, records, nextBatch=None, parent=None):
    ...
    def __setCurrentRecord(self, item, record):
        """
        ...
        for c in range(item.childCount()):
            if self.__setCurrentRecord(item.child(c), record):
                return True
        
        return False
    
    def __updateColumnValues(self, index, hidden):
        """
    ...
    def emitRecordDoubleClicked(self, item):
        ...
        elif item.text(0) == 'Add New Record...' and self.tableType():
    ...
    def markLoadingFinished(self):
        ...
        if self.showAddEntryItem():
            item = XTreeWidgetItem(self, ['Add New Record...'])
    ...
    def refresh(self, reloadData=False, force=False):
        ...
        self._worker.setBatched(self.isPaged())
        self._worker.setBatchSize(self.pageSize())
        ...
        if self.isThreadEnabled() and currset.isThreadEnabled():
            self._worker.setPreloadColumns(self._preloadColumns)
            self._loading = True
    ...
    def restoreXml(self, xml):
        ...
        grp_enabled = xml.get('groupingEnabled')
        if grp_enabled is not None:
            self.setGroupingEnabled(grp_enabled == 'True', autoRefresh=False)
    ...
    def saveXml(self, xml):
        ...
        xml.set('groupingEnabled', str(self.isGroupingEnabled()))
    ...
    def setCurrentRecord(self, record):
        ...
        for i in range(self.topLevelItemCount()):
            if self.__setCurrentRecord(self.topLevelItem(i), record):
                return True
        return False
    ...
    def setGroupingEnabled(self, state, autoRefresh=False):
        """
    ...
    def waitUntilFinished(self):
        ...
        """
        self._worker.waitUntilFinished()
    ...
    def dataRestoreRecords(mimeData):
        ...
        repros = str(mimeData.data('application/x-orb-records'))
        repros = repros.split(';')
