...
class XPopupWidget(QWidget):
    ...
    def eventFilter( self, object, event ):
        ...
        :param      object | <QObject>
                    event  | <QEvent>
        """
        if object == self.positionLinkedTo() and \
           self.isVisible() and \
           event.type() == event.Move and \
           self.currentMode() != self.Mode.Dialog:
            
            deltaPos = event.pos() - event.oldPos()
            self.move(self.pos() + deltaPos)
            return False
        
        elif object == self.positionLinkedTo() and \
             event.type() in (event.Hide, event.Close):
            self.close()
        ...
        elif event.type() in (event.MouseButtonPress, 
    ...
    def popup(self, pos = None):
        ...
        if not self.positionLinkedTo():
            self.setPositionLinkedTo(QApplication.activeWindow())
    ...
    def setPositionLinkedTo(self, widget):
        """
        ...
        :param      widget | <QWidget>
        """
        if widget == self._positionLinkedTo:
            return
        
        self._positionLinkedTo = widget
