...
class XView(QWidget):
    ...
    def setVisible(self, state):
        ...
        :param      state | <bool>
        """
        super(XView, self).setVisible(state)
        ...
        # initialize the view once the view becomes visible the first time
        # the first time the view becomes visible, the initialized signal
        # will be emitted
        if self.isVisible() and not self.isInitialized():
            self.initialize()
        
        # after the initial time the view is loaded, the visibleStateChanged
        # signal will be emitted
        elif not self.signalsBlocked():
            self.visibleStateChanged.emit(state)
