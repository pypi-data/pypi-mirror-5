...
class XViewPanel( XTabWidget ):
    ...
    def eventFilter( self, object, event ):
        if ( event.type() == event.MouseButtonPress ):
            ...
            if ( event.button() == Qt.MidButton or
                ...
                drag = QDrag(self)
                data = QMimeData()
                data.setText('move view:%i' % self.tabBar().tabAt(event.pos()))
    ...
    def markCurrentChanged( self ):
        ...
        """
        widget = self.currentWidget()
        if ( widget and isinstance(widget, XView) ):
            widget.setCurrent()
    ...
    def setLocked( self, state ):
        ...
        # update the tab button sizes
        for tab in range(self.count()):
            btn = tabbar.tabButton(tab, tabbar.RightSide)
            if ( not btn ):
                continue
                
            btn.setMaximumSize(QSize(10, 10))
            btn.move(btn.pos().x(), (tabbar.height() - 10) / 2.0)
