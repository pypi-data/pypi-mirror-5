...
class XLoaderProgressBar(QProgressBar):
    ...
    def paintEvent( self, event ):
        ...
        # draw percent
        perc = self.value() / float(self.maximum())
        painter.setBrush( Qt.gray )
        painter.drawRect( x,(self.height() - 8) / 2, w * \
                             perc, 8)
...
class XLoaderWidget(QWidget):
    ...
    @staticmethod
    def start( widget, processEvents = True, style=None ):
        """
        ...
        loader_name = widget.objectName() + '_Loader'
        loader      = parent.findChild(XLoaderWidget, loader_name)
        
        if not loader:
            loader = XLoaderWidget(parent, style)
            loader.move(widget.pos())
            loader.setObjectName(loader_name)
            loader.setVisible(widget.isVisible())
