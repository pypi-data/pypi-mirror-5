...
class XStream(QObject):
    ...
    def write( self, msg ):
        ...
        self.messageWritten.emit(projex.text.encoded(msg))
...
class XConsoleEdit(XLoggerWidget):
    ...
    def applyCommand( self ):
        ...
        match   = INPUT_EXPR.match(projex.text.encoded(block))
        ...
        self.executeCommand(projex.text.encoded(match.group(1)))
    ...
    def eventFilter( self, obj, event ):
        ...
        if ( event.key() == Qt.Key_Escape ):
            ...
            cursor   = self.textCursor()
            text     = projex.text.encoded(cursor.block().text())
            ...
            for i in range(self._completerTree.topLevelItemCount()):
                ...
                if ( projex.text.encoded(item.text(0)).startswith(text) ):
    ...
    def gotoHome( self ):
        ...
        cursor = self.textCursor()
        block  = projex.text.encoded(cursor.block().text())
    ...
    def waitForInput( self ):
        ...
        newln = '>>> '
        if projex.text.encoded(self.textCursor().block().text()):
            newln = '\n' + newln
