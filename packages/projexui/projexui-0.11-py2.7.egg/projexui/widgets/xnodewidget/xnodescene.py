...
class XNodeScene( QGraphicsScene ):
    ...
    def __init__( self, view, cellWidth = 20, cellHeight = 18 ):
        ...
        self._gridColor                 = QColor(90, 90, 90)
        self._alternateGridColor        = QColor(110, 110, 110)
        self._centerLineColor           = QColor(160, 160, 160)
...
#        processed += nodes
    ...
    def connections( self ):
        ...
        """
        return self.findItems(XNodeConnection)
    ...
    def drawBackground( self, painter, rect ):
        ...
        if self._mainView and not self._mainView.isEnabled():
            bg = QColor('lightGray')
            grid = bg.darker(120)
            agrid     = bg.darker(110)
            lineColor = bg.lighter(120)
        else:
            bg = self.backgroundBrush().color()
            grid = self.gridColor()
            agrid = self.alternateGridColor()
            lineColor = self.centerLineColor()
    ...
    def findNodeByRegex( self, objectRegex ):
        ...
        :return     <XNode> || None
        """
        expr = re.compile(str(objectRegex))
        for item in self.items():
            if ( isinstance(item, XNode) and
    ...
    def setAlternateGridColor(self, color):
        ...
        :param     color | <QColor>
        """
        self._alternateGridColor = QColor(color)
    ...
    def setCenterLineColor(self, color):
        ...
        :return     <QColor>
        """
        self._centerLineColor = QColor(color)
    ...
    def setSceneRect( self, x, y, w, h ):
        ...
        :param      h   <float>
        """
        super(XNodeScene, self).setSceneRect(x, y, w, h)
        self.setDirty()
