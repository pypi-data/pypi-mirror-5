...
def requestCube(ip_connect,name_bbdd,name_cube,col_cube,med_cube,request_data,ifDate,dimension_filters,database_cube_dict,total_data = False,where_tupla=('Date','Id','Id'),request_date_previous = False,complete_name=False,format_cube=False):
    cube=CUBE()
    # print 'MY TAGSSSSS'
    mdx_class=MDX()
    ...
    # NOT DELETE######################
    # for i,v in part_filter.iteritems():
    #     index = i[0].find('_')
    #     a = i[0][0:index]
    #     where[(a,i[1],i[2])]=v
    ################################
    ...
    if(col_cube[0][0] == 'Origin'):
        ...
        # print '////////////////////////PART WHERE',where
        # print '////////////////////////PART FILTER',part_filter
    ...
    if format_cube:
        ...
        json_data=grid.result_to_json(cube,res,database_cube_dict,False,request_data ['row_num'],request_data['page'],total_data,complete_name,format_cube)
        # print '``````````````````````````````````````'
        ...
        # print '====================',json_data_footer
        json_data=json.loads(json_data)
        # print '1111111111111111111111',json_data
        # print '2222222222222222222222222',json.loads(json_data_footer)['rows']
...
def reorderJson(json_data,col_cube):
    ...
    return json_data
def requestCubeHighcharts(ip_connect,name_bbdd,name_cube,col_cube,med_cube,request_data,ifDate,dimension_filters,database_cube_dict,total_data = True,where_tupla=('Date','Id','Id'),request_date_previous = False, format_cube=False):
    ...
    where={}
    part_filter = filterWhere(dimension_filters,request_data,database_cube_dict)
    # print 'COL CUBEEEEEEEE   ',col_cube
    # print 'Medidas CUBEEEEE   ',med_cube
    ...
    if request_data['button']=='ALL':
        json_data=high.result_to_json(cube,res,types,dimension_v_name='Client',request_data=request_data,format_cube= format_cube)
    else:
        json_data=high.result_to_json(cube,res,types,dimension_v_name=request_data['button'],exclude=exclude_rows,request_data=request_data,format_cube= format_cube)
...
def adjustementData(dict_higcharts,value_adjustement):
    ...
    for data in dict_higcharts['v_data']:
        ...
        for i in range(0,len(data)):
            ...
            try:
                # print 'list_index',list_index[j][2]
...
def filterMed(med_cube,select_med):
    ...
    if select_med.lower() == 'asr':
        med_cube=[('Measures','ASR'),('Measures','Calls')]
        return med_cube
    elif select_med.lower() == 'acd' :
        med_cube=[('Measures','ACD'),('Measures','ASR'),('Measures','Calls')]
        return med_cube
    else:
        return med_cube
            ...
            for m in med_cube_prec:
                if m[ind] == k:
                    ...
                    if request_data != False and m[0] =='calldate':
                        v = str(v)
                        v = tzAdjustement(v,int(request_data['tz_adjustement']))
                    v = str(v)
