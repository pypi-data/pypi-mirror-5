...
An object like <olap.xmla.xmla.XMLASource object at 0xa7ae5ac> : it`s the connection with OLAP service
        ...
        if not isAdded(self.key_memcache):
            ...
            try:
                ...
                # cubes=['Scint','ChannelUsage'] cubes=['Cdrt','ChannelUsage']
                self.medidas=self.__getMedidas(bbdd,settings.CUBE_NAMES) 
...
### case where 
    ...
    def with_set_member_part(self,cube,part_rows=[],part_ranking=[]):
        mdx=''
        mdx='WITH SET orderedDimension as topcount(['+part_rows[part_rows.keys()[0]][0]+'].['+part_rows[part_rows.keys()[0]][1]+'].['+part_rows[part_rows.keys()[0]][2]+'].members,100000000,['+part_ranking[2][0]+'].['+part_ranking[2][1]+'])'
        mdx=' MEMBER ranking_medida as RANK(['+part_rows[part_rows.keys()[0]][0]+'].['+part_rows[part_rows.keys()[0]][1]+'].CurrentMember,orderedDimension)'
        return mdx

    def select_rank(self,cube,part_rows=[],part_ranking=[]):
        mdx=''
        mdx='select '
        ...
        return mdx
    def mdx_rank(self,cube,part_rows=[],part_columns=[],part_from='',part_where={},part_NON_EMPTY=True,total_data=False,request_data={},part_ranking=[]):
        mdx=''
        mdx=self.with_set_member_part(part_rows,part_ranking);
        mdx=self.select_rank(part_rows,part_ranking);
...
A string which is a mdx 
        ...
        if cube.dimensiones:
            if part_ranking:
                mdx=self.mdx_rank(cube,part_rows,part_columns,part_from)
            else: 
                mdx='select'
