...
class CUBE:
    ...
    **Attributes: **
        #. dimensiones: A dict, the keys are tuples with the name of the dimension first, in second place the attribute of this dimension 
        and in third place 'All' (to explore all values of that attribute) or the attribute again to explore one to one . And the values are the dimension like is needed to create in a mdx .Example:
        dimensiones={('Client','Client','Client'):'[Client].[Client].[Client]'}
        #. medidas: As the attribute dimension but the keys are different only have two values, the first is 'Measures' for all them , and the second is the measure.Example:
        medidas={('Measures','Calls'):'[Measures].[Calls]'}
        ...
        if isfilter:
            ...
            if total:
                ...
                for k,v in totales.items():
                    ...
                    if total_income != 0: # sino dejo el sumatorio 
                        json_data_new['Margin']= float(total_income) / (float(total_income) - float(total_cost)) * 100


                data['footerData']=json_data_new
            data['rows']=rows
            data['page'] = page
            obj=ujson.dumps(data)
    #############################
            return obj
    ...
    def select_for_filter(self,cube,on_rows=[],medida='',exclude_rows={}):
        ...
        """
          ...
          **Modify:**
            Non modify anything
          **Raises:**
            IOError: The errors and warnning of the views are storage in the archive ....
          **Import**::
              >>> from international.views import *
            >>> from django.core.context_processors import csrf
          Other information ::
        
            # The flow AJAX of this view are for Grid,Filters,Excel ...
            # The extructure for field_query_profile are:
              field_query_profile = [(('caller_number'(1),   'CLI'(2),   0(3),''(4),   2(5) ),   'CS'(6) ),....]
                (1) Name of model
                (2) Name for the title of the column in the grid
                (3) Number of decimal for value
                (4) Text for value
                (5) Ratio of width of the column in the grid
                (6) Profile
        
          * `Read this <http://redis.io/>`_
