...
function shortcutsDateFilter(view){
	...
	finish_minutes = 59 
	finish_seconds = 59 
	// console.log('SHORTCUTS_DATE_FILTERS',view,period,'now-->',now);
...
function datetimePickerFromTo(view) {
	...
	// console.log(view,'TO DATE-->',to_date,'FROM DATE-->',from_date);
...
function refreshDatetimeTo(view)
{
	...
	// from_date = $("input[id*='"+view+"_datepicker_from_']").datetimepicker('getDate')
	$("input[id*='"+view+"_datepicker_to_']").datetimepicker('option', 'minDate', from_date );
	// console.log('REFRESH TO MIN DATE WITH ',from_date)
...
{
	fd_str = $('#'+view+'_datepicker_from_calldate').val();
	// to_date = $("input[id*='"+view+"_datepicker_to_']").datetimepicker('getDate')
	...
	// console.log('REFRESH FROM MAX DATE WITH ',to_date)
}
