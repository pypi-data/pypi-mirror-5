...
class Say(object):
    ...
    options = Options(
        ...
        suffix='',          # suffix each line with this
    ...
    def hr(self, sep=six.u('\u2500'), width=40, vsep=0, **kwargs):
        ...
        vertical = '\n' * vsep
        return self._output([vertical, sep * width, vertical], opts)
    ...
    def title(self, name, sep=six.u('\u2500'), width=15, vsep=0, **kwargs):
        ...
        vertical = '\n' * vsep
        line = sep * width
        return self._output([ vertical, ' '.join([line, formatted, line]), vertical], opts)
    ...
    def _output(self, data, opts):
        ...
        def prefixstr():
            op = opts.prefix
            return unicode(op.next()) if hasattr(op, 'next') else unicode(op)

        def suffixstr():
            op = opts.suffix
            return unicode(op.next()) if hasattr(op, 'next') else unicode(op)

        if opts.indent or opts.wrap or opts.prefix or opts.suffix:
            indent_str = opts.indent_str * opts.indent
            if opts.wrap:
                datastr = '\n'.join(data) if isinstance(data, list) else data
                datalines = textwrap.wrap(datastr, width=opts.wrap, initial_indent=indent_str,
            ...
            if opts.prefix or opts.suffix:
                datalines = [''.join([prefixstr(), line, suffixstr()]) for line in datalines]
            outstr = '\n'.join(datalines)
        else:
            outstr = '\n'.join(data) if isinstance(data, list) else data
