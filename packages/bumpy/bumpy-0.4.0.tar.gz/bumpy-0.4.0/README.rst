...
By default, any tasks in your ``build.py`` or ``bum.py`` file will be
included in the global namespace. Bumpy will also search for a ``bump/``
directory and import any modules it finds there, giving each task found
an appropriate namespace based on its module name. Lastly, if you
``import`` any modules from your ``build.py`` or ``bum.py``, tasks in
those modules will also be given an appropriate namespace.
