...
def main():
  ...
  parser.add_argument('-b','--baseline', action='store_true', default=False, dest='baseline', help='If True, will plot only the baseline systems for comparison. Otherwise it will plot fused systems based on the specified fusion method')
  ...
  if args.nocolor: # plot in gray-scale
    color_mapping = {'ubmgmm':'#000000', 'lgbphs':'#4c4c4c', 'ebgm':'#4c4c4c', 'isv':'#999999'}
  ...
  if args.demandedplot == 1:
    ...
    for faceverif in ('ubmgmm', 'lgbphs', 'ebgm', 'isv'):
      if args.baseline:
        base_dev_file = os.path.join(args.basedir_faceverif, faceverif, '10_licit/nonorm/scores-dev')
        over_dev_file = os.path.join(args.basedir_faceverif, faceverif, '10_spoof/nonorm/scores-dev')
        base_test_file = os.path.join(args.basedir_faceverif, faceverif, '10_licit/nonorm/scores-eval')
        over_test_file = os.path.join(args.basedir_faceverif, faceverif, '10_spoof/nonorm/scores-eval')
    
      else:
        base_dev_file = os.path.join(args.basedir_fvas, args.fusionalg, faceverif, args.antispoofingalg, 'nonorm/10_licit/scores/scores-dev')
        over_dev_file = os.path.join(args.basedir_fvas, args.fusionalg, faceverif, args.antispoofingalg, 'nonorm/10_spoof/scores/scores-dev')
        base_test_file = os.path.join(args.basedir_fvas, args.fusionalg, faceverif, args.antispoofingalg, 'nonorm/10_licit/scores/scores-eval')
        over_test_file = os.path.join(args.basedir_fvas, args.fusionalg, faceverif, args.antispoofingalg, 'nonorm/10_spoof/scores/scores-eval')
    ...
    ax1 = mpl.subplot(111); bob.measure.plot.det_axis([0.1, 99, 0.1, 99]); mpl.grid()
  ...
  if args.demandedplot == 2:
    ...
    for faceverif in ('ubmgmm',): #, 'lgbphs', 'ebgm', 'isv'
      ...
      errors_total = error_utils.epsc_error_rates_1(base_neg, base_pos, over_neg, over_pos, thrs, weights) # errors are returned in a numpy.ndarray in the following order: FAR_w, HTER_w
       
      mpl.plot(weights, [100.*k for k in errors_total[:,1]], color=color_mapping[faceverif], marker=marker_mapping[faceverif], linestyle=linestyle_mapping[faceverif], markevery=10, label = legend_dict[faceverif], linewidth=width)
      ax1 = mpl.subplot(111);
  ...
  if args.demandedplot == 3:
    ...
    for faceverif in ('ubmgmm', 'lgbphs', 'ebgm', 'isv'):
      ...
      weights, thrs = error_utils.epsc_thresholds(base_neg_dev, base_pos_dev, over_neg_dev, over_pos_dev, points=points, criteria=criteria)
      errors = error_utils.spoofing_error_rates(base_neg, base_pos, over_neg, over_pos, thrs) # errors are returned in a numpy.ndarray in the following order: FRR, FAR (impostors), HTER (licit), SFAR
       
      mpl.plot(weights, [100.*k for k in errors[:,3]], color=color_mapping[faceverif], marker=marker_mapping[faceverif], linestyle=linestyle_mapping[faceverif], markevery=10, label = legend_dict[faceverif], linewidth=width)
      ax1 = mpl.subplot(111);
  ...
  if args.demandedplot == 10:
    ...
    for fusionalg in ('SUM',):
      ...
      for faceverif in ('ubmgmm', 'lgbphs', 'ebgm', 'isv'):
        ...
        base_dev_cm_file = os.path.join(args.basedir_fvas, fusionalg, faceverif, args.antispoofingalg, 'nonorm/10_licit/scores/scores-dev')
        over_dev_cm_file = os.path.join(args.basedir_fvas, fusionalg, faceverif, args.antispoofingalg, 'nonorm/10_spoof/scores/scores-dev')
        base_test_cm_file = os.path.join(args.basedir_fvas, fusionalg, faceverif, args.antispoofingalg, 'nonorm/10_licit/scores/scores-eval')
        over_test_cm_file = os.path.join(args.basedir_fvas, fusionalg, faceverif, args.antispoofingalg, 'nonorm/10_spoof/scores/scores-eval')
        ...
        errors_cm = error_utils.spoofing_error_rates(base_neg_cm, base_pos_cm, over_neg_cm, over_pos_cm, thrs_cm) # errors are returned in a numpy.ndarray in the following order: FRR, FAR (impostors), HTER (licit), SFAR

        mpl.plot(weights, [100.*k for k in errors_cm[:,3]], color=color_mapping[faceverif], marker=marker_mapping[faceverif], linestyle=linestyle_mapping[faceverif], markevery=10, label = legend_dict[faceverif], linewidth=width)
