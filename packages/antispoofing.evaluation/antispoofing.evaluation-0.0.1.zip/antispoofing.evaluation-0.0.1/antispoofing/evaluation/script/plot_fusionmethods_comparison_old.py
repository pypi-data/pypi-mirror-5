...
def main():
  ...
  if args.demandedplot == 1:
    ...
    for fusionalg in ('SUM', 'LLR', 'LLR_P'):
      ...
      ax1 = mpl.subplot(111)
      base_dev_cm_file = os.path.join(args.basedir_fvas, fusionalg, args.faceverifalg, args.antispoofingalg, 'nonorm/10_licit/scores/scores-dev')
      over_dev_cm_file = os.path.join(args.basedir_fvas, fusionalg, args.faceverifalg, args.antispoofingalg, 'nonorm/10_spoof/scores/scores-dev')
      base_test_cm_file = os.path.join(args.basedir_fvas, fusionalg, args.faceverifalg, args.antispoofingalg, 'nonorm/10_licit/scores/scores-eval')
      over_test_cm_file = os.path.join(args.basedir_fvas, fusionalg, args.faceverifalg, args.antispoofingalg, 'nonorm/10_spoof/scores/scores-eval')
  ...
  if args.demandedplot == 2:
    ...
    errors_total = error_utils.epsc_error_rates(base_neg, base_pos, over_neg, over_pos, thrs, weights) # errors are returned in a numpy.ndarray in the following order: FAR_w, HTER_w
    # plotting the baseline scores
    mpl.plot(weights, [100.*k for k in errors_total[:,1]], color=color_mapping['baseline'], linestyle=linestyle_mapping['baseline'], linewidth=width, label = 'baseline')
    ...
    for fusionalg in ('SUM', 'LLR', 'LLR_P'):
      ...
      errors_cm_total = error_utils.epsc_error_rates(base_neg_cm, base_pos_cm, over_neg_cm, over_pos_cm, thrs_cm, weights) # errors are returned in a numpy.ndarray in the following order: FAR_w, HTER_w

      mpl.plot(weights, [100.*k for k in errors_cm_total[:,1]], color=color_mapping[fusionalg], linestyle=linestyle_mapping[fusionalg], linewidth=width, label = fusion_label_dict[fusionalg])
      mpl.ylim(ymax=30)
  ...
  if args.demandedplot == 3:
    ...
    # plotting the baseline scores
    mpl.plot(weights, [100.*k for k in errors[:,3]], color=color_mapping['baseline'], linestyle=linestyle_mapping['baseline'], linewidth=4, label = 'baseline')
    ...
    for fusionalg in ('SUM', 'LLR', 'LLR_P'):
      ...
      mpl.plot(weights, [100.*k for k in errors_cm[:,3]], color=color_mapping[fusionalg], linestyle=linestyle_mapping[fusionalg], linewidth=width, label = fusion_label_dict[fusionalg])
      mpl.ylim(ymax=100)
