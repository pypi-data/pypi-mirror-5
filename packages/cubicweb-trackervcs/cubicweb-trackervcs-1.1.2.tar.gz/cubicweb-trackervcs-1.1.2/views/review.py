...
class VersionPatchesTab(EntityView):
    ...
    def entity_call(self, entity):
        rql = ('Any P,GROUP_CONCAT(PO),P,P,PB,PS GROUPBY P,PB,PS '
               'WHERE P patch_revision RE, RE author PO, RE branch PB, P in_state PS, '
               'RE from_repository R, P patch_ticket T, '
               'T done_in V, V eid %(v)s')
        rset = self._cw.execute(rql, {'v': entity.eid})
        self.wview('vcreview.patches.table', rset, 'noresult')
...
def registration_callback(vreg):
    ...
    vcreview_startup.AllActivePatches.rql = (
        'Any P,GROUP_CONCAT(PO),P,P,PB,PS,R,T,V GROUPBY R,P,PB,PS,T,V '
        'ORDERBY RT,PB WHERE P patch_revision RE, RE author PO, RE branch PB, P in_state PS, '
        'RE from_repository R, R title RT, '
        'NOT PS name %s, '
        'P patch_ticket T?, T done_in V?')
    vcreview_startup.UserWorkList.rql = (
        'Any P,GROUP_CONCAT(PO),P,P,PB,PS,COUNT(TR),R,T,V GROUPBY R,P,PB,PS,T,V '
        'ORDERBY RT,PB WHERE P patch_revision RE, RE author PO, RE branch PB, P in_state PS,'
    ...
    vcreview_primary.RepositoryPatchesTable.rql = (
        'Any P,GROUP_CONCAT(PO),P,PB,PS,T,V GROUPBY P,PB,PS,T,V WHERE '
        'P patch_revision RE, RE author PO, RE branch PB, P in_state PS, '
