...
Scapy development uses the Mercurial version control system and is hosted on bitbucket.
Scapy's reference repository is at http://bb.secdev.org/scapy/src.

It provides a freely editable `Wiki <http://bb.secdev.org/scapy/wiki/Home>`_ (please contribute!) that can 
reference issues, changesets, files from the project. It also provides 
a issue management service that is used to avoid forgetting patches or bugs.


How to contribute
=================

The easiest way to contribute is to clone the official scapy repository (http://bb.secdev.org/scapy)
on the bitbucket platform, and then submit your contributions by issuing a pull request (one per feature).
See http://bb.secdev.org/scapy/wiki/contrib/index for more details.

* Found a bug in Scapy? `Add a ticket <http://bb.secdev.org/scapy/issues/new>`_.
* Improve this documentation.
* Program a new layer, issue a pull request, and share it on the mailing list
* Upload packet samples for new protocols on the `packet samples page <http://bb.secdev.org/scapy/wiki/contrib/PacketSamples>`_.
