...
declare function local:isDateTime($thisnode as node()*) {
    if (string-length(string($thisnode)) = 10)
        then datetime:parse-dateTime(string($thisnode), 'yyyy-MM-dd')
    else if (string-length(string($thisnode)) = 0)
        then xs:dateTime('2001-01-01T00:00:00Z')
    else
