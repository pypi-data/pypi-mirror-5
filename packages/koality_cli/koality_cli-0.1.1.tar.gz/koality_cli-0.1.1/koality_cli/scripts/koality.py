...
Usage:
  koality help
  koality generate-config
  koality commit [ARGS...]
  koality push [ARGS...]
  koality <command> [ARGS...]
...
Details:
  ...
  generate-config     - attempt to autocreate a config koality.yml in the 
...
Details:
  ...
  autogenerate-config - attempt to autocreate a config koality.yml in the 
                        repo root that will work in most cases but in some
                        instances may need modifications to properly work
  push                - perform a push that is automatically pre verified
                        by Koality before merging into the repo
...
class CliHandler(object):
	...
	def generate_config(self):
		...
		ConfigCreator().create_config(realpath_repo_root)
