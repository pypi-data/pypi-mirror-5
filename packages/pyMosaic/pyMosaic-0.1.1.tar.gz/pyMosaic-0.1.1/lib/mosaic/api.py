...
class MosaicAtom(object):
    ...
    @abstractproperty
    def unique_id(self):
        """An object that uniquely identifies this atom, to be used
        in identity tests (is, is not, ...).
        """
        raise NotImplementedError()
    ...
    def validate(self):
        ...
        if self.parent is not None:
            validate_type(self.parent, MosaicFragment, "Fragment.parent")
...
class MosaicFragment(collections.Mapping):
    ...
    @abstractproperty
    def parent(self):
        """The parent fragment to which the fragment belongs.
        None for top-level fragments.
        """
        raise NotImplementedError()

    @abstractproperty
    def unique_id(self):
        """An object that uniquely identifies this fragment, to be used
        in identity tests (is, is not, ...).
        """
        raise NotImplementedError()
    ...
    def validate(self):
        ...
        for f in self.fragments:
            f.validate()
            if f.parent.unique_id != self.unique_id:
                raise ValueError("Invalid fragment tree structure")
        ...
        for a in self.atoms:
            a.validate()
            if a.parent.unique_id != self.unique_id:
                raise ValueError("Invalid fragment tree structure")
    ...
    def recursive_bond_iterator(self):
        ...
        """
        return IT.chain(IT.chain.from_iterable(f.recursive_bond_iterator()
                                               for f in self.fragments),
                        iter(self.bonds))
...
def validate_units(unit_spec, text):
    ...
    for unit in unit_spec.split():
        if not (re.match("^[a-zA-Z]+([-]?[0-9]+)?$", unit)
                # unit symbol plus exponent
                or re.match("^[1-9][0-9]*$", unit)
                # integer
