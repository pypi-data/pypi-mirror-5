...
atom_array_dtype = N.dtype([('parent_index', N.uint32),
                            ('label_symbol_index', N.uint32),
                            ('dtype_symbol_index', N.uint32),
                            ('dname_symbol_index', N.uint32),
...
class Universe(api.MosaicUniverse):
    ...
    def __init__(self, universe):
        ...
        def store_fragment(f, iparent, ifrag, iatom, ibond, isite):
            ...
            for a in f.atoms:
                assert len(atom_id) == iatom
                atom_id[a.unique_id] = iatom
                self._atom_array[iatom]['parent_index'] = iparent
                self._atom_array[iatom]['label_symbol_index'] = \
                                                symbol_id[a.label]
                self._atom_array[iatom]['dtype_symbol_index'] = \
                                                symbol_id[a.type]
                self._atom_array[iatom]['dname_symbol_index'] = \
                                                symbol_id[a.name]
            ...
            for a1, a2, order in f.bonds:
                i1 = atom_id[f[a1].unique_id]
                i2 = atom_id[f[a2].unique_id]
                if i1 > i2:
                    i1, i2 = i2, i1
...
class Atom(api.MosaicAtom):
    ...
    def type(self):
        isym_type = \
            self._universe._atom_array[self._iatom]['dtype_symbol_index']
        return self._universe._symbols[isym_type]

    @property
    def name(self):
        isym_name = \
            self._universe._atom_array[self._iatom]['dname_symbol_index']
