...
from this software without _specific prior written permission.
...
def pelgev(xmom):
    ...
    if T3<= 0:
        ...
        if T3>= -0.8:
            para3 = G
            GAM = _sp.exp(_sp._special.gammaln(1+G))
...
def pelgno(xmom):
    ...
    E=_sp.exp(0.5*G**2)
    A=xmom[1]*G/(E*_sp._special.erf(0.5*G))
    U=xmom[0]+A*(E-1)/G
    para = [U,A,G]
    return(para)
...
def pelpe3(xmom):
    ...
    BETA=_sp.sqrt(_sp.pi)*xmom[1]*_sp.exp(_sp._special.gammaln(Alpha)-_sp._special.gammaln(Alpha+0.5))
    para = []
    para.append(xmom[0])
...
def quagam(F,para):
    ...
    if AM1 != 0:
        ...
        if AM1 <= 0:
            ...
            Root = Alpha*(1-1/(9*Alpha) + quastn.quastn(F)/_sp.sqrt(9*Alpha))**3
