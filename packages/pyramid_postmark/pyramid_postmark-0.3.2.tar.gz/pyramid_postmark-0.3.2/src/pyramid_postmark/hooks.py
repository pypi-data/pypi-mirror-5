...
def email_factory(request, from_address, to_address, subject, body,
    ...
    """Instatiate an instance of ``postmark.PMMail``, where the strings provided
      are encoded to utf-8 and stripped::
          ...
          >>> mock_request = Mock()
          >>> mock_request.registry.settings = {'postmark.api_key': 'XXX'}
...
def render_email(request, from_address, to_address, subject, template_spec,
    ...
    if render_template is None: #pragma: no cover
...
def send_email(request, get_batch_mailer=None, join_to_transaction=None,
    ...
    """Provides a function to send one or more emails.
      ...
      Or, if ``paste.testing`` is in the WSGI environ (e.g.: when using WebTest)
      then ``request.send_email`` actually just returns the message data::
      
          >>> mock_request.environ = {'paste.testing': True}
          >>> mock_get_mailer = Mock()
          >>> mock_mailer = Mock()
          >>> mock_get_mailer.return_value = mock_mailer
          >>> mock_msg = Mock()
          >>> mock_msg.to_json_message.return_value = '<msg data>'
          >>> send_email = get_send_email(mock_request,
          ...         get_batch_mailer=mock_get_mailer)
          >>> return_value = send_email(mock_msg)
          >>> return_value['messages']
          ['<msg data>']
          >>> assert not mock_get_mailer.called
    ...
    def send_email(messages, should_join_tx=None, in_background=False):
        ...
        # If we're integration testing, then return the messages.
        if request.environ.get('paste.testing', False):
            return {
                'messages': [item.to_json_message() for item in messages]
            }
