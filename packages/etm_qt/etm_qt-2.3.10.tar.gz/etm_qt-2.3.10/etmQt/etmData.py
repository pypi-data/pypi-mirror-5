...
def getReportData(s, file2uuids, uuid2hash, options={}, export=False,
    ...
    for tup in tups:
        ...
        try:
            eval_fmts = [
                eval(
                    x, {},
                    {'tup': tup, 'QDate': QDate, 'hsh': hsh,
                     'rsplit': rsplit, 'qlcl': qlcl, 's2or3': s2or3})
                for x in fmts]
    ...
    if groupby['report'] == 'c' and not export:
        ...
        if html:
            return(
                tree2Html(
                    tree, width1=groupby['width1'], width2=groupby['width2'],
                    colors=clrs))
        else:
            lst, count, count2id = tree2Text(
                ...
                if hsh['itemtype'] == '*':
                    ...
                    if ed > sd:
                        ...
                        if (typ == 'rm' and 'r' in hsh and
                                ('BYHOUR' in hsh['r'] or
                                    'BYMINUTE' in hsh['r'])):
                            # make sure all the items for this day are the
                            # same so that only one will be displayed
                            st_fmt = '...'
                            et_fmt = ''
...
class ETMCmd(cmd.Cmd):
    ...
    def load_data(self):
        ...
        self.prompt = "\nEnter q to quit, ? to list commands or ? X to show\
 details for command X\netm: "
        self.intro = "Welcome to the etm shell."
        self.doc_header = 'Documented etm commands (enter ? X for details\
 about X)'
