...
from PyQt4.QtCore import (QDate, QDateTime, QLocale)
...
def parse_datetime(dt, timezone='', fmt=rfmt):
    ...
    try:
        ...
        mesg = """\
except: could not parse date = '%s', timezone = '%s', fmt = '%s'
%s""" % (dt, timezone, fmt, f.getvalue())
        print(mesg)
...
def str2opts(s, options={}):
    ...
    for part in groupbylst:
        if groupdate_regex.search(part):
            ...
            term_print(
                str(tr('Ignoring invalid groupby part: "{0}"'.format(part))))
    ...
    for group in groupbylst:
        ...
        if groupdate_regex.search(group):
            ...
            groupby['fmts'].append(
                "qlcl.toString(QDate(tup[-3]), '%s')" % group)
            ...
            if hsh['itemtype'] in [u'+', u'-', u'%']:
                ...
                if 's' in hsh:
                    if 'rrule' in hsh:
                        ...
                        dt = parse(
                            parse_dtstr(hsh['s'], hsh['z'])).replace(
                                tzinfo=None)
                else:
                    # undated
                    dt = ''
...
def getAgenda(allrows, colors=2, days=4, indent=2, width1=54, width2=14,
              calendars=[], html=True, number=True):
    cal_regex = None
    if calendars:
    ...
    for row in rows:
        if cal_regex and not cal_regex.match(row[0][-1]):
            continue
        if row[0][0] == 'day':
            # print('getAgenda', row[-1][0], row[-1][2])
        ...
        if lst:
            ...
            if html:
                ...
                lst, count, count2id = tree2Text(
                    tree, width1=width1,
                    width2=width2, colors=colors,
                    number=number,
                    count=count,
                    count2id=count2id)
                output.extend(lst)
                # output.extend(tree2Text(?tree, width1=width1, width2=width2, colors=colors, number=number))
    # print(count, count2id)
    return((output, count2id))

def getReportData(s, file2uuids, uuid2hash, options={}, export=False,
                  colors=None, html=True, number=True):
    """
    ...
    if groupby['report'] == 'c' and not export:
        ...
        if html:
            return(tree2Html(tree, width1=groupby['width1'], width2=groupby['width2'],
                colors = clrs))
        else:
            lst, count, count2id = tree2Text(
...
def updateCurrentFiles(allrows, file2uuids, uuid2hash, options):
    if options['current_textfile']:
        if 'current_opts' in options and options['current_opts']:
            txt, count2id = getReportData(options['current_opts'],
                file2uuids,
                uuid2hash,
                options,
                colors=0)
        else:
            txt, count2id = getAgenda(allrows,
                colors = 0,
    ...
    if options['current_htmlfile']:
        if 'current_opts' in options and options['current_opts']:
            html, count2id = getReportData(options['current_opts'],
                file2uuids,
                uuid2hash,
                options)
        else:
            html, count2id = getAgenda(allrows,
...
class ETMCmd(cmd.Cmd):
    ...
    def load_data(self):
        ...
        self.prompt = "\nEnter q to quit, ? to list commands or ? X to show details for command X\netm: "
        self.intro = "Welcome to the etm shell."
        self.doc_header = 'Documented etm commands (enter ? X for details about X)'
        self.misc_header = 'Miscellaneous etm commands'
        self.undoc_header = 'Undocumented etm commands'

    # def preloop(self):
    #     self.prompt = "Enter q to quit, ? to list commands or ? X to show details for command X\netm: "
    ...
    def get_itemhash(self, arg_str):
        ...
        if count not in self.count2id:
            print('Item number %d not found' % count)
            return()
        return(self.uuid2hash[self.count2id[count]])
...
by the previous 'c' or 's' command then display the
...
""")
    ...
    def do_e(self, arg_str):
        ...
        f = os.path.join(self.options['datadir'], f)
        hsh = {'file': f, 'line': s}
...
Usage
    e INT
If there is an item number INT among those displayed
by the previous 'c' or 's' command then the open the file
    ...
    def do_n(self, arg_str):
        ...
        if item:
            ...
            if msg:
                print("\n".join(msg))
                return(False)
            if 's' in new_hsh and new_hsh['s']:
                print('new_hsh[s]', new_hsh['s'], type(new_hsh['s']))
                self.currfile = ensureMonthly(self.options, new_hsh['s'])
            new_item = "\n{0}".format(hsh2str(new_hsh, self.options))
            fo = codecs.open(self.currfile, 'a')
            fo.writelines([new_item])
            fo.close()
            print("    appended{0}\n    to\n{1}".format(new_item, self.currfile))
            self.load_data()
        else:  # missing arg_str
            if self.editcmd:
                hsh = {
                    'file': self.currfile,
                    'line': 9999}
...
If ITEM is missing and edit_cmd is specified in etm.cfg, the current
monthly file will opened using edit_cmd for appending the new item.\
""")
...
etm shell loop commands:
    ...
    c ARGS  display a custom report using the remaining
            arguments as the report specification.
    d INT   if there is an item number INT among those displayed
            by the previous 'c' or 's' command then display the
            details of that item.
    ...
    s       display the next few days from the day view
            ...
            views.
    t       toggle the display of item numbers for 'c' and
            's' reports.
    ? ARGS  if ARGS = 'help' then display this help information
...
etm commands:
    ...
    l       begin an interactive shell loop in which the above
            ...
            Enter ? in the shell loop for a listing of
