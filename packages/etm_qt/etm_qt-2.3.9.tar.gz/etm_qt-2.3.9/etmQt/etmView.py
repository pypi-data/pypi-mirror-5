...
from etmQt import etm_rc  # THIS IS NECESSARY!!!
...
class HelpForm(QDialog, Help_Dialog):
    def __init__(self, parent=None, page=0):
        ...
        self.connect(
            self.search, SIGNAL("returnPressed()"), self.doFind)
...
class CompletionTextEdit(QTextEdit):
    ...
    def setCompleter(self, completer):
        ...
        self.connect(
            self.completer, SIGNAL(
                "activated(const QString&)"), self.insertCompletion)
...
class WhichForm(QDialog, Which_Dialog):
    def __init__(self, parent, mode, instance):
        ...
        self.connect(
            self.comboBox, SIGNAL('currentIndexChanged(QString)'),
            self.setChoice)
...
for an action based on this item."""))
        ...
        self.connect(
            self.repsButton, SIGNAL("clicked()"), self.show_repetitions)
        ...
        self.connect(
            self.cloneItemButton, SIGNAL("clicked()"), self.clone_item)

        self.connect(
            self.cloneActionButton, SIGNAL("clicked()"), self.clone_action)
...
to set the finish datetime for this item."""))
    ...
    def show_history(self):
        ...
        QObject.connect(
            self.form.infoButton, SIGNAL("clicked()"), self.showDiffInfo)
        self.form.show()
...
""".format(fmt_date(self.hsh['s'], True), fmt_date(self.bef, True))
        ...
        QObject.connect(
            self.form.infoButton, SIGNAL("clicked()"), self.showInterval)
        self.form.show()
...
class EditForm(QMainWindow, Edit_Dialog):
    def __init__(
        ...
        QObject.connect(
            self.saveButton, SIGNAL("clicked()"), self.fileSave)
        QObject.connect(
            self.quitButton, SIGNAL("clicked()"), self.close)
        QShortcut(
            QKeySequence(Qt.CTRL + Qt.Key_W), self, self.close)
        QShortcut(
        ...
        QObject.connect(
            self.copyButton, SIGNAL("clicked()"), self.editor.copy)
        QObject.connect(
            self.cutButton, SIGNAL("clicked()"), self.editor.cut)
        QObject.connect(
            self.pasteButton, SIGNAL("clicked()"), self.editor.paste)

        self.connect(
            self.editor, SIGNAL("selectionChanged()"), self.updateUi)
        self.connect(
            self.editor.document(), SIGNAL("modificationChanged(bool)"),
            self.updateUi)
        self.connect(
            QApplication.clipboard(), SIGNAL("dataChanged()"), self.updateUi)
    ...
    def writeLinesToFile(self, filename, lines, senddone=True):
        ...
        if 'hg_commit' in self.parent().options:
            command = self.parent().options['hg_commit'].format(
                repo=self.parent().options['datadir'], mesg=self.mode)
            process = QProcess()
...
background: transparent; font-size: 14pt; font-family: CourierNewPSMT"""))
    ...
    #     # FIXME: where did ExtendedComboBox come from?
...
class WeekView(QWidget):
    ...
    def showBusyTimes(self):
        lines = ["<pre>%s %s" %
                 (self.tr("Scheduled times for"),
                 str(self.parent().busyLabel.text()))]
        ampm = False
        s1 = s2 = ''
...
class UiWindow(QMainWindow, MainWindow_Dialog):
    def __init__(self, parent=None, options={}):
        ...
        QObject.connect(
            self.uiFilter, SIGNAL("textChanged(QString)"), self.filter)
        ...
        QObject.connect(
            self.newActionButton, SIGNAL("clicked()"), self.timer_toggle)
        QObject.connect(
            self.pause_restartButton, SIGNAL("clicked()"), self.timer_toggle)
        QObject.connect(
            self.stopButton, SIGNAL("clicked()"), self.timer_finish)

        QObject.connect(
            self.reportButton, SIGNAL("clicked()"), self.createReport)

        QObject.connect(
            self.prevWeekButton, SIGNAL("clicked()"), self.prevPeriod)

        QObject.connect(
            self.nextWeekButton, SIGNAL("clicked()"), self.nextPeriod)

        QObject.connect(
            self.alertsButton, SIGNAL("clicked()"), self.showAlerts)

        QObject.connect(
            self.todayButton, SIGNAL("clicked()"), self.viewChanged)

        QObject.connect(
            self.errorButton, SIGNAL("clicked()"), self.showMessages)

        QObject.connect(
            self.newItemButton, SIGNAL("clicked()"), self.createItem)

        QObject.connect(
            self.mainHelpButton, SIGNAL("clicked()"), self.mainHelp)
        ...
        QObject.connect(
            self.calendarButton, SIGNAL("clicked()"), self.selectCalendars)
        ...
        self.connect(
            self.calendarWidget, SIGNAL("activated(QDate)"), self.setMonthDay)

        self.connect(self, SIGNAL("itemChanged"), self.itemChanged)

        self.connect(
            self.weekview, SIGNAL("doubleclicked(QString, QString)"),
            self.clickedWeek)
        ...
        QObject.connect(
            self.dayTree,
            SIGNAL("activated(QModelIndex)"), self.activated)
        QObject.connect(
            self.tagTree,
            SIGNAL("activated(QModelIndex)"), self.activated)
...
Click here or press Shift Ctrl-T to stop the timer and record the action."""))
    ...
    def aboutHelp(self):
        d = {
            ...
            'group':
"""<a href="http://groups.google.com/group/eventandtaskmanager/topics">\
forum</a>""",
