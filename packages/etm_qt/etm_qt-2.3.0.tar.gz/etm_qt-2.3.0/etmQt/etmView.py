    ...
    def writeLinesToFile(self, filename, lines, senddone=True):
        ...
        try:
            ...
            if tup not in self.recent_edits:
                ...
                self.recent_edits = self.recent_edits[:10]
...
class UiWindow(QMainWindow, MainWindow_Dialog):
    def __init__(self, parent=None, options={}):
        ...
        QShortcut(QKeySequence(Qt.Key_F8), self, self.exportVcal)
    ...
    def edit_recent(self):
        ...
        if which.exec_():
            # print('edit_recent', self.which_file)
...
""" % (datestr, res))
    ...
    def exportVcal(self):
        if not self.options['has_vobject']:
            msg = self.tr("Could not load the required python package 'vobject'.")
        else:
            ok, msg = make_vcal(self.all_rows, self.uuid2hash, self.options['vcal_file'], self.calendars)
            if ok:
                if self.calendars:
                    ...
                    calendars = "all items"
                msg = "Exported {0}\nto {1}".format(calendars, self.options['vcal_file'])
