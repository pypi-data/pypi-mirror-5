...
if term_locale == 'de_DE':
    ...
    rfmt="%Y-%m-%d %H:%M%z"
# print('using rfmt:', term_locale, rfmt)
...
def get_options(etmdir = ''):
    ...
    if os.path.isfile(locale_cfg):
        ...
        fo.close()
        # print('get_options use_locale', use_locale)
    ...
    if changed:
        ...
        yaml.dump(options, fo)
        fo.close()

    # add custom user options if the file exists
    # if os.path.isfile(user_config):
    #     fo = codecs.open(user_config, 'r', encoding)
    #     custom_options = yaml.load(fo)
    #     fo.close()
    ...
    options['has_vobject'] = has_vobject
    options['vcal_file'] = os.path.join(etmdir, "etm.ics")
    ...
    options['lcl'] = lcl
    options['qlcl'] = qlcl
    # print("options:", options['dayfirst'], options['yearfirst'])
...
def fmt_dt(dt, fmt):
    ...
    # print('fmt_dt', type(dt), dt, fmt, dt_fmt)
    return(s2or3(dt_fmt))
...
def tallyByGroup(list_of_tuples, max_level=0, indnt=3, options={},
    ...
    """
        ...
        in each tuple should be a tuple (minutes, value) to be tallied.

             ('Scotland', 'Glasgow', 'North', 'summary sgn', (306, 10)),
             ('Scotland', 'Glasgow', 'South', 'summary sgs', (960, 10)),
             ('Wales', 'Cardiff', 'summary wc', (396, 10)),
             ('Wales', 'Bangor', 'summary wb', (126, 10)),
...
def group_sort(lst):
    # last element of each list component is a (minutes, value) tuple.
...
def parse_date_period(s):
    ...
    try:
        # print('parse_date_period', parts[0])
        # print(parse_datetime(parts[0]))
    ...
    for key in item_keys:
        ...
        if key in hsh and hsh[key]:
            ...
            if keys:
                ...
                tmp = []
                for pair in hsh['f']:
                    tmp.append(";".join([x.strftime(zfmt) for x in pair]))
                sl.append("\n  @f %s" % (',\n       '.join(tmp)))
            else:
...
def getFileItems(full_name, rel_name):
    ...
    """
    fo = codecs.open(full_name, 'r', 'UTF8')
    lines = fo.readlines()
    fo.close()
    # make sure we have a trailing new-line. Yes, we really need this.
    lines.append('\n')
...
def get_rrulestr(hsh):
    ...
    # print('get_rrulestr processing', hsh)
    lst = []
    l = []
    if type(lofh) == dict:
        lofh = [lofh]
    for h in lofh:
        ...
        lst.append(";".join(l))
    # print('get_rrulestr returning', lst)
    return("\n".join(lst))
...
def checkhsh(hsh):
    ...
    elif hsh['itemtype'] in ['~'] and 'e' not in hsh:
        messages.append("An entry for @e is required for actions.")
...
def getReportData(s, file2uuids, uuid2hash, options={}, export=False, colors=None, html=True):
    ...
    for tup in tups:
        ...
        if groupby['report'] == 'c':
            ...
            row.append(timeValue(hsh, options))
        rows.append(row)
    ...
    try:
            ...
            if at_key == 'a':
                ...
                if at_key in ['a', 't']:
                    if comma_regex.search(at_val):
                        hsh[at_key] = comma_regex.split(at_val)
                    else:
                        hsh[at_key] = [at_val]
                elif at_key == 's':
        ...
        if 'b' in hsh:
            try:
                ...
                msg.append("the value of @b should be an integerhsh[b]: '%s'" % hsh['b'])
        if 'f' in hsh:
...
def getViewData(bef, file2uuids={}, uuid2hash={}, options={}):
    ...
    datetimes = []
    busytimes = {}  # date -> [(isocal-datekey, start_minutes, end_minutes, uuid,  total_minutes, f)]
        ...
        for u in file2uuids[f]:
            ...
            for dt in dates:
                ...
                if hsh['itemtype'] == '+':
                    ...
                    add2list(rows, item)
                    continue
    # updateCurrentFiles(rows, file2uuids, uuid2hash, options)
    return(rows, busytimes, busydays, alerts, datetimes)
        ...
        if row[0][0] == 'day':
            if row[0][1] >= beg_fmt and day_count <= days + 1:
                ...
                if row[0][1] != last_day:
                    last_day = row[0][1]
                    day_count += 1
                    # print('day_count', day_count, last_day)
                if day_count <= days:
                    day.append(row)
        elif row[0][0] == 'now':
...
def updateCurrentFiles(allrows, file2uuids, uuid2hash, options):
    if options['current_textfile']:
        # print('updating current_textfile')
        ...
        if txt and not txt[0]:
            txt.pop(0)
        fo = codecs.open(options['current_textfile'], 'w', "UTF-8")
        fo.writelines("\n".join(txt))
        fo.close()
    if options['current_htmlfile']:
        # print('updating current_htmlfile')
        if 'current_opts' in options and options['current_opts']:
        ...
        if not html[0]:
            html.pop(0)
        fo = codecs.open(options['current_htmlfile'], 'w', "UTF-8")
...
def make_vcal(rows, uuid2hash, vcal_file, calendars=None):
    """
        Return calendar as string if fname is ''.
    """
    if not has_vobject:
        return(False, 'Could not load vobject')
    ret_val = False
    cal_regex = None
    if calendars:
        cal_pattern = r'^%s' % '|'.join([x[2] for x in calendars if x[1]])
    ...
    for row in rows:
        ...
        # skip other types for now
            ...
            continue
        element.add('uid').value = uniqueId()
        element.add('summary').value = hsh['_summary']
        if 'q' in hsh:
            element.add('priority').value = hsh['_p']
        if 'l' in hsh:
            element.add('location').value = hsh['l']
        if 't' in hsh:
            element.add('categories').value = hsh['t']
        if 'd' in hsh:
            element.add('description').value = hsh['d']
        if 'm' in hsh:
            element.add('comment').value = hsh['m']
        ...
        if item_type in ['un', 'du', 'cu', 'nu']: # undated task
            ...
            element.add('comment').value = timedelta2Str(hsh['e'])
            allday =False
            ...
            if 'e' in hsh:
                etu = (dtu + hsh['e'])
                element.add('dtend').value = etu
        ret_val = True
    (name, ext) = os.path.splitext(vcal_file)
    pname = "%s.ics" % name
    try:
        cal_str = cal.serialize()
        # pprint(cal_str.split('\n'))
    except Exception:
        f = StringIO()
        traceback.print_exc(file=f)
