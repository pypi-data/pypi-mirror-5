...
class UiWindow(QMainWindow, MainWindow_Dialog):
    ...
    def jumpToDate(self):
        ...
        if ok:
            ...
            dt = parse(parse_datetime(str(datestr)), dayfirst=True, yearfirst=True).replace(tzinfo = None)
            # print(type(dt), dt)
            if dt:
                self.scrollToDate(dt.date())
                return(True)
    ...
    def timer_finish(self, create = True):
        ...
        if self.timer_hsh:
            ...
            inital_entry = "~ %s @s %s @e %s" % (self.timer_summary,
                fmt_datetime(datetime.now(), self.options), fmt_period(self.timer_delta))
