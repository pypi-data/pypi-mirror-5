...
class Weather(QObject):
	...
	def getWeather(self):
		...
		if not (self.weather_location.startswith('p=') or self.weather_location.startswith('w=')):
			# assume p=
					...
					if hash:
						if first:
							...
							for key in ['sunrise', 'sunset']:
								if key in data:
									if key == 'sunrise':
										string += "%s %s, " % (self.tr('sunrise'),
										 data[key])
									else:
										string += "%s %s" % (self.tr('sunset'),
										 data[key])
							ret.append(string)
							#### Forecast ####
...
def getSunMoon(loc=None, d=None):
	...
	if len(sunmoon_location) == 2:
		...
		return (0, 'There is a problem with the setting for \'sunmoon_location\' in ~/.etmrc.')
	...
	try:
		if python_version2:
			...
			response = urlopen(url, bytes(url_data, 'utf-8'))
	except URLError as e:
		if hasattr(e, 'reason'):
			msg = """\
