...
import platform
import sip
from icalendar import Calendar, Event, Todo, Journal, Timezone, TimezoneStandard, TimezoneDaylight
from icalendar.parser import param_value, tzid_from_dt
from icalendar.caselessdict import CaselessDict
from icalendar.prop import vDate, vDatetime, vDuration
import pytz
...
# sys_path = os.path.join(os.path.split(os.__file__)[0], "site-packages")
...
from PyQt4.QtCore import (PYQT_VERSION_STR, QDate, QDateTime, QFile, QRegExp,
    QVariant, Qt, QAbstractItemModel, SIGNAL, QLocale)

from time import sleep
import glob

def tr(string):
    return(QApplication.translate('etmData', string))
...
def getMercurial():
    hg = PathSearch('hg')
    if hg:
        history_command = '%s log --style compact --template "{rev}: {desc}\\n" -R {repo} -p -r "tip":0 {file}' % hg
        commit_command = '%s commit -q -A -R {repo} -m "{mesg}"' % hg
        init_command = '%s init {0}' % hg
    else:
        history_command = commit_command = init_command = ''
    return(history_command, commit_command, init_command)
...
def mail_report(message, smtp_from = None, smtp_server = None, smtp_id = None, smtp_pw = None, smtp_to = None):
    import smtplib
...
def send_mail(smtp_to, subject, message, files = [], smtp_from = None, smtp_server = None, smtp_id = None, smtp_pw = None):
    import smtplib
    from email.MIMEMultipart import MIMEMultipart
    from email.MIMEBase import MIMEBase
    from email.MIMEText import MIMEText
...
today = datetime.now(tzlocal()).replace(hour=0, minute=0,
...
def get_options(etmdir = ''):
    ...
    if os.path.isfile(locale_cfg):
        fo = codecs.open(locale_cfg, 'r', encoding)
        use_locale = yaml.load(fo)
        fo.close()

    time_zone = get_localtz()[0]

    # for adjusting vertical height in tree views if positive
    default_rowsize = 0
    ...
    hg_history, hg_commit, hg_init = getMercurial()
    ...
    if os.path.isfile(config):
        fo = codecs.open(config, 'r', encoding)
        user_options = yaml.load(fo)
        fo.close()
    else:
        user_options = {'datadir' : default_datadir}
        fo = codecs.open(config, 'w', encoding)
        # fo = open(config, 'w')
    ...
    changed = False
    if user_options:
        if 'actions_timercmd' in user_options and user_options['actions_timercmd']:
            user_options['action_timer']['running'] = user_options['actions_timercmd']
    ...
    options['reprdatetimefmt'], options['etmdatetimefmt'], options['rfmt']) = get_fmts(options)
    options['config'] = config
    options['datafile'] = datafile
    options['scratchfile'] = os.path.join(options['datadir'], ".scratch")
    # options['icsitem_file'] = os.path.join(etmdir, "etmitem.ics")
    # options['icscal_file'] = os.path.join(etmdir, "etmcal.ics")

    if options['action_minutes'] not in [1, 6, 12, 15, 30, 60]:
        print("Invalid action_minutes setting: %s. Reset to 1." %
            options['action_minutes'])
        options['action_minutes'] = 1

    z = gettz(options['local_timezone'])
    if z is None:
        print("Error: bad entry for local_timezone in etm.cfg: '%s'" % options['local_timezone'])
...
def get_fmts(options):
    ...
    return(daybegin_fmt, dayend_fmt, reprtimefmt, reprdatetimefmt, etmdatetimefmt, rfmt)
...
Error code: %s.""" % e.code
        ...
        if python_version2:
            ...
            res = response.read().decode(encoding)
            vstr = rsplit('\s+', res)[0]

        if version < vstr:
            return(1, '----------------------------------------\nA newer version of etm, %s, is available at people.duke.edu/~dgraham/etmqt.' % (vstr))
        else:
...
def fmt_time(dt, omitMidnight=False, options = {}):
    # fmt time, omit leading zeros and, if ampm, convert to lowercase and omit trailing m's
...
def expandTree(tree, node=('','_'), level=0):
    if type(node) == tuple:
        if type(node[1]) == tuple:
            t = id2Type[node[1][1]]
            s = "%s %s" % (unicode(t), unicode(node[1][2]))
            rmlft = 50 - 3*level
            if len(s) > rmlft:
                s = "%s..." % s[:(rmlft-3)]
            print("{0}{1}\t{2}".format("   "*level, s, node[1][3]))
        else:
            print("%s%s" % ("   "*level, node[1]))
    else:
        ...
        if type(node) == tuple:
            if type(node[1]) == tuple:
                ...
                s = "%s%s%s %-*s %s%s" % (tab*level, s_c, unicode(t),
                    ...
                    unicode(truncate(node[1][3], width2)), e_c)
                html_lst.append(s)
            else:
...
def tree2Text(tree, indent=2, width1=54, width2=14, colors=0):
    ...
    def t2H(tree, node=('','_'), level=0):
        ...
        for n in nodes:
            t2H(tree, n, level)
    t2H(tree)
    return([x[indent:] for x in text_lst])


def tallyByGroup(list_of_tuples, max_level=0, indnt=3, options={},
    export = False):
    """
...
def group_sort(lst):
    # last element of each list component is a (minutes, value, expense, charge) tuple.
...
def dump_data(options, uuid2hashes, file2uuids, file2lastmodified, bad_datafiles, messages):
...
def parse_datetime(dt, timezone = '', fmt = rfmt):
    ...
    try:
        ...
        mesg = "except: could not parse date = '%s', timezone = '%s', fmt = '%s'\n%s" % (
                dt, timezone, fmt, f.getvalue())
        print(mesg)
...
def parse_dtstr(dtstr, timezone="", fmt=rfmt):
    ...
    if type(dtstr) in [str, unicode]:
        if dtstr == 'now':
            if timezone:
                dt = datetime.now().replace(tzinfo = tzlocal()).astimezone(gettz(timezone)).replace(tzinfo = None)
            else:
                dt = datetime.now()
        else:
            dt = parse(dtstr)
...
def hsh2str(hsh, options = {}):
    '''
        ...
        1. Add @+ datetime to orig and make copy sans all repeating info and with @s datetime.
        2. Add &r datetime - oneminute to each _r in orig and make copy with @s datetime
        3. Add &f datetime to selected job.
    '''
    if u'_summary' not in hsh: hsh[u'_summary'] = ''
    ...
    for key in item_keys:
        ...
        if key == 'a' and '_a' in hsh:
            ...
            for alert in hsh["_a"]:
                ...
                tr = "@a %s" % ", ".join([fmt_period(x) for x in triggers])
                if actions:
                    tr += ": %s" % ", ".join(actions)
                    if arguments:
                        arg_strings = []
        ...
        if key in hsh and hsh[key]:
            value = hsh[key]
            if keys:
                # @r or @j --- value will be a list of hashes or possibly, in the  case of @a, a list of lists.  f will be the first key for @r and t will be the first for @a
                tmp = []
                for h in value:
                    if unicode(keys[0]) not in h:
                        print("error:", keys[0], "not in", h)
                        continue
                    tmp.append('%s@%s %s' % (prefix, at_key, lst2str(h[unicode(keys[0])])))
                    for amp_key in keys[1:]:
                       if amp_key in h:
                            if at_key == 'j' and amp_key == 'f':
                                pairs = []
                                for pair in h['f']:
                                    pairs.append(";".join([x.strftime(zfmt) for x in pair if x]))
                                v = (', '.join(pairs))
                                ...
                                try:
                                    ...
                                    v = h['e']
                                    print("error: could not parse h['e']: '%s'" % h['e'])
                ...
                sl.append("@%s %s" % (key,
                    fmt_datetime(value, options = options)))
            elif key == 'e':
                sl.append("@%s %s" % (key,
                    fmt_period(value)))
            elif key == 'f':
...
def getFiles(root):
    ...
    for path, dirs, files in os.walk(root):
        # exclude dirs
        dirs[:] = [os.path.join(path, d) for d in dirs if not fnmatch.fnmatch(d, excludes)]

        # exclude/include files
        files = [os.path.join(path, f) for f in files if not fnmatch.fnmatch(f, excludes)]
...
def getFileItems(full_name, rel_name):
    """
        Group the lines in file f into logical items and return them.
    """
    fo = codecs.open(full_name, 'r', encoding)
    lines = fo.readlines()
    fo.close()
    # make sure we have a trailing new-line. Yes, we really need this.
    lines.append('\n')
...
def items2Hashes(list_of_items, options = {}):
    ...
    for item, rel_name, linenums in list_of_items:
        ...
        try:
            ...
            raise ValueError("exception in fileinfo:", rel_name, linenums, "\n", hsh)
        ...
        if itemtype == '#':
            ...
            for job in jobs:
                ...
                job["_summary"] = "%s %d/%d: %s" % (summary, job_num, num_jobs, job['j'])
                del job['j']
                if 'q' not in job:
...
def get_rrulestr(hsh, key_hsh=rrule_hsh):
    ...
    for h in lofh:
        ...
        if 'u' in h:
            dt = parse(parse_dtstr(h['u'], hsh['z'])).replace(tzinfo = None)
...
def get_rrule(hsh):
    ...
    if 'o' in hsh and hsh['o'] == 'r' and 'f' in hsh :
        ...
        dtstart = parse(parse_dtstr(hsh['s'], hsh['z'])).replace(tzinfo = None)
    else:
    ...
    if '-' in hsh:
        ...
        if parts:
            for part in map(str, parts):
                rlst.append("EXDATE:%s" % parse(part).strftime(sfmt))
    rrulestr = "\n".join(rlst)
    try:
...
def checkhsh(hsh):
    ...
    if hsh['itemtype'] in ['*', '~', '^'] and 's' not in hsh:
        messages.append("An entry for @s is required for events, actions and occasions.")
    ...
    if ('a' in hsh or 'r' in hsh) and 's' not in hsh:
        messages.extend(["An entry for @s is required for items with", "either @a or @r entries."])
    if ('+' in hsh or '-' in hsh) and 'r' not in hsh:
        messages.extend(["An entry for @r is required for items with", "either @+ or @- entries."])
    return(messages)
...
def str2opts(s, options={}):
    ...
    if 'calendars' in options:
        cal_pattern = r'^%s' % '|'.join([x[2] for x in options['calendars'] if x[1]])
        filters['cal_regex'] = re.compile(cal_pattern)
    s = str(s)
    pos_fields = []
    neg_fields = []
    parts = minus_regex.split(s)
    head = parts.pop(0)
    report = head[0]
    groupbystr = head[1:].strip()
    if not report or report not in ['a', 'c'] or not groupbystr:
        return({})
    ...
    for part in groupbylst:
        if groupdate_regex.search(part):
            dated['groupby'] = True
            filters['dates'] = True
        elif part not in ['c', 'u'] and part[0] not in ['k', 'f',]:
            term_print(str(tr('Ignoring invalid groupby part: "{0}"'.format(part))))
    ...
    for group in groupbylst:
        ...
        if groupdate_regex.search(group):
            ...
            # groupby['fmts'].append("QDate(tup[-3]).toString('%s')" % group)
            groupby['fmts'].append("qlcl.toString(QDate(tup[-3]), '%s')" % group)

        elif '[' in group:
            if group[0] == 'f':
                if ':' in group:
                    groupby['fmts'].append("'/'.join(rsplit('/', hsh['fileinfo'][0])%s)" % (group[1:]))
                    groupby['tuples'].append("'/'.join(rsplit('/', hsh['fileinfo'][0])%s)" % (group[1:]))
                else:
                    groupby['fmts'].append("rsplit('/', hsh['fileinfo'][0])%s" % (group[1:]))
                    groupby['tuples'].append("rsplit('/', hsh['fileinfo'][0])%s" % (group[1:]))
            elif group[0] == 'k':
                if ':' in group:
                    groupby['fmts'].append("':'.join(rsplit(':', hsh['%s'])%s)" % (group[0], group[1:]))
                    groupby['tuples'].append("':'.join(rsplit(':', hsh['%s'])%s)" % (group[0], group[1:]))
                else:
                    groupby['fmts'].append("rsplit(':', hsh['%s'])%s" % (group[0], group[1:]))
                    groupby['tuples'].append("rsplit(':', hsh['%s'])%s" % (group[0], group[1:]))
    ...
    for part in parts:
        key = unicode(part[0])
        if key in ['b', 'e']:
            # dated[key] = parse(parse_datetime(part[1:])).replace(
            #     tzinfo=None)
            ...
            for t in value:
                if t[0] == '!':
                    filters['neg_fields'].append(('t',
                        re.compile(r'%s' % t[1:], re.IGNORECASE)))
                else:
                    filters['pos_fields'].append(('t'),
                        re.compile(r'%s' % t, re.IGNORECASE))
        elif key == 'o':
            value = unicode(part[1:].strip())
            if value[0] == '!':
            ...
            value = unicode(part[1:].strip())
            if value[0] == '!':
                filters['neg_fields'].append((key,
                    re.compile(r'%s' % value[1:], re.IGNORECASE)))
            else:
                filters['pos_fields'].append((key,
                    re.compile(r'%s' % value, re.IGNORECASE)))
    if 'b' not in dated:
        dated['b'] = parse(parse_datetime(options['report_begin'])).replace(tzinfo=None)
    if 'e' not in dated:
        dated['e'] = parse(parse_datetime(options['report_end'])).replace(tzinfo=None)
    ...
    for f in file2uuids:
        ...
        for uuid in file2uuids[f]:
            ...
            if skip:
                # try the next uuid
                continue
            for f, r in filters['pos_fields']:
                if f not in hsh or not r.search(hsh[f]):
                    skip = True
                    break
            if skip:
                # try the next uuid
                continue
            for f, r in filters['neg_fields']:
                if f in hsh and r.search(hsh['f']):
                    skip = True
                    break
            if skip:
                # try the next uuid
...
def reportDT(dt, include, options={}):
    ...
    if dt.hour == 0 and dt.minute == 0:
        if not include:
            return('')
        return(s2or3(qlcl.toString(QDate(dt), "MMM d yyyy")))
    else:
        if options['ampm']:
            if include:
                return(str(QDateTime(dt).toString("%s h:mma" % include)))
            else:
                return(str(QDateTime(dt).toString("h:mma")))
        else:
            if include:
                return(str(QDateTime(dt).toString("%s hh:mm" % include)))
            else:
                return(str(QDateTime(dt).toString("hh:mm")))
...
def makeReportTuples(uuids, uuid2hash, groupby, dated, options={}):
    ...
    """
    today_datetime = datetime.now().replace(hour=0, minute=0,
        second=0, microsecond=0)
    ...
    for uuid in uuids:
        ...
        if dated['groupby']:
            ...
            if 'f' in hsh and hsh['f']:
                dt = parse(parse_dtstr(hsh['f'][-1][0], hsh['z'])).astimezone(tzlocal()).replace(tzinfo = None)
            ...
            for dt in dates:
                ...
                for g in groupby['tuples']:
                    if groupdate_regex.search(g):
                        item.append(qlcl.toString(QDate(dt), g))
                    elif g in ['c', 'u']:
                        item.append(hsh[g])
                    else: # should be f or k
                        item.append(eval(g))
                item.extend([tstr2SCI[tstr][0],
                    tstr,
                    dt,
            ...
            if hsh['itemtype'] in [u'+', u'-', u'%']:
                ...
                if 's' in hsh:
                    if 'rrule' in hsh:
                        ...
                        dt = parse(parse_dtstr(hsh['s'], hsh['z'])).replace(tzinfo = None)
                else:
                    # undated
                    dt = ''
            for g in groupby['tuples']:
                if groupdate_regex.search(g):
                    item.append(str(QDate(dt).toString(g)))
                else:
                    try:
                        res = eval(g)
                        item.append(res)
...
def getReportData(s, file2uuids, uuid2hash, options={}, export=False, colors=None, html=True):
    """
    ...
    if not groupby:
        return([str(tr('invalid groupby setting'))])
    ...
    for tup in tups:
        hsh = uuid2hash[tup[-1]]
        try:
            eval_fmts = [eval(x, {}, {'tup': tup, 'QDate' : QDate, 'hsh' : hsh, 'rsplit': rsplit, 'qlcl': qlcl, 's2or3': s2or3}) for x in fmts]
        except Exception as e:
            print('except', e, [str(x) for x in fmts])
            continue
        if filters['dates']:
            ...
            # the sort string will be in tup[-3], the display string in tup[-2]
            dt = tup[-2]
        row = (cols.format(*eval_fmts)).split('::')
        if groupby['report'] == 'c':
            row.append((tup[-1], tup[-4], setSummary(hsh, parse(dt)), dt, ''))
        else:
    ...
    if groupby['report'] == 'c' and not export:
        if colors is not None:
            ...
            clrs = groupby['colors']
        if html:
            return(tree2Html(makeTree(rows, sort=False), width1=groupby['width1'], width2=groupby['width2'],
                colors = clrs))
        else:
            return(tree2Text(makeTree(rows, sort=False), width1=groupby['width1'], width2=groupby['width2'],
                colors = clrs))
    else:
        if groupby['report'] == 'a' and 'depth' in groupby and groupby['depth']:
        ...
        if export:
            ...
            if groupby['report'] == 'c':
                ...
                csv = [head]
                lst = tallyByGroup(rows, max_level = depth, options = options, export = True)
                for row in lst:
            ...
            return(tallyByGroup(rows, max_level = depth, options = options))
    ...
    try:
        ...
        for at_part in at_parts:
            ...
            if at_key == 'a':
                ...
                if alert_parts:
                    action_parts = [x.strip() for x in alert_parts[0].split(';')]
                    actions = [unicode(x.strip()) for x in
                ...
                for amp_part in amp_parts:
                    ...
                    if amp_key == 'q':
                        ...
                        part_hsh[amp_key] = parse(parse_datetime(amp_val)).replace(tzinfo=None)

                    elif amp_key == 'e':
                        try:
                            part_hsh['e'] = parse_period(amp_val)
                        except:
                            msg.append("error: could not parse hsh['e']: '%s'" % amp_val)

                    else:
                        m = range_regex.search(amp_val)
                        if m:
                            if m.group(3):
                                part_hsh[amp_key] = [x for x in range( int(m.group(1)), int(m.group(3)))]
                            else:
                ...
                try:
                    ...
                    msg.append("error appending '%s' to hsh[%s]" % (part_hsh, at_key))
            else:
                # value will be a scalar or list
                if at_key in ['a', 't']:
                    if comma_regex.search(at_val):
                        hsh[at_key] = [x for x in comma_regex.split(at_val) if x]
                    else:
                        hsh[at_key] = [at_val]
                elif at_key == 's':
                    ...
                    try:
                        ...
                        msg.append("error: could not parse hsh['e']: '%s'" % at_val)
                elif at_key == 'p':
                    hsh['p'] = int(at_val)
        ...
        if 's' in hsh:
            try:
                hsh['s'] = parse(parse_datetime(hsh['s'], hsh['z'])).replace(tzinfo = None)
            except:
        ...
        if 'b' in hsh:
            try:
                ...
                msg.append("the value of @b should be an integer: hsh['b'] = '%s'" % hsh['b'])
        if 'f' in hsh:
            ...
            for pair in pairs:
                pair = pair.split(';')
                done = parse(parse_datetime(pair[0], hsh['z'])).replace(tzinfo = None)
                if len(pair) > 1:
                    due = parse(parse_datetime(pair[1], hsh['z'])).replace(tzinfo = None)
                else:
                    due = done
        ...
        if 'j' in hsh:
            for i in range(len(hsh['j'])):
                ...
                if 'f' in job:
                    ...
                    # done = parse(parse_dtstr(pair[0], hsh['z'])).replace(tzinfo = None)
                    if len(pair) > 1:
                        due = parse(parse_datetime(pair[1], hsh['z'])).replace(tzinfo = None)
                        # due = parse(parse_dtstr(pair[1], hsh['z'])).replace(tzinfo = None)
                    else:
                        due = ''
...
def add2list(lst, item, expand=True):
    ...
    if expand and len(item) == 3:
        # this is a tree entry, so we need to expand the middle tuple for makeTree
        try:
            entry = []
            entry.append(item[0])
...
# def get_data(options = {}, new = [], modified = [], deleted = [], use_pickle=True):
def get_data(options = {}, dirty=False, use_pickle=True):
    ...
    if dirty and not messages:
        dump_data(options, uuid2hash, file2uuids, file2lastmodified, bad_datafiles, messages)
...
def getDoneAndTwo(hsh, keep=False):
    ...
    next = None
    following = None
    today_datetime = datetime.now(gettz(hsh['z'])).replace(hour=0, minute=0,
        second=0, microsecond=0, tzinfo=None)
    if 'f' in hsh and hsh['f']:
        if type(hsh['f']) in [str, unicode]:
            parts = str(hsh['f']).split(';')
            done = parse(parse_dtstr(parts[0], hsh['z']).replace(tzinfo = None))
            if len(parts) < 2:
                due = done
            else:
                due = parse(parse_dtstr(parts[1], hsh['z']).replace(tzinfo = None))
    ...
    if 'rrule' in hsh:
        ...
        if 'f' in hsh:
            ...
            next = parse(parse_dtstr(hsh['s'], hsh['z'])).replace(tzinfo = None)
    # if next and type(next) != datetime:
    #     print("getDoneAndTwo", type(done), done, type(next), next, type(following), following)
...
def timedelta2Sentence(td):
    ...
    if string == 'none':
        ...
        return(str(tr("{0} from now")).format(string))
...
def add_busytime(id, sd, sm, em, evnt_summary, busytimes, busydays, relpath):
    ...
    if ((sm, em, id, evnt_summary)) not in busytimes[timekey][1]:
        ...
        bisect.insort( busytimes[timekey][1], (sm, em, id, evnt_summary, relpath))
...
def setItemPeriod(hsh, start, end, short=False, options = {}):
    ...
    end_tup = (end.year, end.month, end.day)
    sy = start.year
    ey = end.year
    sm = start.month
    em = end.month
    ...
    if start == end:                    # same time - zero extent
        if short:
            ...
            period = "%s %s" % (fmt_time(start, options=options), fmt_date(start, True))
    elif (sy, sm, sd) == (ey, em, ed):  # same day
        if short:
            period = "%s - %s" % (fmt_time(start, options = options), fmt_time(end, options = options))
        else:
            period = "%s - %s %s" % (fmt_time(start, options = options), fmt_time(end, options = options),
                fmt_date(end, True))
    else:
        period = "%s %s - %s %s" % (fmt_time(start, options = options), fmt_date(start, True),
...
def getViewData(bef, file2uuids={}, uuid2hash={}, options={}):
    ...
    datetimes = []
    busytimes = {}  # isodate -> [(start_minutes, end_minutes, uuid,  time period, subject, f)]
    busydays = {}   # date -> totalminutes
    occasions = {}  # isodate -> [(uuid, subject)]
    alerts = [] # for today only [(start_minutes, alert_minutes, action, uuid, f)]
    for f in file2uuids:
        ...
        for u in file2uuids[f]:
            ...
            if hsh['itemtype'] in [u'+', u'-', u'%']:
                ...
                if done:
                    ...
                    for done, due in hsh['f'][-options['show_finished']:]:
                        item = [('day', done.strftime(sortdatefmt), tstr2SCI[typ][0], hsh['_p'], '', f),
                            (fmt_date(done),),
                ...
                if next:
                    ...
                    if hsh['itemtype'] == '%':
                        ...
                        if 'prereqs' in hsh and hsh['prereqs']:
                            ...
                            if pastdue:
                                ...
                                typ = 'av'
                    item = [('folder', (f, tstr2SCI[typ][0]), next, hsh['_summary'], f), tuple(folders), (u, typ,
                        setSummary(hsh, next), time_str, etmdt)]
                    add2list(rows, item)
                    if 'k' in hsh:
                        keywords = [x.strip() for x in hsh['k'].split(':')]
                        # hsh['k'] = ':'.join(keywords)
                        item = [('keyword', (hsh['k'], tstr2SCI[typ][0]), next, hsh['_summary'], f), tuple(keywords), (u, typ,
                            setSummary(hsh, next), time_str, etmdt)]
                        add2list(rows, item)
                    if 't' in hsh:
                        for tag in hsh['t']:
                            item = [('tag', (tag, tstr2SCI[typ][0]), next, hsh['_summary'], f), (tag,), (u, typ,
                                setSummary(hsh, next), time_str, etmdt)]
                            add2list(rows, item)
                else: # undated
                    dts = "none"
                    ...
                    etmdt = "%s %s" % (dtl.strftime(etmdatefmt),
                        fmt_time(dtl, True, options=options))
                    item = [('folder', (f, tstr2SCI[typ][0]), '', hsh['_summary'], f), tuple(folders), (u, typ,
                        setSummary(hsh, ''), '')]
                    add2list(rows, item)
                    if 'k' in hsh:
                        keywords = [x.strip() for x in hsh['k'].split(':')]
                        # hsh['k'] = ":".join(keywords)
                        item = [('keyword', (hsh['k'], tstr2SCI[typ][0]), '', hsh['_summary'], f), tuple(keywords), (u, typ,
                            setSummary(hsh, ''), '', etmdt)]
                        add2list(rows, item)
                    if 't' in hsh:
                        for tag in hsh['t']:
                            item = [('tag', (tag, tstr2SCI[typ][0]), next, hsh['_summary'], f), (tag,), (u, typ,
                ...
                if dt:
                    try:
                        etmdt = "%s %s" % (dt.strftime(etmdatefmt),
                        ...
                        print(hsh)
                    if hsh['itemtype'] == '*':
                        sdt = "%s %s" % (fmt_time(dt, True, options = options), fmt_date(dt, True))
                    elif hsh['itemtype'] == '~':
                        if 'e' in hsh:
                            sdt = "%s: %s" % (fmt_period(hsh['e']),
                                fmt_date(dt, True))
                        else:
                            sdt = ""
                    else:
                ...
                if hsh['itemtype'] == '*':
                    ...
                    typ = type2Str[hsh['itemtype']]
                item = [('folder', (f, tstr2SCI[typ][0]), dt, hsh['_summary'], f), tuple(folders), (u, typ,
                    setSummary(hsh, dt), sdt, etmdt)]
                add2list(rows, item)
                if 'k' in hsh: # and hsh['k'] != 'none':
                    keywords = [x.strip() for x in hsh['k'].split(':')]
                    # hsh['k'] = ":".join(keywords)
                    item = [('keyword', (hsh['k'], tstr2SCI[typ][0]), dt, hsh['_summary'], f), tuple(keywords), (u, typ,
                        setSummary(hsh, dt), sdt, etmdt)]
                    add2list(rows, item)
                if 't' in hsh and hsh['t'] != 'none':
                    for tag in hsh['t']:
                        # this_hsh = deepcopy(hsh)
                        item = [('tag', (tag, tstr2SCI[typ][0]), dt, hsh['_summary'], f), (tag,), (u, typ,
                            setSummary(hsh, dt), sdt, etmdt)]
                        add2list(rows, item)
            ...
            if hsh['itemtype'] == '$':
                cat = 'In Basket'
                item = [('next', (0, cat), tstr2SCI['ib'][0], 0, hsh['_summary'], f), (cat,),
                    (u, 'ib', hsh['_summary'], '')]
                add2list(rows, item)
                continue
            if hsh['itemtype'] == '?':
                cat = 'Someday maybe'
                item = [('next', (2, cat), tstr2SCI['so'][0], 0, hsh['_summary'], f), (cat,),
                    (u, 'so', hsh['_summary'], '')]
                add2list(rows, item)
                continue
            ...
            if dts == "none":
                ...
                item = [('next', (1, hsh['c'], hsh['_p'], exttd), tstr2SCI[typ][0], hsh['_p'], hsh['_summary'], f),
            ...
            if 'rrule' in hsh:
                ...
                thisdate = parse(parse_dtstr(hsh['s'], hsh['z'])).astimezone(tzlocal()).replace(tzinfo = None)
                dates.append(thisdate)
                add2Dates(datetimes, (thisdate, f))
            if 'f' in hsh and hsh['f']:
                # make sure finish dates are indexed
                thisdate = parse(parse_dtstr(hsh['f'][-1][0], hsh['z'])).astimezone(tzlocal()).replace(tzinfo = None)
                add2Dates(datetimes, (thisdate, f))
            if 'e' in hsh:
                try:
                    e = hsh['e']
                    minutes = e.days * 3600 + e.seconds//60
                except:
                    print("bad hsh['e']", hsh['e'], hsh['summary'],
                        hsh['entry'])
                    minutes = 0
            else:
                e = timedelta(minutes=0)
                minutes = 0
            ...
            for dt in dates:
                ...
                if 'e' in hsh:
                    ...
                    tmpl_hsh['e'] = ''
                    etl = dtl
                tmpl_hsh['time_span'] = setItemPeriod(hsh, dtl, etl, options = options)
                tmpl_hsh['busy_span'] = setItemPeriod(hsh, dtl, etl, True, options = options)
                    ...
                    for alert in hsh['_a']:
                        ...
                        for td in time_deltas:
                            ...
                            if adt.date() == today_date:
                                ...
                                this_hsh['alert_time'] = fmt_time(adt, True, options = options)
                ...
                if 'f' in hsh and 'rrule' not in hsh:
                    ...
                    if hsh['itemtype'] == '%':
                        ...
                        if 'prereqs' in hsh and hsh['prereqs']:
                            ...
                            cat = 'Available'
                            sn = (1, tstr2SCI[typ][0])
                    item = [('now', sn, dtl, hsh['_p'], summary, f), (cat,),
                ...
                if hsh['itemtype'] == '!':
                    typ = 'ns'
                    item = [('day', sd.strftime(sortdatefmt), tstr2SCI[typ][0], hsh['_p'], '', f),
                        (fmt_date(dt),),
                        (u, typ,
                            summary, '', etmdt)]
                    add2list(rows, item)
                ...
                if hsh['itemtype'] == '*':
                    ...
                    evnt_summary = "%s: %s" % (tmpl_hsh['busy_span'],tmpl_hsh['summary'])
                    if et != st:
                    ...
                    if ed > sd:
                        ...
                        add2list(rows, item)
                        add_busytime(u, sd, sm, day_end_minutes, evnt_summary, busytimes, busydays, f)
                        sd += oneday
                        i = 0
                        item_copy = []
                        while sd < ed:
                            ...
                            item_copy[i][2][3] = '%s ~ %s' % (options['daybegin_fmt'],
                                options['dayend_fmt'])
                            ...
                            add_busytime(u, sd, 0, day_end_minutes, evnt_summary, busytimes, busydays, f)
                            sd += oneday
                            i += 1
                        # the last day tuple
                        if em:
                            ...
                            item_copy[i][2] = tuple(item_copy[i][2])
                            add2list(rows, item_copy[i])
                            add_busytime(u, sd, 0, em, evnt_summary, busytimes, busydays, f)
                    else:
                        # single day event or reminder

                        if typ == 'rm' and 'r' in hsh and (
                            'BYHOUR' in hsh['r'] or 'BYMINUTE' in hsh['r']):
                            # make sure all the items for this day are the same so that only one will be displayed
                            st_fmt = '...'
                            et_fmt = ''
                            # put this reminder first
                            st = etmdt = parse('00:00')

                        item = [('day', sd.strftime(sortdatefmt) , tstr2SCI[typ][0], hsh['_p'], st.strftime(sorttimefmt), f),
                            (fmt_date(sd),),
                            (u, typ, summary,
                                '%s%s' % (
                                    st_fmt,
                        ...
                        add2list(rows, item)
                        add_busytime(u, sd, sm, em, evnt_summary, busytimes, busydays, f)
                        continue
                #--------------- other dated items ---------------#
                if hsh['itemtype'] in ['+', '-', '%']:
                    if 'e' in hsh:
...
def getAgenda(allrows, colors=2, days=4, indent=2, width1=54, width2=14, calendars=[], html=True):
    cal_regex = None
    if calendars:
    ...
    for row in rows:
        ...
        if row[0][0] == 'day':
            if row[0][1] >= beg_fmt and day_count <= days + 1:
                # process day rows until we get to days+1 so that all items from days are included
                if row[0][1] != last_day:
                    last_day = row[0][1]
                    day_count += 1
                if day_count <= days:
    ...
    for lst in [day, now, next]:
        if lst:
            tree = makeTree(lst)
            if html:
                output.extend(tree2Html(tree, indent=indent, width1=width1, width2=width2, colors=colors))
            else:
                output.extend(tree2Text(tree, width1=width1, width2=width2, colors=colors))
    return(output)
...
def updateCurrentFiles(allrows, file2uuids, uuid2hash, options):
    if options['current_textfile']:
        if 'current_opts' in options and options['current_opts']:
            txt = getReportData(options['current_opts'],
                file2uuids,
                uuid2hash,
                options,
                colors=0)
        else:
            txt = getAgenda(allrows,
                colors = 0,
        ...
        if txt and not txt[0]:
            txt.pop(0)
        fo = codecs.open(options['current_textfile'], 'w', encoding)
        fo.writelines("\n".join(txt))
        fo.close()
    if options['current_htmlfile']:
        if 'current_opts' in options and options['current_opts']:
            html = getReportData(options['current_opts'],
                file2uuids,
                uuid2hash,
                options)
        else:
            html = getAgenda(allrows,
        ...
        if not html[0]:
            html.pop(0)
        fo = codecs.open(options['current_htmlfile'], 'w', encoding)
        fo.writelines('<!DOCTYPE html> <html> <head> <meta charset="utf-8">            </head><body><pre>%s</pre></body>' % "\n".join(html))
        fo.close()
...
def hsh2ical(hsh):
    ...
    if 'z' in hsh:
        ...
        # pprint(tz.__dict__)
        # element.add('tzid', hsh['z'])
    else:
        tz = None
    if 's' in hsh:
        dt = hsh[u's']
        # print('type hsh[s]', type(hsh['s']), hsh['s'])
        # dz = parse(parse_dtstr(hsh['s'], hsh['z'])).replace(tzinfo = tz)
        # print('dt', type(dt), dt)
        dz = dt.replace(tzinfo=tz)
        tzinfo = dz.tzinfo
        tzname = tzinfo.tzname(dz)
        tzid = tzid_from_dt(dz)
        # print('tzinfo utcoffset', tzinfo.utcoffset(dz))
        # print('tzinfo dst', tzinfo.dst(dz))
        # print('tzinfo tzname', tzinfo.tzname(dz), tzid, tz)
        # print('dt', dt, 'dz', dz)
        dt = dz
        dd = dz.date()
    else:
    ...
    if u'_r' in hsh:
        ...
        for r in rlst:
            if r['f'] == 'l':
                if '+' not in hsh:
                    ...
                    continue
                # list only kludge: make it repeat daily for a count of 1 using the first element from @+ as the starting datetime

                dz = parse(parse_dtstr(hsh['+'].pop(0), hsh['z'])).replace(tzinfo = tzinfo)
                # print('dz', dz)
                dt = dz
                dd = dz.date()
        ...
        if '+' in hsh:
            element.add('rdate', hsh['+'])
        if '-' in hsh:
            element.add('exdate', hsh['-'])
    ...
    item_type = type2Str[hsh['itemtype']]

    if hsh['itemtype'] in ['-', '+', '%']:
        done, next, following = getDoneAndTwo(hsh)
        # print('TASK:', summary, done, next, following)
        if 's' in hsh:
            element.add('dtstart', dt)
        if done:
            # print('f', hsh['f'])
            # finz = parse(parse_dtstr(done, hsh['z']))
            finz = done.replace(tzinfo = tzinfo)
            fint = vDatetime(finz)
            # print('adding completed from done', finz)
            element.add('completed', fint)
        if next:
            # duez = parse(parse_dtstr(next, hsh['z']))
            duez = next.replace(tzinfo = tzinfo)
            duet = vDatetime(duez)
            dued = vDate(duez)
            # print('adding due from next', duez)
...
def export_ical_item(hsh, vcal_file):
    ...
    # tzc.add('x-lic-location', 'US/Eastern')
...
def export_ical(uuid2hash, vcal_file, calendars=None):
    ...
    # tzs = TimezoneStandard()
    # tzs.add('tzname', 'EST')
    # tzs.add('dtstart', datetime(1970, 11, 1, 3, 0, 0))
    # tzs.add('rrule', {'freq': 'yearly', 'bymonth': 11, 'byday': '1su'})
    # tzs.add('TZOFFSETFROM', timedelta(hours=-4))
    # tzs.add('TZOFFSETTO', timedelta(hours=-5))
    ...
    # tzd = TimezoneDaylight()
    # tzd.add('tzname', 'EDT')
    # tzd.add('dtstart', datetime(1970, 3, 8, 2, 0, 0))
    # tzd.add('rrule', {'freq': 'yearly', 'bymonth': 3, 'byday': '2su'})
    # tzd.add('TZOFFSETFROM', timedelta(hours=-5))
    # tzd.add('TZOFFSETTO', timedelta(hours=-4))
...
def import_ical(fname):
    ...
    for comp in cal.walk():
        ...
        summary = comp.get('summary')#.to_ical()
        clst = [t, summary]
        if start:
        ...
        if rrule:
            ...
            for key in keys:
                if key == 'FREQ':
                    ...
                    rlst.append("&%s %s" % (ical_rrule_hsh[key],
...
# def make_ical(rows, uuid2hash, vcal_file, calendars=None):
#     """
#         Return calendar as string if fname is ''.
#     """
#     ret_val = False
#     cal_regex = None
#     if calendars:
...
#         if cal_regex and not cal_regex.match(row[0][-1]):
#             continue
#         if row[0][0] not in ['day']:
#             # we will only export dated items
#             continue
#         date = row[0][1]
#         if row[0][4]:
#             time = row[0][4]
#         else:
#             time = '0000'
#         dt = parse("%sT%s" % (date, time))
#         dtl = dt.replace(tzinfo = tzlocal())
...
#         else:
#             continue
#         element.add('uid', uniqueId())
#         # use the setSummary version
#         element.add('summary', row[-1][2])
#         # element.add('summary', hsh['_summary'])
#         if 'q' in hsh:
#             element.add('priority', hsh['_p'])
#         if 'l' in hsh:
#             element.add('location', hsh['l'])
...
#         if item_type in ['pt', 'pd', 'pc', 'av', 'ds', 'cs', 'un', 'du', 'cu']: # dated task
#             element.add('due', dd)
#             allday =False
#         elif item_type in ['fn']: # completed task
#             element.add('completed', dtu)
#             allday =False
#             element.add('dtstart', dd)

#         if item_type in ['un', 'du', 'cu', 'nu']: # undated task
#             allday =False
#         elif item_type in ['ns']:
#             element.add('dtstart', dtu)
#         elif item_type in ['oc']: # allday
#             allday = True
#             element.add('dtstart', dd)
#         elif item_type in ['ac']: # action
#             if 'e' in hsh:
#                 element.add('comment', timedelta2Str(hsh['e']))
#             allday =False
#         elif item_type in ['ev', 'rm']:
#             allday = False
#             element.add('dtstart', dtu)
#             if 'e' in hsh and hsh['e']:
#                 etu = (dtu + hsh['e'])
#                 element.add('dtend', etu)
#         cal.add_component(element)
#         ret_val = True
#     (name, ext) = os.path.splitext(vcal_file)
#     pname = "%s.ics" % name
...
class ETMCmd(cmd.Cmd):
    ...
    """
    # global file2uuids, uuid2hash, options
    def __init__(self, rows=[], file2uuids={}, uuid2hash={}, options={}, completekey='tab'):
    ...
    intro = 'Welcome to the etm shell. Type help or ? to list commands.\n=========================================================='
    doc_header = "Enter 'help X' (or '? X') for details about command X\nDocumented etm commands:"
    misc_header = 'Miscellaneous etm commands'
    undoc_header = 'Undocumented etm commands'

    def preloop(self):
        self.prompt = "Enter 'q' to quit or 'help' (or '?') for usage information\netm: "

    def do_s(self, arg_str):
        # term_print(tr('Agenda'))
        self.mk_rep('s')
...
next {0} days together with any now and next items.
""".format(self.options['agenda_days']))

    def do_m(self, arg_str):
        f = self.options['report_specifications']
        if not f or not os.path.isfile(f):
            term_print("This option requires a valid report_specifications setting in etm.cfg.")
            return()
        with open(f, 'r') as fo:
...
from {0}:""".format(f))
            ...
            return()
        rep_spec = lines[n-1].strip()
        term_print('report using #{0}: "{1}":'.format(n, rep_spec))
...
Options include:
    ...
    def help_c(self):
        print("""\
Generate a custom report. Usage:
    c <groupby> [options]
where groupby can include semicolon separated date specifications
and elements from:
    c context
...
Options include:
    ...
    def help_help(self):
        print("Usage: X [options]    where X is either a or c.")
        print("Use 'help X' or '? X' for detailed help on command X.\n")

    def mk_rep(self, arg_str):
        if arg_str == 's':
            txt = getAgenda(self.rows,
...
def main(etmdir='', argv=[]):
    ...
    # qlcl = options['qlcl']
    # if use_locale:
    #     QLocale.setDefault(options['qlcl'])
    ...
    uuid2hash, file2uuids, file2lastmodified, bad_datafiles, messages = \
        get_data(options = options, use_pickle=True)
    if messages:
        for msg in messages:
            print(msg)
    rows, busytimes, busydays, alerts, dates, occasions = getViewData(bef, file2uuids, uuid2hash, options=options)

    for key in uuid2hash:
        print(key, uuid2hash[key], "\n")

    if len(argv) > 1:
        if argv[1] in ['a', 'c', 's', 'm', '?']:
            c = ETMCmd(rows, file2uuids, uuid2hash, options)
            c.onecmd(' '.join(argv[1:]))
        else:
            c = ETMCmd(rows, file2uuids, uuid2hash, options)
            c.cmdloop()
