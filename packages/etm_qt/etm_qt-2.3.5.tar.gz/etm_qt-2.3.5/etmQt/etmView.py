...
from etmQt.etmData import getToday, makeTree, tree2Html, expandTree,  leadingzero, get_data, getViewData, updateCurrentFiles, getReportData, tstr2SCI, oneday, reprdatefmt, fmt_date, fmt_dt, fmt_time, fmt_datetime, fmt_period, fmt_weekday, s2or3, datetime2minutes, getPrevNext, expand_template, oneminute, send_mail, send_text, calyear, str2opts, getAgenda, export_ical_item, import_ical, export_ical

from etmQt.etmData import get_changes, uniqueId, add2list, zonelist,  hsh2str, parse_dtstr, parse_datetime, etmdatefmt, timedelta2Str, mail_report

from etmQt.etmData import get_options, getMercurial, checkForNewerVersion, date_calculator
...
import locale, re, platform, csv
from dateutil import __version__ as dateutil_version

# load icons, make with:
# pyrcc4 -o etm_rc.py etm.qrc
from etmQt import etm_rc
...
from etmQt.ui_etmBrowser import Ui_Dialog as Browser_Dialog
class BrowserForm(QDialog, Browser_Dialog):
    def __init__(self, parent=None):
        super(BrowserForm, self).__init__(parent)
        self.setupUi(self)
        self.parent = weakref.ref(parent)
        self.setStyleSheet('font-size: %dpt' % self.parent().options['fontsize'])
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_W), self,
            self.close)


from etmQt.ui_etmCalendar import Ui_Dialog as Calendar_Dialog
class CalendarYearForm(QDialog, Calendar_Dialog):
    def __init__(self, parent=None):
        ...
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_P), self,
            self.printCalendar)
...
from etmQt.ui_etmHelp import Ui_Dialog as Help_Dialog
class HelpForm(QDialog, Help_Dialog):
    def __init__(self, parent=None, page=0):
        ...
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_O), self,
            self.parent().edit_config)

        self.connect(self.closeButton, SIGNAL("clicked()"), self.close)
        self.connect(self.search, SIGNAL("returnPressed()"),
            self.doFind);

        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_F), self,
            self.openSearch)

        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_G), self,
            self.doFind)

        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_W), self,
            self.close)

        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL + Qt.Key_G), self,
            self.doFindBackwards)
    ...
    def doFind(self, backwards=False):
        ...
        if r:
            ...
            self.showStatusMessage("{0} {1}".format(self.tr("not found"), where))
...
class PHL(QSyntaxHighlighter):
    ...
    UUIDREGEX = r"@i\s+[a-z0-9]{8}\-[a-z0-9]{4}\-[a-z0-9]{4}\-[a-z0-9]{4}\-[a-z0-9]{12}\b"
    ...
    def __init__(self, parent=None):
        ...
        for key in PHL.TYPEKEYS:
            ...
            PHL.Rules.append((QRegExp(pattern),
                PHL.typeColor[key][0]))

        PHL.Rules.append((QRegExp(
                "|".join([r"\s&{0}\s".format(repeat_key)
                for repeat_key in PHL.REPEATKEYS])),
                "repeat_key"))

        PHL.Rules.append((QRegExp(
                "|".join([r"\s@{0}\s".format(at_key)
                for at_key in PHL.ATREPEATKEYS])),
                "repeat_key"))

        PHL.Rules.append((QRegExp(
                "|".join([r"\s&{0}\s".format(job_key)
                for job_key in PHL.JOBKEYS])),
                "job_key"))

        PHL.Rules.append((QRegExp(
                "|".join([r"\s@{0}\s".format(at_key)
                for at_key in PHL.ATJOBKEYS])),
                "job_key"))

        PHL.Rules.append((QRegExp(
                "|".join([r"\s@{0}\s".format(at_key)
                for at_key in PHL.ATKEYS])),
                "at_key"))

        PHL.Rules.append((QRegExp(r"\s@{0}\s[^@&]*".format(PHL.ATERROR)),
                "error"))

        PHL.Rules.append((QRegExp(r"\s&{0}\s[^@&]*\b".format(PHL.AMPERROR)),
                "error"))

        PHL.Rules.append((QRegExp(PHL.UUIDREGEX),
                "uuid"))

        PHL.Rules.append((QRegExp(r"#.*"),
                "comment"))
    ...
    def highlightBlock(self, text):
        ...
        if self.normal_format:
            self.setFormat(0, textLength,
                       PHL.Formats[self.normal_format])
        else:
            self.setFormat(0, textLength,
                       PHL.Formats["normal"])
...
class CompletionTextEdit(QTextEdit):
    def __init__(self, parent=None):
        ...
        # self.setPlainText(QString(STARTTEXT))
    ...
    def insertCompletion(self, completion):
        tc = self.textCursor()
        extra = (self.length(completion) -
            self.length(self.completer.completionPrefix()))
        # if self.completer.completionPrefix().right(1) == ' ':
...
from etmQt.ui_etmList import Ui_Dialog as List_Dialog
class ListForm(QDialog, List_Dialog):
    def __init__(self, parent=None, title = '', items = [], tooltip = None):
...
from etmQt.ui_etmWhich import Ui_whichDialog as Which_Dialog
    ...
    def __init__(self, parent, mode, instance):
        ...
        if mode == 'file':
            ...
            if suggested_file:
                ...
                currfile = self.parent().ensureMonthly()
            itemList = []
            self.fileList = []
            current = 0
            ...
            for i in range(len(choices)):
                ...
                self.fileList.append(os.path.join(self.parent().options['datadir'], recent_edits[i][0]))
        ...
        self.connect(self.comboBox, SIGNAL('currentIndexChanged(QString)'), self.setChoice)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_L), self, self.showList)


    def setChoice(self, txt):
        if self.mode == 'file':
            self.parent().which_file = os.path.join(self.datadir, str(self.comboBox.currentText()))
...
from etmQt.ui_etmDetails import Ui_detailDialog as Details_Dialog
class DetailForm(QDialog, Details_Dialog):
    def __init__(self, parent=None, receiver=None):
        ...
        if self.parent().timer_status == 'stopped':
            self.cloneActionButton.setToolTip(self.tr("Click here or press Ctrl-T to start the timer\nfor an action based on this item."))
        else:
        ...
        self.connect(self.historyButton, SIGNAL("clicked()"), self.show_history)

        self.connect(self.repsButton, SIGNAL("clicked()"), self.show_repetitions)

        self.connect(self.itemButton, SIGNAL("clicked()"), self.edit_item)

        self.connect(self.fileButton, SIGNAL("clicked()"), self.edit_file)

        self.connect(self.finishButton, SIGNAL("clicked()"), self.finish)

        self.connect(self.cloneItemButton, SIGNAL("clicked()"),
            self.clone_item)

        self.connect(self.cloneActionButton, SIGNAL("clicked()"),
            self.clone_action)

        self.connect(self.minusButton, SIGNAL("clicked()"), self.remove)

        self.connect(self.moveButton, SIGNAL("clicked()"), self.move)

        self.connect(self.helpButton, SIGNAL("clicked()"), self.detailsHelp)
        ...
        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL + Qt.Key_H), self, self.show_history)
    ...
    def exportIcalItem(self):
        ...
        if num == 1:
            ...
            msgBox.setInformativeText(self.tr("Save this item to the file\n\n{0}?".format(self.options['icsitem_file'])))
    ...
    def setHash(self, hsh):
        ...
        self.finishButton.setEnabled(
            hsh['itemtype'] in [u'-', u'+', u'%'] and (u'_r' in hsh or u'f' not in hsh))
        # why not enable deleting hidden items?
        # self.minusButton.setEnabled(hsh['itemtype'] != '#')
        ...
        if '_r' in hsh:
            if hsh['_r'][0]['f'] != 'l':
                ...
                self.repeating = False
        # self.repeating = ('_r' in hsh)
        self.hsh = hsh

        if self.repeating:
            self.minusButton.setToolTip(self.tr("Click here or press Ctrl-D to delete one or more repetitions of this item."))
            self.minusButton.setIcon(QIcon(":/date_delete.png"))

            if self.grouptask:
                self.finishButton.setToolTip(self.tr("Click here or press Ctrl-F to set the completion datetime for this job."))
            else:
                self.finishButton.setToolTip(self.tr("Click here or press Ctrl-F when a task is selected\nto update the latest completion datetime for the task."))
        else:
            self.minusButton.setIcon(QIcon(":/item_delete.png"))

            if hsh['itemtype'] == u'~':
                self.minusButton.setToolTip(self.tr("Click here or press Ctrl-D to delete this action"))
            else:
                self.minusButton.setToolTip(self.tr("Click here or press Ctrl-D to delete this item."))
            if self.grouptask:
                self.finishButton.setToolTip(self.tr("Click here or press Ctrl-F to set the completion datetime for this job."))
            else:
                self.finishButton.setToolTip(self.tr("Click here or press Ctrl-F when a task is selected\nto set the finish datetime for this item"))
        self.repsButton.setEnabled(self.repeating)
        self.hsh['_filename'] = os.path.join(self.parent().options['datadir'], hsh['fileinfo'][0])
    ...
    def processCommand(self, command):
        ...
        if process.waitForStarted():
            ...
            if error:
                QMessageBox.warning(self, "etm error",
                    "{0}".format(str(error)))
                return((0, error))
            else:
                if python_version2:
                    ...
                    return((1, str(result, encoding=encoding)))
    ...
    def show_history(self):
        self.emit(SIGNAL("history"), self.editor)
        command = self.parent().options['hg_history'].format(repo=self.parent().options['datadir'], file=self.hsh['_filename'], rev="{rev}", desc="{desc}")

        res, qstr = self.processCommand(command)
        lines = qstr.split('\n')
        self.form = ListForm(self.parent(), title = "{0}: {1}".format(self.tr('change history'), self.hsh['fileinfo'][0]),
            items = lines)
        font = QFont(self.font())
        font.setPointSize(font.pointSize()-1)
        self.form.setFont(font)
        self.form.resize(580, 240)
        QObject.connect(self.form.infoButton, SIGNAL("clicked()"),
            self.showDiffInfo)
        self.form.show()
...
""")
    ...
    def show_repetitions(self):
        ...
        if gotall:
            ...
            self.txt = """\
Repetitions that fall on or after the item's starting date, {0}, and before {1} plus the first repetition, if any, after this interval.
        """.format(fmt_date(self.hsh['s'], True), fmt_date(self.bef, True))
        self.form = ListForm(self.parent(), title = '{0}: {1}   ({2} {3})'.format(self.tr('repetitions'), self.hsh['_summary'], self.tr("times are"), self.hsh['z']),
    ...
    def showInterval(self):
        mbox = QMessageBox(self)
        mbox.setText(self.tr('repetitions for {0}'.format(self.hsh['_summary'])))
        mbox.setInformativeText(self.txt)
        mbox.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        mbox.show()

    def help(self):
        self.emit(SIGNAL("help"), self.editor)
        print('help')

    def finish(self):
        self.emit(SIGNAL("finish"), self.editor)
        ...
        if next:
            ...
            ddn = next.replace(tzinfo=tzlocal()).astimezone(gettz(self.hsh['z'])).replace(tzinfo=None)
        ...
        if self.hsh['itemtype'] == u'+':
            ...
            if m:
                ...
                self.hsh['_j'][int(num)-1]['f'] = [(dt.replace(tzinfo=None), ddn)]
                finished = True
            ...
            if not dt:
                return()
            dtz = dt.replace(tzinfo=tzlocal()).astimezone(gettz(self.hsh['z'])).replace(tzinfo = None)
            if not ddn:
                ddn = dtz
    ...
    def getDate(self, prompt):
        ...
        dt = None
        while ok and not dt:
            fdate, ok = QInputDialog.getText(self,
                self.tr("datetime"), "%s:" % prompt,
                text = txt)
            if ok:
                try:
                    dt = parse(parse_datetime(str(fdate)), dayfirst=self.options['dayfirst'], yearfirst=self.options['yearfirst']).replace(tzinfo = None)
    ...
    def move(self):
        self.emit(SIGNAL("move"), self.editor)
        form = getEditForm(self.parent(), instance='file', hsh = self.hsh, receiver = self.receiver)
        if form:
            newfile = form.getFileName()
            if not newfile:
                return(False)
            ...
            added = form.addLinesToFile(filename = newfile,
                newlines = newlines, senddone = False)
            if not added:
                # warn message
                QMessageBox.warning(self, "etm",
                        self.tr("Adding item to {1} failed - aborted removing item from {2}: {3}".format(newfile, oldfile, e)))
                form.close()
                return(False)
            removed = form.removeLinesFromFile(filename = oldfile, begline = begline, endline = endline, senddone=True)
            if removed:
                self.close()
            form.close()

    def doRemove(self, ret):
        form = getEditForm(self.parent(), instance='file',
        ...
        if form:
            ...
            if self.rev_str:
                ...
                contents = "# %s"  % hsh2str(self.hsh, self.parent().options)
    ...
    def edit_file(self):
        form = getEditForm(self.parent(), instance = 'edit',
            filename = self.hsh['_filename'],
            hsh = self.hsh,
            receiver = self.receiver)
        if form:
    ...
    def doIt(self, mode):
        ...
        if ret == 1:
            ...
            # proceed as in ret = 4
            datestr, doit = QInputDialog.getText(self,
                self.tr("datetime"), "%s:" % (
                    self.tr("new date and time to replace\n%s" % self.hsh['_dt'])))
            if doit:
                new_dt = parse(str(datestr)).replace(tzinfo=tzlocal()).astimezone(gettz(self.hsh['z']))
                if new_dt:
                    old_dt = parse(self.hsh['_dt']).replace(tzinfo=tzlocal()).astimezone(gettz(self.hsh['z']))
                    self.hsh.setdefault('+', []).append(
                        new_dt.strftime(sfmt))
                    self.hsh.setdefault('-', []).append(
                        old_dt.strftime(sfmt))
                    self.rev_str = edit_str = hsh2str(self.hsh, self.parent().options)
            ...
            # make a copy without repetion entries and with this date as hsh['s']
            ...
            dt = parse(self.hsh['_dt']).replace(tzinfo=tzlocal()).astimezone(gettz(self.hsh['z']))
        ...
        if mode == 'remove':
            if self.hsh['_which'] == 4:
                filename = self.hsh['_filename']
                fn, bl, el = self.hsh['fileinfo']
                form = getEditForm(self.parent(), instance= 'remove', receiver = self.receiver)
                if form:
                    form.removeLinesFromFile(filename, bl, el)
                    form.close()
                    self.close()
            ...
            filename = self.hsh['_filename']
            fn, bl, el = self.hsh['fileinfo']
            form = getEditForm(self.parent(), instance='file', receiver = self.receiver)
            if form:
                form.replaceLinesInFile(filename, bl, el, lines)
                form.close()
                self.close()
            ...
            form = getEditForm(self.parent(), instance='edit', hsh = edit_hsh,
                receiver = self.receiver)
            if form:
    ...
    def remove(self):
        if not self.repeating:
            ...
            msgBox.setInformativeText(self.tr("Do you really want to delete this item?\n\nThis action cannot be undone."))
            msgBox.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
            msgBox.setDefaultButton(QMessageBox.Ok)
            # must use exec_ here
            reply = msgBox.exec_()
    ...
    def clone_item(self):
        ...
        for key in [u'i']:
            if key in hsh_cpy:
                del hsh_cpy[key]
        form = getEditForm(self.parent(), instance='edit', hsh = hsh_cpy, receiver = self.receiver)
        if form:
            form.setWindowTitle("{0}: {1}".format(self.tr("copy"), self.hsh['_filetext']))
            form.rev_hsh = {}
            form.rev_str = ''
            form.filename = None
            form.hsh = None
            form.suggested_file = hsh_cpy['_filename']
            # do this in editform
    ...
    def clone_action(self):
        ...
        if self.hsh['itemtype'] == u'~':
            if self.hsh['s'].date() == datetime.now().date():
                ...
                msgBox.setInformativeText("Restart the timer for the existing action (yes) or create a new timer (no)?")
            ...
            for key in [u'r', u'rrule', u'_r', u'p', u'q', u'+', u'-' ]:
                if key in hsh_cpy:
                    del hsh_cpy[key]
            self.parent().timer_subject = hsh2str(hsh_cpy, self.parent().options)
        self.parent().timer_hsh = hsh_cpy
    ...
    def close(self):
        self.emit(SIGNAL("close"), self.editor)
        self.reject()
...
def getEditForm(parent, instance=None, hsh=None, filename=None, receiver=None, highlight=True, use_completer=True, options = {}):
    msg_hsh = {
    ...
    # 'file' will be set for finish, move, remove when the form will not be opened
    ...
    if instance in msg_hsh.keys() and instance in parent.editor_instances:
        ...
        return(None)
    elif instance in ['file', 'edit', 'recent'] and ('file' in parent.editor_instances or 'edit' in parent.editor_instances or 'recent' in parent.editor_instances):
        if instance == 'file':
            if 'file' in parent.editor_instances:
                msg = tr('Another file operation is in process and must be completed before this can be done.')
            else: # edit
                msg = tr('The editor must be closed before this can be done.')
        else: # edit
            if 'file' in parent.editor_instances:
                msg = tr('A file operation is in process and must be completed before the editor can be opened.')
            else: # edit
                msg = tr('Only once instance of the editor can be open at one time.')
        QMessageBox.warning(parent, "etm", msg)
        return(None)
    else:
        form = EditForm(parent=parent,
            instance = instance,
            filename = filename,
            hsh = hsh,
            receiver = receiver,
        ...
        form.resize(parent.options['window_width'], parent.options['window_height']-30)
        return(form)


from etmQt.ui_etmEditor import Ui_editDialog as Edit_Dialog
class EditForm(QMainWindow, Edit_Dialog):
    def __init__(self, parent=None, instance = None, filename = None, hsh = None,
        ...
        self.connect(self.helpButton, SIGNAL("clicked()"), self.editHelp)
        self.connect(self.search, SIGNAL("returnPressed()"),
            self.doFind);

        QShortcut(QKeySequence(Qt.Key_F1), self, self.editHelp)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_I), self,
            self.insertScratchPad)
        multiple_whitespace_regex = re.compile(r' {2,}|\t+')

        if True:
            self.editor = CompletionTextEdit(self.parent())
            if ('auto_completions' in self.parent().options and self.parent().options['auto_completions'] and os.path.isfile(self.parent().options['auto_completions'])):
                try:
                    words = []
                ...
                words = []
                self.showStatusMessage(self.tr("'auto_completions' not found or empty"))
        ...
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_W), self,
            self.close)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_Return), self,
            self.fileSaveAndClose)


        QObject.connect(self.copyButton, SIGNAL("clicked()"),
            self.editor.copy)
        QObject.connect(self.cutButton, SIGNAL("clicked()"),
            self.editor.cut)
        QObject.connect(self.pasteButton, SIGNAL("clicked()"),
            self.editor.paste)
        ...
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_S), self,
            self.fileSave)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_R), self,
            self.reloadFile)
        ...
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_Z), self,
            self.updateUi)
        ...
        if self.filename is not None:
            ...
            if self.linenum:
                ...
                cursor.movePosition(QTextCursor.Down, QTextCursor.MoveAnchor, self.linenum - 1)
    ...
    def insertScratchPad(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "etm -- read Error",
    ...
    def okToContinue(self):
        if self.editor.document().isModified():
            ...
            msgBox.setInformativeText(self.tr("Do you want to verify and save your changes?"))
    ...
    def writeLinesToFile(self, filename, lines, senddone=True):
        ...
        try:
            ...
            ret = [self.filename]
            tup = (relpath(filename, self.parent().options['datadir']), self.now.strftime(self.parent().options['etmdatetimefmt']))
            if tup not in self.recent_edits:
                self.recent_edits.insert(0, tup)
                del self.recent_edits[10:]
                # print('writeLinesToFile')
                # pprint(self.recent_edits)
        except (IOError, OSError) as e:
            QMessageBox.warning(self, "etm -- Save Error",
        ...
        if 'hg_commit' in self.parent().options:
            command = self.parent().options['hg_commit'].format(repo=self.parent().options['datadir'], mesg=self.mode)
            process = QProcess()
            process.startDetached(command)
        self.showStatusMessage(self.tr("changes saved"))
        return True

    def replaceLinesInFile(self, filename=None, begline=None, endline=None, newlines=None, senddone=True):
    ...
    def removeLinesFromFile(self, filename=None, begline=None, endline=None, senddone=True):
    ...
    def loadFile(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "etm -- Load Error",
        ...
        if self.filename is None:
            ...
            if 's' in new_hsh and new_hsh['itemtype'] in ['*']:
                ...
                if 0 <= h <= 6 and not (h==0 and m==0):
                    ...
                    msgBox.setInformativeText("Do you really want to schedule this for %s?" % fmt_time(new_hsh['s'], options=self.parent().options))
            ...
            fn, begline, endline = self.hsh['fileinfo']
            filename = self.hsh['_filename']
            # editlines = map(unicode, self.new_str.split('\n'))
            ...
            if begline >= 0:
                ...
                if rev_str:
                    ...
                    revlines = []
                return self.replaceLinesInFile(filename, begline, endline,
                    revlines + newlines)
            else:
    ...
    def ensureMonthly(self):
        retval = None
        if 'monthly' in self.parent().options and self.parent().options['monthly']:
            monthly = os.path.join(self.parent().options['datadir'], self.parent().options['monthly'])
            ...
            if not os.path.isfile(currfile):
                fo = codecs.open(currfile, 'w', encoding)
                fo.write("")
                fo.close()
    ...
    def fileSaveAs(self):
        ...
        fn, begline, endline = self.hsh['fileinfo']
        filename = self.hsh['_filename']
        # editlines = map(str, self.new_str.split('\n'))
        editlines = [unicode(x) for x in self.new_str.split('\n')]
        newlines = editlines
        return self.addLinesToFile(filename, newlines)
...
class TreeFilter(QSortFilterProxyModel):
    ...
    def filterAcceptsRowItself(self, sourceRow, sourceParent):
        return(super(TreeFilter,self).filterAcceptsRow(sourceRow,sourceParent))
...
from etmQt.ui_etmReport import Ui_reportDialog as Report_Dialog
class ReportForm(QDialog, Report_Dialog):
    def __init__(self, parent=None, colors=2):
        ...
        self.resize(520, 340)
        reports = []
        if ('report_specifications' in self.parent().options and self.parent().options['report_specifications'] and os.path.isfile(self.parent().options['report_specifications'])):
            try:
                fo = codecs.open(self.parent().options['report_specifications'], 'r', encoding)
        ...
        # connect signals
        self.reportBox.lineEdit().textEdited[unicode].connect(self.pFilterModel.setFilterFixedString)
        self.completer.activated.connect(self.on_completer_activated)

        self.connect(self.reportBox, SIGNAL("currentIndexChanged(int)"),
            self.indexChanged);

        self.connect(self.reportBox, SIGNAL("editTextChanged(QString)"),
            self.textChanged);

        self.connect(self.reportBox, SIGNAL("activated(QString)"),
            self.activated);
        ...
        self.connect(self.closeButton, SIGNAL("clicked()"),
            self.close)
        self.connect(self.printButton, SIGNAL("clicked()"),
            self.printReport)
        self.connect(self.exportButton, SIGNAL("clicked()"),
            self.exportReport)
        self.connect(self.reportButton, SIGNAL("clicked()"),
            self.createReport)
        self.connect(self.saveButton, SIGNAL("clicked()"),
            self.saveReports)
        self.connect(self.minusButton, SIGNAL("clicked()"),
            self.reportRemove)
        self.connect(self.mailButton, SIGNAL("clicked()"),
            self.mailReport)
        ...
        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL + Qt.Key_R), self, self.parent().editReportSpecs)
        ...
        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL + Qt.Key_G), self,
            self.doFindBackwards)
        # QShortcut(QKeySequence(Qt.Key_Return), self.search,
        #     self.find)
        QShortcut(QKeySequence(Qt.Key_Escape), self,
            self.closeSearch)
    ...
    def createReport(self):
        ...
        self.html = getReportData(opstr, self.parent().file2uuids,
    ...
    def exportReport(self):
        opstr = self.reportBox.lineEdit().text()
        rows = getReportData(opstr, self.parent().file2uuids,
            self.parent().uuid2hash, self.parent().options, True)
        filename, pat = QFileDialog.getSaveFileNameAndFilter(self, 'Save File', '.', filter = self.tr("CSV files (*.csv)"), options = QFileDialog.DontUseNativeDialog)
        if filename:
    ...
    def textChanged(self, text):
        if self.reportBox.lineEdit().isModified():
            self.reportBox.lineEdit().setStyleSheet(QString("background: lightgoldenrodyellow; font-size: 14pt; font-family: Courier New"))
            self.reportBox.setToolTip(
                self.tr('Press return to make modification\ntemporarily available.'))
        else:
            self.reportBox.lineEdit().setStyleSheet(QString("background: transparent; font-size: 14pt; font-family: Courier New"))
            self.reportBox.setToolTip(
                self.tr('Press Ctrl-R to\ncreate report.'))
    ...
    def okToContinue(self):
        if self.modified:
            ...
            msgBox.setInformativeText(self.tr("Do you want to save the changes?"))
            msgBox.setStandardButtons(QMessageBox.Save | QMessageBox.Discard | QMessageBox.Cancel)
            msgBox.setDefaultButton(QMessageBox.Save)
            reply = msgBox.exec_()
            # reply = msgBox.show()
    ...
    def saveReports(self):
        res = False
        if self.modified:
            items = [str(self.reportBox.itemText(i)) for i in range(self.reportBox.count())]
            try:
                fo = codecs.open(self.parent().options['report_specifications'], 'w', encoding)
                for item in items:
                    fo.write("%s\n" % item)
                self.saveButton.setEnabled(False)
                self.modified = False
    ...
    def reportRemove(self):
        ...
        if text == curr_text:
            # only remove the item is the displayed value is the same as the stored value
    ...
    def mailReport(self):
        to, ok = QInputDialog.getText(self,
            self.tr("send email"),
...
from etmQt.ui_etmCalendars import Ui_Dialog as Calendars_Dialog
...
class WeekView(QWidget):
    ...
    def __init__(self, parent=None, uuid2hash=None):
        ...
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_B), self,
            self.showBusyTimes)
    ...
    def getDetails(self, event):
        ...
        numerator = int(round(.5 + (7 * (x - WeekView.XMARGIN - self.indent_width)/span)))
        day = max(1, min(numerator, 7)) - 1
        self.dayhourmin = '%s:%s:%s' % (day, hours, mins)
        id = ''
        summary = ''
        occasions = "\n".join([x[1] for x in self.occasion_lst[day]])
        for (s, e, i, summary, f, mtch) in self.busy_lst[day]:
            ...
            # id = ''
        self.day = day
        self.time = time = "%s:%02d" % (hours, mins)
        self.uuid = id
    ...
    def mouseDoubleClickEvent(self, event):
        if event.button() != Qt.LeftButton:
            ...
            self.emit(SIGNAL("doubleclicked(QString, QString)"), self.dayhourmin, self.uuid)

    def keyPressEvent(self, event):
        QWidget.keyPressEvent(self, event)

    def setWeekdays(self, day_lst, busy_lst, occasion_lst, today_col = 0, curr_minutes = 0):
    ...
    def paintEvent(self, event=None):
        ...
        segColor = QColor(Qt.blue).dark(120)
        segLineColor = segColor.dark()
        lineColor = QColor(Qt.gray).dark(90)
        ...
        for i in range(1, num_hours+1):
            y += segHeight
            painter.setPen(lineColor)
            painter.drawLine(WeekView.XMARGIN + self.indent_width, y,
        ...
        for i in range(7):
            ...
            if self.today_col and self.today_col - 1 == i:
                ...
                painter.fillRect(x+i*segWidth, start_y,
            ...
            for tup in self.busy_lst[i]:
                ...
                end_y =   ((t2 - t1)*segHeight)/60.0
                painter.fillRect(x+i*segWidth, start_y,
                         segWidth, end_y, busyColor)

    def showBusyTimes(self):
        lines = ["<pre>%s %s" % (self.tr("Scheduled times for"), str(self.parent().busyLabel.text()))]
        ampm = False
        s1 = s2 = ''
        if 'ampm' in self.parent().options and self.parent().options['ampm']:
            ampm = True
...
from etmQt.ui_etmView import Ui_uiMainWindow as MainWindow_Dialog
class UiWindow(QMainWindow, MainWindow_Dialog):
    def __init__(self, parent=None, options={}):
        ...
        if 'calendars' in options and options['calendars']:
            ...
            default_pattern = r'^%s' % '|'.join([x[2] for x in self.default_calendars if x[1]])
            self.default_regex = re.compile(default_pattern)
            self.cal_pattern = r'^%s' % '|'.join([x[2] for x in self.calendars if x[1]])
        ...
        self.connect(self.uiView, SIGNAL("activated(int)"),
            self.viewChanged);
        ...
        QObject.connect(self.uiFilter, SIGNAL("textChanged(QString)"), self.filter)
        ...
        QObject.connect(self.newActionButton, SIGNAL("clicked()"),
            self.timer_toggle)
        QObject.connect(self.pause_restartButton, SIGNAL("clicked()"),
            self.timer_toggle)
        QObject.connect(self.stopButton, SIGNAL("clicked()"),
            self.timer_finish)

        QObject.connect(self.reportButton, SIGNAL("clicked()"),
            self.createReport)

        QObject.connect(self.prevWeekButton, SIGNAL("clicked()"),
            self.prevPeriod)

        QObject.connect(self.nextWeekButton, SIGNAL("clicked()"),
            self.nextPeriod)

        QObject.connect(self.alertsButton, SIGNAL("clicked()"),
            self.showAlerts)

        QObject.connect(self.todayButton, SIGNAL("clicked()"),
            self.viewChanged)

        QObject.connect(self.errorButton, SIGNAL("clicked()"),
            self.showMessages)

        QObject.connect(self.newItemButton, SIGNAL("clicked()"),
            self.createItem)

        QObject.connect(self.mainHelpButton, SIGNAL("clicked()"),
            self.mainHelp)
        ...
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_F), self,
            self.filterFocus)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_N), self,
            self.createItem)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_T), self,
            self.timer_toggle)
        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL +  Qt.Key_T), self,
            self.timer_finish)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_R), self,
            self.createReport)
        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL + Qt.Key_O), self,
            self.edit_config)
        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL + Qt.Key_C), self,
            self.edit_completions)
        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL + Qt.Key_R), self,
            self.editReportSpecs)
        QShortcut(QKeySequence(Qt.SHIFT + Qt.CTRL + Qt.Key_H), self,
            self.edit_recent)

        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_C), self,
            self.selectCalendars)

        QShortcut(QKeySequence(Qt.Key_Comma), self,
            self.dayView)
        QShortcut(QKeySequence(Qt.Key_Period), self,
            self.weekView)
        QShortcut(QKeySequence(Qt.Key_Slash), self,
            self.monthView)
        QShortcut(QKeySequence(Qt.Key_Semicolon), self,
            self.nowView)
        QShortcut(QKeySequence(Qt.Key_Apostrophe), self,
            self.nextView)
        QShortcut(QKeySequence(Qt.Key_BracketLeft), self,
            self.folderView)
        QShortcut(QKeySequence(Qt.Key_BracketRight), self,
            self.keywordView)
        QShortcut(QKeySequence(Qt.Key_Backslash), self,
            self.tagView)


        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_A), self,
            self.showAlerts)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_J), self,
            self.jumpToDate)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_P), self,
            self.openScratchPad)
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_S), self,
        ...
        QObject.connect(self.dayTree,
            SIGNAL("doubleClicked(QModelIndex)"), self.clickedSlot)
        QObject.connect(self.nowTree,
            SIGNAL("doubleClicked(QModelIndex)"), self.clickedSlot)
        QObject.connect(self.nextTree,
            SIGNAL("doubleClicked(QModelIndex)"), self.clickedSlot)
        QObject.connect(self.folderTree,
            SIGNAL("doubleClicked(QModelIndex)"), self.clickedSlot)
        QObject.connect(self.keywordTree,
            SIGNAL("doubleClicked(QModelIndex)"), self.clickedSlot)
        QObject.connect(self.tagTree,
            SIGNAL("doubleClicked(QModelIndex)"), self.clickedSlot)
        QObject.connect(self.dayTree, SIGNAL("activated(QModelIndex)"),
                    self.activated)
        QObject.connect(self.tagTree, SIGNAL("activated(QModelIndex)"),
                    self.activated)
    ...
    # def loadFileData(self, new=[], modified=[], deleted=[]):
    def loadFileData(self, dirty=False):
        self.uuid2hash, self.file2uuids, self.file2lastmodified, self.bad_datafiles, self.messages = get_data(self.options,dirty)
        return True
    ...
    def loadViewData(self):
        ...
        (self.all_rows, self.all_busytimes, self.all_busydays,
            self.all_alerts, self.all_dates, self.occasions) = getViewData(bef,
            self.file2uuids, self.uuid2hash, options=self.options)
        updateCurrentFiles(self.all_rows, self.file2uuids, self.uuid2hash, self.options)
        self.all_alerts = [x for x in self.all_alerts
            if x[0] >= curr_minutes]
    ...
    def showMessages(self, force=True):
        if self.messages:
            ...
            html = self.tr("No error messages were generated when data files were last loaded.")
    ...
    def applyCalendarsFilter(self):
        if self.all_rows:
            ...
            if self.alerts:
                self.alertsButton.setToolTip(self.tr("Click here or press <em>Ctrl-A</em> to see the remaining alerts for today."))
    ...
    def refreshViews(self):
        ...
        if self.next_tree and root_key in self.next_tree:
            ...
            self.addItems('next', self.next_model, self.next_tree[root_key], self.next_tree)
        ...
        if self.now_tree and root_key in self.now_tree:
            ...
            self.index2uuid['now'] = {}
            self.index2item['now'] = {}
            self.addItems('now', self.now_model, self.now_tree[root_key], self.now_tree)
        ...
        if self.day_tree and root_key in self.day_tree:
            ...
            self.index2uuid['day'] = {}
            self.index2item['day'] = {}
            self.addItems('day', self.day_model, self.day_tree[root_key], self.day_tree)
        ...
        if self.folder_tree and root_key in self.folder_tree:
            ...
            self.addItems('folder', self.folder_model, self.folder_tree[root_key], self.folder_tree)
        ...
        if self.keyword_tree and root_key in self.keyword_tree:
            ...
            self.addItems('keyword', self.keyword_model, self.keyword_tree[root_key], self.keyword_tree)
        ...
        if self.tag_tree and root_key in self.tag_tree:
            ...
            self.index2uuid['tag'] = {}
            self.index2item['tag'] = {}
            self.addItems('tag', self.tag_model, self.tag_tree[root_key], self.tag_tree)
        ...
        # we need these expanded to resize the 2nd column and let the first take the remaining space
        ...
        # resize the last column (2) and let the first column (1) take the remaining space when the window is resized. Too cool!
        for tree in [self.nowTree, self.nextTree, self.dayTree,
            self.folderTree, self.keywordTree, self.tagTree]:
    ...
    def edit_recent(self):
        if not self.recent_edits:
            QMessageBox.warning(self, "etm", self.tr("No files have been changed in this session."))
            return()

        indx = 0
        which = WhichForm(self, 'recent', '')
        if which.exec_():
            form = getEditForm(self, instance = 'recent',
                filename = self.which_file,
                receiver = self.itemChanged)
            if form:
                form.setWindowTitle("{0}".format(relpath(self.which_file, self.options['datadir'])))
                form.show()
                form.helpPage = 1
                form.show()

    def edit_config(self):
        form = getEditForm(self, instance='config',
    ...
    def edit_completions(self):
        form = getEditForm(self, instance='complete',
    ...
    def editReportSpecs(self):
        form = getEditForm(self, instance='report',
                filename = self.options['report_specifications'],
                receiver = self.configChanged,
                highlight = False,
                use_completer = False)
        if form:
            form.setWindowTitle("{0}".format(self.options['report_specifications']))
            form.helpPage = 3
            form.show()
        ...
        if ok:
            ...
            for calendar in self.calendars:
                if calendar[1]:
                    ...
                    break
            if selections:
                self.cal_pattern = r'^%s' % '|'.join([x[2]
                    for x in self.calendars if x[1]])
            else:
                self.cal_pattern = ''
    ...
    def scrollToDate(self, curr_day = None):
        ...
        self.dayTree.scrollTo(self.dayTree.currentIndex(), hint=QAbstractItemView.PositionAtTop)
    ...
    def setWeek(self, curr_day = datetime.today()):
                ...
                for item in self.occasions[isokey]:
                    ...
                    if matching:
                        if not self.cal_regex.match(item[-1]):
                            continue
                        mtch = (self.default_regex.match(it[-1])
                                    is not None)
                    else:
                        mtch = True
                    it.append(mtch)
        ...
        mend = weekend.month
        if mbeg == mend:
            header = "{0} - {1}".format(fmt_dt(weekbeg,'%b %d'), fmt_dt(weekend, '%d, %Y'))
        elif ybeg == yend:
            header = "{0} - {1}".format(fmt_dt(weekbeg, '%b %d'), fmt_dt(weekend, '%b %d, %Y'))
        else:
            header = "{0} - {1}".format(fmt_dt(weekbeg, '%b %d, %Y'), fmt_dt(weekend, '%b %d, %Y'))
        # header = s2or3(leadingzero.sub('', header))
        header = leadingzero.sub('', header)
        self.busyLabel.setText("{0} {1}:  {2}".format(self.tr("Week"), wn, header))
        self.weekview.setWeekdays(weekdays, busy_lst, occasion_lst, self.today_col, curr_minutes)
    ...
    def getDayColor(self, num_minutes):
        ...
        max_b = 1           # must be <= 1
        max_minutes = 480
        brightness = min(max_b, min_b + (max_b-min_b)* num_minutes/float(max_minutes))
    ...
    def clickedWeek(self, dayhourmin, uuid):
        ...
        if uuid:
            ...
            dt += days*oneday
            self.createItem(self.tr("new event"), text = "*  @s %s" % fmt_datetime(dt, options = self.options), modified = False)
    ...
    def expandTo(self):
        if self.current_view in self.indexTree:
            depth, ok = QInputDialog.getInt(self,
                self.tr("expand to depth"),
    ...
    def itemChanged(self, filename):
        ...
        if res:
            ...
            # loadViewData updates self.all_alerts and through call to applyCalendarsFilter, updates self.alerts and show/hide todayButt
            if res:
                self.refreshViews()
        self.showStatusMessage(self.tr('changes saved'))
        if self.yScroll:
    ...
    def viewChanged(self, int=4):
        self.uiView.setCurrentIndex(int)
        if int in self.indexHsh:
            self.current_view = self.indexHsh[int]
        self.stackedWidget.setCurrentIndex(self.comboxItem[int])
    ...
    def setLeaf(self, text, type_key, tooltip='', col3=''):
        ...
        if type_key and type_key in tstr2SCI:
            if self.options['report_colors']:
                if self.options['report_colors'] == 2 or type_key[1] in [13,14,15]:
        ...
        for text in elements:
            if text in tree:
                ...
                if item_type and item_type in tstr2SCI:
                    if self.options['report_colors']:
                        if self.options['report_colors'] == 2 or item_type[1] in [13,14,15]:
                            item2.setForeground(QColor(tstr2SCI[item_type][1]))
    ...
    def timeout(self):
        ...
        new, modified, deleted = get_changes(self.options, self.file2lastmodified)
        if new or modified or deleted:
            if 'filechange_alert' in self.options and self.options['filechange_alert']:
                scmd = self.options['filechange_alert']
                process = QProcess()
                process.startDetached(scmd)

            res = self.loadFileData(dirty = True)
            self.loadViewData() # this updates self.alerts and show/hide todayButton
            self.refreshViews()
        elif newday:
            self.loadViewData()
            self.refreshViews()
        ...
        if self.alerts:
            ...
            while td < 0 and self.alerts:
                ...
                if td < 0:
                    self.alerts.pop(0)
            if td == 0:
                if 'alert_wakecmd' in self.options and self.options['alert_wakecmd']:
                ...
                while td == 0:
                    ...
                    if 's' in actions:
                        if 'alert_soundcmd' in self.options and self.options['alert_soundcmd']:
                            scmd = expand_template(self.options['alert_soundcmd'], hsh)
                            process.startDetached(scmd)
                        else:
                            QMessageBox.warning(self, "etm",
                    self.tr("A sound alert failed. The setting for 'alert_soundcmd' is missing from your etm.cfg."))
                    if 'd' in actions:
                        if 'alert_displaycmd' in self.options and self.options['alert_displaycmd']:
                            dcmd = expand_template(self.options['alert_displaycmd'], hsh)
                            process.startDetached(dcmd)
                        else:
                            QMessageBox.warning(self, "etm",
                    self.tr("A display alert failed. The setting for 'alert_displaycmd' is missing from your etm.cfg."))
                    if 'v' in actions:
                        if 'alert_voicecmd' in self.options and self.options['alert_voicecmd']:
                            vcmd = expand_template(self.options['alert_voicecmd'], hsh)
                            process.startDetached(vcmd)
                        else:
                            QMessageBox.warning(self, "etm",
                    self.tr("An email alert failed. The setting for 'alert_voicecmd' is missing from your etm.cfg."))
                    if 'e' in actions:
                        missing = []
                        for field in [
                            'smtp_from',
                        ...
                        if missing:
                            QMessageBox.warning(self, "etm",
                    self.tr("An email alert failed. Settings for the following variables are missing from your etm.cfg: %s." % ", ".join(["'%s'" %x for x in missing])))
                        else:
                            subject = hsh['summary']
                            message = expand_template(self.options['email_template'], hsh)
                            arguments = hsh['_alert_argument']
                            recipients = [str(x).strip() for x in
                                arguments[0]]
                            if len(arguments) > 1:
                                attachments = [str(x).strip() for x in  arguments[1]]
                            else:
                                attachments = []
                    ...
                    if 'm' in actions:
                        ...
                        mbox.setIconPixmap(QPixmap(":/etm.png").scaledToWidth(60))
                        mbox.setText(expand_template('!summary!', hsh))
                        mbox.setInformativeText(expand_template(self.options['alert_template'], hsh))
                        mbox.show()
                    ...
                    if 't' in actions:
                        ...
                        if missing:
                            QMessageBox.warning(self, "etm",
                    self.tr("A text alert failed. Settings for the following variables are missing from your 'emt.cfg': %s." % ", ".join(["'%s'" %x for x in missing])))
                        else:
                            message = expand_template(self.options['sms_message'], hsh)
                            subject = expand_template(self.options['sms_subject'], hsh)
                            arguments = hsh['_alert_argument']
                            if arguments:
        ...
        if self.timer_status == 'running':
            if  'running' in self.options['action_status'] and self.options['action_status']['running']:
                tcmd = expand_template(self.options['action_status']['running'], self.timer_status_hsh)
                os.system(tcmd)
        elif self.timer_status == 'paused':
            if  'paused' in self.options['action_status'] and self.options['action_status']['paused']:
                tcmd = expand_template(self.options['action_status']['paused'], self.timer_status_hsh)
                os.system(tcmd)
        ...
        if (self.options['action_interval'] and self.timer_minutes > 0
            ...
            if self.timer_status == 'running':
                if  'running' in self.options['action_timer'] and self.options['action_timer']['running']:
                    tcmd = self.options['action_timer']['running']
                    process.startDetached(tcmd)
            elif self.timer_status == 'paused':
                if  'paused' in self.options['action_timer'] and self.options['action_timer']['paused']:
    ...
    def jumpToDate(self):
        datestr, ok = QInputDialog.getText(self,
            self.tr("jump to date"), "%s:" % self.tr("date"))
        if ok:
            dt = parse(parse_datetime(str(datestr)), dayfirst=self.options['dayfirst'], yearfirst=self.options['yearfirst']).replace(tzinfo = None)
            if dt:
                self.scrollToDate(dt.date())
                return(True)
        return(False)
    ...
    def openScratchPad(self, modified=False):
        if 'scratchpad' in self.editor_instances:
            QMessageBox.warning(self, "etm", self.tr('scratchpad is already open'))
        else:
            form = getEditForm(self, instance='scratch', filename=self.scratch_file, highlight=False, use_completer=False)
            if form:
    ...
    def dateCalculator(self):
        datestr, ok = QInputDialog.getText(self,
            self.tr("datetime calculator"), "%s" % self.tr("""\
...
""" % (datestr, res))
    ...
    def timer_finish(self, create = True):
        ...
        self.timer_status = 'stopped'
        if  'stopped' in self.options['action_status'] and self.options['action_status']['stopped']:
            tcmd = expand_template(self.options['action_status']['stopped'], self.timer_status_hsh)
            os.system(tcmd)

        if self.timer_hsh:
            hsh = self.timer_hsh
            form = getEditForm(self, instance='edit', hsh = self.timer_hsh,
                receiver = self.itemChanged)
            if form:
            ...
            inital_entry = "~ %s @s %s @e %s" % (self.timer_summary,
                fmt_datetime(datetime.now(), self.options), fmt_period(self.timer_minutes*oneminute))
            res = self.createItem(title= self.tr("action"), text = inital_entry, modified = True)
            if not res:
                return()
        self.newActionButton.setEnabled(True)
        self.newActionButton.setToolTip("Click here or press 'a' to create a new action timer.")
        self.pause_restartButton.hide()
        self.timerLabel.hide()
        self.stopButton.hide()
        if self.timer_status == 'running':
    ...
    def timer_start(self, hsh={}):
        ...
        summary, ok = QInputDialog.getText(self,
            self.tr("new action"), "%s:" % self.tr("summary"), text=text)
        if ok:
    ...
    def timer_toggle(self, hsh={}):
        ...
        if self.timer_status == 'stopped':
            ...
            self.pause_restartButton.setToolTip(self.tr("Click here or press Ctrl-T to pause the action timer."))
            self.stopButton.setToolTip(self.tr("Click here or press Shift Ctrl-T to stop the timer and record the action."))
            self.timerLabel.setStyleSheet('color: green')
            self.timerLabel.setToolTip("'{0}': {1}".format(self.timer_summary, self.tr('running')))
        elif self.timer_status == 'running':
            self.timer_delta += datetime.now() - self.timer_last
            self.timer_status = 'paused'
            self.pause_restartButton.setToolTip("Click here or press Ctrl-T to restart the action timer.")
            self.pause_restartButton.setIcon(QIcon(":/action_restart.png"))
            self.timerLabel.setStyleSheet('color: red')
            self.timerLabel.setToolTip("'{0}': {1}".format(self.timer_summary, self.tr('paused') ))
            if  'paused' in self.options['action_timer'] and self.options['action_timer']['paused']:
                tcmd = self.options['action_timer']['paused']
                process = QProcess()
                process.startDetached(tcmd)
    ...
    def getTimer(self):
        ...
        if self.timer_status == 'paused':
            ...
            seconds = (self.timer_delta + datetime.now() - self.timer_last).seconds
        else:
            seconds = 0
            self.timer_minutes = 0
        if seconds > 0:
        ...
        # self.timer_status_hsh['summary'] = str(self.timer_summary)
    ...
    def createItem(self, title=None, text="", modified=False):
        ...
        form = getEditForm(self, instance='edit', receiver = self.itemChanged)
        if form:
    ...
    def showAgenda(self):
        ...
        form.resize(
            (self.options['agenda_width1']+self.options['agenda_width2'])*10+16,
             380)
        form.show()
        return(True)
    ...
    def aboutHelp(self):
        ...
        QMessageBox.about(self, "About etm",
                """<b>Event and Task Manager</b> v {0[version]}
                <p>This application provides a format for using plain text files to store actions, events, notes, and tasks and a PyQt based GUI for creating and modifying items as well as viewing them.
                <p>Copyright &copy; {0[copyright]} {0[dev]}. All rights reserved. This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 3 of the License, or (at your option) any later version.
                <p>Python {0[python]}, dateutil {0[dateutil]}, Qt {0[qt]}, SIP {0[sip]}, PyQt {0[pyqt]} on {0[platform]}.
                <center>{0[home]} &nbsp;&nbsp; {0[group]} &nbsp;&nbsp; {0[gpl]} &nbsp;&nbsp;</center>""".format(d))


    def showAlerts(self):
        header = "{0:^7}\t{1:^7}\t{2:<8}{3:<30}".format(self.tr('alert'), self.tr('event'), self.tr('type'), self.tr('summary'))
        divider = '-'*60
        if self.alerts:
            for alert in self.alerts:
                s = '%s\n%s\n%s' % \
                (header, divider,
                    "\n".join(["{0:^7}\t{1:^7}\t{2:<8}{3:<30}".format
                    (x[1]['alert_time'],
                    x[1]['_event_time'],
    ...
    #     fname = unicode(QFileDialog.getOpenFileName(self, 'Import ICS File', d, filter = self.tr("ICS files (*.ics)"), options = QFileDialog.DontUseNativeDialog))
    #     if not fname:
    #         return()
    #     print('importIcal:', fname)
    #     newlines = import_ical(fname)
    #     print('\n'.join(newlines))
    #     d = self.options['datadir']
    #     fname = unicode(QFileDialog.getOpenFileName(self, '', d, filter = self.tr("etm data files (*.txt)"), options = QFileDialog.DontUseNativeDialog))
    #     if not fname:
    #         return()
    #     fo = codecs.open(fname, 'r', encoding)
    #     oldlines = fo.readlines()
    #     fo.close()
    #     while len(oldlines) > 1 and not oldlines[-1]:
    #         oldlines.pop(-1)
    #     lastline = len(oldlines)
    #     fo = codecs.open(fname, 'a', encoding)
    #     fo.writelines("\n".join(newlines))
    #     fo.close()

    #     form = EditForm(parent=self,
    #         instance = 'import',
    #         filename = fname,
    #         hsh = None,
    #         receiver = self.itemChanged,
    ...
    #     if form:
    #         form.resize(self.options['window_width'], self.options['window_height']-30)
    #         form.setWindowTitle("{0}".format(self.options['auto_completions']))
    #         form.helpPage = 1
    #         # form.mode = 'import'
    #         cursor = form.editor.textCursor()
    #         cursor.movePosition(QTextCursor.Down, QTextCursor.MoveAnchor, lastline)
    #         form.editor.setTextCursor(cursor)
    ...
    def importIcal(self):
        ...
        while len(newlines) > 0 and not newlines[-1]:
            newlines.pop(-1)
        d = self.options['datadir']
        fname = unicode(QFileDialog.getSaveFileName(self, '', d, filter = self.tr("etm data files (*.txt)"),
        ...
        fo = codecs.open(fname, 'w', encoding)
        fo.writelines("\n".join(lines))
        fo.close()
        ...
        if form:
            ...
            form.setWindowTitle("{0}".format(self.options['auto_completions']))
            form.helpPage = 1
            # set the cursor at the end and then move it back to the beginning of the addition to leave the addition selected
            cursor = form.editor.textCursor()
            cursor.movePosition(QTextCursor.End, QTextCursor.MoveAnchor)
            cursor.movePosition(QTextCursor.StartOfLine, QTextCursor.KeepAnchor)
            cursor.movePosition(QTextCursor.Up, QTextCursor.KeepAnchor, addedlines-1)
    ...
    def exportIcal(self):
        # print('starting exportIcal')
        # ok, msg = make_ical(self.all_rows, self.uuid2hash, self.options['icscal_file'], self.calendars)
        msgBox = QMessageBox(self)
        msgBox.setIcon(QMessageBox.Question)
        msgBox.setText(self.tr("iCalendar Export"))
        msgBox.setInformativeText(self.tr("Save items in selected calendars to the file\n\n{0}?".format(self.options['icscal_file'])))
        msgBox.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
        msgBox.setDefaultButton(QMessageBox.Ok)
        # must use exec_ here
        reply = msgBox.exec_()
        ...
        ok, msg = export_ical(self.uuid2hash, self.options['icscal_file'], self.calendars)
        if ok:
            if self.calendars:
                calendars = "items in calendars:\n      {0}".format(
                    ", ".join(["'{0}'".format(x[0]) for x in self.calendars if x[1]]))
            else:
                calendars = "all items"
            msg = "Exported {0}\nto {1}".format(calendars, self.options['icscal_file'])
    ...
    def showWeather(self):
        if self.getWeather:
            ...
            QMessageBox.information(self, "etm", self.tr("The setting for 'weather_location' in etm.cfg is required for this operation."))
    ...
    def showSunMoon(self, enabled=True):
        if self.getSunMoon:
            ...
            QMessageBox.information(self, "etm", self.tr("The setting for 'sunmoon_location' in etm.cfg is required for this operation."))
...
def main(etmdir = ''):
    ...
    loadfile = window.loadFileData()
    loadview = window.loadViewData()
