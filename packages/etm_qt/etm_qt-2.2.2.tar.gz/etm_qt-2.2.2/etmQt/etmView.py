...
if platform.python_version() >= '3':
    ...
    def utf8(s):
        # print('utf8', s, type(s), unicode(s), str(s.encode('utf-8')).decode('utf-8'))
...
class PHL(QSyntaxHighlighter):
    ...
    def highlightBlock(self, text):
        ...
        # print('highlightBlock', text, type(text))
...
class DetailForm(QDialog, Details_Dialog):
    def __init__(self, parent=None, receiver=None):
        ...
        QShortcut(QKeySequence(Qt.CTRL + Qt.Key_H), self, self.show_history)
        ...
        if process.waitForStarted():
            ...
            if error:
                ...
                if python_version2:
                    ...
                    return((1, str(result, encoding='utf-8')))
...
def getEditForm(parent, instance=None, hsh=None, filename=None, receiver=None, highlight=True, use_completer=True, options = {}):
    ...
    if instance in msg_hsh.keys() and instance in parent.editor_instances:
        ...
        return(None)
    elif instance in ['file', 'edit'] and ('file' in parent.editor_instances or 'edit' in parent.editor_instances):
        if instance == 'file':
            ...
            if ('auto_completions' in self.parent().options and self.parent().options['auto_completions'] and os.path.isfile(self.parent().options['auto_completions'])):
                try:
                    ...
                    for word in f.readlines():
                        ...
                        if python_version2:
                            words.append(u'%s' % word.decode('utf-8'))
                        else:
                            words.append(word)
    ...
    def ensureMonthly(self):
        ...
        if 'monthly' in self.parent().options and self.parent().options['monthly']:
            ...
            if not os.path.isfile(currfile):
                fo = codecs.open(currfile, 'w', 'UTF8')
                fo.write("")
                fo.close()
...
class ReportForm(QDialog, Report_Dialog):
    def __init__(self, parent=None, colors=2):
        ...
        self.resize(520, 340)
        # print('checking for reports', self.parent().options)
        reports = []
        if ('report_specifications' in self.parent().options and self.parent().options['report_specifications'] and os.path.isfile(self.parent().options['report_specifications'])):
            try:
                fo = codecs.open(self.parent().options['report_specifications'], 'r', 'UTF8')
    ...
    def on_completer_activated(self, text):
        if text:
            ...
            # print("on_completer_activated", self.reportBox.itemText(index))
    ...
    def saveReports(self):
        ...
        if self.modified:
            ...
            try:
                fo = codecs.open(self.parent().options['report_specifications'], 'w', 'UTF8')
                for item in items:
                    fo.write("%s\n" % item)
                self.saveButton.setEnabled(False)
                self.modified = False
...
class WeekView(QWidget):
    ...
    def mouseMoveEvent(self, event):
        id, summary = self.getDetails(event)
    ...
    def setWeekdays(self, day_lst, busy_lst, today_col = 0, curr_minutes = 0):
...
class UiWindow(QMainWindow, MainWindow_Dialog):
    ...
    def loadViewData(self):
        ...
        (self.all_rows, self.all_busytimes, self.all_busydays,
            self.all_alerts, self.all_dates) = getViewData(bef,
    ...
    def setWeek(self, curr_day = datetime.today()):
        ...
        self.weekview.setWeekdays(weekdays, busy_lst, self.today_col, curr_minutes)
    ...
    def clickedSlot(self, modelIndex):
        ...
        # print('view', view_indx, view, self.current_view)
    ...
    def itemChanged(self, filename):
        # print('get_changes itemChanged 1')
    ...
    def configChanged(self, filename):
        ...
        self.options = options
        # print('configChanged', options['datadir'], current_options['datadir'])
    ...
    def timeout(self):
        ...
        if self.timer_status == 'running':
            # run action_timercmd every acton_minutes
            if (self.options['action_interval'] and self.timer_minutes > 0
              and self.timer_minutes % self.options['action_interval'] == 0):
                if  'action_timercmd' in self.options and self.options['action_timercmd']:
                    tcmd = self.options['action_timercmd']
    ...
    def jumpToDate(self):
        ...
        if ok:
            # print("jumpToDate", type(datestr), datestr)
            # dt = parse(parse_dtstr(str(datestr))).replace(tzinfo = None)
...
""" % (datestr, res))
    ...
    def timer_finish(self, create = True):
        ...
        if self.timer_hsh:
            # print('self.timer_hsh is not empty')
            hsh = self.timer_hsh
            ...
            if form:
                ...
                if self.timer_hsh['e'] == timedelta(seconds = 0):
                    ...
                    form.filename = self.timer_hsh['_filename']
