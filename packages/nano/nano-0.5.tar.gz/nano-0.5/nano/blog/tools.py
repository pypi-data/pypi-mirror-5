...
def get_nano_blog_entries(special_tags=('devel','milestone'), cutoff=2):
    ...
    special_tags: iterable collection of strings, default 'devel','milestone'
    """
    special_news = ()
    news = ()
    if NANO_BLOG_TAGS:
        if NANO_BLOG_TAGS.__name__ == 'taggit':
            special_news = _Entry.objects.filter(tags__slug__in=special_tags)
        elif NANO_BLOG_TAGS.__name__ == 'tagging':
            special_news = _Entry.tagged.with_any(special_tags)
        special_news = special_news.order_by('-pub_date')[:cutoff]
    if special_news:
        news = _Entry.objects.exclude(id__in=[e.id for e in special_news])
    else:
        news = _Entry.objects.all()
