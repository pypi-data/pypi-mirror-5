    ...
    def pprint(self, ostream=None, verbose=False):
        ...
        if len(self._data):
            for val in self._data.itervalues():
                val.pprint(indent, verbose=verbose)
        else:
            Block.pprint(self, ostream=indent, verbose=verbose)
...
class Disjunct(Block):
    def __new__(cls, *args, **kwds):
        ...
        if args == ():
            return SingletonDisjunct.__new__(SingletonDisjunct)
        else:
...
class SingletonDisjunct(_DisjunctData, Disjunct):
    ...
    def __getstate__(self):
        ans = Disjunct.__getstate__(self)
        ans2 = _DisjunctData.__getstate__(self)
        for attr, val in ans2.iteritems():
            if attr not in ans:
                ans[attr] = val
    ...
    def __setstate__(self, state):
        # Important: call Disjunct before _DisjunctData so that the Disjunct
        # hierarchy has an opportunity to correct weakrefs before
        # _DisjunctData performs a blanket assignment
        Disjunct.__setstate__(self, state)
        _DisjunctData.__setstate__(self, state)
...
register_component(Disjunct, "Disjunctive blocks in a model.")
register_component(Disjunction, "Disjunction expressions in a model.")
