.. image:: https://secure.travis-ci.org/unice/unice.portlet.mosaique.png
   :target: https://travis-ci.org/unice/unice.portlet.mosaique

Introduction
============

This portlet shows a mosaique item as mosaique in the portlet.
...
Add `unice.portlet.mosaique` to your buildout and re-run buildout.
...
the option to add a Mosaique Portlet.

Next, select a mosaique item to display. It can be any type of item. You can
select which of the item's fields you want to display in the portlet's mosaique
area:

* Title (either as a link to the mosaique item or as plain text)
* Description
* Date
* Body text
* Image

When the mosaique item doesn't have a selected field, it should work without
...
* portlet header title as rendered on page
  (use mosaique's title or select custom)
* hide portlet header altogether
* conditionally show a portlet footer with a link to the mosaique item
