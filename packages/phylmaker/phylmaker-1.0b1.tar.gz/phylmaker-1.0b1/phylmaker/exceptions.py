"""
	Exceptions used by Phylmaker
	
	Copyright (c) 2013, Mel Collins <mel@raumkraut.net>
	Released under the terms of the X11/MIT License
	(see included LICENSE file)
"""

class FMConnectionError(IOError):
	""" The process of making an FM XML API call failed. """
class FMFeatureNotImplementedError(NotImplementedError):
	""" Attempted to use an as yet unimplemented feature. """
class FMNotSupportedError(NotImplementedError, ValueError):
	""" Attempted to perform an action impossible on the XML API. """

class FMDatabaseError(Exception):
	""" The server responded with an error code. """
	def __init__(self, code):
		self.code = code
		message = '{0}: {1}'.format(code, self.MESSAGES.get(code, '???'))
		super(FMDatabaseError, self).__init__(message)
