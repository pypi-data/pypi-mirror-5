...
class InputFormatBase(InputFormat):
    ...
    def __init__(self, options):
        ...
        self._src = options.get('src', None)
...
class InputFormat_Text_Lines(InputFormatBase):
    ...
    """
    def __init__(self, options):
        InputFormatBase.__init__(self, options)
...
class InputFormat_Text_CSV(InputFormatBase):
    ...
    def __init__(self, options):
        InputFormatBase.__init__(self, options)
        self._dialect = options.get('dialect', 'excel')
        self._csv_header = options.get('csv_header', 'no')
        self._struct = options.get('struct', 'list')
        self._line_num = 0   # of last read line, 1-based.
