...
class HolmiumNose(Plugin):
    ...
    def options(self, parser, env):
        ...
        parser.add_option("", "--holmium-browser", dest="ho_browser", type = "choice", choices = list(holmium.browser_mapping.keys()), help="the selenium driver to invoke")
    ...
    def configure( self, options, conf ):
        if options.ho_enabled:
            ...
            if self.remote_url:
                caps.update(holmium.capabilities[self.browser])
            ...
            self.driver_initializer_fn = lambda:holmium.browser_mapping[self.browser](**args)
            self.enabled = True
    ...
    def beforeTest(self, test):
        ...
        try:
            ...
            if config:
                self.config = holmium.Config(config, holmium_vars)
        except Exception as e:
