...
class Page(object):
    ...
    def __getattribute__(self, key):
        ...
        if not object.__getattribute__(self, "home"):
            holmium.log.debug("home url not set, attempting to update.")
        ...
        if isinstance(attr, types.MethodType):
            ...
            def wrap(*args, **kwargs):
                ...
                if not resp:
                    holmium.log.debug("method %s returned None, using fluent response" % attr.func_name)
                    resp = self
                return resp
            return wrap
        return attr
...
class ElementGetter(object):
    ...
    def __init__(self, locator_type, query_string, base_element=None, timeout=1, value=lambda el:el):
        ...
        holmium.log.debug("locator:%s, query_string:%s, timeout:%d" %
    ...
    def get_element(self, method = None):
        if self.base_element:
            if isinstance(self.base_element, types.LambdaType):
                ...
                holmium.log.error("unknown base_element type used: %s" % type(self.base_element))
        else:
            _meth = method
        holmium.log.debug("looking up locator:%s, query_string:%s, timeout:%d" %
        ...
        if self.timeout:
            try:
                ...
                holmium.log.debug("unable to find element %s after waiting for %d seconds" % (self.query_string, self.timeout))
...
class Element(ElementGetter):
    ...
    by querying via one of :class:`holmium.Locators`
    """
...
class Elements(ElementGetter):
    ...
    objects by querying via one of :class:`holmium.Locators`
    """
