...
class StaticRoute(object):
    ...
    def __init__(self, name, roles, urls, connex):
        ...
        self.write = None
        self.route_ref = myhash(connex)
        self.identity_ack = False
        self.serial = 0
        self.ws = None
        self.their_roles = None  # their roles answered in identity ack(with *)
...
def ac_on_close(self, event):
    ...
    self.broadcast_event('EV_STATIC_ROUTE_CLOSED', route=route);
...
def ac_input_message(self, event):
    ...
    if message_type == '__identity_card__':
        ...
        self.broadcast_event('EV_STATIC_ROUTE_OPENED', route=this_route);
...
GROUTER_FSM = {
    'event_list': (
        ...
        'EV_STATIC_ROUTE_OPENED: top output',
        'EV_STATIC_ROUTE_CLOSED: top output',
    ...
    *Output-Events:*
    ...
    def mt_show_routes(self):
        ...
        for key in static_routes:
            ...
            s += ' route_ref: %s |name:%r |roles: %r |their_roles:%r |%s |%s\n' % (
                key,
                r.gaplic_name,
                r.roles,
                r.their_roles,
        ...
        for key in dynamic_routes:
            ...
            s += '  route_ref: %s |name:%r |roles:%r |their_roles:%r\n' % (
                key,
                r.gaplic_name,
                r.roles,
                r.their_roles
    ...
    def _send_event_to_external_route(
        ...
        if gaplic_name is None:
            gaplic_name = route.gaplic_name
        if role is None:
            role = route.roles
