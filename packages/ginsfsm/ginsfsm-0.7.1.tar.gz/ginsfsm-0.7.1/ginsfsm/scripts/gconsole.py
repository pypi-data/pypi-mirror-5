...
def ac_command_response(self, event):
    ...
    """
    #self.set_timeout(5)
    action_return = event.__action_return__
...
class GConsole(GObj):
    ...
    *Output-Events:*
    ...
    def cmd_show_external_routes(self, route_ref):
        self.gaplic.router._send_event_to_external_route(
            route_ref,
            None,  # dest gaplic.name: the route's gaplic
            None,  # dest roles; the route's roles
            'router',
