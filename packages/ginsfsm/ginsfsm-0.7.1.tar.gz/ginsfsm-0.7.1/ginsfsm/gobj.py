...
GOBJ_GCONFIG = {
    ...
    '__new_subscriptor_event__': [
        str, None, 0, None,
        'If exists, the subscribe_event method will send that event to the obj'
        ' to inform of a new subscriber.'],
    ...
    def subscribe_event(self, event_name, subscriber_gobj, **kw):
        ...
        Possible values for **kw** arguments:
            ...
            * `__first_shot__`: ``Bool``
              ...
              If True then generate a broadcast event to the new subscriber.
              It's necesary to have __new_subscriptor_event__ flag,
              and it's responsability of the gobj to do the first shot.
        """
        ...
        if self.config.__new_subscriptor_event__:
            self.send_event(
                self,
                self.config.new_susbcriptor_event,
