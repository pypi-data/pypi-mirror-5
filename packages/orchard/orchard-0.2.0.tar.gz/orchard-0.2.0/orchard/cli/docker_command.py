...
class DockerCommand(Command):
    ...
    def run(self, options):
        ...
        Options:
            ...
            -p PORTS    Expose a container's port to the host (format: -p EXTERNAL_PORT:CONTAINER_PORT)
        ...
        if options['-p']:
            match = re.search('^\d+:\d+$', options['-p'])

            if match:
                ports = [options['-p']]
            else:
                sys.stderr.write("Ports argument format: -p EXTERNAL_PORT:CONTAINER_PORT\n")
                exit(1)
