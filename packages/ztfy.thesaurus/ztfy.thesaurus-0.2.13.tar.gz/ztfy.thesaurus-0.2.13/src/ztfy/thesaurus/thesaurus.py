...
class Thesaurus(Persistent, Contained):
    ...
    def findTerms(self, query=None, extract=None, autoexpand='on_miss', glob='end', limit=None, exact=False):
        ...
        search = ThesaurusIndexSearch((self.catalog, 'label'), {'query': u' '.join((u'%s%s%s' % (start, m, end)
                                                                                    for m in query.split()
                                                                                          if len(m) > 2)),
                                                                'autoexpand': autoexpand,
                                                                'ranking': False})
        terms += sorted(query_util.searchResults(search, limit=limit), key=lambda x: x.label)
        if extract:
